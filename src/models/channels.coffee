...
channelSchema.statics.getChannelLeaderBadges = (domain, count, userId, channelIds, checkIsLeader, callback) ->
  self = @
  getBadges = (channelIds) ->
    ...
    async.filter channelIds, (channelId, cb) ->
      if checkIsLeader
        ...
        images.countInChannelByOwner domain, channelId, userId, domain.intercept (count) ->
          leaderDataHash[channelId] = { count:count }
          cb true
    , (channelsUserLeadsIds) -> # async.filter doesnt bubble error...
      async.reduce channelsUserLeadsIds, [],
        (mostPopular, channelId, cb) ->
          images.count 'tags.channel':channelId, highestCountItems(count, mostPopular, channelId, cb)
      , (err, mostPopular) ->
        countHash = {}
        mostPopUserLedChannelIds = mostPopular.map (channel) ->
          countHash[channel._id] = channel.count
          channel._id
        self.find(_id:$in:mostPopUserLedChannelIds, { name:1, aliases:1 }).lean().exec domain.intercept (channels) ->
          channels.forEach (channel) ->
            channel.count = countHash[channel._id]
            channel.leaderPosition = leaderDataHash[channel._id].position
            channel.leaderImagesCount = leaderDataHash[channel._id].count
          callback null, channels
  if not channelIds # if no channelId provided then check all channels user has images in
  ...
  , (err, channelsUserLeadsData) ->
    if err then callback err else
      if channelsUserLeadsData.length <= size
        async.map channelsUserLeadsData, extendWithNameAndCount(domain).bind(this)
        , callback
      else
