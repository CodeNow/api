...
Runnables =
    ...
    handler = (err, image) ->
      if err then cb err else
        users.findUser _id: userId, (err, user) ->
          if err then new error { code: 500, msg: 'error looking up user' } else
            if not user then new error { code: 404, msg: 'user not found' } else
              runnableId = encodeId image._id
              user.vote runnableId, (err) ->
                if err then new error { code: 500, msg: 'error updating vote count' } else
  ...
  createContainer: (userId, from, cb) ->
    ...
    images.findOne _id: from, (err, image) ->
      if err then cb err else
        if not image then new error { code: 400, msg: 'could not find source image to fork from' } else
  ...
  delete: (userId, runnableId, cb) ->
    ...
    projects.findOne _id: runnableId, (err, project) ->
      if err then new error { code: 500, msg: 'error querying mongodb' } else
        if not project then new error { code: 404, msg: 'runnable not found' } else
          if project.owner.toString() is userId.toString() then removeProject() else
            users.findUser _id: userId, (err, user) ->
              if err then new error { code: 500, msg: 'error looking up user' } else
                if not user then new error { code: 404, msg: 'user not found' } else
                  if user.permission_level <= 1 then new error { code: 403, msg: 'permission denied' } else
  ...
  isOwner: (userId, runnableId, cb) ->
    runnableId = decodeId runnableId
    projects.findOne _id: runnableId, (err, project) ->
      if err then new error { code: 500, msg: 'error looking up runnable' } else
        if not project then new error { code: 404, msg: 'runnable not found' } else
  ...
  start: (userId, runnableId, cb) ->
    ...
    projects.findOne _id: runnableId, (err, project) ->
      if err then new error { code: 500, msg: 'error looking up runnable' } else
        if not project then new error { code: 404, msg: 'runnable not found' } else
          if project.owner.toString() isnt userId.toString() then new error { code: 403, msg: 'permission denied' } else
            project.start (err) ->
              if err then cb err else
                project.containerState (err, state) ->
                  if err then cb err else
  ...
  getVotes: (runnableId, cb) ->
    ...
    users.find('votes.runnable': runnableId).count().exec (err, count) ->
      if err then new error { code: 500, msg: 'error counting votes in mongodb' } else
  ...
  listAll: (sortByVotes, limit, page, cb) ->
    if not sortByVotes
      ...
      users.aggregate voteSortPipeline(limit, limit*page), (err, results) ->
        if err then new error { code: 500, msg: 'error aggragating votes in mongodb' } else
          async.map results, (result, cb) ->
            projects.findOne _id: result._id, (err, runnable) ->
              if err then new error { code: 500, msg: 'error retrieving project from mongodb' } else
    ...
    projects.findOne _id: runnableId, (err, project) ->
      if err then new error { code: 500, msg: 'error looking up runnable' } else
        if not project then new error { code: 404, msg: 'runnable not found' } else
          ...
          if not tag then new error { code: 404, msg: 'tag not found' } else
            cb null, tag
      ...
      if err then new error { code: 500, msg: 'error looking up runnable' } else
        if not project then new error { code: 404, msg: 'runnable not found' } else
          if project.owner.toString() isnt userId.toString()
            users.findUser _id: userId, (err, user) ->
              if err then new error { code: 500, 'error looking up user' } else
                if not user then new error { code: 500, 'user not found' } else
                  if user.permission_level < 2 then new error { code: 403, msg: 'permission denied' } else
                    project.tags.push name: text
                    tagId = project.tags[project.tags.length-1]._id
                    project.save (err) ->
                      if err then new error { code: 500, msg: 'error saving tag' } else
        ...
        if not project then new error { code: 404, msg: 'runnable not found' } else
          if project.owner.toString() isnt userId.toString()
            user = users.findOne _id: userId, (err, user) ->
              if err then new error { code: 500, msg: 'error looking up user' } else
                if not user then new error { code: 500, msg: 'user not found' } else
                  if user.permission_level < 2 then new error { code: 403, msg: 'permission denied' } else
                    project.tags.id(tagId).remove()
                    project.save (err) ->
                      if err then new error { code: 500, msg: 'error removing tag from mongodb' } else cb()
          else
            project.tags.id(tagId).remove()
            project.save (err) ->
              if err then new error { code: 500, msg: 'error removing tag from mongodb' } else cb()
  ...
  listFiles: (runnableId, content, dir, default_tag, path, cb) ->
    ...
    containers.findOne _id: runnableId, (err, container) ->
      if err then new error { code: 500, msg: 'error looking up runnable' } else
        if not container then new error { code: 404, msg: 'runnable not found' } else
    ...
    projects.findOne _id: runnableId, (err, project) ->
      if err then new error { code: 500, msg: 'error looking up runnable' } else
        if not project then new error { code: 404, msg: 'runnable not found' } else
          ...
          if not file then new error { code: 404, msg: 'file not found' } else
