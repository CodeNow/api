...
describe 'api', ->
  ...
  it 'should be able to invoke container cleanup as an admin user ::base', (done) ->
    user = sa.agent()
    helpers.createServer configs, done, (err, instance) ->
      if err then done err else
  ...
  it 'should be able to invoke cache refresh as an admin user ::base', (done) ->
    user = sa.agent()
    helpers.createServer configs, done, (err, instance) ->
      if err then done err else
        helpers.createAdminUser user, (err, token) ->
          if err then done err else
            user.get("http://localhost:#{configs.port}/cache")
              .set('runnable-token', token)
              .end (err, res) ->
                if err then done err else
                  res.should.have.status 200
