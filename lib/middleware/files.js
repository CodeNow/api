...
var files = module.exports = createModelMiddleware('file', {
  ...
  sync: function (req, res, next) {
    ...
    async.waterfall([
      ...
      function (cb) {
        console.log('one');
        volumes.readAllFiles(container, ignoreFilepaths, function (err, data) { console.log('callback', err); cb(err, data); })
      },
      function (allFiles, cb) {
        console.log('two');
        Container.findById(container._id, function (err, data) { var results = { allFiles: allFiles, container: data }; cb(err, results); })
      },
      function (results, cb) {
        console.log('sup');
