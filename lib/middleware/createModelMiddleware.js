...
ModelMiddleware.prototype = {
  ...
  set: function (data) {
    ...
    return function (req, res, next) {
      var localData = utils.replacePlaceholders(req, data);
      req[this.key].set(localData);  // is this.key correct??
      next();
    };
  },
  ...
  checkConflict: function (req, res, next) {
    var paramId = req.params && (req.params.id || req.params[this.key+'Id']);
    var conflictId = req[this.key] && req[this.key]._id;
