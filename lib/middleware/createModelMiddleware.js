...
function getModelMiddlewareClass() {
  ...
  _(ModelMiddleware.prototype).extend({
    ...
    modelExtendRequestData: function () {
      // inherit requestData methods
      var self = this;
      var requestData = new RequestData(self.key);
      self.model = self.model || {};
      for (var key in requestData) {
        var dataMethod = requestData[key];
        // if requestData key is method, and method doesnt already exists on self.model
        if (typeof dataMethod === 'function' && !self.model[key]) {
          self.model[key] = dataMethod.bind(requestData);
        }
      }
    },
      ...
      respondStream: function (req, res, next) {
        ...
        function resWriteDoc (doc, cb) {
          if (doc.returnJSON) {
            ...
            writing = false;
            json = doc.toJSON ? doc.toJSON() : doc;
            var str = JSON.stringify(json);
            if (firstWrite) {
              firstWrite = false;
            }
...
module.exports = function createModelMiddleware (keyOrModel, extend, key) {
