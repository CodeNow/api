...
var setDefault = function (defVal) {
  return function (val) {
    return val || defVal;
  };
};
...
var utils = module.exports = {
  ...
  formatPaging: function () {
    return utils.series(
      ...
      mw.query('limit').mapValues(max(configs.maxPageLimit)),
      mw.query('page', 'limit').number().require()
    );
  },
  ...
  require: function (key) {
    return function (req, res, next) {
      ...
      if (!req[key]) {
        return next(error(404, utils.capitalize(key)+' not found'));
      }
      next();
    };
  },
