...
var users = module.exports = {
  ...
  register: function (req, res, next) {
    ...
    function save (password, cb) {
      ...
      user.lower_username = user.username.toLowerCase();
      user.save(cb);
    }
  },
  ...
  checkPassword: function (req, res, next) {
    if (configs.passwordSalt) {
      bcrypt.compare(req.body.password + configs.passwordSalt, req.user.password, req.domain.intercept(function (matches) {
        if (!matches) {
