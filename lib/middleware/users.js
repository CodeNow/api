...
module.exports = {
  fetchSelf: function (req, res, next) {
    User.findUser(req.domain, { _id: req.user_id }, req.domain.intercept(function (user) {
      if (!user) {
  ...
  fetchUser: function (req, res, next) {
    if (req.params.userid) {
      if (!req.params.userid.match(/^[0-9a-fA-F]{24}$/)) {
        res.json(404, { message: 'user not found' });
      } else if (req.params.userid.toString() !== req.user_id.toString() && !req.self.isModerator) {
  ...
  saveAllUser: function (req, res, next) {
