...
var images = module.exports = createModelMiddleware(Image, {
  ...
  getFeedPage: function (req, res, next) {
    ...
    // use zrevrange to get high -> low position
    var page = parseInt(req.query.page);
    var limit = parseInt(req.query.limit);
    var start = page * limit;
    var end = start + limit - 1;
    async.parallel({
      imageIds: redis.zrevrange.bind(redis, 'imagefeed_' + feed, start, end),
      length: redis.zcard.bind(redis, 'imagefeed_' + feed)
    }, function(err, results) {
      if (err) {
        return next(err);
      }
      req.imageIds = results.imageIds;
      req.paging = {
  ...
  getSortedList: function (req, res, next) {
    if (req.imageIds) {
      // for the odd case if we just pass one in, not in an array
      var imageIds = Array.isArray(req.imageIds) ? req.imageIds : [req.imageIds];
      async.map(imageIds, function(imageId, cb) {
        Image.findById(imageId, { files: 0 }, cb);
      }, function(err, results) {
        if (err) {
          return next(err);
        }
