...
var images = module.exports = createModelMiddleware(Image, {
  checkRedisHealth: function (req, res, next) {
    redis.exists('imagefeed_global', function (err, check) {
      if (err) {
        return next(err);
      }
      if (check === 1) {
  ...
  getFeedPage: function (req, res, next) {
    ...
    var start = parseInt(req.query.page) * parseInt(req.query.limit);
    var end = start + parseInt(req.query.limit) - 1;
    redis.zrevrange('imagefeed_' + feed, start, end, function(err, imageIds) {
      if (err) {
        return next(err);
      }
      req.params.query = { _id: { $in: imageIds } };
      next();
    });
  },
