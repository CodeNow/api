...
var images = module.exports = createModelMiddleware(Image, {
  ...
  getFeedPage: function (req, res, next) {
    ...
    function combineChannelsAndPage (err, results) {
      ...
      var combinedImages = _.reduce(results, function (memo, result) {
        _.each(result.imageIds, function(id, index, list) {
          memo.push(id[0]);
        });
        return memo;
      }, []);

      req.query._id = combinedImages.slice(start, end + 1);
      req.paging = {
        lastPage: Math.ceil(combinedImages.length / limit)
      };
