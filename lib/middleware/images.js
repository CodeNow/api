...
var images = module.exports = createModelMiddleware(Image, {
  ...
  getFeedPage: function (req, res, next) {
    ...
    function getAllChannelsFromRedis(channel, callback) {
      async.parallel({
        imageIds: redis.zrevrange.bind(redis, 'imagefeed_' + channel, 0, -1, 'WITHSCORES'),
        length: redis.zcard.bind(redis, 'imagefeed_' + channel)
      }, function(err, results) {
        if (err) {
          return next(err);
        }
        var data = [];
        for (var i = 0; i < results.imageIds.length; i += 2) {
          data.push([results.imageIds[i], results.imageIds[i+1]]);
        }
        results.imageIds = data;
    ...
    function combineChannelsAndPage (err, results) {
      ...
      if (channels.length > 1) {
        ...
        combinedImagesWithScores = results[0].imageIds;
      }
      ...
      req.paging = {
        lastPage: Math.ceil(combinedImagesWithScores.length / limit)
      };
  ...
  findPageInChannels: function (channelsKey) {
    return function (req, res, next) {
      var channels = keypather.get(req, channelsKey) || [];
      var channelIds = channels.map(mutils.pluck('_id'));
      ...
      images.findPage('query')(req, res, next);
    };
  },
