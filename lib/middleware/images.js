...
var images = module.exports = createModelMiddleware(Image, {
  ...
  getFeedPage: function (req, res, next) {
    ...
    function combineChannelsAndPage (err, results) {
      ...
      req.params.allImages = combinedImagesWithScores;
  ...
  getRemainingTags: function (req, res, next) {
    ...
    function goTJ (callback) {
      var time = process.hrtime();
      Image.distinct('tags.channel', { _id: { $in: images }}, intercept(function (channelIds) {
        Channel.find({ _id: { $in: channelIds }}, { name:1 }, intercept(function (channels) {
          async.map(channels, function (channel, cb) {
            Image.count({
              'tags.channel': channel._id,
              _id: { $in: images }
            }, intercept(function (count) {
              channel = channel.toJSON();
              channel.count = count;
              cb(null, channel);
            }));
          }, intercept(function (channels) {
            req.filterTagsTJ = channels;
            var diff = process.hrtime(time);
            console.log('[feedTags:TJ]', diff[0] + diff[1] / 1e9);
            callback();
          }));
        }));
      }));
    ...
    function goBK (callback) {
      var time = process.hrtime();
      var match = {
        $match: {
          _id: {
            $in: images.map(utils.createObjectId)
          }
        }
      };
      var project = {
        ...
        Channel.find({_id: {$in: onlyIds}}, function (err, results) {
          ...
          _.each(data, function (channelResult) {
            ...
            req.filterTags.push({
              ...
              count: channelResult.images,
              name: channel.name
            });
          });
          var diff = process.hrtime(time);
          console.log('[feedTags:BK]', diff[0] + diff[1] / 1e9);
          callback();
        });
      });
    }
