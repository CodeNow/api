...
var images = module.exports = createModelMiddleware(Image, {
  ...
  getFeedPage: function (req, res, next) {
    var channelIds = req.channel && req.channel.length ?
      req.channel.map(pluck('_id')) : ['global'];
    ...
    feeds.range(start, end, function (err, images, paging, feedResults) {
      if (err) {
        next(err);
      }
      else {
  ...
  getRemainingTags: function (req, res, next) {
    ...
    if (req.feedResults) {
      var allFeedImageIds = req.feedResults.map(pluck(0)); // imageId is pos 1
      doAggregation(allFeedImageIds);
    }
    else {
