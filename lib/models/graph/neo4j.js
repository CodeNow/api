...
Neo4j.prototype.getNodeCount = function (nodeLabel, cb) {
  ...
  this._query(query, {}, function (err, data) {
    if (err) { cb(err); }
    else if (!data || !data['count(*)']) { cb(null, -1); }
...
Neo4j.prototype.writeNode = Neo4j.prototype._writeUniqueNode = function (node, cb) {
  ...
  log.trace(logData, 'Neo4j.prototype.writeNode');
  var nodeProps = [];
  ...
  this._query(q, p, function (err, data) {
    if (err) {
      log.error(put({ err: e }, logData), 'Neo4j.prototype.writeNode error');
      return cb(err);
    }
    if (!data || !data.n) {
...
Neo4j.prototype._query = function (q, p, cb) {
  ...
  log.trace(logData, 'Neo4j.prototype._query');
