...
Neo4j.prototype.getNodes = function (start, steps, cb) {
  ...
  steps.forEach(function (step) {
    ...
    if (step.Out) {
      s = step.Out;
      query += '-[:' + s.edge + ']->(' + node + ':' + s.node + ')';
      returnVars.push(node);
    } else if (step.In) {
      s = step.In;
      query += '<-[:' + s.edge + ']-(' + node + ':' + s.node + ')';
      returnVars.push(node);
    ...
    if (s && s.props) {
      var props = node + 'Props'; // e.g., cProps
      s.propsName = props;
      s.propsAsQuery = Object.keys(s.props).map(function (key) {
  ...
  steps.forEach(function (step) {
    if (keypather.get(step, 'In.props')) {
      where = where.concat(step.In.propsAsQuery);
      params[step.In.propsName] = step.In.props;
    } else if (keypather.get(step, 'Out.props')) {
      where = where.concat(step.Out.propsAsQuery);
      params[step.Out.propsName] = step.Out.props;
    }
  });
  var q = [ query ];
