...
Neo4j.prototype.writeConnections = function(connections, cb) {
  ...
  var self = this;
  async.mapSeries(
    connections, function(conn, mapCb) {
      conn.push(mapCb);
...
Neo4j.prototype.writeNodes = function(nodes, cb) {
  ...
  var self = this;
  async.forEach(
    nodes, function(n, eachCb) {
      self._writeUniqueNode(n, eachCb);
    }, function(err) {
      if (err) {
        return cb(err);
      }
...
Neo4j.prototype.writeNode = Neo4j.prototype._writeUniqueNode = function(node, cb) {
  ...
  this._query(q, p, function(err, data) {
    if (err) {
      log.error(put({
        err: err
      }, logData), 'Neo4j.prototype.writeNode error');
      return cb(err);
    }
    if (!data || !data.n) {
...
Neo4j.prototype._query = function(q, p, cb) {
  ...
  t.on('error', function(e) {
    log.error(put({
      err: e
    }, logData), 'Neo4j.prototype._query error');
    err = e;
  });
  t.commit();
};
