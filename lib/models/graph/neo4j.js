...
Neo4j.prototype.getNodes = function (start, steps, cb) {
  ...
  steps.forEach(function (step) {
    ...
    if (s && s.node && s.node.props) {
      ...
      s.propsName = props;
      s.propsAsQuery = Object.keys(s.node.props).map(function (key) {
  ...
  steps.forEach(function (step) {
    if (keypather.get(step, 'In.node.props')) {
      ...
      params[step.In.propsName] = step.In.node.props;
    } else if (keypather.get(step, 'Out.node.props')) {
      where = where.concat(step.Out.propsAsQuery);
      params[step.Out.propsName] = step.Out.node.props;
    }
  });
  var q = [ query ];
