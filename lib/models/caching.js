...
(function() {
    ...
    return redis_client.get('sort_cache.block_set_dirty', function(err, value) {
      if (err) {
        ...
        if (value !== 'true') {
          ...
          return async.parallel([redis_client.setex.bind(redis_client, 'sort_cache.block_set_dirty', 3600, 'true'), redis_client.set.bind(redis_client, "sort_cache.dirty", 'true')], cb);
        }
      }
    });
  };
  ...
  getFilteredCachedResults = function(limit, index, channels, cb) {
    ...
    }, listFields, function(err, selected) {
  ...
  updateAllCaches = function(req, res) {
    return users.findUser(req.domain, {
      ...
      if (err) {
        return res.json(500, {
          message: 'error looking up user in mongodb'
        });
      } else {
        if (!user) {
          return res.json(500, {
          ...
          if (!user.isModerator) {
            ...
            return isCacheDirty(function(err, dirty) {
              if (err) {
                return res.json(500, {
                  message: 'error checking cache dirty flag'
                });
              } else {
                if (!dirty) {
                  return res.json({
                    message: 'cache is not dirty, skipping refresh'
                  });
                } else {
                  return markCacheAsClean(function(err) {
                    if (err) {
                      return res.json(500, {
                        message: 'error marking cache as clean'
                      });
                    } else {
                      return updateAllFilteredCachedResults(function(err) {
                        if (err) {
                          return res.json(500, {
                            message: 'error refreshing filtered redis cache'
                          });
                        } else {
                          return updateAllUnfilteredCachedResults(function(err) {
                            if (err) {
                              return res.json(500, {
                                message: 'error refreshing redis cache'
                              });
                            } else {
                              return res.json({
                                message: 'redis cache refreshed'
                              });
                            }
                          });
                        }
