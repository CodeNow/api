...
GitHub.prototype._updatePullRequestsComments = function (instance, cb) {
  ...
  async.each(repos, function (repo, callback) {
    self._updatePullRequestsCommentsForRepo(repo.lowerRepo, repo.lowerBranch, instance.owner, callback);
  }, cb);
};


GitHub.prototype._updatePullRequestsCommentsForRepo = function (shortRepo, branch, owner, cb) {
  ...
  async.parallel(tasks, function (err, results) {
    ...
    async.map(ids, function (number, callback) {
      var gitInfo = {
        owner: {
          login: owner.login || owner.username
        },
        number: number,
