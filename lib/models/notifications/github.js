...
GitHub.prototype.notifyOnPullRequest = function (gitInfo, instances, cb) {
  console.log('notifyOnGitHubPullRequest', formatArgs(arguments));
  cb = cb || noop;
...
GitHub.prototype._getCommit = function (githubId, repo, commitId, cb) {
  User.findByGithubId(githubId, function (err, user) {
    if (err || !user) { return cb(err); }
    var gh = new GitHub({token: user.accounts.github.accessToken});
    gh.getCommit(repo, commitId, cb);
  });
};

GitHub.prototype._updateComment = function (gitInfo, instances, callback) {
  var self = this;
  this.github.findCommentByUser(gitInfo.repoName, gitInfo.number,
    ...
    function (err, comment) {
      ...
      self.github.updateComment(gitInfo.repoName, comment.id, newMessage, callback);
    });
};
