...
Notifications.prototype.notifyOnBuild = function (commitLog, contextVersions, cb) {
  ...
  if (this.hipchat) {
    tasks.push(async.apply(this.hipchat.notifyOnBuild.bind(this.hipchat), commitLog, contextVersions));
  }
  if (tasks.length > 0) {
...
Notifications.prototype.notifyOnInstances = function (commitLog, contextVersions, instances, cb) {
  var tasks = [];
  if (this.slack) {
    tasks.push(async.apply(this.slack.notifyOnInstances.bind(this.slack), commitLog, contextVersions, instances));
  }
  if (this.hipchat) {
    tasks.push(async.apply(this.hipchat.notifyOnInstances.bind(this.hipchat), commitLog, contextVersions, instances));
  }
  if (tasks.length > 0) {
    async.parallel(tasks, cb);
  } else {
