...
function Notifications (settings, owner) {
  this.settings = settings;
  this.owner = owner;
  if (settings) {
    if (this.settings.slack) {
      this.slack = new Slack(this.settings.slack, owner);
    }
    if (this.settings.hipchat) {
      this.hipchat = new HipChat(this.settings.hipchat, owner);
    }
  }
}
...
Notifications.prototype.notifyOnNewBranch = function (githubPushInfo, cb) {
  ...
  if (slack) {
    tasks.push(slack.notifyOnNewBranch.bind(slack, githubPushInfo));
  }
  // if (hipchat) {
  //   tasks.push(hipchat.notifyOnNewBranch.bind(hipchat, githubPushInfo));
  // }
  if (tasks.length > 0) {
