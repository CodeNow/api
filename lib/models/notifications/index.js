...
Notifications.prototype.notifyOnInstance = function (commitLog, contextVersions, cb) {
  var tasks = [];
  if (this.slack) {
    tasks.push(async.apply(this.slack.notifyOnInstance.bind(this.slack), commitLog, contextVersions));
  }
  if (this.hipchat) {
    tasks.push(async.apply(this.hipchat.notifyOnInstance.bind(this.hipchat), commitLog, contextVersions));
  }
  if (tasks.length > 0) {
    async.parallel(tasks, cb);
  } else {
