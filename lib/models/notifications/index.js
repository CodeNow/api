...
Notifications.prototype.notifyOnGitHubPullRequest = function (githubPushInfo, instances, cb) {
  // our env parsing cannot parse boolean correctly atm
  ...
  if (github) {
    tasks.push(
      github.createGitHubPRInstancesLinks.bind(github, instances, githubPushInfo));
  }
  if (tasks.length > 0) {
    async.parallel(tasks, cb);
  } else {
