...
Notifier.prototype.sendDirectMessage = function (gitUser, message, cb) {
  ...
  this.findSlackUser(gitUser, function (err, slackUser) {
    ...
    self.slackClient.openDM(userId, function (err, resp) {
      if (err || !resp || !resp.channel) {
        return cb();
      }
      ...
      self.slackClient._apiCall('chat.postMessage', opts, cb);
    });
  });
};
...
Slack.prototype.findAllUsers = function (callback) {
  this.slackClient._apiCall('users.list', {}, function (err, resp) {
    if (err) {
      return callback(err);
    }
