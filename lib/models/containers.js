...
containerSchema.methods.tagWithChannel = function (channelId, cb) {
  var query = {
    _id: this._id,
  ...
  var self = this;
  Container.findOneAndUpdate(query, update).lean().exec(function (err, updatedContainer) {
    if (err) {
      return cb(err);
    }
    if (updatedContainer) {
      self.set(updatedContainer);
    }
    cb();
  });
};
  ...
  if (typeof content === 'string') {
    ...
    volumes.streamFile(domain,
      ...
      domain.intercept(function () {
        ...
        var ext = path.extname(name);
        if (cacheContents(ext)) {
          volumes.readFile(domain,
            self.servicesToken,
            self.file_root,
            name,
            filePath,
            domain.intercept(function (file_content) {
  ...
  function renameFile () {
    ...
    if (!oldCached && newCached) {
      volumes.readFile(domain,
        ...
        file.name,
        file.path,
        domain.intercept(function (content) {
          file.content = content;
          self.last_write = new Date();
          self.save(domain.intercept(function () {
            cb(null, file);
...
containerSchema.methods.createDirectory = function (domain, name, path, cb) {
  var self = this;
  volumes.createDirectory(domain,
    this.servicesToken,
    this.file_root,
    name,
    path,
...
containerSchema.methods.deleteFile = function (domain, fileId, recursive, cb) {
  ...
  if (!file) {
    ...
    volumes.removeDirectory(domain,
      this.servicesToken,
      this.file_root,
      file.name,
      file.path,
