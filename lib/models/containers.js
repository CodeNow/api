var ObjectId, Schema, async, cacheContents, configs, containerSchema, crypto, decodeId, encodeId, error, exts, implementations, minus, mongoose, path, plus, proxy, request, slash, sync, underscore, uuid, volumes, _, __indexOf = [].indexOf || function (item) {
...
containerSchema.statics.destroy = function (domain, id, cb) {
  var self = this;
  return this.findOne({ _id: id }, domain.intercept(function (container) {
    if (!container) {
      ...
      return request({
        ...
        return self.remove({ _id: id }, domain.intercept(function () {
          return cb();
        }));
      }));
    }
...
containerSchema.methods.syncFiles = function (domain, cb) {
  ...
  return sync(domain, this.servicesToken, this, function (err) {
    if (err) {
      ...
      return self.save(domain.intercept(function () {
        return cb(null, self);
      }));
    }
  });
  ...
  if (typeof content === 'string') {
    ...
    return volumes.streamFile(domain, this.servicesToken, this.file_root, name, filePath, content, function (err) {
      ...
      if (err) {
        ...
        ext = path.extname(name);
        if (cacheContents(ext)) {
          return volumes.readFile(domain, self.servicesToken, self.file_root, name, filePath, function (err, file_content) {
            if (err) {
              return cb(err);
            } else {
  ...
  if (!foundFile) {
    ...
    return volumes.streamFile(domain, this.servicesToken, this.file_root, foundFile.name, foundFile.path, content, function (err) {
      ...
      if (err) {
        ...
        if (cacheContents(ext)) {
          return volumes.readFile(domain, self.servicesToken, self.file_root, foundFile.name, foundFile.path, function (err, file_content) {
            if (err) {
              return cb(err);
            } else {
...
containerSchema.methods.renameFile = function (domain, fileId, newName, cb) {
  ...
  function renameFile () {
    var oldExt = path.extname(oldName);
    var newExt = path.extname(newName);
    var oldCached = cacheContents(oldExt);
    var newCached = cacheContents(newExt);
...
containerSchema.methods.deleteFile = function (domain, fileId, recursive, cb) {
  ...
  if (!file) {
    ...
    volumes.deleteFile(domain, this.servicesToken, this.file_root, file.name, file.path, domain.intercept(function () {
      file.remove();
      self.last_write = new Date();
      return self.save(domain.intercept(cb()));
    }));
  } else {
    volumes.removeDirectory(domain, this.servicesToken, this.file_root, file.name, file.path, recursive, domain.intercept(function () {
      if (recursive) {
        ...
        for (var _j = 0, _len1 = toDelete.length; _j < _len1; _j++) {
          elem = toDelete[_j];
          elem.remove();
        }
      }
