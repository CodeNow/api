...
ContainerSchema.methods.atomicUpdateCommitStatusAndName = function (update, cb) {
  // TODO: expand this to all statuses
  var container = this;
  var status = update.status;
  var name = update.name || container.name;
  async.waterfall([
    checkImageNameConflict,
    atomicStatusUpdate
  ], cb);
  function checkImageNameConflict (cb) {
    if (status === 'Committing new') {
      Image.findOne({ name: name }, { _id: 1 }, function (err, image) {
        if (err) {
          return cb(err);
        }
        if (image) {
  ...
  function atomicStatusUpdate (cb) {
    ...
    }, cb); // if update unsuccessful, callsback null
  }
};
