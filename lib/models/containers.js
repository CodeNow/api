...
var ObjectIdKeys = Object.keys(containerSchema.paths).filter(function (property) {
  var schema = containerSchema.paths[property];
  return schema.instance === 'ObjectID';
});
...
containerSchema.methods.encodeJSON = function (cb) {
  var json = this.toJSON();
  ['_id', 'parent', 'target'].forEach(function (key) {
    var val = json[key];
...
containerSchema.methods.getTags = function (cb) {
  ...
  async.map(tags, Channel.findById, cb);
};

containerSchema.statics.ObjectIdKeys = function () {
  return _.clone(ObjectIdKeys);
};
