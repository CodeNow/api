...
function handleDockerEvent (successHandler, errorHandler data) {
  ...
  mutex.lock(function (err, success) {
    if (err) {
      ...
      errorHandler(Boom.conflict('Event is currently being updated by another API host.'));
    }
    else {
...
function listen (errorHandler) {
  ...
  messenger.on('runnable:docker:die', handleContainerDieEvent.bind(errorHandler));
}
...
function cleanup () {
  // messenger.removeAllListeners('runnable:docker:docker_daemon_down', handleDockerDaemonDownEvent);
  // messenger.removeAllListeners('runnable:docker:docker_daemon_up', handleDockerDaemonUpEvent);
  messenger.removeAllListeners('runnable:docker:die');
}
