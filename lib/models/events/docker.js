    ...
    userStoppedContainer.lock(function (err, success) {
      ...
      if (stoppedByUser) {
        Instance.inspectAndUpdateByContainer(containerId, function (err) {
          logIfErr(err); // log error and continu
          eventMutex.unlock(logIfErr);
          userStoppedContainer.unlock(logIfErr);
        });
      }
      else {
        // user was responsible for the container die event.
        // we skipped `die` event once. Now remove flag.
        userStoppedContainer.unlock(function (err) {
          err = err || Boom.conflict('Event is being handled by another subsystem');
          unlockAndLogErr(eventMutex, err);
        });
      }
    });
  });
