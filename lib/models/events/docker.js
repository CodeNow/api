  ...
  activeApi.isMe(function (err, meIsActiveApi) {
    ...
    self.getEventLock(data.uuid, function (err, eventMutex) {
      ...
      if (isImageBuilder(data)) {
        return handleImageComplete(data, function(err) {
          logIfErr(err);
...
function handleImageComplete (data, cb) {
  // find context by id
  // retry if no started
  var name = data.inspectData.name.replace('/', '');
  var retry = process.env.IMAGE_RETRY_COUNT;

  dogstatsd.increment('api.events.docker.die.builder', ['id:'+name]);
  handle();
  function handle () {
    ContextVersion.findByBuildId(name, function(err, cv) {
      ...
      if (cv && cv.length === 0) {
        if (retry > 0) {
          return cb(new Error('can not find context version with buildId'));
        } else {
          dogstatsd.increment('api.events.docker.die.builder.retry', ['id:'+name]);
      ...
      docker.getBuildInfo(name, function(err, buildData) {
        if (err) { return  cb(err); }
        if (buildData.buildFailed) {
          debug('set build failed', buildData);
          err = Boom.badRequest('Building dockerfile failed with errorcode: '+
            data.inspectData.errorcode || 1, {
              docker: {
                containerId: data.inspectData.Id,
                ContextVersionId: cv._id,
                log: buildData.buildLog
              }
          });
          cv.setBuildError(err, cb);
        } else {
          debug('set build complete success', buildData);
          cv.setBuildCompleted(buildData, cb);
        }
      });
      // push to reg
      var split = cv.build.dockerTag.split(':');
