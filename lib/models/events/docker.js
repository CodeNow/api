...
function handleContainerDie (data, cb) {
  ...
  flags.get(containerId + '-die-flag', function (err, reply) {
    ...
    // don't do anything
    if (reply.toString() === 'ignore') {
      cb(null);
      // we skipped `die` event once. Now remove skip flag.
      flags.del(containerId + '-die-flag', function (err, reply) {
        if (err) {
          error.log('failed to remove die flag lock flag for container', err);
        }
        else {
          debug('remove die flag lock flag for container', containerId);
        }
      });
    } else {
