...
function handleContainerDie (data, cb) {
  ...
  Instance.findByContainerId(containerId, function (err, instance) {
    ...
    applyInstanceAction('stop', instance, cb);
  });
}
...
function applyInstanceAction (actionFn, instance, cb) {
  ...
  User.findByGithubId(instance.owner.github, function (err, user) {
    ...
    runnable.newInstance(instance.shortHash)[actionFn]({force: true}, function (err, resp) {
      if (err) {
        error.log(err);
        return cb(err);
      }
      console.log('we just called ' + actionFn + ' instance method', err, resp.container);
      cb(null, resp);
    });
  });
}
