...
DockerEvents.prototype.listen = function (cb) {
  ...
  console.log('should be here, punk');
  this.subscribeAll();
  cb();
};

DockerEvents.prototype.close = function (cb) {
  debug('close', arguments, this.eventLockCount, new Error().stack);
  cb = cb || noop;
  if (this.closeHandler) {
...
DockerEvents.prototype.once = function (name, handler) {
  name = process.env.DOCKER_EVENTS_NAMESPACE + name;
  messenger.once(name, handler);
};

DockerEvents.prototype.off = DockerEvents.prototype.removeListener = function (name, handler) {
  name = process.env.DOCKER_EVENTS_NAMESPACE + name;
  messenger.removeListener(name, handler);
};
