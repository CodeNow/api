...
var log = require('logger').child({ module: 'events:docker' }, true);
...
function handleImageBuilderDie (data, cb) {
  ...
  function updateBuildError (containerId, buildInfo, cb) {
    ...
    errorMessage += ' - ' + keypather.get(Labels, 'sessionUser.accounts.github.displayName');
    errorMessage += ' - [' + keypather.get(Labels, 'sessionUser.accounts.github.username') + ']';
