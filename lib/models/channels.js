...
channelSchema.statics.mostPopAffectedByUser = function (domain, size, userId, callback) {
  var self = this;
  images.distinct('tags.channel', {
    owner: userId
  }, domain.intercept(function (channelIds) {
    highestImageCount(domain, size, channelIds,
      domain.intercept(function (popularChannelsData) {
        async.map(popularChannelsData, function (channelData, cb) {
          self.extendWithNameAndCount(domain).call(self, channelData,
            domain.intercept(function (channel) {
              images.countInChannelByOwner(domain, channel._id, userId,
                domain.intercept(function (userImagesCount) {
...
channelSchema.statics.isLeader = function (domain, userId, channelId, cb) {
  images.distinct('owner', { 'tags.channel': channelId }, domain.intercept(function (ownerIds) {
    async.reduce(ownerIds, [], function (leaders, ownerId, cb) {
      images.countInChannelByOwner(domain, channelId, ownerId, highestCountItems(3, leaders, {
        _id: ownerId
      }, cb));
    }, domain.intercept(function (leaders) {
      var data = null;
...
channelSchema.statics.leaderBadgesInChannelsForUser =
  function (domain, size, filterChannelIds, userId, callback) {
  var self = this;
  async.reduce(filterChannelIds, [], function (badges, channelId, cb) {
    self.isLeader(domain, userId, channelId, domain.intercept(function (leaderData) {
      if (leaderData) {
...
channelSchema.statics.listChannelsInCategory = function (domain, categories, categoryName, cb) {
  ...
  }, domain.intercept(function (category) {
    if (!category) {
      cb(error(404, 'could not find category'));
    } else {
      redis_client.get('listChannelsInCategory:' + category._id,
        domain.intercept(function (listChannelsInCategoryCache) {
          redis_client.get('listChannelsInCategoryValid:' + category._id,
            domain.intercept(function (valid) {
              if (valid && listChannelsInCategoryCache) {
                ...
                self.find({
                  ...
                  async.map(channels, function (channel, cb) {
                    ...
                    redis_client.set('listChannelsInCategory:' + category._id,
...
channelSchema.statics.relatedChannels = function (domain, channelNames, cb) {
  ...
  this.find({ aliases: { $in: lowerNames } }, domain.intercept(function (channels) {
        ...
        if (!channel) {
          ...
          categories.findOne({ aliases: text }, domain.intercept(function (category) {
            if (category) {
              ...
              categories.createImplicitCategory(domain, text, domain.intercept(function (category) {
                channel.tags.push({ category: category._id });
...
var highestCountItems = function (size, memo, doc, cb) {
  return function (err, count) {
    if (err) {
      ...
      var inserted = memo.some(function (memoItem, i) {
