...
(function() {
  ...
  channelSchema.statics.mostPopAffectedByUser = function(domain, size, userId, callback) {
    var self;
    self = this;
    return images.distinct('tags.channel', {
      owner: userId
  ...
  channelSchema.statics.isLeader = function(domain, userId, channelId, cb) {
    return images.distinct('owner', {
      'tags.channel': channelId
    }, domain.intercept(function(ownerIds) {
      return async.reduce(ownerIds, [], function(leaders, ownerId, cb) {
        return images.countInChannelByOwner(domain, channelId, ownerId, highestCountItems(3, leaders, {
          _id: ownerId
        }, cb));
  ...
  channelSchema.statics.relatedChannels = function(domain, channelNames, cb) {
    ...
    }, domain.bind(function(err, channels) {
      var channelIds;
      if (err) {
        throw err;
      } else {
  ...
  highestImageCount = function(domain, size, channelIds, callback) {
    return async.reduce(channelIds, [], function(popularChannelsData, channelId, cb) {
      return images.count({
        ...
        return highestCountItems(size, popularChannelsData, {
          _id: channelId
        }, cb)(null, count);
      }));
    }, callback);
