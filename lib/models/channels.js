...
channelSchema.methods.getTags = function (cb) {
  ...
  var categoryIds = this.tags.map(utils.pick('_id'));
  async.map(categoryIds, Category.findBy, cb);
};
...
channelSchema.statics.findByIdAndAddAlias = function (id, alias) {
  alias = alias.toLowerCase();
  var args = Array.prototype.slice.call(arguments, 2); // slice off id, alias
  var query = {
    _id: id,
    aliases: { $ne: alias }
  };
  ...
  args.unshift(query, update);
  this.findOneAndUpdate.apply(this, args);
};
    ...
    highestImageCount(domain, size, channelIds, domain.intercept(function (popularChannelsData) {
      async.map(popularChannelsData, function (channelData, cb) {
        self.extendWithNameAndCount(domain).call(self, channelData, domain.intercept(function (channel) {
          images.countInChannelByOwner(domain, channel._id, userId, domain.intercept(function (userImagesCount) {
