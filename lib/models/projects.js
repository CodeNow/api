...
ProjectSchema.statics.createAndSaveFromDockerfile = function (content, props, cb) {
  ...
  Context.createAndSaveFromDockerfile(content, function (err, context) {
    if (err) { return cb(err); }

    defaultEnv.contexts.push({
      context: context._id,
      version: last(context.versions)._id // context only has just created version
    });
...
ProjectSchema.methods.createAndSaveEnvFromDefault = function (newEnv, cb) {
  var project = this;
  var source = project.getDefaultEnv();
...
ProjectSchema.methods.createAndSaveEnvFromEnvId = function (newEnv, sourceId, cb) {
  cb = cb || noop;
  var project = this;
  var source = project.getEnvWithId(sourceId);
  if (!source) {
...
ProjectSchema.methods.validateEnv = function (env, cb) {
  ...
  else if (project.getEnvWithName(env.name)) {
...
ProjectSchema.methods.getDefaultEnv = function () {
  return this.getEnvWithId(this.defaultEnvironment);
};
...
ProjectSchema.methods.getEnvWithId = function (envId) {
...
ProjectSchema.methods.getEnvWithName = function (name) {
  return this.environments &&
