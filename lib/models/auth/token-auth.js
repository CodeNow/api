...
var debug = require('debug')('runnable-api:token-auth:model');
...
function isRequested (session) {
  var requested = keypather.get(session, 'requiresToken');
  debug('isRequested: requested', !!requested);
  return !!requested;
}
/**
...
TokenAuth.createWithSessionCookie = function (session, cookie, cb) {
  debug('createWithSessionCookie: session', session, 'cookie', cookie);
