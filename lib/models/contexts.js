...
ContextSchema.methods.listFiles = function (versionId, prefix, cb) {
...
ContextSchema.methods.getFile = function (versionId, key, cb) {
  ...
  if (fileIndex === -1) {
    return cb(Boom.badRequest('invalid resource key'));
  }
...
ContextSchema.methods.findVersion = function (versionId, cb) {
  ...
  if (versionId === 'latest') {
    ...
    var i = findIndex(this.versions, function (v) { return v._id.toString() === versionId.toString(); });
    if (i === -1) {
