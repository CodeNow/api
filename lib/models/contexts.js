...
var debug = require('debug')('context:model');
...
ContextSchema.methods.uploadResource = function (s3Url, content, callback) {
  ...
  s3.putObject(data, function (err, res) {
    debug('uploaded resource ' + s3Url.pathname);
    callback(err, res);
  });
};
...
ContextSchema.methods.copy = function (callback) {
  ...
  newContext.set(_.pick(this, copyAttributes));
  ...
  if (this.dockerfile) {
    ...
    var origDockerfile = url.parse(this.getDockerfileUrl());
    var destDockerfile = url.parse(newContext.dockerfile);
    tasks.dockerfile = s3.copyObject.bind(s3, {
      Bucket: destDockerfile.hostname,
      CopySource: join(destDockerfile.hostname, origDockerfile.pathname.slice(1)),
      Key: destDockerfile.pathname.slice(1)
    });
  }
  ...
  if (this.source.length && this.source[0].sourceType === 'local') {
    ...
    async.waterfall([
      ...
      function (resources, callback) {
        _.forEach(resources.Contents, function (file) {
          debug('adding file to copy: ' + file.Key);
          if (file.Key[0] === '/') {
            file.Key = file.Key.substring(1);
          }
          var resource = file.Key.split('/').slice(2).join('/');
          if (_.last(file.Key) === '/') {
            resource += '/';
          }
          var newFileUrl = url.parse(newContext.getResourceUrl(resource));
          tasks[newFileUrl.pathname.slice(1)] = s3.copyObject.bind(s3, {
            Bucket: newFileUrl.hostname,
            CopySource: join(newFileUrl.hostname, file.Key),
            Key: newFileUrl.pathname.slice(1)
          });
          callback();
        });
      }
