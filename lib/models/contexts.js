...
var ContextSchema = new Schema({
   ...
   *  @property {string} versions[].tag Tag of the Docker image (name:tag)
  ...
  versions: {
    ...
    // FIXME: this shouldn't default. we should add the first one on a successful build
    'default': [{
      tag: 'v0',
      created: Date.now
    }]
...
/** Create the source directory in S3 for the context and saves the URL in the context
  * @param {function} callback Receives (err, {@link module:Context})
  */
