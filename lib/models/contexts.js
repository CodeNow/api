...
var hasKeypath = require('101/has-keypaths');
...
ContextSchema.methods.listFiles = function (versionId, prefix) {
  // assumed that we have validated that the versionId is valid already
  // otherwise, we just return an empty list
...
ContextSchema.methods.findVersion = function (versionId, cb) {
  ...
  if (versionId === 'latest') {
    ...
    var i = findIndex(this.versions, hasKeypath({ '_id.toString()': versionId.toString() }));
    if (i === -1) {
