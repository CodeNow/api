...
userSchema.methods.register = function (domain, cb) {
  var self = this;
  async.waterfall([
    findConflicts,
    checkConflicts,
  ...
  function checkConflicts (user, cb) {
    if (user) {
      var collision = self.email === user.email ? 'email' : 'username';
  ...
  function hashPassword (cb) {
    bcrypt.hash(self.password + configs.passwordSalt, 10, cb);
  }
  function save (password, cb) {
    self.permission_level = 1;
    self.lower_username = self.username.toLowerCase();
