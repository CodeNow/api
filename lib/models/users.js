  ...
  setPassword = function (password) {
    return self.findOne({
      ...
      if (user) {
        ...
        return self.findByIdAndUpdate(userId, cmd, domain.intercept(function (user) {
          return cb(null, user);
        }));
      }
    }));
...
userSchema.methods.addVote = function (domain, runnableId, cb) {
  var found, vote, _i, _len, _ref, self = this;
  found = false;
  _ref = this.votes;
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    vote = _ref[_i];
  ...
  if (found) {
    ...
    return this.save(domain.intercept(function () {
      vote = self.votes[self.votes.length - 1].toJSON();
