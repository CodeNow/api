...
var volumes = {
  ...
  readAllFiles: function (container, ignores, cb) {
    var subDomain = container.servicesToken;
    var srcDir = container.file_root;
    console.log('making request', subDomain);
    request({
      pool: false,
      ...
      console.log('container_id for TJ', container._id, err, res && res.statusCode, res && res.body);
      console.log(subDomain, configs.domain);
      if (err) {
      	console.log('err', err);
        cb(err);
      } else if (res.statusCode === 502) {
