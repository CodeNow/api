...
InfraCodeVersionSchema.statics.createCopyById = function (infraCodeVersionId, environmentId, cb) {
  ...
  InfraCodeVersion.findById(infraCodeVersionId, function (err, source) {
    ...
    infraCodeVersion.environment = environmentId;
    // Since source files won't have an environment set, don't set it if it doesn't exist
    if (source.environment && source.environment !== environmentId) {
      infraCodeVersion.parentEnvironment = source.environment;
    }
