...
InfraCodeVersionSchema.methods.createFs = function (data, cb) {
  ...
  async.waterfall([
    ...
    debug('createFS async.waterfall final', formatArgs(arguments));
    cb();
  });
  var fullpath = join(data.path, data.name);
  function checkParentDirExists (cb) {
    debug('checkParentDirExists');
  ...
  /**
   * Must bind 'data' events for other listeners before
   * S3 upload, S3 upload will close stream making data events
   * unavailable. Therefore we compute hash of file simultaniously w/
   * S3 upload operation
   */
  function createInBucketAndHash (cb) {
    debug('createInBucket');
    if (last(fullpath) === '/' || data.isDir) {
      ...
      async.parallel([
        ...
        if (err) {
          debug('createInBucketAndHash error', formatArgs(arguments));
        }
        cb(err, results[0], results[1]);
      });
    }
  }
  function updateMongo (hash, s3Data, cb) {
    debug('updateMongo');
    ...
    if (last(fileData.Key) === '/') {
      debug('updateMongo - directory');
      fileKey = fileData.Key.slice(0, -1);
      dirKey = fileData.Key;
      update();
    }
    else {
      debug('updateMongo - file');
...
/*
      ...
      hasher(
        data.body,
        // only trim whitespace if filename "Dockerfile"
        !/^Dockerfile$/i.test(path.basename(fileKey)),
        function (err, hash) {
          debug('hasher cb', err, hash);
          if (err) { return cb(err); }
          fileData.hash = hash;
          update();
        });
