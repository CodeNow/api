...
InfraCodeVersionSchema.methods.createFs = function (data, cb) {
  ...
  function updateMongo (s3Data, cb) {
    ...
    if (last(fileData.Key) === '/') {
      ...
      fileData.hash = hashString(data.body);
    }
    // atomic update
...
InfraCodeVersionSchema.methods.getHash = function (cb) {
  InfraCodeVersion.findOne({
    ...
    cb(null, hashString(jsonHash(hashMap)));
  });
};
...
function hashString(data) {
  return crypto.createHash('md5').update(
    data
      ...
      .replace(/^[ ]+/g, '')        // trim whitespace before line
      .replace(/[ ]+\n$/g, '\n')    // trim whitespace after line
      .toString().trim()).digest('hex');
}
