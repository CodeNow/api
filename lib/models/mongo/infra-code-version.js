...
InfraCodeVersion.methods.listFiles = function (prefix) {
  ...
  files.forEach(function (file) {
    ...
    var validPrefixIndex = findIndex(prefix,
      function (p) { return file.Key.indexOf(p, startIndex) !== -1; });
    if (validPrefixIndex === -1) { return; }
    var prefixIndex = file.Key.indexOf(prefix[validPrefixIndex], startIndex);
    if (prefixIndex === -1) { return; }
    var endOfPrefixIndex = prefixIndex + prefix[validPrefixIndex].length;
