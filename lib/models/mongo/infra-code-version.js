...
InfraCodeVersionSchema.statics.createCopyById = function (infraCodeVersionId, newContextId, cb) {
  ...
  InfraCodeVersion.findById(infraCodeVersionId, function (err, source) {
    ...
    if (newContextId) {
      // If we are moving this infracode to a new context, we need
      // to keep it from deduping
      infraCodeVersion.context = newContextId;
      infraCodeVersion.edited = true;
    } else {
