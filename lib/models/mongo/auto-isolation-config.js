 ...
 *
 * @param {ObjectId} instanceId            - Id of the parent instance
 * @param {Object[]} model - New references to the instances that are part of this config
 * @returns {Promise}
 * @resolves {AutoIsolationConfig} - newly updated model
 */
AutoIsolationSchema.statics.updateAutoIsolationDependencies = function (instanceId, model) {
  const log = logger.child({
  ...
  return AutoIsolationConfig.findOneAndUpdateAsync(query, model, { upsert: true, new: true })
}
