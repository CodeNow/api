 ...
 * @param {ObjectId} aigId - id of the AutoIsolationConfig
 * @resolves  if auto-isolation-config was marked as deleted
 */
AutoIsolationSchema.statics.markAsDeleted = function (aigId) {
  const log = logger.child({
    method: 'markAsDeleted',
    aigId
  })
  log.info('called')
  return AutoIsolationSchema.findOneAndUpdateAsync({
    _id: objectId(aigId),
    deleted: {
      $exists: false
    }
 ...
 */
AutoIsolationSchema.statics.findByIdAndAssert = function (aigId) {
  const log = logger.child({
    method: 'findByIdAndAssert',
    aigId
  })
  log.info('called')
  return AutoIsolationConfig.findOneActive({ _id: aigId })
}
