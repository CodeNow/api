...
UserSchema.methods.findGithubUserByGithubId = function (githubId, cb) {
  ...
  var accessToken = this.accounts.github.accessToken
  Users.findOneAsync({ 'accounts.github.id': githubId })
    .catch(function (err) {
      log.warn(put({
        err: err
      }, logData), 'findGithubUserByGithubId User findOneByIdAsync failed')
    })
    .then(function (user) {
    ...
    .then(function (user) {
      if (!user) {
        throw Boom.badRequest('The github user could not be found with id ' + githubId)
      }
      return user
    })
    .asCallback(cb)
