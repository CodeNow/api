...
/**
 * Fetches a user's info by their github username.  This uses the app's key to make authenticated
 * requests
 * @param githubId {String}   githubId of the User to find
 * @param cb       {Function} callback
 */
UserSchema.statics.anonymousFindGithubUserByGithubId = function (githubId, cb) {
  var logData = {
    tx: true,
    githubId: githubId
  }
  log.info(logData, 'findGithubUserByGithubId')
  Users.findOneAsync({ 'accounts.github.id': githubId })
    .catch(function (err) {
      log.warn(put({
        err: err
      }, logData), 'anonymousFindGithubUserByGithubId User findOneByIdAsync failed')
    })
    .then(function (user) {
      if (keypather.get(user, 'accounts.github')) {
        return user.accounts.github._json || user.accounts.github
      } else {
    ...
    .then(function (user) {
      if (!user) {
        throw new Error('The github user could not be found with id ' + githubId)
      }
      return user
    })
    .asCallback(cb)
...
/**
 * Find all members in a Github organization who are also Runnable users
 *
...
UserSchema.methods.findGithubOrgMembersByOrgName = function (githubOrgName, cb) {
  ...
  github.getOrgMembers(githubOrgName, function (err, members) {
    ...
    var memberIds = members.map(pluck('id'))
    Users.publicFindBy('accounts.github.id', { $in: memberIds }, cb)
  })
}

/**
 * Find all users with a Github username or who are part of a Github organization.
 * Only a `githubUsername` or `githubOrgName` parameter can be passed.
 *
 * @param {Object}   opts
 * @param {Function} cb {Error, Array}
 */
