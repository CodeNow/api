...
UserSchema.methods.findUserByGithubOrgNameorUsername = function (opts, cb) {
  if (!isObject(opts)) {
    cb(new Error('Query argument must be an object'), null)
  } else if (opts.githubUsername && opts.githubOrgName) {
    cb(new Error('Query argument must only contain either a githubUsername or a githubOrgName'), null)
    ...
    cb(new Error('Not enough parameters provided'))
  }
}
