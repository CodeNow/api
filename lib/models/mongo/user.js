 ...
 * @param {Object}   opts
 */
UserSchema.methods.findUsersByGithubOrgNameOrUsername = function (opts) {
  return Promise
    .try(function () {
      if (!isObject(opts)) {
        throw Boom.badRequest('Query argument must be an object', opts)
      } else if (opts.githubUsername && opts.githubOrgName) {
        throw Boom.badRequest(
          'Query object must contain only one of the following properties: ' +
          '`githubUsername`, `githubOrgName`', opts)
      } else if (!opts.githubUsername && !opts.githubOrgName) {
        throw Boom.badRequest('Not enough parameters provided', opts)
      }
    })
    .then(function () {
      if (opts.githubOrgName) {
        return OrganizationService.getMembersByOrgName(opts.githubOrgName)
      } else {
        return Users.findByGithubUsernameAsync(opts.githubUsername)
      }
    })
}
