...
UserSchema.statics.addOrUpdateSlackAccount = function (userId, account, cb) {
  var self = this;
  this.update({
    _id: userId,
    'accounts.slack.orgs.githubId': {$ne: account.githubId}},
    {$push: {'accounts.slack.orgs': account}}, function (err, rowsUpdated) {
      if (err) { return cb(err); }
      if (rowsUpdated === 0) {
        self.update({
          _id: userId,
          'accounts.slack.orgs.githubId': account.githubId},
          {$set: {'accounts.slack.orgs.$': account}}, cb);
      }
      else {
        cb(null, rowsUpdated);
      }
...
UserSchema.statics.findSlackAccount= function (githubUsername, githubOrgOrUserId, cb) {
  this.publicFindOneByGithubUsername(githubUsername, function (err, user) {
    if (err || !user) { return cb(err); }
    var slackAccounts = keypath.get(user, 'accounts.slack.orgs');
    if (slackAccounts) {
      var account = find(slackAccounts, hasProps({ githubId: githubOrgOrUserId }));
      if (account) {
