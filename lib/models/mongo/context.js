...
ContextSchema.methods.isPublic = function (cb) {
  log.trace({ isPublic: this.isPublic }, 'isPublic')
  if (!this.public) {
    return cb(Boom.forbidden('Context is private'))
  }
  cb(undefined, this)
}

ContextSchema.statics.findByVersions = function (contextVersions, cb) {
  log.trace({ contextVersions }, 'findByVersions')
  const contextIds = contextVersions.map(pluck('context'))
  this.findByIds(contextIds, cb)
}

const Context = module.exports = mongoose.model('Contexts', ContextSchema)
