  ...
  async.parallel({
    currentDependencyDiff: function getCurrentDependencyDiff (cb) {
      self.getDependencies(function (err, dependencies) {
        if (err) { return cb(err); }
        var lowerNames = dependencies.map(pluck('lowerName'));
        var toRemove = dependencies.slice(); // clone
        var diff = {
          toAdd: []
        };
        var re =
        ...
        self.env.forEach(function (e) {
          ...
          if (match && match[1]) {
            var index = lowerNames.indexOf(match[1]);
            if (index === -1) {
    ...
    var tasks = [];
    results.currentDependencyDiff.toAdd.forEach(function (lowerName) {
      var i = find(
        results.getInstanceNames,
        hasKeypaths({ 'lowerName': lowerName }));
      if (i) {
        // oh my god, teejus
        var hn = runnableHostname.elastic({
    ...
    results.currentDependencyDiff.toRemove.forEach(function (dep) {
      tasks.push(self.removeDependency.bind(self, dep));
    });
    if (tasks.length) {
