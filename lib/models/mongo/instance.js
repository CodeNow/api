...
InstanceSchema.methods.removeSelfFromGraph = function (cb) {
  ...
  self.getParentAsync()
    .then(function (masterInstance) {
      return Instance.findAndUpdateAsync({
        dependencies: {
          $elemMatch: {
            instanceId: self._id
          }
    ...
    .tap(function (numUpdated) {
      if (!numUpdated) {
        // the update failed
        throw new Error('No deps updated!')
      }
    })
    .finally(function () {
  ...
  Promise.props({
    ...
    .then(function (results) {
      ...
      function toAddTask (dep) {
        return self.addDependencyAsync(dep)
      }
      function toRemTask (dep) {
        return self.removeDependencyAsync(dep.instanceId)
      }
...
function cleanDep (dep) {
  return {
    elasticHostname: dep.elasticHostname,
...
function getDepFromInstance (instance, elasticHostname) {
  if (keypather.get(instance, 'dependencies.length')) {
    ...
    return deps.length ? cleanDep(deps[0]) : null
  }
}
 ...
 * @param {string} elasticHostname - Elastic hostname associated with this instance
 * @param {function} cb callback
 */
InstanceSchema.methods.addDependency = function (instance, elasticHostname, cb) {
  if (isFunction(elasticHostname)) {
    cb = elasticHostname
  }
  elasticHostname = instance.elasticHostname.toLowerCase()
  var log = logger.log.child({
    instanceId: keypather.get(this, '_id'),
    instanceName: keypather.get(this, 'name'),
 ...
 * @param {function} cb callback
 */
InstanceSchema.methods.removeDependency = function (instanceId, cb) {
  var self = this
  return Instance.findOneAndUpdateAsync({
    _id: this._id
  }, {
