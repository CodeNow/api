...
InstanceSchema.statics.findInstanceIdByShortHash = function (shortHash) {
  return Instance.findInstanceByShortHash(shortHash)
    .get('_id')
}

/**
 * Given an array of instances, fetch all of the parent instances
 *
 * @param {Instance[]} instances
 *
 * @resolves {Instance[]} parent instances
 * @throws {Instance.NotFoundError} If instance wasn't found
 */
...
/**
 * Takes a query to fetch the instance, and assert if it's not found
 * @param {Object} query - query to fetch instance by
 *
 * @returns {Instance} requested Instance
 * @throws {Instance.NotFoundError} If instance wasn't found
 * @private
 */
InstanceSchema.statics._findOneAndAssert = function (query) {
 ...
 *
 * @resolves {Instance} instance
 * @throws {Instance.NotFoundError} If instance wasn't found
 */
InstanceSchema.statics.fetchMasterPodInstanceOfChild = function (childInstance) {
  const log = logger.log.child({
    method: 'InstanceSchema.statics.fetchMasterPodInstanceOfChild'
  })
  const query = {
    'contextVersion.context': childInstance.contextVersion.context,
    masterPod: true
  }
  log.info({query}, 'called')
  return Instance._findOneAndAssert(query)
}

InstanceSchema.statics.findInstanceByShortHash = function (shortHash) {
  const log = logger.log.child({
    shortHash,
    method: 'InstanceSchema.statics.findInstanceByShortHash'
  })
  log.info('called')
 ...
 */
InstanceSchema.statics.findInstanceById = function (instanceId) {
  const log = logger.log.child({
    instanceId,
    method: 'InstanceSchema.statics.findInstanceById'
  })
  log.info('called')
  return Instance.findByIdAsync(instanceId)
    .tap(function (instance) {
      if (!instance) {
        throw new Instance.NotFoundError({ instanceId })
      }
    })
}

/**
 * Find instances by Cluster Hash generated by create route
 * @param {String} githubId - id github
 * @param {String} clusterName - name of parent cluster
 * @param {String} clusterUuid - UUID of parent cluster
 * @returns {Promise}
 * @resolves {Instances} instance mongo model
 */
InstanceSchema.statics.findInstancesByClusterUUID = (githubId, clusterName, clusterUuid) => {
  let query = {
    '$match': {
      'owner.github': githubId,
      'name': { $regex: clusterName, $options: 'g' },
      'clusterCreateJobUuid': clusterUuid
    }
  }
...
/**
 * Find child of an instance using the instanceId and the isolationId from
 * @param {Instance} parentInstance - master instance of the child we're looking for
 * @param {ObjectId} isolationId    - Id of the isolation to look into
 * @resolves {Instance} instance mongo model
 * @throws   {Instance.NotFoundError} When instance lookup failed
 * @throws   {Error}                  When Mongo fails
 */
InstanceSchema.statics.findIsolatedChildOfParentInstance = function (parentInstance, isolationId) {
  const log = logger.log.child({
    method: 'InstanceService.findIsolatedChildOfParentInstance',
    parentInstance, isolationId
  })
  log.info('called')
  const query = {
    'contextVersion.context': parentInstance.contextVersion.context,
    isolated: isolationId
  }
  return Instance._findOneAndAssert(query)
}
...
/**
 * Fetch all instances from an isolation
 *
 * @param    {ObjectId}   isolationId - Id of the isolation to look into
 *
 * @resolves {Instance[]} instance mongo model
 ...
 * @throws   {Error}                  When Mongo fails
 */
InstanceSchema.statics.fetchIsolatedInstances = function (isolationId) {
  const log = logger.log.child({
    method: 'fetchIsolatedInstances',
    isolationId
  })
  log.info('called')
  const query = {
...
/**
 * Fetches the MasterPod parent of an instance by an instanceId.  This uses the contextId
 * to match with the masterPod.
 *
 * @resolves {Instance} MasterPod Instance that is the parent of the givent instanceId
 * @throws  {Instance.NotFoundError} When instance lookup failed
 */
InstanceSchema.statics.findParentByChildId = function (instanceId) {
  const log = logger.log.child({
    instanceId,
    method: 'InstanceSchema.statics.findParentByChildId'
  })
  log.info('called')
  return Instance.findInstanceById(instanceId)
    .then(instance => Instance.fetchMasterPodInstanceOfChild(instance))
}
