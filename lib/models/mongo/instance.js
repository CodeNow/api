...
InstanceSchema.methods.updateInspectedContainerData = function (cb) {
  ...
  this.inspectContainers(function (err, containers) {
    ...
    async.forEach(containers, function (container, cb) {
      var i = findIndex(Instance.containers, hasProperties({dockerContainer: container.Id}));
      if (i === -1) {
        // let's try just not updating anything
        cb();
      } else {
        Instance.update({
          'containers._id': Instance.containers[i]._id
        }, {
          $set: {
            'containers.$.ports': container.NetworkSettings.Ports
          }
        });
      }
    }, cb);
