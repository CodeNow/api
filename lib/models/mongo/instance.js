...
InstanceSchema.statics.findContextVersionsForRepo = function (lowerRepo, cb) {
  // NOTE: why do we have `match` twice? what first `match` is doing?
  Instance.aggregate([
    {
      $match: {
...
InstanceSchema.statics.findInstancesLinkedToBranch = function (lowerRepo, lowerBranch, cb) {
  // TODO add here check that instance.linked = true
  var query = {
    'contextVersion.appCodeVersions.lowerRepo': lowerRepo,
    'contextVersion.appCodeVersions.lowerBranch': lowerBranch
  };
 ...
 * @param {Function} cb  callback(err, instance)
 */
InstanceSchema.methods.modifySetContainerInspectErr = function (err, cb) {
  Instance.findByIdAndUpdate(this._id, {
    $set: {
