...
/**
 * write a node for a hostname into the graph
 * @param {string} hostname
 * @param {function} callback
 */
InstanceSchema.methods.upsertHostIntoGraph = function (hostname, cb) {
  var client = new Graph();
  var node = {
    label: 'Hostname',
    props: {
...
InstanceSchema.methods.addDependency = function (instance, hostname, cb) {
  ...
  var connections = [
    [start, 'dependsOn', end],
    [end, 'hasHostname', hostnameNode]
  ];
  async.series([
    client.graph.writeNode.bind(client.graph, hostnameNode),
    client.graph.writeConnections.bind(client.graph, connections)
  ], function (err) {
    if (err) { return cb(err); }
    var i = pick(instance.toJSON(), ['id', 'lowerName', 'owner', 'contextVersion']);
...
InstanceSchema.methods.getDependencies = function (params, cb) {
  ...
  client.graph.getNodes(start, steps, function (err, nodes, allNodes) {
    if (params.hostname) { nodes = allNodes.b; }
