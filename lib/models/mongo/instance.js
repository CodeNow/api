...
function findMatchingInstanceByHostname(val, instanceList) {
  ...
  Promise.props({
    ...
    .then(function (results) {
      ...
      var envDeps = linkedHostnames.map(function findAllMatchingInstancesByHostname(val) {
        ...
        // if no isolated instances were found, check the current deps
        if (!instance) {
          instance = findMatchingInstanceByHostname(val, deps)
        }
        if (!instance) {
