...
function NotFoundError (query) {
  BaseError.call(this, 'Instance not found', {
    query: query
  }, {
    level: 'debug'
  })
...
InstanceSchema.statics.NotFoundError = NotFoundError
 ...
 * @param {Object} reporting      reporting options
 */
function IncorrectStateError (expectedStatus, instance) {
  const state = keypather.get(instance, 'container.inspect.State')
  const id = keypather.get(instance, 'id')
  const status = keypather.get(state, 'Status')
  BaseError.call(this, 'Instance not in correct state', {
    actualStatus: status,
...
InstanceSchema.statics.IncorrectStateError = IncorrectStateError
...
InstanceSchema.statics.assertNotStartingOrStopping = function (instance) {
  var State = keypather.get(instance, 'container.inspect.State')
  var log = logger.log.child({
    State: State,
 ...
 * @throws {NotFoundError} If instance with containerId not found
 * @throws {IncorrectStateError} If instance not in starting state
 */
InstanceSchema.statics.findOneStarting = function (instanceId, containerId) {
  ...
  return Instance.findOneAsync(query)
    .tap(function checkInstanceState (instance) {
      ...
      if (status !== 'starting') {
        throw new IncorrectStateError('starting', instance)
      }
    })
}
...
InstanceSchema.statics.markAsCreating = function (instanceId, contextVersionId, containerId, containerInfo) {
  ...
  return Instance.findOneAndUpdateAsync(query, update)
    .tap(function checkInstanceState (instance) {
      if (!instance) {
        log.error({ query: query }, 'failed to find instance with context version and not container')
        throw new NotFoundError(query)
      }
    })
}
