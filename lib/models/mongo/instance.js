...
InstanceSchema.methods.getElasticHostname = function (ownerGitHubUsername) {
  if (!ownerGitHubUsername) { throw new Error('ownerGitHubUsername is required') }
...
InstanceSchema.methods.setDependenciesFromEnvironment = function (ownerGitHubUsername, cb) {
  ...
  ownerGitHubUsername = ownerGitHubUsername.toLowerCase()
    ...
    masterInstances: function getNamesFromMongo (cb) {
      ...
      Instance.find(query, fieldsForHostname, function (err, instances) {
        ...
        instances.forEach(function (i) {
          i.hostname = i.getElasticHostname(ownerGitHubUsername)
        })
        cb(null, instances)
      })
    }
  ...
  Promise.all([
    ...
    .then(function () {
      return Promise.all(instances.map(function (instance) {
        ...
        var setData = {
          contextVersion: instance.contextVersion.toJSON()
        }
