...
InstanceSchema.methods.inspectAndUpdate = function (cb) {
  ...
  docker.inspectContainer(self.container, function (err, inspect) {
    if (err) {
      self.modifySetContainerInspectErr(dockerContainer, err, function (err2, instance) {
        err = err || err2;
        if (err) { return cb(err); }
        cb(null, instance);
      });
      ...
      self.modifySetContainerInspect(dockerContainer, inspect, cb);
    }
  });
};
 ...
 * @param {Function} cb  callback(err, instance)
 */
InstanceSchema.methods.modifyContainerCreateErr = function (dockerContainer, err, cb) {
  // Note: update instance only if cv (build) has not changed (not patched)
  var query = {
    _id: this._id,
 ...
 * @param {function} cb - callback
 */
InstanceSchema.methods.modifySetContainerInspect =
 ...
 * @param  {Function} cb  callback
 */
InstanceSchema.methods.modifySetContainerInspectErr = function (dockerContainer, err, cb) {
  // Note: update instance only if cv (build) has not changed (not patched)
  var query = {
    _id: this._id,
    'container.dockerContainer': dockerContainer
  ...
  Instance.findOneAndUpdate(query, {
    ...
    if (err) {
      ...
      // just log this secondary error, this route is already errored
      error.log(Boom.conflict('Container error was not set, container has changed'));
      cb(null, self);
    }
    else {
      cb(err, instance);
