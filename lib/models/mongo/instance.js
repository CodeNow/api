 ...
 * removes all connections to this instance
 * and updates all dependents to point to master
 * if `Neo.ClientError.Statement.EntityNotFound` happens - ignore it
 * @param  {Function} cb (err, thisInstance)
 */
InstanceSchema.methods.removeSelfFromGraphAndIgnore404 = function (cb) {
  this.removeSelfFromGraph(function (err, result) {
    if (err) {
