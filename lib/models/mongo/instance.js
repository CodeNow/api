 ...
 * @param {Function} cb     callback
 */
InstanceSchema.statics.findMasterInstancesNotMatchingContexts = function (contextIds, cb) {
  log.trace({
    tx: true,
    contextIds: contextIds
  }, 'findMasterInstancesNotMatchingContexts')
  var query = {
    masterPod: true,
    shouldNotAutoFork: false,
    'contextVersion.context': { $nin: contextIds.map(objectId) }
  }
  Instance.find(query, cb)
}
 ...
 * @param {function} cb           - Callback with signature (err, instance).
 */
InstanceSchema.statics.findIsolationChildrenWithRepo = function (isolationId, repo, cb) {
  log.info({
    tx: true,
    repo: repo,
    isolationId: isolationId.toString()
  }, 'InstanceSchema.statics.findIsolationChildrenWithRepo')
  var query = {
    isolated: isolationId,
