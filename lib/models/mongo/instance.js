...
InstanceSchema.methods.setDependenciesFromEnvironment = function (ownerUsername, cb) {
  ...
  async.parallel({
    ...
    masterInstances: function getNamesFromMongo (cb) {
      ...
      Instance.find(query, fieldsForHostname, function (err, instance) {
        if (err) { return cb(err); }
    ...
    var tasks = addDeps
      .map(toTask('addDependency'))
      .concat(
        remDeps.map(toTask('removeDependency'))
      );
    ...
    function toTask (method) {
      return function (dep) {
        return self[method].bind(self, dep, dep.hostname);
      };
    }
  });
};
