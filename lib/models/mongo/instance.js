  ...
  async.parallel({
    ...
    if (isEmpty(deps)) {
      ...
      envDeps.forEach(function (envDep) {
        if (!isEmpty(remDeps)) {
          var index = -1
          remDeps.every(function (remDep, i) {
            if (depsEqual(envDep, remDep)) {
              index = i
              return false // Return false so .every short circuits
            }
            return true
          })
          if (index >= 0) {
            // don't remove it from the deps, so remove it from remDeps
            remDeps.splice(index, 1)
          } else {
            // doesn't exist, so add it.
            addDeps.push(envDep)
          }
        } else {
          // Nothing is left in rem, so just add the rest
          addDeps.push(envDep)
        }
      })
    }
