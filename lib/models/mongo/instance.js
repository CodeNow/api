...
var logger = require('middlewares/logger')('models:mongo:instance');
...
InstanceSchema.methods.populateOwnerAndCreatedBy = function (sessionUser, cb) {
  logger.log.trace({tid: true}, 'populateOwnerAndCreatedBy');
  var self = this;
  async.parallel({
...
InstanceSchema.methods.generateGraphNode = function () {
  ...
  logger.log.trace({tid: true, node: node}, 'generateGraphNode');
  return node;
};
...
InstanceSchema.methods.setDependenciesFromEnvironment = function (ownerUsername, cb) {
  logger.log.trace({tid: true}, 'setDependenciesFromEnvironment');
  var originalCb = cb;
  cb = function () {
...
InstanceSchema.methods.populateModels = function (cb) {
  logger.log.trace({tid: true}, 'populateModels');
