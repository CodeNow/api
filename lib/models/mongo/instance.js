...
InstanceSchema.statics.addDefaultIsolationOpts = function(query) {
  log.info({
    tx: true,
    query: query
  }, 'InstanceSchema.addDefaultIsolationOpts');
  if (isObject(query)) {
    query = clone(query);
  } else {
    query = {};
  ...
  if (query.isIsolationGroupMaster) {
    return query;
  }
  if (query.$or) {
...
InstanceSchema.methods.setContainerStateToStopping = function(cb) {
  ...
  log.trace(logData, 'InstanceSchema.methods.setStateToStopping');
...
InstanceSchema.statics.populateOwnerAndCreatedByForInstances = function(sessionUser, instances, cb) {
  ...
  if (instances.length === 0) {
    ...
    async.waterfall([function checkIfSessionUserIsHelloRunnable(checkCallback) {
...
InstanceSchema.methods.getSelfFromGraph = function(cb) {
  ...
  client.graph.getNodes(node, [], function(err, nodes) {
    ...
    if (!nodes.length) {
      return cb(new Error('could not retrieve node from graph'));
    }
    cb(null, nodes[0]);
  });
};
...
InstanceSchema.methods.setDependenciesFromEnvironment = function(ownerUsername, cb) {
  log.trace({
    tx: true
  }, 'setDependenciesFromEnvironment');
  var self = this;
  var originalCb = cb;
  cb = function() {
...
InstanceSchema.methods.updateStaleCv = function(cb) {
  ...
  if (keypather.get(this, 'contextVersion.build.started') &&
    ...
    var self = this;
    ContextVersion.findById(this.contextVersion._id, {
      'build.log': 0
    }, function(err, cv) {
      if (err) {
        return cb(err);
 ...
 */
InstanceSchema.methods.modifyContainer = function(contextVersionId, dockerContainer, dockerHost, cb) {
  log.trace({
    tx: true,
...
InstanceSchema.methods.emitInstanceUpdate = function(action, cb) {
  ...
  async.waterfall([
    User.findByGithubId.bind(User, this.createdBy.github), function populateInstance(user, cb) {
      async.parallel([
