...
var NaviEntries = require('models/mongo/navi-entry');
...
/**
 * finds all running or starting instances based on dockerHost
 * @param  {string}   dockerHost format: http://10.0.0.1:4242
 * @param  {Function} cb         (err, InstanceArray)
 */
InstanceSchema.statics.findActiveInstancesByDockerHost = function (dockerHost, cb) {
  log.info({
    tx: true,
    dockerHost: dockerHost
  }, 'InstanceSchema.statics.findActiveInstancesByDockerHost findOneByContainerId');
  Instance.find({
...
InstanceSchema.methods.populateOwnerAndCreatedBy = function (sessionUser, cb) {
  ...
  }, 'InstanceSchema.methods.populateOwnerAndCreatedBy');
  var self = this;
  async.parallel({
...
/**
 * get the direct hostname of the instance
 * @param {string} ownerUsername instance owner's username
 * @return {string} directHostname
 */
InstanceSchema.methods.getDirectHostname = function (ownerUsername) {
  if (!ownerUsername) { throw new Error('ownerUsername is required'); }
  return runnableHostname.direct({
    shortHash: this.shortHash,
    instanceName: this.name,
    ownerUsername: ownerUsername,
    branch: this.getMainBranchName(),
...
InstanceSchema.methods.modifyContainerInspect =
  function (dockerContainer, containerInspect, cb) {
    ...
    Instance.findOneAndUpdate(query, { $set: $set }, function (err, instance) {
      ...
      NaviEntries.handleInstanceStatusChange(instance, function (err) {
        if (err) { return cb(err); }
        cb(null, instance);
      });
    });
 ...
 *   - unknown
 */
InstanceSchema.methods.status = function (cb) {
  var jesusBirthday = '0001-01-01T00:00:00Z';
  var container = keypather.get(this, 'container');
  var build = keypather.get(this, 'build');
  if (container) {
    // Starting/Stopping are not docker API response properties, custom inserted by API
    if (keypather.get(container, 'inspect.State.Starting')) {
      return cb(null, 'starting');
    }
    if (keypather.get(container, 'inspect.State.Stopping')) {
      return cb(null, 'stopping');
    }
    if(!keypather.get(container, 'inspect.State.Running')) {
      if (keypather.get(container, 'inspect.State.ExitCode') === -1) {
        return cb(null, 'stopped');
      }
      if (keypather.get(container, 'inspect.State.StartedAt') === jesusBirthday) {
        return cb(null, 'neverStarted');
      }
      return cb(null, 'crashed');
    }
  ...
  function getBuild (cb) {
    if (instance.build instanceof ObjectId) {
      Build.findById(instance.build, cb);
    } else {
      cb(null, instance.build);
    }
  }

  getBuild(function (err, build) {
    if (err) { return cb(err); }
    if (!build) {
      return cb(null, 'unknown');
    }
    if (build.completed && build.failed){
      return cb(null, 'buildFailed');
    }
    return cb(null, 'building');
  });
};
