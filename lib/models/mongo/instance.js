      ...
      return instance.convertAliasToDependency(params.hostname)
        ...
        .catch(Instance.NotFoundError, () => {
          ...
          if (params.hostname) {
            ...
            dependencies = dependencies.filter(dep => self._id.toString() !== dep.id)
          }
 ...
 * @throws Instance.NotFoundError - When we're looking for a dependency that isn't a masterpod nor isolated
 * @throws Instance.NotFoundError - When the Dependency Instance fetch fails
 *
 */
InstanceSchema.methods.convertAliasToDependency = function (alias) {
  var log = logger.log.child({
    instanceId: keypather.get(this, '_id'),
  ...
  return Promise.try(() => {
    if (!alias) {
      throw new Instance.NotFoundError('No alias was given')
    }
    const base64Alias = new Buffer(alias).toString('base64')
    const aliasModel = this.aliases[base64Alias]
    if (!aliasModel) {
      throw new Instance.NotFoundError('Alias was not found')
    }
    const query = {
      'contextVersion.context._id': aliasModel.contextId
    }
    if (this.masterPod) {
      query.masterPod = true
    } else if (this.isolation) {
      query.isolation = this.isolation
    } else {
    ...
    return Instance.findOneBy(query)
      .then(instance => {
        if (!instance) {
          throw new Instance.NotFoundError('Instance Dependency (by alias) could not be found', query)
        }
      })
  })
}
