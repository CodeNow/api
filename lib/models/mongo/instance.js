 ...
 * @param  {finishedTime}  time as string when conatiner was stopped/died
 * @param  {Function} cb               callback(err, instance)
 */
InstanceSchema.methods.setContainerFinishedState = function (finishedTime, cb) {
  // TODO we might revisit setting `Restarting` to false.
  // we don't care about that field just now
  var updateFields = {
  ...
  Instance.findOneAndUpdate(query, {
    $set: updateFields
  }, function (err, instance, w) {
    console.log('qweqwe', instance, w);
    if (err) {
      cb(err);
    }
    else if (!instance) { // instance was not in running state
      Instance.findById(this._id, cb);
    }
    else {
