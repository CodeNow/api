...
InstanceSchema.methods.generateGraphNode = function (alias) {
  let val = {
    elasticHostname: alias || this.elasticHostname,
    instanceId: this._id,
    name: this.name
  }
  if (alias) {
...
/**
 * Searches through a list of instances to find the first one that matches the context
 * @param {String} contextId        - ObjectId (as string) of the contextId to match
 * @param {[Instance]} instanceList - Instances to search to find one matching the contextId
 * @returns {Instance|null} instance with the given hostname
 */
function findMatchingInstanceByContextId (contextId, instanceList) {
  return find(instanceList, function (instance) {
    return keypather.get(instance, 'contextVersion.context.toString()') === contextId
  })
}
...
InstanceSchema.methods.getAliases = function () {
  var log = logger.log.child({
    instanceId: keypather.get(this, '_id'),
    instanceName: keypather.get(this, 'name'),
    method: 'InstanceSchema.methods.getAliases'
  })
  log.info('called')
...
/**
 *
 * @param {Object}     instancesOpts
 * @param {Instance[]} instancesOpts.dependentInstances
 * @param {Instance[]} instancesOpts.isolatedInstances
 * @param {Instance[]} instancesOpts.masterInstances
 * @param {Boolean=}   isAlias
 *
 * @returns {Function}
 * @private
 */
InstanceSchema.statics._createGraphNodesFromDeps = function (instancesOpts, isAlias) {
  var dependentInstances = instancesOpts.dependentInstances // existing dependencies
  var isolatedInstances = instancesOpts.isolatedInstances // instances in the same isolation group
  var masterInstances = instancesOpts.masterInstances
  // We need to do different kinds of matching depending on if this is an existing instance
  var matchingMethod = isAlias ? findMatchingInstanceByContextId : findMatchingInstanceByHostname
  return function (value) {
    var val = isAlias ? value.contextId : value
    var alias = isAlias ? value.alias : null

    var instance = matchingMethod(val, dependentInstances)
    // first check the isolated instances.
    if (!instance && isolatedInstances.length) {
      instance = matchingMethod(val, isolatedInstances)
    }
    if (!instance) {
      instance = matchingMethod(val, masterInstances)
    }
    if (instance) {
      return instance.generateGraphNode(alias)
    }
  }
}
...
InstanceSchema.methods.setDependenciesFromEnvironment = function (ownerUsername, cb) {
  ...
  Promise.props({
    existingGraphNodes: self.dependencies.map(dep => dep.toJSON()),
    dependentInstances: self.getDependenciesAsync(),
    ...
    .then(function (results) {
      var existingDeps = results.existingGraphNodes // existing dependencies
      ...
      var linkedAliases = self.getAliases()

      var envDeps = linkedHostnames.map(hostname => Instance._createGraphNodesFromDeps(results)(hostname))
      var aliasDeps = linkedAliases.map(alias => Instance._createGraphNodesFromDeps(results, true)(alias))
      ...
      var remDeps = subtract.sub(existingDeps, allDeps)
      var addDeps = subtract.sub(allDeps, existingDeps)
      ...
      function toRemTask (dep) {
        return self.removeDependency(dep.instanceId, dep.elasticHostname)
      }
 ...
 * @param {ObjectId} instanceIdToFind - ObjectId of the instance to find
 *
 * @returns {GraphNode|null} Either the matching node for the given hostname, or null
 */
function getDepFromInstance (instance, instanceIdToFind) {
  if (keypather.get(instance, 'dependencies.length')) {
    var deps = instance.dependencies.filter(function (dep) {
      return dep.instanceId.toString() === instanceIdToFind.toString()
    })
 ...
 *
 * @param {Instance|GraphNode} instance - Either the full instance to add, or the already-creaeted
 *                                          graph node to add to THIS instance.  Best to send the
 *                                          graph node
 *
...
InstanceSchema.methods.addDependency = function (instance) {
  if (instance.generateGraphNode) {
    instance = instance.generateGraphNode()
  }
  ...
  return Instance.findByIdAsync(this._id)
    ...
    .then(function (updatedInstance) {
      return getDepFromInstance(updatedInstance, instance._id)
    })
    .finally(function () {
      self.invalidateContainerDNS()
    })
 ...
 */
InstanceSchema.methods.removeDependency = function (instanceId, hostnameOrAlias) {
  var self = this
  var query = { instanceId }
  if (hostnameOrAlias) {
    query.elasticHostname = hostnameOrAlias
  }
...
InstanceSchema.statics.findForkableMasterInstances = function (repo, branch, cb) {
  var log = logger.log.child({
    repo: repo,
    branch: branch,
    method: 'InstanceSchema.statics.findForkableMasterInstances'
  })
  log.info('called')
  var query = {
    masterPod: true,
 ...
 */
InstanceSchema.statics.findInstancesForBranchAndBuildHash = function (repo, branch, contextId, hash) {
  var log = logger.log.child({
    repo: repo,
    branch: branch,
    contextId: contextId,
...
InstanceSchema.statics.populateModels = function (instances, cb) {
  var log = logger.log.child({
    ...
    method: 'InstanceSchema.statics.populateModels'
  })
  log.info('called')
  var instancesByCvId = {}
...
InstanceSchema.methods.populateModels = function (cb) {
  ...
  self.updateCv().asCallback(count.next)
...
InstanceSchema.statics.emitInstanceUpdates = function (sessionUser, query, action, cb) {
  var log = logger.log.child({
    ...
    query: query,
    action: action,
    method: 'InstanceSchema.statics.emitInstanceUpdates'
  })
  log.info('called')
  this.find(query, function (err, instances) {
    if (err) {
      log.error({ err: err }, 'InstanceSchema.findBy contextVersion._id failure')
      return cb(err)
    }
    log.trace({
      instances: instances.map(function (instance) {
    ...
    }, 'InstanceSchema.findBy contextVersion._id success')
...
InstanceSchema.methods.emitInstanceUpdate = function (sessionUser, action, cb) {
  var log = logger.log.child({
    instanceId: keypather.get(this, '_id'),
    instanceName: keypather.get(this, 'name'),
    method: 'InstanceSchema.methods.emitInstanceUpdate'
  })
  log.info('called')
  var self = this
  async.parallel([
