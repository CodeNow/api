...
InstanceSchema.statics.populateModels = function (instances, cb) {
  ...
  instances = instances.filter(function (instance) {
    ...
    if (container.dockerContainer && noInspectData) {
      ...
      instancesByCvId[instance.contextVersion._id] = instance
      instancesByBuildId[instance.build] = instance
      return true
    }
  })
  Promise.all([
    ContextVersion.findAsync({ _id: { $in: Object.keys(instancesByCvId) } }, {'build.log': 0})
      .each(function (cv) {
        instancesByCvId[cv._id]._doc.contextVersion = cv
      }),
    Build.findAsync({ _id: { $in: Object.keys(instancesByBuildId) } })
      .each(function (build) {
        instancesByBuildId[build._id]._doc.build = build
      })
  ])
    .then(function () {
