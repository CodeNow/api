 ...
 */
InstanceSchema.methods.modifySetContainer = function (containerInspect, cb) {
  var instance = this;
  var info = containerInspect;
  var started = instance.container.create.started;
  instance.container = { // set it on the model, mongoose can cast things
  ...
  if (ports) {
    instance.container.ports = ports;
  }
  // find and modify goes straight to db driver, so casting above is important!
  instance.update({
 ...
 * @param {Function} cb  callback(err, instance)
 */
InstanceSchema.methods.modifyContainerCreateError = function (err, cb) {
  Instance.findByIdAndUpdate(this._id, {
    $set: {
      'container.create.completed': Date.now(),
      'container.create.error': pick(err, ['message', 'stack'])
    }
  }, cb);
};
