...
InstanceSchema.statics.findInstancesBuiltByDockerHost = function (dockerHost, cb) {
  ...
  Instance.find(query, function (err, instances) {
    if (err) {
      log.error({ err: err }, 'error when finding instance')
      return cb(err)
    }
    log.trace({ count: instances.length }, 'found instances')
    cb(null, instances)
  })
}
...
InstanceSchema.statics.findInstancesBuildingOnDockerHost = function (dockerHost, cb) {
  ...
  Instance.find(query, function (err, instances) {
    if (err) {
      log.error({ err: err }, 'error when finding instances')
      return cb(err)
    }
    log.trace({ count: instances.length }, 'found instances')
    cb(null, instances)
...
InstanceSchema.statics.setContainerError = function (instanceId, containerId, errMsg) {
  ...
  var log = logger.log.child({
    ...
    query: query,
    setData: setData,
    method: 'InstanceSchema.methods.setContainerError'
  })
  log.info('setContainerError called')
  // Note: update instance only if container not changed
  return Instance.findOneAndUpdateAsync(query, {
    $set: setData
  })
  .tap(function (instance) {
    if (!instance) {
      log.error('instance with container not found')
      throw Boom.notFound('instance with container not found', {
        containerId: containerId,
        instanceId: instanceId
      })
    }
    log.trace('Instance.setContainerError success')
  })
  .catch(function (err) {
    log.error({
      updateErr: err
    }, 'Instance.setContainerError error')
    throw err
  })
}
...
InstanceSchema.statics.setContainerCreateError = function (instanceId, contextVersionId, errMsg) {
  ...
  var log = logger.log.child({
    ...
    query: query,
    setData: setData,
    method: 'InstanceSchema.methods.setContainerCreateError'
  })
  log.info('setContainerCreateError called')
  // Note: update instance only if context version has not changed
  return Instance.findOneAndUpdateAsync(query, {
    $set: setData
  })
  .tap(function (instance) {
    ...
    log.trace('Instance.setContainerCreateError success')
  })
  .catch(function (err) {
    log.error({
      updateErr: err
    }, 'Instance.setContainerCreateError error')
    throw err
  })
}
