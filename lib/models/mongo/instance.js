...
InstanceSchema.methods.populateModels = function (cb) {
  ...
  const count = createCount(3, callback)
  self.populate('build', count.next)
  self.updateCv().asCallback(count.next)
  ClusterDataService.populateInstanceWithClusterInfo(self).asCallback(count.next)

  function callback (_err) {
    if (_err) { return cb(_err) }
    log.trace('populated all models', self)
    const json = self.toJSON()
    log.trace('populated all models and created json', json)
