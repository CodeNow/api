...
/**
 * Check if autodeploy is enabled for the instance.
 */
InstanceSchema.statics.isAutoDeployOn = function (shortHash, cb) {
  debug('isAutoDeployOn', shortHash);
  var fields = { _id: 1, shortHash: 1, locked: 1 };
  this.findOneByShortHash(shortHash, fields, function (err, inst) {
    if (err) { return cb(err); }
    if (!inst) {
      return cb(Boom.notFound('Instance with shortHash not found', {
        shortHash: shortHash,
        report: false
      }));
    }
    return cb(null, !inst.locked);
  });
};
  ...
  async.parallel({
    currentDependencyDiff: function getCurrentDependencyDiff (cb) {
      self.getDependencies(function (err, dependencies) {
        ...
        var re =
          new RegExp('([0-9a-z-_]+)-' + ownerUsername + '.' + process.env.USER_CONTENT_DOMAIN);
    ...
    results.currentDependencyDiff.toAdd.forEach(function (instance) {
      ...
      if (i) {
        var hn = i.lowerName + '-' + ownerUsername + '.' + process.env.USER_CONTENT_DOMAIN;
