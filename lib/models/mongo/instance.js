...
function (sessionUser, instances, cb) {
    ...
    async.waterfall([
      function checkIfSessionUserIsHelloRunnable (cb) {
        if (sessionUser.accounts.github.id === process.env.HELLO_RUNNABLE_GITHUB_ID) {
          // just use the first created by - chances are that he can get all the users
          sessionUser.findGithubUserByGithubId(createdByGithubIds[0], cb);
        } else {
          cb(null, sessionUser);
        }
      },
  ...
  function populateField (user, keyIds, mapToUpdateList, fieldPath, cb) {
