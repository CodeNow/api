 ...
 */
InstanceSchema.statics.populateModels = function (instances, sessionUser, cb) {
  const log = logger.log.child({
  ...
  Promise.all([
    function () {
      ...
      try {
        ...
        AutoIsolationConfig.findAsync({
          ...
          .then((clusters) => {
            ...
            instances.forEach(function (instance) {
              ...
              if (aig) {
                const cluster = find(clusters, hasProps({ autoIsolationConfigId: aig._id.toString() }))
                if (cluster) {
                  instance._doc.inputClusterConfig = Object.assign({}, cluster.toJSON(), {
        ...
        log.error({ err }, 'org was not found')
      }
    },
...
InstanceSchema.methods.populateModels = function (cb) {
  const log = logger.log.child({
    instanceId: keypather.get(this, '_id'),
    instanceName: keypather.get(this, 'name'),
    method: 'InstanceSchema.methods.populateModels'
  })
  log.info('called')
  const self = this
  ...
  const count = createCount(2, callback)
