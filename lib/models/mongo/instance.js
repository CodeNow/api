...
InstanceSchema.methods.populateOwnerAndCreatedBy = function (sessionUser, cb) {
  logger.log.trace({tx: true}, 'populateOwnerAndCreatedBy');
  var self = this;
  async.parallel({
...
InstanceSchema.methods.generateGraphNode = function () {
  ...
  logger.log.trace({tx: true, node: node}, 'generateGraphNode');
  return node;
};
...
InstanceSchema.methods.setDependenciesFromEnvironment = function (ownerUsername, cb) {
  logger.log.trace({tx: true}, 'setDependenciesFromEnvironment');
  var originalCb = cb;
  cb = function () {
...
InstanceSchema.methods.populateModels = function (cb) {
  logger.log.trace({tx: true}, 'populateModels');
