...
/**
 * finds all instances running or starting on the dock (based on dockerHost)
 * @param  {String}   dockerHost format: http://10.0.0.1:4242
 * @param  {Function} cb         (err, InstanceArray)
 */
InstanceSchema.statics.findInstancesRunningOrStartingByDockerHost = function (dockerHost, cb) {
  var query = {
  ...
  log.info(logData, 'InstanceSchema.statics.findInstancesRunningOrStartingByDockerHost')
  Instance.find(query, function (err, instances) {
    if (err) {
      log.error(put({ err: err }, logData), 'findInstancesRunningOrStartingByDockerHost error')
      return cb(err)
    }
    log.trace(put({ count: instances.length }, logData), 'findInstancesRunningOrStartingByDockerHost success')
    cb(null, instances)
  })
}
...
InstanceSchema.methods.populateOwnerAndCreatedBy = function (sessionUser, cb) {
  ...
  var ownerPopulated = keypather.get(this, 'owner.username') && keypather.get(this, 'owner.gravatar')
  var createdByPopulated = keypather.get(this, 'createdBy.username') && keypather.get(this, 'createdBy.gravatar')
  if (ownerPopulated && createdByPopulated) {
    log.trace(logData, 'InstanceSchema.methods.populateOwnerAndCreatedBy success - already populated')
    return cb(null, self)
  }
  async.parallel({
...
InstanceSchema.statics.populateOwnerAndCreatedByForInstances = function (sessionUser, instances, cb) {
  ...
  if (instances.length === 0) {
    ...
    var filteredInstances = instances.filter(function (instance) {
      var ownerPopulated = keypather.get(instance, 'owner.username') && keypather.get(instance, 'owner.gravatar')
      var createdByPopulated = keypather.get(instance, 'createdBy.username') && keypather.get(instance, 'createdBy.gravatar')
      return !ownerPopulated || !createdByPopulated
    })
    if (filteredInstances.length === 0) {
      log.trace(logData, 'InstanceSchema.statics.populateOwnerAndCreatedByForInstances - all instances populated already')
      return done()
    }
    var instancesByOwnerGithubId = groupBy(filteredInstances, 'owner.github')
    var ownerGithubIds = Object.keys(instancesByOwnerGithubId)
    var instancesByCreatedByGithubId = groupBy(filteredInstances, 'createdBy.github')
        ...
        mapToUpdateList[githubId].forEach(function (instance) {
          ...
          instance.update({
            ...
            if (err) {
              ...
              }, logData), 'InstanceSchema.statics.populateOwnerAndCreatedByForInstances - populateField')
            }
          })
        })
...
/**
 * Only update the context version if there is a build that is started but not yet completed
 * @param  {Function} cb callback
 */
...
InstanceSchema.statics.emitInstanceUpdates = function (sessionUser, query, action, cb) {
  ...
  log.info(logData, 'InstanceSchema.statics.updateStaleCvByContextVersionIds')
  this.find(query, function (err, instances) {
    if (err) {
      log.error(
        put({ err: err }, logData),
        'updateStaleCvByContextVersionIds: InstanceSchema.findBy contextVersion._id failure'
      )
      return cb(err)
    }
    log.trace(
