...
/**
 * Mark instance as deleted by id
 * @param {String} instanceId
 * @param {Function} cb Callback function
 */
InstanceSchema.statics.markAsDeletedById = function (id, cb) {
  var update = {
    $set: {
      deleted: true
    }
  }
  log.info({ tx: true, id: id, update: update }, 'InstanceSchema.statics.markAsDeletedById')
  Instance.findByIdAndUpdate(id, update, cb)
}

/**
 * Mark all instances as deleted by id. Search instances that has `instaceShortHash` as parent
 * @param {String} instaceShortHash - instance hash used for search all forked instances
 * @param {Function} cb Callback function
 */
InstanceSchema.statics.markAsDeletedByParent = function (instaceShortHash, cb) {
  var query = {
    parent: instaceShortHash
  }
  var update = {
    $set: {
      deleted: true
  ...
  log.info({
    tx: true, instaceShortHash: instaceShortHash,
    query: query, update: update
  }, 'InstanceSchema.statics.markAsDeletedByParent')
 ...
 * Also instances shouldn't have `deleted=true`
 ...
 * We only care about main repos (additionalRepo=false).
 * Also instances shouldn't be deleted
 * @param  {String}   repo   full repo name (username/reponame)
 * @param  {String}   branch branch name
 * @param  {Function} cb     callback
 */
