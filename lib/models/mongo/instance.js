...
var isObjectId = require('utils/is-object-id-string')
...
InstanceSchema.statics.findOneStopping = function (instanceId, containerId, cb) {
  ...
  log.trace(logData, 'InstanceSchema.statics.findOneStopping')
  Instance.findOne({
    _id: instanceId,
    'container.dockerContainer': containerId,
    'container.inspect.State.Stopping': {
...
InstanceSchema.statics.markAsStopping = function (instanceId, containerId, cb) {
  ...
  log.trace(logData, 'InstanceSchema.statics.markAsStopping')
  Instance.findOneAndUpdate({
    _id: instanceId,
    'container.dockerContainer': containerId,
    'container.inspect.State.Starting': {
...
InstanceSchema.methods.updateCv = function (cb) {
  ...
  ContextVersion.findById(this.contextVersion._id, {'build.log': 0}, function (err, cv) {
    if (err) { return cb(err) }
    var cvDoc = cv.getSubdoc()
    self.update({
      $set: { contextVersion: cvDoc }
    }, function (updateErr) {
      self._doc.contextVersion = cvDoc
      cb(updateErr, self)
    })
  })
}
  ...
  Promise.all([
    ...
    .then(function () {
      return instances.map(function (instance) {
        var instanceDoc = instance._doc
        instanceDoc.id = instance._id.toString()
        return instanceDoc
      })
    })
    ...
    .asCallback(cb)
    .then(function () {
      return Promise.all(instances.filter(function (instance) {
        return instance.contextVersion.build.started && !instance.contextVersion.build.completed;
      }).map(function (instance) {
        var query = {
          _id: instance._id
        }
        var setData = {
          contextVersion: instance.contextVersion.getSubdoc()
        }
        return Instance.findOneAndUpdateAsync(query, { $set: setData })
      }))
    })
    .catch(function (err) {
      log.error(put({
        err: err.message
      }, logData), 'populateModels failure')
      error.log(err)
    })
}
/**
...
InstanceSchema.methods.modifyContainerInspect = function (dockerContainer, containerInspect, cb) {
  var logData = {
    ...
    containerInspect: containerInspect.id
  }
...
InstanceSchema.statics.emitInstanceUpdates = function (sessionUser, query, action, cb) {
  var logData = {
    tx: true,
    sessionUser: keypather.get(sessionUser, 'accounts.github.id'),
    query: query,
    action: action
  }
  ...
  this.find(query, function (err, instances) {
    ...
    }, logData), 'emitInstanceUpdates: InstanceSchema.findBy contextVersion._id success')
