...
/**
 * get branch name of the contextVersion's first appCodeVersion
 * @return {string} branchName
 */
InstanceSchema.methods.getBranchName = function () {
  return keypather.get(this, 'contextVersions.appCodeVersions[0].branch');
};
...
InstanceSchema.methods.getElasticHostname = function (ownerUsername) {
  ...
  return runnableHostname.elastic({
    ...
    branch: i.getBranchName(),
    masterPod: i.masterPod,
...
InstanceSchema.methods.setDependenciesFromEnvironment = function (ownerUsername, cb) {
  ...
  async.parallel({
    ...
    var envDeps = self.env.map(function (env) {
      var val = env.split('=')[1];
      var instance = find(masters, hasKeypaths({
        'hostname.toLowerCase()': val.toLowerCase()
      }));
      if (instance) {
    ...
    function toAddTask (dep) {
      return console.log.bind(console, 'ADD', dep.hostname, dep.name);
      // return self.addDependency.bind(self, dep, dep.hostname);
    }
    function toRemTask (dep) {
      return console.log.bind(console, 'REM', dep.hostname, dep.name);
      // return self.removeDependency.bind(self, dep, dep.hostname);
    }
  });
};
function depsEqual (depA, depB) {
  // we assume deps have the same keys
  if (depA.contextVersion.toJSON) { depA.contextVersion = depA.contextVersion.toJSON(); }
  if (depB.contextVersion.toJSON) { depB.contextVersion = depB.contextVersion.toJSON(); }
  depA = keypather.flatten(depA);
  depB = keypather.flatten(depB);
  return Object.keys(depA).reduce(function (key) {
    var valA = depA[key];
    var valB = depB[key];
