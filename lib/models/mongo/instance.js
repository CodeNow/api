 ...
 * @param {Function} cb           - Callback with signature (err, instance).
 */
InstanceSchema.statics.findInstancesInIsolationWithSameRepo = function (isolationId, repoName, cb) {
  var query = {
    isolated: isolationId,
    'contextVersion.appCodeVersions': {
      $elemMatch: {
  ...
  }, 'InstanceSchema.statics.findInstancesInIsolationWithSameRepo')
  this.find(query, cb)
}
...
/**
 * Update the instances that exist for a given repository.
 * @param {ObjectId} isoaltionId  - ID of the isolation
 * @param {String}   repoName     - Repository for which to search (e.g. "Runnable/cli").
 * @param {Object}   updates      - Updates to be set on all instances
 * @param {Function} cb           - Callback with signature (err, instance).
 */
InstanceSchema.statics.updateInstancesInIsolationWithSameRepo = function (isolationId, repoName, updates, cb) {
  var query = {
    isolated: isolationId,
    'contextVersion.appCodeVersions': {
      $elemMatch: {
  ...
  log.info({
    ...
    query: query,
    updates: updates
  }, 'InstanceSchema.statics.updateInstancesInIsolationWithSameRepo')
  var update = {
    $set: updates
  }
  Instance.update(query, update, { multi: true }, cb)
