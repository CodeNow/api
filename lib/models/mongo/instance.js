...
InstanceSchema.methods.getGithubUsername = function (sessionUser, cb) {
  var instance = this;
  if (instance.owner && instance.owner.username) {
    // already populated
    return cb(null, instance);
  }
  if (!this.owner.github) {
  ...
  sessionUser.findGithubUserByGithubId(this.owner.github, function (err, user) {
    ...
    if (!user) {
      cb(Boom.notFound('Owner not found'));
    }
...
InstanceSchema.methods.populateModels = function (sessionUser, cb) {
  var self = this;
  ...
  function populate (err, instance) {
    ...
    instance.getGithubUsername(sessionUser, count.inc().next);
