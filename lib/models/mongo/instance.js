...
InstanceSchema.methods.generateGraphNode = function (ignoreIsolationProperty) {
  var log = logger.log.child({
    ...
    ignoreIsolationProperty: ignoreIsolationProperty,
    method: 'InstanceSchema.methods.generateGraphNode'
  })
  log.info('called')
  var node = {
    label: 'Instance',
...
InstanceSchema.methods.getSelfFromGraph = function (cb) {
  var log = logger.log.child({
    instanceId: keypather.get(this, '_id'),
    instanceName: keypather.get(this, 'name'),
    method: 'InstanceSchema.methods.getSelfFromGraph'
  })
  log.info('called')
  var client = new Graph()
...
InstanceSchema.methods.upsertIntoGraph = function (cb) {
  var log = logger.log.child({
    instanceId: keypather.get(this, '_id'),
    instanceName: keypather.get(this, 'name'),
    method: 'InstanceSchema.methods.upsertIntoGraph'
  })
  log.info('called')
  var client = new Graph()
  var node = this.generateGraphNode()
...
InstanceSchema.methods.addDependency = function (instance, hostname, cb) {
  ...
  self.getSelfFromGraphAsync()
    .catch(function () {
      log.trace('Self not in graph, upserting')
      return self.upsertIntoGraphAsync()
    })
    .then(function () {
      return instance.getSelfFromGraphAsync()
        .catch(function () {
          log.trace('Dependent instance not in graph, upserting')
          return instance.upsertIntoGraphAsync()
        })
    })
    .then(function () {
      log.trace('Verified both nodes exist in graph, writing connections')
      return Promise.fromCallback(function (cb) {
        client.graph.writeConnections(connections, cb)
      })
    })
    .then(function () {
      log.trace('connections updated')
...
InstanceSchema.methods.removeDependency = function (instance, cb) {
  ...
  var log = logger.log.child({
    ...
    dependentInstanceId: keypather.get(instance, '_id'),
    dependentInstanceNAme: keypather.get(instance, 'name'),
    method: 'InstanceSchema.methods.removeDependency'
  })
  log.info('called')
...
InstanceSchema.methods.updateCv = function () {
  ...
  log.trace(logData, 'updateCv')
  return ContextVersion.findByIdAsync(cvId, {'build.log': 0})
    .tap(function (cv) {
      if (!cv) {
        log.error(logData, 'Not context version found for an instance')
...
InstanceSchema.statics.emitInstanceUpdates = function (sessionUser, query, action, cb) {
  ...
  this.find(query, function (err, instances) {
    ...
    log.trace({
      instances: instances.map(toJSON)
