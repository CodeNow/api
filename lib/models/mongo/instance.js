...
InstanceSchema.methods.setContainerStateToStarting = function (cb) {
  ...
  Instance.findOneAndUpdate(query, {
    ...
    if (err) {
      ...
      return cb(Boom.badRequest('container is already starting or stopping'));
    }
    cb(null, result);
  });
};
...
InstanceSchema.methods.inspectAndUpdate = function (cb) {
  log.trace({
    tx: true
  }, 'InstanceSchema.methods.inspectAndUpdate');
  var self = this;
  ...
  docker.inspectContainer(self.container, function (err, inspect) {
    if (err) {
      log.trace({
        tx: true,
        err: err
      }, 'InstanceSchema.methods.inspectAndUpdate docker.inspectContainer error');
      self.modifyContainerInspectErr(dockerContainer, err, function (err2, instance) {
        if (err2) {
          log.trace({
            tx: true,
            err: err2
          }, 'InstanceSchema.methods.inspectAndUpdate modifyContainerInspectErr error');
          return cb(err2);
        }
        else {
          log.trace({
            tx: true
          }, 'InstanceSchema.methods.inspectAndUpdate modifyContainerInspect success');
      ...
      log.trace({
        tx: true
      }, 'InstanceSchema.methods.inspectAndUpdate docker.inspectContainer success');
...
InstanceSchema.methods.modifyContainerInspect =
  function (dockerContainer, containerInspect, cb) {
    ...
    log.trace(logData, 'InstanceSchema.methods.modifyContainerInspect');
    ...
    Instance.findOneAndUpdate(query, { $set: $set }, function (err, instance) {
      if (err) {
        log.error(put({
          err: err
        }, logData), 'InstanceSchema.methods.modifyContainerInspect '+
          'Instance.findOneAndUpdate error');
        cb(err);
      }
      else if (!instance) { // changed or deleted
        log.error(logData, 'InstanceSchema.methods.modifyContainerInspect '+
                  'Instance.findOneAndUpdate instance not found');
        cb(Boom.conflict('Container was not deployed, instance\'s container has changed'));
      }
      else {
        log.trace(logData, 'InstanceSchema.methods.modifyContainerInspect '+
                  'Instance.findOneAndUpdate success');
        cb(err, instance);
      }
    });
  };
...
InstanceSchema.methods.modifyContainerInspectErr = function (dockerContainer, err, cb) {
  log.trace({
    ...
    dockerContainer: dockerContainer,
    err: err
  }, 'InstanceSchema.methods.modifyContainerInspectErr');
  // Note: update instance only if cv (build) has not changed (not patched)
  var query = {
    _id: this._id,
    'container.dockerContainer': dockerContainer
 ...
 * @param {Object} err - optional, will be added to instance document
 * @param {Function} cb
 */
...
InstanceSchema.methods.isPublic = function (cb) {
  log.trace({
    tx: true
  }, 'InstanceSchema.methods.isPublic');
  var err;
  if (!this.public) {
...
InstanceSchema.methods.setContainerFinishedState = function (finishedTime, exitCode, cb) {
  log.trace({
    ...
    exitCode: exitCode
  }, 'InstanceSchema.methods.setContainerFinishedState');
...
InstanceSchema.methods.emitInstanceUpdate = function (action, cb) {
  log.trace({
    tx: true,
    action: action
  }, 'InstanceSchema.methods.emitInstanceUpdate');
  var instance = this;
