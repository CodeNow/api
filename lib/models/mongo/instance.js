 ...
 */
InstanceSchema.statics.isAutoDeployOn = function (id, cb) {
  debug('isAutoDeployOn', id);
  this.findById(id, function (err, inst) {
    if (err) { return cb(err); }
    if (!inst) {
      return cb(Boom.notFound('Instance with id not found', {
        id: id,
        report: false
      }));
    }
    return cb(null, !inst.locked, inst);
  });
};
