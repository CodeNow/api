 ...
 * and dock ip.
 * @param {Instance} instance - instance to in which to invalidate the dns
 *
 * TODO move this out of the mongoose model when we come to a conclusion on
 *   overarching models or controllers
 */
InstanceSchema.statics.invalidateContainerDNS = function (instance) {
  ...
  if (!hasKeypaths(instance, ['container.dockerHost'])) {
    log.warn({
  ...
  if (!hasKeypaths(instance, ['container.inspect.NetworkSettings.IPAddress'])) {
    log.warn({
      method: 'InstanceSchema.methods.invalidateContainerDNS'
  ...
  var hostIpMatch = instance.container.dockerHost
  ...
  var localIp = instance.container.inspect.NetworkSettings.IPAddress
...
InstanceSchema.methods.setDependenciesFromEnvironment = function (ownerUsername, cb) {
  ...
  Promise.props({
    ...
    .finally(function () {
      return Instance.invalidateContainerDNS(self)
    })
    .asCallback(cb)
}
...
InstanceSchema.methods.addDependency = function (instance, hostname, cb) {
  ...
  self.getSelfFromGraphAsync()
    ...
    .finally(function () {
      ...
      Instance.invalidateContainerDNS(self)
      Instance.invalidateContainerDNS(instance)
    })
    .asCallback(cb)
}
