...
InstanceSchema.methods.setDependenciesFromEnvironment = function (ownerUsername, cb) {
  ...
  async.parallel({
    ...
    var linkedHostnames = []
    if (keypather.get(self, 'attrs.contextVersion.appCodeVersions.length')) {
      self.attrs.contextVersion.appCodeVersions.forEach(function (acv) {
        if (keypather.get(acv, 'transformRules.replace.length')) {
    ...
    self.env.forEach(function (env) {
      linkedHostnames.push(env.split('=')[1])
    })

    var envDeps = linkedHostnames.map(function findMatchingInstancesByHostname(env) {
      var val = env
