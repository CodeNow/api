...
class NotFound extends BaseError {
  constructor (query, opts, reporting) {
    const level = reporting && reporting.level || 'info'
    super(`instance not found with query: ${query}`, opts, reporting)
    this.setLevel(level)
  }
}
InstanceSchema.statics.NotFound = NotFound

/**
 * Error thrown instance is not in the expected state
 */
class IncorrectState extends BaseError {
  constructor (expectedStatus, instance, reporting) {
    const level = reporting && reporting.level || 'info'
    const state = keypather.get(instance, 'container.inspect.State')
    const status = keypather.get(state, 'Status')
    super(`instance was not in correct state: ${expectedStatus}`, {
      actualStatus: status,
...
InstanceSchema.statics.IncorrectState = IncorrectState
 ...
 * @throws {NotFound} If instance with containerId not found
 * @throws {IncorrectState} If instance not in starting state
 */
InstanceSchema.statics.findOneStarting = function (instanceId, containerId) {
  ...
  return Instance.findOneAsync(query)
    .tap(function checkInstanceState (instance) {
      ...
      if (status !== 'starting') {
        throw new IncorrectState('starting', instance)
      }
    })
}
