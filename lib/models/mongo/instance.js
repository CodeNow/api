...
InstanceSchema.methods.inspectAndUpdate = function (container, dockerHost, cb) {
  ...
  docker.inspectContainer(container, function (err, inspect) {
    if (err) {
      self.modifySetContainerInspectErr(err, function (err2, instance) {
        err = err || err2;
        if (err) { return cb(err); }
        cb(null, instance);
      });
...
/**
 * find contextVersionIds on instances by repo and possibly branch
...
// Find all instances that follow specific repo and branch
...
InstanceSchema.methods.populateModels = function (cb) {
  ...
  if (container.dockerContainer && noCache) {
    self.inspectAndUpdate(self.container, self.container.dockerHost, populate);
  }
  else {
    populate(null, self);
  }
 ...
 */
InstanceSchema.methods.modifySetContainer =
  function (contextVersionId, containerInspect, dockerHost, cb) {
    ...
    Instance.findOneAndUpdate(query, { $set: updateFields }, function (err, instance) {
      if (err) {
        cb(err);
      }
      else if (!instance) {
        cb(Boom.conflict('Container was not deployed, build has changed'));
      }
      else {
        cb(err, instance);
      }
...
InstanceSchema.methods.modifyContainerCreateErr = function (contextVersionId, err, cb) {
  ...
  Instance.findOneAndUpdate(query, {
    ...
    if (err) {
      ...
      error.log(Boom.conflict('Container error was not set, build has changed'));
      cb(null, self);
    }
    else {
      cb(err, instance);
 ...
 * @param {Function} cb  callback(err, instance)
 */
InstanceSchema.methods.modifySetContainerInspectErr = function (contextVersionId, err, cb) {
  // Note: update instance only if cv (build) has not changed (not patched)
  var query = {
    _id: this._id,
    'contextVersion._id': contextVersionId
