 ...
 * @param   {ObjectID} body.build                  - build Id
 ...
 * @param   {ObjectID} body.isolated               - id of the isolation model
 ...
 * @param   {ObjectID} body.parent                 - id of the instance this was forked from
...
/**
 * Given a body object full of parameters, create an instance
 * @param    {Object}   body              - body object from the route
 * @param    {Boolean}  body.autoForked   - true if this instance was autoforked
 * @param    {ObjectID} body.build        - build Id
 * @param    {[String]} body.env          - array of envs ['abc=123']
 * @param    {Object}   body.ipWhitelist  - contains enabled, which when true, means the container
 *                                            is disconnected from outside traffic
 * @param    {ObjectID} body.isolated     - id of the isolation model
 * @param    {Boolean}  body.masterPod    - true if this instance is the masterpod
 * @param    {String}   body.name         - name of this instance
 * @param    {Object}   body.owner        - owner of this instance
  ...
  return Instance.validateBody(body, true)
    ...
    .then(function (build) {
      ...
      return Promise.props({
        contextVersion: ContextVersion.findByIdAsync(build.contextVersions[0]),
        shortHash: InstanceCounter.nextHashAsync(),
        owner: sessionUser.findGithubUserByGithubIdAsync(body.owner.github),
    ...
    .then(function (results) {
      ...
      if (!ownerUsername) {
        log.error('owner not found on Github')
        throw Boom.notFound('owner not found on Github')
      }
      return Instance.createAsync(put(body, {
        ...
        .then(function (instance) {
          ...
          return Promise.props({
            ...
            ownerUsername: Promise.resolve(ownerUsername)
          })
        })
    })
    .tap(function (results) {
      ...
      if (!results.contextVersion) {
        throw Boom.notFound('context version not found')
      }
      if (keypather.get(results, 'contextVersion.build.completed') !== keypather.get(results, 'instance.contextVersion.build.completed')) {
    ...
    .tap(function (results) {
      if (results.contextVersion.successful) {
        ...
        if (!manual) {
          ...
          rabbitMQ.instanceDeployed({
            instanceId: results.instance._id,
            cvId: results.contextVersion._id
          })
        }
        rabbitMQ.createInstanceContainer({
          instanceId: results.instance._id,
          contextVersionId: results.contextVersion._id,
