...
InstanceSchema.methods.generateGraphNode = function () {
  return {
    ...
    props: {
      ...
      owner_github: this.owner.github
    }
  };
};
...
InstanceSchema.methods.upsertIntoGraph = function (cb) {
  var client = new Graph();
  var node = this.generateGraphNode();
  client.graph.writeNode(node, cb);
};

InstanceSchema.methods.addDependency = function (instance, cb) {
  var client = new Graph();
  ...
  client.graph.writeConnection(start, 'dependsOn', end, function (err) {
    if (err) { return cb(err); }
    cb(null, pick(instance.toJSON(), ['id', 'lowerName', 'owner']));
  });
};
...
InstanceSchema.methods.getDependencies = function (cb) {
  var client = new Graph();
  var start = this.generateGraphNode();
  var steps = [{
    Out: {
  ...
  client.graph.getNodes(start, steps, function (err, nodes) {
    // fix owner_github -> owner.github
    if (err) { return cb(err); }
    nodes.forEach(function (n) {
      if (n.owner_github && !n.owner) {
        n.owner = { github: n.owner_github };
        delete n.owner_github;
      }
    });
    cb(null, nodes);
  });
