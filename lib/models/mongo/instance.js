...
InstanceSchema.methods.setContainerStateToStopping = function (cb) {
  ...
  Instance.findOneAndUpdate({
    ...
    if (err) {
      ...
      return cb(Boom.conflict('container is already starting or stopping'));
    }
    cb.apply(this, arguments);
  });
};
...
InstanceSchema.methods.invalidateContainerDNS = function () {
  ...
  if (!hasKeypaths(this, ['container.dockerHost'])) {
    log.warn('invalidateContainerDNS: instance missing `container.dockerHost`');
    return;
  }

  if (!hasKeypaths(this, ['inspect.NetworkSettings.IPAddress'])) {
    log.warn([
      'invalidateContainerDNS: instance missing ',
  ...
  log.debug({
    channel: channel,
    localIp: this.inspect.NetworkSettings.IPAddress
  }, 'Container DNS Invalidate');

  // Publish the invalidate event
  pubsub.publish(channel, this.inspect.NetworkSettings.IPAddress);
};
