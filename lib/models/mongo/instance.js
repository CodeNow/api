 ...
 * @param {Function} cb
 */
InstanceSchema.methods.modifyContainerInspectStartErr = function (cb) {
  log.trace({
    tx: true,
    instanceId: this._id,
    instanceName: this.name
  }, 'InstanceSchema.methods.modifyContainerInspectStartErr');
  Instance.findOneAndUpdate({
    ...
    if (updateErr) {
      log.error({
        tx: true,
        err: updateErr
      }, 'InstanceSchema.methods.modifyContainerInspectStartErr findOneAndUpdate error');
      return cb(updateErr);
    }
    log.trace({
      tx: true,
      instance: instance
    }, 'InstanceSchema.methods.modifyContainerInspectStartErr findOneAndUpdate success');
    cb(updateErr, instance);
  });
};

/**
 * update container inspect if stop request errors
 * removes optimistic set "stopping" property
 * @param {Function} cb
 */
InstanceSchema.methods.modifyContainerInspectStopErr = function (cb) {
  log.trace({
    tx: true,
    instanceId: this._id,
    instanceName: this.name
  }, 'InstanceSchema.methods.modifyContainerInspectStopErr');
  Instance.findOneAndUpdate({
    _id: this._id
  }, {
    $unset: {
    ...
    if (updateErr) {
      log.error({
        tx: true,
        err: updateErr
      }, 'InstanceSchema.methods.modifyContainerInspectStopErr findOneAndUpdate error');
      return cb(updateErr);
    }
    log.trace({
      tx: true,
      instance: instance
    }, 'InstanceSchema.methods.modifyContainerInspectStopErr findOneAndUpdate success');
    cb(updateErr, instance);
  });
};
