...
/**
 * finds all instances based on dockerHost
...
InstanceSchema.statics.findInstancesByDockerHost = function (dockerHost, cb) {
  ...
  log.info(logData, 'InstanceSchema.statics.findInstancesByDockerHost')
  Instance.find(query, function (err, instances) {
    if (err) {
      log.error(put({ err: err }, logData), 'findInstancesByDockerHost error')
      return cb(err)
    }
    log.trace(put({ count: instances.length }, logData), 'findInstancesByDockerHost success')
    cb(null, instances)
  })
}
 ...
 * @param  {Function} cb         (err, InstanceArray)
 */
InstanceSchema.statics.findInstancesBuilByDockerHost = function (dockerHost, cb) {
  var query = {
    'container.dockerHost': dockerHost,
  ...
  log.info(logData, 'InstanceSchema.statics.findInstancesBuilByDockerHost')
  Instance.find(query, function (err, instances) {
    if (err) {
      log.error(put({ err: err }, logData), 'findInstancesBuilByDockerHost error')
      return cb(err)
    }
    log.trace(put({ count: instances.length }, logData), 'findInstancesBuilByDockerHost success')
    cb(null, instances)
  })
}
