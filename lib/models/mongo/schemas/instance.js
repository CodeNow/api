...
var InstanceSchema = module.exports = new Schema({
  ...
  /** Short instance name. Not unique inside organization, but should be unique inside cluster.
   *  However we are not checking that. Should be used mostly for presentation purposes.
   *  @type String */
  shortName: {
    type: String
  },
  /** Unique cluster id. this is used to identify instances before an ICC id is created.
   *  @type String */
  clusterCreateId: {
    type: String
  },
  ...
  /** Defaults to true (will not create new instances for new branches)
   *  @type boolean */
  shouldNotAutofork: {
    type: Boolean,
    'default': true
  ...
  /** True if its a masterPod and it's ever had branches added
   * @type Boolean */
  hasAddedBranches: {
    type: Boolean,
    'default': false
  ...
  /**
   * If defined, command that runs the application
   * @type {Object}
   */
  ...
  /**
   * If defined, ports that will be exposed in the container
   * @type {Array<Number>}
   */
  ports: {
  ...
  /**
   * Boolean indicating if the instance is to be used for testing
   * @type Boolean
   */
  isTesting: Boolean,
  /**
   * Boolean indicating if the instance is a test reporter
   * @type Boolean
   */
  isTestReporter: Boolean,
  /** Instance that this instance is built to test from
   *  @type ObjectId */
  testingParentId: {
    type: ObjectId,
    ref: 'Instance',
    validate: validators.objectId({ model: 'Instance', literal: 'Parent Testing Instance Hash' })
  },
  /**
  ...
  /**
   * Aliases is an object with key/value pairs with the alias, instanceName, and
   * contextId.  The keys are converted to Base64 to remove any illegal characters
   * that may cause issues.
   *
   * @type {Object} aliases                   - indexed by the alias -> base64
   * @type {Object} aliases[key]              - alias model
   * @type {Object} aliases[key].alias        - Hostname that this instance can
   *                                              use to reference the aliased one
   * @type {Object} aliases[key].contextId    - Id of the context on the masterPod
   *                                              this alias is referencing
   * @type {Object} aliases[key].instanceName - Instance name of the masterPod this
   *                                              alias is referencing
   */
  aliases: {
    type: Schema.Types.Mixed,
    'default': {}
...
// Support the isTesting search on container.life-cycle.died
InstanceSchema.index({
