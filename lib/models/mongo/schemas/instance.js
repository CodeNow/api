...
var InstanceSchema = module.exports = new Schema({
    ...
    type: [
      {
        ...
        name: {
          ...
          validate: validators.alphaNum({model: 'Instance', literal: 'OutputView\'s Name'})
        },
        type: {
          type: String,
          required: 'Instances require an OutputView with a type',
          validate: validators.alphaNum({model: 'Instance', literal: 'OutputView\'s Type'})
        }
      }
    ],
    'default': []
  ...
  deploy: {
    started: {
      type: Date,
      validate: validators.beforeNow({model: 'Instance', literal: 'Deployed'})
    },
    error: {
      type: {
        message: String,
