...
var ContextVersionSchema = module.exports = new Schema({
  ...
  /**
   * The dockRemoved status has been confirmed by the user,
   * this will exist even after the dockRemoved is set to false after being true
   * Used to render a modal to the user that their container has been "migrated" to a better "server"
   * @type Boolean
   */
  ...
  build: {
    ...
    state: {
      type: String,
      validate: validators.dockerState({model: 'ContextVersion', literal: 'Build Started'})
    },
