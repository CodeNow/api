...
var ContextVersionSchema = module.exports = new Schema({
  ...
  /** type: ObjectId */
  createdBy: {
    required: 'Context Versions require an CreatedBy',
    type: {
      github: {
        type: Number,
  ...
  environment: {
    type: ObjectId,
    required: 'Versions require an Environment',
    validate: validators.objectId({model:'Version', literal: 'Environment'})
  },
  /** type: ObjectId */
  context: {
    type: ObjectId,
...
ContextVersionSchema.path('owner').validate(numberRequirement, 'Invalid Owner Id');
