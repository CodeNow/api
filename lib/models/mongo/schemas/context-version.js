...
//        validate: validators.number({ model: 'ContextVersion', literal: 'Github Owner' })
  ...
  /** type: object */
  owner: {
    // TODO: add required only when we hunt bug on production
...
ContextVersionSchema.index({
  ...
  'infraCodeVersion': 1,
  'appCodeVersions.transformRules.hash': 1
});
ContextVersionSchema.index({
  'appCodeVersions.lowerRepo': 1,
  'appCodeVersions.commit': 1,
...
ContextVersionSchema.post('save', function (doc) {
  ...
  if (!doc.owner) {
    var err = Boom.badImplementation('context version was saved without owner', {
      cv: doc
    });
    error.log(err);
  }
