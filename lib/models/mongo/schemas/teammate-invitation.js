...
var TeammateInvitation = module.exports = new Schema({
  ...
  owner: {
    required: 'The Github user who sent the invitation is required',
    type: {
      github: {
        type: Number
      }
...
TeammateInvitation.pre('save', function (next) {
  ...
  if (!this.recipient || (!this.recipient.github)) {
    ...
    err.name = 'ValidationError'
    next(err)
  } else if (!this.owner || (!this.owner.github)) {
    err = Boom.badRequest("Invitation's owner github ID is required")
    err.name = 'ValidationError'
    next(err)
    ...
    err = Boom.badRequest("Invitation's owner github ID must be a number")
    err.name = 'ValidationError'
    next(err)
