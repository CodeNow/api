...
function lengthCheck(input) {
  ...
  return validate(input, 'len', 1, input.maxLength)
}
function invalidCheck(input) {
  input.message = input.model + "'s " + input.literal + input.invalidCheck;
  return validate(input, input.validType, input.arguments);
}
extend('matches', function (arguments) {
  if (this && typeof this.str === "string") {
    return this.str.match(arguments);
  }
  return false;
});

/**
 * Common Validators contain common validations used between the various schema models, as well as helper function
 * wrappers to generate validation error messages.  These wrappers take in an input object that should resemble this
 *
 * input {
 ...
 * }
 *
 * @type {{name: name, email: email, alphaNumName: alphaNumName, description: description, urlSafe: urlSafe, dockerId: dockerId, objectId: objectId, dockerHost: dockerHost}}
 */
module.exports.commonValidators= {
  name: function(input) {
    ...
    input.validType = 'isAlphanumeric';
