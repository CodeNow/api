...
const BaseSchema = module.exports = {
  methods: {},
  statics: {}
}
...
BaseSchema.NotFoundError = class extends BaseError {
  constructor (modelName, opts, level) {
    super(modelName + ' not found', opts, { level: level || 'debug' })
  }
}
BaseSchema.CreateFailedError = class extends BaseError {
  constructor (modelName, opts, level) {
    super(modelName + ' failed to be created', opts, { level: level || 'debug' })
  }
}
BaseSchema.NotChangedError = class extends BaseError {
  constructor (modelName, opts, level) {
    super(modelName + ' wasn\'t changed', opts, { level: level || 'debug' })
  }
}
BaseSchema.IncorrectStateError = class extends BaseError {
  constructor (modelName, expectedStatus, actualStatus, level) {
    super(
      modelName + ' expected ' + expectedStatus + ' but got ' + actualStatus,
      {
