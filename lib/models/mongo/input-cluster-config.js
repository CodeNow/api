 ...
 *
 * @param {ObjectId} inputClusterConfigId - _id of the inputClusterConfig
 *
 * @resolves {InputClusterConfig} input-cluster-config config model
 * @rejects {InputClusterConfig.NotFoundError}  if active model wasn't found
 */
InputClusterConfigSchema.statics.findSimilarActive = function (inputClusterConfigId) {
  const log = logger.child({
    method: 'findSimilarActive',
    inputClusterConfigId
  })
  log.info('called')
  return InputClusterConfig.findOneAndAssert(inputClusterConfigId)
    .then((cluster) => {
      const query = {
        repo: cluster.repo,
