  ...
  if (!dockerInfo.dockerTag) {
    ...
    ContextVersion.findOneAndUpdate({
      ...
      $set: {
        ...
        'build.log64': dockerInfo.buildLog64,
      ...
      if (duplicates.length) {
        ...
        latestDupeWithSameBranches(function (err, latestExactDupe) {
          if (err) {
            ...
            equalDates(latestDupe.build.started, latestExactDupe.build.started)) {
            // if the latest exact dupe matches the latest build's date just use it.
            cb(null, latestExactDupe);
          }
          else {
            // copy everything from latest dupe
