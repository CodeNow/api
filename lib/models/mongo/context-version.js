...
ContextVersionSchema.statics.createDeepCopy = function (user, version, cb) {
  log.trace({
    tx: true,
    sessionUser: keypather.get(user, 'accounts.github.id'),
...
/**
 * Generate a query to query for completed builds
 * @param {Boolean} buildCompleted - Whether the queried build should be completed or not
 * @returns {Object} query - query object for mongo
 */
ContextVersionSchema.statics.generateQueryForBuildCompleted = function (buildCompleted) {
  log.trace({
    tx: true,
    buildCompleted: buildCompleted
  }, 'ContextVersionSchema.statics.generateQueryForBuildCompleted')
  if (!isBoolean(buildCompleted) && (!isString(buildCompleted))) {
    throw Boom.badRequest('`buildCompleted` must be a boolean or a boolean string')
  }
  if (isString(buildCompleted)) {
    if (!includes(['true', 'false'], buildCompleted)) {
      throw Boom.badRequest('`buildCompleted` must be a boolean string')
    }
    buildCompleted = /true/.test(buildCompleted)
  }
  return {
    'build.completed': {
      $exists: buildCompleted
    }
  }
}
