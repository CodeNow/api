...
ContextVersionSchema.methods.populateOwner = function (sessionUser, cb) {
  ...
  var timer = monitorDog.timer('api.cv.populate_owner.timer')
...
ContextVersionSchema.methods.dedupeInfra = function (cb) {
  ...
  var timer = monitorDog.timer('api.cv.dedupe_infra')
...
ContextVersionSchema.methods.dedupe = function (callback) {
  ...
  var timer = monitorDog.timer('api.context-version.dedupe.timer')
  async.waterfall([
    dedupeInfra,
    function (cb) {
      var dedupeSelfTimer = monitorDog.timer('api.context-version.dedupe-self.timer')
      dedupeSelf(function () {
        dedupeSelfTimer.stop()
...
ContextVersionSchema.methods.modifyAppCodeVersionWithLatestCommit = function (user, cb) {
  ...
  var timer = monitorDog.timer('api.context-version.modifyACVWithLatestCommit.timer')
  ...
  async.each(updatableAdditionalRepos, function (acv, eachCb) {
    ...
    log.trace(logData, 'modifyAppCodeVersionWithLatestCommit finish')
    timer.stop()
    if (err) {
      return cb(err)
    }
...
ContextVersionSchema.methods.dedupeBuild = function (callback) {
  ...
  var timer = monitorDog.timer('api.cv.dedupe_build.timer')
  var self = this
