 ...
 * @returns {Object} query - query object for mongo
 */
ContextVersionSchema.statics.generateExistsQuery = function (query, keys) {
  log.trace({
    tx: true
  }, 'ContextVersionSchema.statics.generateExistsQuery')
  var resultQuery = {}
  if (!Array.isArray(keys)) {
    throw Boom.badRequest('keys must be an array of strings')
  }
  keys.forEach(function (key) {
    var val = keypather.get(query, key)
    if (isBoolean(val)) {
      resultQuery[key] = {
        $exists: val
      }
      keypather.del(query, key)
    } else if (val === null || val === undefined) {
      keypather.del(query, key)
      return
    } else {
      throw Boom.badRequest('Provided key must be a boolean or a boolean string')
    }
  })
  return resultQuery
}
