...
ContextVersionSchema.methods.setBuildDupsCompleted = function (cb) {
  ...
  if (!self.build.complete) {
    ...
    ContextVersion.findOneAndUpdate(query, {
      ...
      multi: true
    }, function(err, num, update) {
      console.log('xanand num updated', num, update);
      if (err) {
        cb(err);
      } else {
        cb(null);
...
ContextVersionSchema.methods.dedupeBuild = function (callback) {
  ...
  function findHashDupes (hash, cb) {
    ...
    ContextVersion.find(query, null, opts, function (err, duplicates) {
      if (err) { return cb(err); }
      // return if no dups
      if (duplicates.length === 0) { return cb(); }

      // use first dup and set self as started
      cb(null, duplicates[0]);
    });
  }

  function replaceIfDupe(dupeCV, cb) {
    if (typeof dupeCV === 'function') {
      cb = dupeCV;
      dupeCV = null;
    }
    if (dupeCV) {
      console.log('zanand build dedup');
      self.copyBuildFromContextVersion(dupeCV, cb);
    } else {
      self.update({
        $set: {
