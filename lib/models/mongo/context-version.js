 ...
 *         {String} props.advance
 *         {Array}  props.appCodeVersions
 * @param  {String} dockerFileContent
 * @return {ContextVersion}
 */
ContextVersionSchema.statics.createWithDockerFileContent = function (props, dockerFileContent) {
  const infraCodeVersion = new InfraCodeVersion({
    context: props.context
  })
  return infraCodeVersion.initWithDefaultsAsync()
  .then(() => {
    return infraCodeVersion.saveAsync()
  })
  .then(() => {
    return infraCodeVersion.createFsAsync({
      name: 'Dockerfile',
      path: '/',
  ...
  .then(() => {
    props.infraCodeVersion = infraCodeVersion._id
    const contextVersion = new ContextVersion(props)
    return contextVersion.saveAsync()
  })
}


ContextVersionSchema.statics.createWithNewInfraCode = function (props) {
  const log = logger.child(Object.assign({
    method: 'createWithNewInfraCode'
  }, props))
  log.info('called')
  const infraCodeVersionOpts = {
    context: props.context
  }
  if (props.parentInfraCodeVersion) {
    infraCodeVersionOpts.parent = props.parentInfraCodeVersion
  }
  ...
  return infraCodeVersion.initWithDefaultsAsync()
    .then((newInfraCodeVersion) => {
      ...
      .catch((err) => {
        log.error({ err }, 'failed to save infraCodeVersion')
    ...
    .then((savedInfraCodeVersion) => {
      ...
      return contextVersion.saveAsync()
        ...
        .catch((err) => {
          log.error({ err }, 'failed to save contextVersion')
          infraCodeVersion.bucket().removeSourceDir(noop)
          throw err
        })
    })
...
ContextVersionSchema.statics.createDeepCopy = function (user, version, cb) {
  ...
  log.trace('ContextVersionSchema.statics.createDeepCopy')
  if (version.build) {
...
ContextVersionSchema.methods.dedupe = function (callback) {
  ...
  log.info('ContextVersionSchema.methods.dedupe')
  var self = this
  if (!this.owner) {
  ...
  function dedupeInfra (cb) {
    ...
    self.dedupeInfra(function (err) {
      if (err) {
        ...
        log.trace('dudupe self.dedupeInfra success')
      }
      cb(err)
    })
  }
...
ContextVersionSchema.methods.modifyAppCodeVersionWithLatestCommit = function (user, cb) {
  ...
  log.info('modifyAppCodeVersionWithLatestCommit called')
  var self = this
...
ContextVersionSchema.statics.modifyAppCodeVersionByRepo = function (versionId, repo, branch, commit, cb) {
  ...
  log.trace('modifyAppCodeVersionByRepo called')
