...
ContextVersionSchema.methods.getGithubUserInformation = function (userAccessToken, cb) {
  if (!keypather.get(this, 'build.triggeredAction.appCodeVersion.repo')) { return cb(null, this); }
  var self = this;
  var github = new Github({ token: userAccessToken });
  var repo = self.build.triggeredAction.appCodeVersion.repo;
  var commit = self.build.triggeredAction.appCodeVersion.commit;
  github.getUsernameForCommit(repo, commit, function (err, username) {
    self.build.triggeredAction.appCodeVersion.username = username;
    cb(err, self);
  });
};

ContextVersionSchema.statics.createFirstVersionForEnv = function (props, cb) {
 ...
 * @param cb callback
 */
ContextVersionSchema.methods.setBuildStarted = function (user, dockerHost, build, cb) {
  ...
  Object.keys(build).forEach(function (key) {
    update.$set['build.'+key] = build[key];
  });

  var contextVersion = this;
  var query;

  if (build.triggeredAction.appCodeVersion) {
    query = {
      _id: contextVersion._id,
      'build.started': {
        $exists: false
      },
      'appCodeVersions.lowerRepo': build.triggeredAction.appCodeVersion.repo.toLowerCase()
    };
    update.$set['appCodeVersions.$.commit'] = build.triggeredAction.appCodeVersion.commit;
  }
  else {
    query = {
      _id: contextVersion._id,
