...
ContextVersionSchema.methods.dedupeBuild = function (callback) {
  ...
  function findPendingDupes (cb) {
    var query = {
      ...
      'build.started': { $ne: self.build.started }
    };
    query = addAppCodeVeresionQuery(self, query);
    var opts = {
      sort : 'build.started',
  ...
  function findCompletedDupes (pending, cb) {
    ...
    ContextVersion.find(query, null, opts, function (err, duplicates) {
      ...
      // if none found, no completed dups exist
      if (duplicates.length === 0) { return cb(null, pending, null); }

      // use oldest dupe
      cb(null, pending, duplicates[0]);
    });
  }
  ...
  function replaceIfDupe(pending, completed, cb) {
    if (pending) {
      console.log('xanand pending found');
      self.copyBuildFromContextVersion(pending, cb);
    } else if (completed) {
      console.log('xanand completed found');
      self.copyBuildFromContextVersion(completed, cb);
    } else {
      console.log('xanand nodupe');
      cb(null, self);
    }
  }
};
