...
ContextVersionSchema.methods.setBuildStarted = function (user, dockerHost, buildProps, cb) {
  ...
  if (buildProps.triggeredAction.appCodeVersion) {
    query.appCodeVersions = {
      lowerRepo: buildProps.triggeredAction.appCodeVersion.repo.toLowerCase()
    };
    ...
    InfraCodeVersion.findById(contextVersion.infraCodeVersion, function (err, infraCodeVersion) {
      if (err) {
        ...
        if (!infraCodeVersion.edited) {
          ...
          InfraCodeVersion.remove({ _id: infraCodeVersion._id }, cb);
        } else {
          cb();
        }
      } else {
  ...
  function setContextVersionBuildStarted(count, notUsed, cb) {
    // For some fucking reason, remove will sometimes send 1 arguments, or 2 (plus the cb).
    // Can't really tell why it does one or the other, but it's fucking awful.
    // So we do this to fix it.
    if (typeof count === 'function') {
      cb = count;
    } else if (notUsed === 'function') {
      cb = notUsed;
    }
    console.log('count', count, 'nU', notUsed, 'cb', cb);
