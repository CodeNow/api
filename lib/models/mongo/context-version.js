...
var BaseError = require('error-cat/errors/base-error')
...
/**
 * Error thrown when an contextVersion is not found
 * @param {Object} query     query made for contextVersion
 * @param {Object} data      extra error data
 * @param {Object} reporting reporting options
 */
function NotFoundError (query) {
  BaseError.call(this, 'ContextVersion not found', {
    query: query
  }, {
    level: 'debug'
  })
...
ContextVersionSchema.statics.NotFoundError = NotFoundError
 ...
 * @param {Object} reporting      reporting options
 */
function IncorrectStateError (expectedState, contextVersion) {
  const state = keypather.get(contextVersion, 'state')
  const id = keypather.get(contextVersion, 'id')
  BaseError.call(this, 'ContextVersion not in correct state', {
    actualState: state,
...
util.inherits(IncorrectStateError, BaseError)
ContextVersionSchema.statics.IncorrectStateError = IncorrectStateError
 ...
 * @throws  {NotFoundError}  If cv with contextVersionId not found
 * @throws  {IncorrectStateError} If cv not in creating state
 */
ContextVersionSchema.statics.findOneCreating = function (contextVersionId) {
  var query = {
    _id: contextVersionId
  }
  return ContextVersion.findOneAsync(query)
    .tap(function checkInstanceState (contextVersion) {
      if (!contextVersion) {
        log.error({ query: query }, 'failed to find contextVersion')
        throw new NotFoundError(query)
      }
      // state should not exist here.
      if (contextVersion.state) {
        throw new IncorrectStateError('creating', contextVersion)
      }
    })
}
 ...
 * @param {Object}   dockerInfo
 * @param {boolean}  dockerInfo.failed - true if build failed
 * @param {string}   dockerInfo.dockerImage - output image name and tag of build
...
ContextVersionSchema.statics.updateBuildCompletedByBuildId = function (buildId, dockerInfo, cb) {
  var logger = log.child({method: 'updateBuildCompletedByBuildId'})
  logger.info('ContextVersionSchema.statics.updateBuildCompletedByBuildId called')
  // The docker image is only required if the build succeeded
  if (dockerInfo.failed) {
    return ContextVersion.updateFailedBuild(buildId, dockerInfo.error.message).asCallback(cb)
  } else {
    return ContextVersion.updateSuccessfulBuild(buildId, dockerInfo.dockerImage).asCallback(cb)
  }
}

ContextVersionSchema.statics.updateFailedBuild = (buildId, errorMessage) => {
  var logger = log.child({ method: 'updateWithFailedBuild' })
  logger.info('updateWithFailedBuild called')
  ...
  return ContextVersion.updateByKeyAndEmit('build._id', buildId, update)
}

ContextVersionSchema.statics.updateSuccessfulBuild = (buildId, dockerImage) => {
  var logger = log.child({method: 'updateSuccessfulBuild'})
  logger.info('updateSuccessfulBuild called')

  var update = {
    $set: {
      'build.completed': Date.now(),
      'build.failed': false,
      'build.dockerImage': dockerImage,
...
ContextVersionSchema.statics.updateByKeyAndEmit = (property, matchValue, update) => {
  return ContextVersion.updateByAsync(property, matchValue, update, { multi: true })
    .then(() => {
      return ContextVersion.findByAsync(property, matchValue)
    })
...
ContextVersionSchema.statics.updateBuildErrorByContainer = function (dockerContainer, err, cb) {
  log.info('ContextVersionSchema.statics.updateBuildErrorByContainer')
  var now = Date.now()
  ...
  return ContextVersion.updateByKeyAndEmit('build.dockerContainer', dockerContainer, update)
}
...
ContextVersionSchema.methods.getMainAppCodeVersion = function () {
  log.trace('ContextVersionSchema.methods.getMainAppCodeVersion')
...
ContextVersionSchema.methods.updateBuildHash = function (hash, cb) {
  log.info({ hash: hash }, 'contextVersion: setHash')
  var self = this
  var query = {
    $set: {
      'build.hash': hash
...
ContextVersionSchema.methods.getAndUpdateHash = function (cb) {
  log.info('ContextVersionSchema.methods.getAndUpdateHash')
  var self = this
...
ContextVersionSchema.methods.dedupeBuild = function (callback) {
  log.info('ContextVersionSchema.methods.dedupeBuild')
  var self = this
...
ContextVersionSchema.methods.copyBuildFromContextVersion = function (dupeCv, cb) {
  ...
  log.info({
    dupeCv: dupeCv._id
  }, 'ContextVersionSchema.methods.copyBuildFromContextVersion')
...
function checkIfDedupedShouldBeUpdated (dupeCv, dedupedCv, cb) {
  ...
  dupeCv.findSelf(function (err, foundDupeCv) {
    ...
    if (!foundDupeCv) {
      log.trace(logData, 'checkIfDedupedShouldBeUpdated: not found')
...
function getSetForDedupe (deduped, dupe) {
  var dupeCopyFields = ['build', 'dockerHost', 'state']
