...
ContextVersionSchema.methods.setBuildStarted = function (user, dockerHost, buildProps, cb) {
  ...
  function findAndCheckInfraCodeEditedFlag(cb) {
    InfraCodeVersion.findById(contextVersion.infraCodeVersion, function (err, infraCodeVersion) {
      if (err) {
      ...
      } else if (infraCodeVersion.edited) {
        // Set the infracode edited field to false, since it's no longer able to be edited
        InfraCodeVersion.update({
          _id: contextVersion.infraCodeVersion
        }, {
          $set: {
            edited: false
          }
        ...
        // a parent, that means it's a source file
  ...
  function setContextVersionBuildStarted(count, notUsed, cb) {
    if (!count) {
      cb(Boom.conflict('InfraCodeVersion could not get set', {
        debug: {
          contextVersion : contextVersion._id,
          infraCodeVersion : contextVersion.infraCodeVersion
        }
  ...
  function afterSetBuildStarted(updatedContextVersion) {
