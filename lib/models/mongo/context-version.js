 ...
 * @param  {String} commitish - commit or branch name
 * @return {Object}
 * @return {String} .repo
 ...
 */
ContextVersionSchema.statics.createAppcodeVersion = function (sessionUser, repoName, commitish) {
  const log = logger.child({
    method: 'createAppcodeVersion',
    sessionUser, repoName, commitish
  })
  log.trace('called')
  const token = sessionUser.accounts.github.accessToken
  const github = new Github({ token })
  // let githubData = {
  //   githubRepoInfo: github.getRepoAsync(repoName)
  // }
  // if (commitish) {
  //   log.trace('before branch')
  //   githubData.githubBranchInfo = github.getBranchAsync(repoName, commitish)
  //   log.trace('after branch')
  // }
  log.trace('before repo')
  return github.getRepoAsync(repoName)
  .then((githubRepoInfo) => {
    if (commitish) {
      log.trace('before branch')
      return github.getBranchAsync(repoName, commitish)
  ...
  .then((resp) => {
    ...
    const additionalGithubData = {
      keys: github.createHooksAndKeys(repoName)
    }
    const defaultBranch = githubRepoInfo.default_branch
    const branchName = githubBranchInfo ? githubBranchInfo.name : defaultBranch
    if (githubBranchInfo) {
      additionalGithubData.branchInfo = Promise.resolve(githubBranchInfo)
    } else {
      additionalGithubData.branchInfo = github.getBranchAsync(repoName, defaultBranch)
    }
    return Promise.props(additionalGithubData)
    .then((githubInfo) => {
      return {
        repo: repoName,
  ...
  .catch((err) => {
    log.error({ err }, 'error get data from github')
    return {}
  })
}
...
ContextVersionSchema.statics.modifyAppCodeVersionByRepo = function (versionId, repo, branch, commit, pullRequest, cb) {
  ...
  if (pullRequest) {
    updates['appCodeVersions.$.pullRequest'] = pullRequest
  }
