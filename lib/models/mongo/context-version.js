...
ContextVersionSchema.methods.setBuildRunning = function (dockerInfo, cb) {
  var self = this;
  if (!dockerInfo.dockerTag) {
    return cb(Boom.badRequest('ContextVersion requires dockerTag'));
  }
  ContextVersion.findOneAndUpdate({
    ...
    if (!updatedContextVersion) {
      var message = self.build.started ?
        'Context version build is already built.' :
        'Context version build has not started.';
      return cb(Boom.badRequest(message, {
        debug: { self : self._id }
      }));
    }

    messenger.emitContextVersionUpdate(updatedContextVersion, 'build_running');
...
ContextVersionSchema.methods.setBuildCompleted = function (dockerInfo, cb) {
  ...
  ContextVersion.findOneAndUpdate({
    ...
    $set: {
      'build.completed' : now,
      'build.duration': now - new Date(self.build.started),
    ...
    messenger.emitContextVersionUpdate(updatedContextVersion, 'build_complete');
    cb(null, updatedContextVersion);
  });
};
...
ContextVersionSchema.methods.setBuildError = function (err, cb) {
  var self = this;
  var now = Date.now();
  var log = keypather.get(err, 'data.docker.log') || '';
  log = log.toString().replace(/\0/g, '');
  var started = self.build.started || now; // started wont be set if mavis findDock fails
  ContextVersion.findOneAndUpdate({
    _id: self._id,
    'build.started': {
      $exists: true
    ...
    messenger.emitContextVersionUpdate(updatedContextVersion, 'build_complete_err');
    cb(err, updatedContextVersion);
  });
};
