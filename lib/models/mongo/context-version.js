...
/**
 * finds build that are started with on build container created
 * preconditions:
 *   build has started
 *   build does not have a container
 *   build has not finished
 * @param {String} contextVersionId  id of context version
 * @returns {Promise}
 *   @resolves {Object} single mongoose context version model
 */
ContextVersionSchema.statics.findBuildStarted = function (contextVersionId) {
  var query = {
    '_id': contextVersionId,
  ...
  log.info({ query: query }, 'ContextVersionSchema.statics.findBuildStarted')

  return ContextVersion.findOneAsync(query)
}

/**
 * set build starting state for all builds with passed id
 * preconditions:
 *   build has not yet finished
 *   build has started
 *   state is not started
 * @param {String} buildId  id of build to update
 * @returns {Promise}
 *   @resolves {Object} number of models updated
 */
ContextVersionSchema.statics.setBuildStarting = function (buildId, dockerHost) {
  var query = {
    'build._id': buildId,
    'build.finished': {
      $exists: false
    ...
    state: { $ne: ContextVersionSchema.statics.buildStarted }
  }
  var update = {
    $set: {
  ...
  log.info({ query: query, update: update }, 'ContextVersionSchema.statics.setBuildStarting')
