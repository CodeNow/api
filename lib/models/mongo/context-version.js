...
function writeToStream (stream, logs) {
  return function (chunk, cb) {
    stream.write(logs.slice(chunk.start, chunk.end))
    setTimeout(cb)
  }
}
...
ContextVersionSchema.methods.writeLogsToPrimusStream = function (stream) {
  ...
  dogstatsd.increment('build_logs.array_length', logs.length)
  var processingTimer = monitor.timer('build_logs.processing', true)
  var chunkArray = []
  for (var x = 0; x < logs.length; x += process.env.BUILD_LOG_PER_BATCH_LIMIT) {
    var nextX = x + process.env.BUILD_LOG_PER_BATCH_LIMIT
    chunkArray.push({
      start: x,
      end: (logs.length < nextX) ? logs.length : nextX
    })
  }
  processingTimer.stop('build_logs.processing')

  var timer = monitor.timer('build_logs.streaming', true)
  async.eachSeries(chunkArray, writeToStream(stream, logs), function () {
    stream.end()
...
var copyFields = [
  ...
  'advanced',
  'dockRemoved',
  'dockRemovedNeedsUserConfirmation',
...
ContextVersionSchema.methods.findPendingDupe = function (cb) {
  ...
  ContextVersion.find(query, null, opts, function (err, duplicates) {
    ...
    log.trace(put({ oldestPending: oldestPending ? oldestPending.toJSON() : undefined }, logData),
...
ContextVersionSchema.methods.copyBuildFromContextVersion = function (dupeCv, cb) {
  ...
  self.modifySelf({ $set: $set }, function (err, dedupedCv) {
    ...
    if (!dupeCv.build.completed) {
      log.trace(put({ dedupedCv: dedupedCv.toJSON() }, logData), 'copyBuildFromContextVersion: !dupeCv.build.completed')
      // check for race condition (read checkIfDedupedShouldBeUpdated's doc)
      checkIfDedupedShouldBeUpdated(dupeCv, dedupedCv, cb)
    } else {
      log.trace(put({ dedupedCv: dedupedCv.toJSON() }, logData), 'copyBuildFromContextVersion: dupeCv.build.completed emit')
...
function checkIfDedupedShouldBeUpdated (dupeCv, dedupedCv, cb) {
  ...
  dupeCv.findSelf(function (err, foundDupeCv) {
    ...
    foundDupeCv = foundDupeCv.toJSON()
    ...
    if (hasNotChanged) {
      log.trace(put({ foundDupeCv: foundDupeCv }, logData), 'checkIfDedupedShouldBeUpdated: dupe has not changed')
    ...
    log.trace(put({ query: query, update: $set, foundDupeCv: foundDupeCv }, logData), 'checkIfDedupedShouldBeUpdated: dupe has changed')
    ContextVersion.findOneAndUpdate(query, { $set: $set }, function (err, updatedDedupedCv) {
      ...
      var updatedDedupedCvJson = updatedDedupedCv.toJSON()
      if (!updatedDedupedCv) {
        log.trace(put({ updatedDedupedCv: updatedDedupedCvJson }, logData), 'checkIfDedupedShouldBeUpdated: deduped cv was already complete')
        return cb(null, dedupedCv)
      }
      log.trace(put({ updatedDedupedCv: updatedDedupedCvJson }, logData), 'checkIfDedupedShouldBeUpdated: deduped cv was not complete and was just marked as completed')
...
/**
 * Set the dockRemoved flag to false, and set the dockRemovedNeedsUserConfirmation to true if dockRemoved was true.
 * @param {function} cb
 */
ContextVersionSchema.methods.handleRecovery = function (cb) {
  var logData = {
    tx: true,
    id: this._id
  }
  log.trace(logData, 'ContextVersionSchema.methods.handleRecovery')
  this.modifySelf({
    $set: {
      dockRemoved: false,
      dockRemovedNeedsUserConfirmation: this.dockRemoved
    }
  }, function (err, newCv) {
    if (err) {
      log.error(put({
        err: err
      }, logData), 'ContextVersionSchema.methods.handleRecovery - Error')
      return cb(err)
    }
    if (newCv) {
      log.trace(put({
        dockRemoved: newCv.dockRemoved,
        dockRemovedNeedsUserConfirmation: newCv.dockRemovedNeedsUserConfirmation
      }, logData), 'ContextVersionSchema.methods.handleRecovery - Success')
    }
    cb(null)
  })
}
