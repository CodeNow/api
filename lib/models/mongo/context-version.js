...
ContextVersionSchema.methods.setBuildCompleted = function (dockerInfo, cb) {
  ...
  if (!dockerInfo.dockerTag) {
    ...
    // TODO: remove once every context Version has hash
    if (contextVersion.hash) {
      query.hash = contextVersion.hash;
    } else {
      query._id = contextVersion._id;
    }
 ...
 */
ContextVersionSchema.methods.dedupeBuild = function (buildProps, cb) {
  var self = this;
  // TODO: remove once every context Version has hash
  if(!self.hash) {return cb(null, self); }
  // search for contextVersions with same hash and check build status
  var query = {
  ...
  ContextVersion.find(query, null, opts, function (err, duplicates) {
    if (err) { return cb(err); }
    // return if no dups
    if (duplicates.length === 0) { return cb(null, self); }

    // use first dup and set self as started
    var latestDupe = duplicates[0];
    self.setBuildStarted(latestDupe.user, latestDupe.dockerHost, buildProps, cb);
  });
};
