...
ContextVersionSchema.methods.writeLogsToPrimusStream = function (stream, cb) {
  ...
  if (isString(logs)) {
    ...
    return cb(new Error('cannot stream logs that are not strings or arrays'))
  }
  var timer = new Timer()
  timer.startTimer('build_logs.streaming', function () {
    stream.write(logs)
    stream.end()
    timer.stopTimer('build_logs.streaming', cb)
  })
}
...
/**
 * get memory limit to assign to this cv's container
 * @param {Object[]} appCodeVersions from a context version
 * @returns {String} memory in bytes to limit to
 */
ContextVersionSchema.statics.getMemoryLimit = function (appCodeVersions) {
  var logData = {
    tx: true,
    appCodeVersions: appCodeVersions
  }
  log.info(logData, 'ContextVersionSchema.statics.getMemoryLimit')

  var isRepo = ContextVersion.getMainAppCodeVersion(appCodeVersions)
  return isRepo
