...
ContextVersionSchema.methods.setBuildCompleted = function (dockerInfo, buildContainerId, cb) {
  var contextVersion = this;
...
ContextVersionSchema.methods.updateBuildError = function (err, buildContainerId, cb) {
  if (isFunction(buildContainerId)) {
    cb = buildContainerId;
    buildContainerId = null;
  }
  var contextVersion = this;
  var now = Date.now();
  ...
  if (buildContainerId) {
    $set.containerId = buildContainerId;
  }
