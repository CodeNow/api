...
ContextVersionSchema.methods.dedupeBuild = function (callback) {
  ...
  function findHashDupes (hash, cb) {
    ...
    ContextVersion.find(query, null, opts, function (err, duplicates) {
      if (err) { return cb(err); }
      // error if nothing found (should at least find this)
      if (duplicates.length === 0) { return cb(Boom.badImplementation('no contextVersions found for hash')); }

      // return if just self
      if (duplicates.length === 1) { return cb(); }
