...
ContextVersionSchema.methods.findLatestBasicVersion = function (cb) {
  var query = {
    'createdBy': this.createdBy,
    'context': this.context,
    'build.completed': {$exists: true},
    'created': {$lte: this.created},
    '_id': {$ne: this._id}, // ignore self
    'advanced': {$ne: true}
  };
  var opts = {
    sort: '-build.started',
  ...
  ContextVersion.findOne(query, opts, function (err, contextVersion) {
    // use oldest dupe (might be null)
    if (!contextVersion) {
      cb(Boom.notFound('No previous basic version found'));
    } else {
      cb(err, contextVersion);
    }
  });
