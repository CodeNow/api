...
ContextVersionSchema.methods.createDeepCopy = function (user, environmentId, cb) {
  ContextVersion.createDeepCopy(user, this, environmentId, cb);
};

ContextVersionSchema.statics.createDeepCopies = function (user, versions, environmentId, cb) {
  var copies = [];
  async.map(versions, function (version, cb) {
    version.createDeepCopy(user, environmentId, function (err, version) {
      if (err) {
...
function (user, contextVersions, contextVersionsToUpdate, cb) {
  ...
  if (!contextVersions.every(hasProps(['build']))) {
    ...
    // which ensures that the build files are locked down. If this is not the case, we cannot use it
...
ContextVersionSchema.methods.updateAppCodeVersion = function (appCodeVersionId, data, cb) {
  ...
  if (data.branch) {
    ...
    update.$set['appCodeVersions.$.lockCommit'] = false;
    update.$unset['appCodeVersions.$.commit'] = 1;
  }
  else if (data.commit) {
    update.$set['appCodeVersions.$.commit'] = data.commit;
    update.$set['appCodeVersions.$.lockCommit'] = true;
    update.$unset['appCodeVersions.$.branch'] = 1;
    update.$unset['appCodeVersions.$.lowerBranch'] = 1;
  }
  else {
