  ...
  if (!dockerInfo.dockerTag) {
    ...
    ContextVersion.findOneAndUpdate({
      ...
      if (err) {
        ...
        buildStream.endBuildStream(contextVersion._id, function() {
...
ContextVersionSchema.statics.addGithubRepoToVersion =
  ...
  ContextVersion.findOneAndUpdate({
    ...
    'appCodeVersions.lowerRepo': { $ne: repoInfo.repo.toLowerCase() }
  }, {
      ...
      async.waterfall([
        ...
        if (updateErr) {
          ...
          ContextVersion.findOneAndUpdate({
            ...
            $pull: {
              appCodeVersions: {
                lowerRepo: repoInfo.repo.toLowerCase()
              }
            }
          }, function (err, doc) {
          ...
          ContextVersion.findOneAndUpdate({
            _id: contextVersionId,
            'appCodeVersions.lowerRepo': repoInfo.repo.toLowerCase()
          }, {
              $set: {
            ...
            if (err) { cb(err); }
            else if (numDocs === 0) { cb(Boom.badImplementation('could not save deploy keys')); }
            else { cb(null); }
          });
        }
      });
...
ContextVersionSchema.methods.addGithubRepo = function (user, repoInfo, cb) {
  ...
  // before we add it, lets see if we already have it in the list of appcodeversions
  var found = find(contextVersion.appCodeVersions, hasKeypaths({'repo': repoInfo.repo}));
  if (found) {
    return cb(Boom.conflict('Github Repository already added'));
  }
  ...
  async.waterfall([
    ...
    github.addDeployKeyIfNotAlready.bind(github, repoInfo.repo),
    function (deployKeys, cb) {
      repoInfo.publicKey = deployKeys.publicKey;
      repoInfo.privateKey = deployKeys.privateKey;
      contextVersion._pushAppCodeVersion(repoInfo, cb);
    }
  ], cb);
};
