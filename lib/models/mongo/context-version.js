...
function emitIfCompleted(cv) {
  ...
  if (cv.build.completed) {
    log.trace({
      tx: true
    }, 'emitIfCompleted completed true');
    messenger.emitContextVersionUpdate(cv, 'build_completed');
  } else {
    log.trace({
      tx: true
    }, 'emitIfCompleted completed false');
  }
}
...
ContextVersionSchema.methods.dedupe = function(callback) {
  ...
  if (!this.owner) {
    ...
    error.log(Boom.badImplementation('context version owner is null during dedupe', {
      cv: this
    }));
  }
  if (this.started) {
      ...
      if (!latestDupe) {
        ...
        if (self.appCodeVersions.length === 0) {
          ...
          if (!latestDupe.owner) {
            ...
            error.log(Boom.badImplementation(msg, {
              cv: latestDupe
            }));
          }
          cb(null, latestDupe);
  ...
  function latestDupeWithSameBranches(cb) {
    query.$and.map(function(acvQuery, i) {
      if (acvQuery.appCodeVersions.$elemMatch) {
        acvQuery.appCodeVersions.$elemMatch.lowerBranch = self.appCodeVersions[i].lowerBranch;
      }
      return acvQuery;
    });
...
ContextVersionSchema.statics.updateContainerByBuildId = function(opts, cb) {
  ...
  ContextVersion.updateBy('build._id', opts.buildId, update, {
    multi: true
  }, cb);
};
...
ContextVersionSchema.statics.updateBuildErrorByBuildId = function(buildId, err, cb) {
  logger.log.info({
    tx: true
  }, 'ContextVersionSchema.statics.updateBuildErrorByBuildId');
  var now = Date.now();
...
ContextVersionSchema.statics.updateBuildErrorByContainer = function(dockerContainer, err, cb) {
  logger.log.info({
    tx: true
  }, 'ContextVersionSchema.statics.updateBuildErrorByContainer');
  var now = Date.now();
  var log = keypather.get(err, 'data.docker.log') || [];
  var update = {
    $set: {
...
ContextVersionSchema.statics.addGithubRepoToVersion = function(user, contextVersionId, repoInfo, cb) {
  ...
  ContextVersion.findOneAndUpdate({
    ...
    'appCodeVersions.lowerRepo': {
      $ne: lowerRepo
    }
  }, {
    $push: {
      appCodeVersions: repoInfo
    }
  }, function(err, doc) {
    ...
    if (err) {
      ...
      async.waterfall([
        github.getRepo.bind(github, repoInfo.repo), function(repo, headers, cb) {
...
ContextVersionSchema.methods.pullAppCodeVersion = function(appCodeVersionId, cb) {
  ...
  var contextVersion = this;
  var found = find(contextVersion.appCodeVersions, hasKeypaths({
...
ContextVersionSchema.methods.modifyAppCodeVersionWithLatestCommit = function(user, cb) {
  ...
  async.each(updatableAdditionalRepos, function(acv, eachCb) {
    ...
    github.getBranch(acv.repo, acv.branch, function(err, branch) {
      ...
      self.modifyAppCodeVersion(acv._id, {
        commit: commit
      }, eachCb);
    });
  }, function(err) {
...
ContextVersionSchema.methods.findPendingDupe = function(cb) {
  log.info({
    tx: true
  }, 'contextVersion.methods.findPendingDupe');
  var self = this;
  var query = {
    'build.completed': {
      $exists: false
  ...
  self.update(query, function(err) {
    if (err) {
      log.error({
        err: err,
        tx: true
      }, 'contextVersion: setHash error');
      return cb(err);
    }
    log.trace({
      hash: hash,
      tx: true
    }, 'contextVersion: setHash success');
