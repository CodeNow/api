...
/**
 * Finds duplicates of the given ContextVersion
 *
 * @param  {ContextVersion} contextVersion - cv with which to find a matching completed dupe
 * @param  {Boolean}        buildCompleted - True if looking for completed builds
 *
 ...
 */
function findDupes (contextVersion, buildCompleted) {
  var log = logger.log.child({
    tx: true,
    buildCompleted: buildCompleted,
    method: 'findDupes'
  })
  log.info('findDupes called')

  var query = {
    'build._id': { $ne: contextVersion.build._id }, // ignore contextVersion
    'build.completed': { $exists: buildCompleted || false },
  ...
  var opts = {
    sort: (buildCompleted) ? '-build.started' : 'build.started',
    limit: 1
  }
...
ContextVersionSchema.statics.findCompletedDupe = function (contextVersion) {
  return findDupes(contextVersion, true)
}
...
ContextVersionSchema.statics.findPendingDupe = function (contextVersion) {
  return findDupes(contextVersion, false)
    ...
    .then(function fetchPending (updatedNewCv) {
      return ContextVersion.findCompletedDupe(updatedNewCv)
        .then(function checkDupes (completedDupe) {
          ...
          if (completedDupe) {
            log.info('checkDupes: skipping, using completed duplicate')
            return completedDupe
          }
          return ContextVersion.findPendingDupe(updatedNewCv)
            .catch(function (err) {
              log.info({ err: err }, 'findPendingDupe: failed to find dupe')
              throw err
            })
        })
...
ContextVersionSchema.statics.copyBuildFromContextVersion = function (newCv, dupeCv) {
  ...
  return ContextVersion.findByIdAndUpdateAsync(newCv._id, { $set: $set })
    .catch(function (err) {
      log.error({ err: err }, 'DEDUPE COPY FAILED')
...
function getSetForDedupe (deduped, dupe) {
  ...
  if (dupe.dockerHost) {
    $set.dockerHost = dupe.dockerHost
  }
  if (dupe.containerId) { // I don't think we use this anymore
    $set.containerId = dupe.containerId
  }
