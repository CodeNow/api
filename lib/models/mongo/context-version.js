...
ContextVersionSchema.methods.writeLogsToPrimusStream = function (stream, cb) {
  ...
  var timer = new Timer()
  timer.startTimer('build_logs.fromMongo', function () {
    var logArrayOfArrays = []  // Don't push this till the loop
    //while (logs.length) {
    //  logArrayOfArrays.push(logs.splice(0, process.env.BUILD_LOG_SYNCHRONOUS_LOOP_LIMIT))
    //}
    // we want to async this so that node doesn't loop forever writing to the
    // stream.
    writeToStream(logs, function () {
      timer.stopTimer('build_logs.fromMongo', cb)
    })
    //async.eachSeries(logArrayOfArrays, writeToStream, function (err) {
    //  timer.stopTimer('build_logs.fromMongo', noop)
    //  stream.end()
    //  if (err) {
    //    log.error(put({
    //      err: err
    //    }, logData), 'writeLogsToPrimusStream failure')
    //    error.report(err)
    //  }
    //  cb(err)
    //})
    function writeToStream (logs, callback) {
...
ContextVersionSchema.methods.findPendingDupe = function (cb) {
  log.info({ tx: true }, 'contextVersion.methods.findPendingDupe')
  var self = this
  var query = {
  ...
  ContextVersion.find(query, null, opts, function (err, duplicates) {
    if (err) {
      log.error({ err: err, tx: true },
        'contextVersion.methods.findPendingDupe: CV.find error')
      return cb(err)
    }
