...
function checkIfDedupedShouldBeUpdated (dupeCv, dedupedCv, cb) {
  ...
  dupeCv.findSelf(function (err, foundDupeCv) {
    log.trace(put({ err: err, foundDupeCvId: foundDupeCv._id }, logData), 'checkIfDedupedShouldBeUpdated: findSelf')
    if (err) { return cb(err) }
    if (!foundDupeCv.build.completed) {
    ...
    ContextVersion.findOneAndUpdate(query, { $set: $set }, function (err, updatedDedupedCv) {
      log.trace(put({ err: err, updatedDedupedCvId: updatedDedupedCv._id }, logData), 'checkIfDedupedShouldBeUpdated: findSelf')
      if (err) {
        return cb(err)
      }
      if (!updatedDedupedCv) {
