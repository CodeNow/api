...
ContextVersionSchema.methods.dedupeBuild = function (callback) {
  ...
  function findCompletedDupe (pendingDupe, cb) {
    log.info('dedupeBuild: findCompletedDupe')

    // always use oldest pending duplicate if it exists
    if (pendingDupe) {
      log.info('dedupeBuild: findCompletedDupe: skipping, using pending duplicate')
  ...
  function checkOwnerMatch (dupe, cb) {
    log.info('dedupeBuild: checkOwnerMatch')
    if (
      (isObject(dupe) && isObject(dupe.owner)) &&
      (self.owner.github !== dupe.owner.github)
    ) {
      log.info('dedupeBuild: owners do not match, stopping dedupe')
      cb(null, null)
    } else {
      cb(null, dupe)
    }
  ...
  function replaceIfDupe (dupe, cb) {
    log.info('dedupeBuild: replaceIfDupe')
    if (dupe) { // dupe found
      log.info('dedupeBuild: replaceIfDupe dupe found')
      dogstatsd.increment('api.contextVersion.build.deduped')
      self.copyBuildFromContextVersion(dupe, cb)
    } else {
      log.info('dedupeBuild: replaceIfDupe no dupe')
