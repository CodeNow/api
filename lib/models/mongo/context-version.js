...
ContextVersionSchema.statics.findContextVersionById = (contextVersionId) => {
  return ContextVersion.findContextVersion({
 ...
 * @throws {ContextVersion.NotFoundError} if not found
 */
ContextVersionSchema.statics.findContextVersion = (query) => {
...
ContextVersionSchema.methods.populateOwner = function (sessionUser, cb) {
  ...
  log.trace('ContextVersionSchema.methods.populateOwner')
  var self = this
  if (!sessionUser) {
...
ContextVersionSchema.statics.findOneCreating = (contextVersionId) => {
  ...
  return ContextVersion.findContextVersion(query)
    .tap((contextVersion) => {
...
ContextVersionSchema.statics.addGithubRepoToVersion = function (user, contextVersionId, repoInfo, cb) {
  ...
  ContextVersion.findOneAndUpdate({
    ...
    if (err) {
      ...
      github.createHooksAndKeys(repoInfo.repo)
      .catch((updateErr) => {
        ...
        .then((contextVersion) => {
          if (!contextVersion) {
            return cb(Boom.badImplementation('could not remove the repo from your project'))
          }
          return cb()
        })
        .catch((err) => {
          return cb(updateErr || err)
        })
      })
      .then((opts) => {
        const deployKeys = opts.deployKeys
        const repoInfo = opts.repoInfo
        ...
        return ContextVersion.findOneAndUpdateAsync({
          ...
          $set: {
            'appCodeVersions.$.defaultBranch': repoInfo.default_branch,
        ...
        .then((contextVersion) => {
          if (!contextVersion) {
            return cb(Boom.badImplementation('could not save deploy keys'))
          }
          return cb()
        })
        .catch((err) => {
