...
ContextVersionSchema.methods.dedupe = function (callback) {
  ...
  function latestDupeWithSameBranches (cb) {
    query.$and.map(function (acvQuery, i) {
      if (acvQuery.appCodeVersions.length) {
...
ContextVersionSchema.methods.updateBuildError = function (err, cb) {
  var contextVersion = this;
  var now = Date.now();
  var dockerInfo = keypather.get(err, 'data.docker');
  var log = dockerInfo.log;

  if (dockerInfo.versionId && contextVersion._id !== dockerInfo.versionId) {
    var message = 'The wrong log is being saved this cv! logId: ' +
      dockerInfo.versionId + ' , thisId: ' + contextVersion._id;
    var doubleError = Boom.conflict(message);
    error.log(doubleError);
    log = message;
  }
  else if (log) {
