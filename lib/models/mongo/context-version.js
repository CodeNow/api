 ...
 * @param {string}   buildId         contextVersion.build._id
 * @param {string}   dockerContainer image builder container id
 * @param {string}   dockerTag       tag of image which context version is associated
 * @param {string}   dockerHost      dockerHost on which the build is building (or was built)
 * @param {Function} cb              callback
 */
ContextVersionSchema.statics.updateContainerByBuildId =
  function (buildId, dockerContainer, dockerTag, dockerHost, cb) {
    var update = {
      $set: {
        'build.dockerContainer': dockerContainer,
        'build.dockerTag': dockerTag,
        dockerHost: dockerHost,
      }
    };
    var opts = { multi: true };
    ContextVersion.updateBy('build._id', buildId, update, opts, cb);
  };
...
ContextVersionSchema.statics.updateBuildCompletedByContainer =
  function (dockerContainer, dockerInfo, cb) {
    ...
    var opts = { multi: true };
    ContextVersion.updateBy('build.dockerContainer', dockerContainer, update, opts, cb);
    ...
    ContextVersion.findBy('build.dockerContainer', dockerContainer, function (err, versions) {
      ...
      versions.forEach(function (version) {
        messenger.emitContextVersionUpdate(version, 'build_completed');
      });
    });
  };
