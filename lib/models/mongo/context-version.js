...
ContextVersionSchema.statics.updateContainerByBuildId = function (opts, cb) {
  var $set = {}
  if (opts.buildContainerId) {
    $set['build.dockerContainer'] = opts.containerIp
  }
  if (opts.tag) {
    $set['build.dockerTag'] = opts.tag
  }
  if (opts.containerIp) {
    $set['build.ipAddress'] = opts.containerIp
  }
  opts.buildId = objectId(opts.buildId)
  ContextVersion.updateBy('build._id', opts.buildId, { $set: $set }, { multi: true }, cb)
}

ContextVersionSchema.statics.findByBuildId = function (buildId, cb) {
  buildId = objectId(buildId)
  ContextVersion.findBy('build._id', buildId, cb)
}
