 ...
 * @param opts.noCache         {Boolean} Action that caused the build
 * @param domain               {Boolean} Domain to get the tid from
 ...
 *
 */
ContextVersionSchema.methods.buildSelf = function (sessionUser, opts, domain) {
  var logData = {
    tx: true,
    _id: this._id,
    opts: opts
  ...
  return Promise.try(function () {
    if (keypather.get(this, 'build.started')) {
      throw Boom.conflict('cannot build a context version that is already building or built')
    }
  }).bind(this)
    .then(function () {
      return this.modifyAppCodeVersionWithLatestCommitAsync(sessionUser)
    })
    .then(function () {
      if (!opts.noCache) {
        return this.dedupeAsync()
      }
      return this
    })
    .then(function (contextVersion) {
      if (keypather.get(contextVersion, 'build.started')) {
        ...
        if (contextVersion._id.toString() !== this._id.toString()) {
        ...
        return this.createBuildJob(sessionUser, opts, domain)
      }
    })
}

ContextVersionSchema.methods.createBuildJob = function (sessionUser, opts, domain) {
  var logData = {
    tx: true,
    _id: this._id,
    opts: opts
  ...
  return this.setBuildStartedAsync(sessionUser, opts)
    .bind(this)
    .then(function () {
      return this.populateOwnerAsync(sessionUser)
    })
    .then(function () {
      var oldBuildId = keypather.get(this, 'build._id')
      if (!opts.noCache) {
        log.trace(logData, 'METHOD: dedupBuild')
        return this.dedupeBuildAsync()
          .return(oldBuildId)
      }
      return oldBuildId
    })
    .then(function (oldBuildId) {
      if (oldBuildId.toString() === this.build._id.toString()) {
        rabbitMQ.createImageBuilderContainer({
          manualBuild: opts.triggeredAction.manual || false,
          sessionUserGithubId: sessionUser.accounts.github.id,
          ownerUsername: this.owner.username,
          contextId: this.context.toString(),
          contextVersionId: this._id.toString(),
