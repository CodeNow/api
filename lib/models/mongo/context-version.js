...
ContextVersionSchema.methods.modifyAppCodeVersionWithLatestCommit = function (user, cb) {
  ...
  var updatableAdditionalRepos = this.appCodeVersions.filter(function (acv) {
    return acv.additionalRepo;// && acv.autoUpdate;
  });
  console.log('updatableAdditionalRepos', updatableAdditionalRepos);
  // we run this sequentualy to avoid race with db
  async.eachSeries(updatableAdditionalRepos, function (acv, eachCb) {
    var github = new Github({ token: githubToken });
    github.getBranch(acv.repo, acv.branch, function (err, branch) {
      if (err) {
        return eachCb(err);
      }
      var sha = keypather.get(branch, 'commit.sha');
      console.log('fethced latest commit for the branch', sha, branch);
      eachCb(null)
    });
  }, function (err) {
    if (err) {
