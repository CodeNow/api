...
ContextVersionSchema.statics.createDeepCopyForNewUser = function (user, version, cb) {
  var ContextVersion = this;
  // If this is a public copy, and the owner of the sessionUser does not
  // 'own' it, then we need to copy the contexts, then update these new cvs

  // First, create a copy of the context
  Context.copyByIdForNewOwner(version.context, user,
    function (err, newContext) {
      var newVersion = new ContextVersion(pick(version, copyFields));
      newVersion.createdBy = {
        github: user.accounts.github.id
      };
      newVersion.context = newContext._id;
      if (!version.infraCodeVersion) {
        cb(Boom.badImplementation('version is missing infraCodeVersion'));
      }
      else {
        InfraCodeVersion.createCopyById(version.infraCodeVersion, newContext._id,
          function (err, newInfraCodeVersion) {
            if (err) {
              return cb(err);
            }
