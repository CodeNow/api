...
const log = require('logger').child({ module: 'ContextVersion' })
...
ContextVersionSchema.statics.updateBuildCompletedByBuildId = function (buildId, dockerInfo, cb) {
  ...
  var state = ContextVersion.states.buildErrored
  var required = ['failed']
  ...
  ContextVersion.updateBy('build._id', buildId, update, opts, function (err) {
    ...
    ContextVersion.findBy('build._id', buildId, {'build.log': false}, function (err, versions) {
      if (err) { return cb(err) }
        ...
        return contextVersion.populateOwnerAsync(sessionUser)
          .then(function (contextVersion) {
            rabbitMQ.createImageBuilderContainer({
              ...
              tid: keypather.get(domain, 'runnableData.tid')
            })
          })
          .return(contextVersion)
      }
