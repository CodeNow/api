...
ContextVersionSchema.methods.dedupeBuild = function (callback) {
  log.info({
    tx: true
  }, 'ContextVersionSchema.methods.dedupeBuild');
  var self = this;
...
ContextVersionSchema.methods.clearDockerHost = function (cb) {
  ...
  log.trace(logData, 'clearDockerHost');
  ContextVersion.findOneAndUpdate({
    _id: this._id
  }, {
    'dockerHost': null,
    ...
    if (err) {
      log.error(put({
        err: err
      }, logData), 'clearDockerHost findOneAndUpdate error');
      return cb(err);
    }
    log.trace(logData, 'clearDockerHost findOneAndUpdate success');
    cb(null, cv);
  });
};
