...
ContextVersionSchema.methods.findCompletedDupe = function (cb) {
  var self = this;
  var query = addAppCodeVersionQuery(self, {
 ...
 */
ContextVersionSchema.methods.setHash = function (hash, cb) {
  var self = this;
  var query = {
    $set: {
      'build.hash' : hash
...
ContextVersionSchema.methods.dedupeBuild = function (callback) {
  ...
  function getHash (cb) {
    ...
    InfraCodeVersion.findById(icvId, function (err, icv) {
      ...
      log.trace(logData, 'dedupeBuild: getHash ICV.findById success');
      icv.getHash(cb);
    });
  }
  ...
  function setHash (hash, cb) {
    log.info(put({ hash: hash }, logData), 'dedupeBuild: setHash');
    self.setHash(hash, function (err) {
      if (err) {
        log.error(put({ err: err }, logData), 'dedupeBuild: setHash error');
        return cb(err);
      }
      log.trace(put({ hash: hash }, logData), 'dedupeBuild: setHash success');
      cb();
    });
  }
  ...
  function findPendingDupes (cb) {
    ...
    self.findPendingDupe(function (err, oldest) {
      if (err) {
  ...
  function findCompletedDupes (pendingDupe, cb) {
    ...
    self.findCompletedDupe(function (err, completedDupe) {
      if (err) {
        log.info(
          put({err: err}, logData),
          'dedupeBuild: findCompletedDupes: failed to find dupe'
        );
        cb(err);
      }
