...
ContextVersionSchema.methods.setBuildCompleted = function (dockerInfo, cb) {
  ...
  else if (dockerInfo.versionId && contextVersion._id !== dockerInfo.versionId) {
...
ContextVersionSchema.methods.setBuildDupsCompleted = function (cb) {
  var self = this;
  if (!self.build.complete) {
    cb(Boom.badRequest('ContextVersion should be built'));
  }
  else {
    var query ={
      'hash': self.hash,
    ...
    query = addAppCodeVeresionQuery(self, query);

    ContextVersion.findOneAndUpdate(query, {
      $set: {
        'build.completed' : self.build.completed,
        'build.duration': self.build.duration,
        'build.dockerTag': self.build.dockerTag,
        'build.dockerImage': self.build.dockerImage,
        'build.log': self.build.log,
...
ContextVersionSchema.methods.dedupeBuild = function (callback) {
  ...
  function findPendingDupes (completed, cb) {
    var query = {
      'build.completed': { $exists: false },
      'build.hash': self.build.hash,
      'build.started': {$ne: self.started}
    };
    query = addAppCodeVeresionQuery(self, query);
    var opts = {
      sort : 'build.started',
    ...
    ContextVersion.find(query, null, opts, function (err, duplicates) {
      ...
      if (duplicates.length === 0) { return cb(null, completed, null); }

      // use oldest dupe
      cb(null, completed, duplicates[0]);
    });
  }

  // find youngest completed builds, (excluding self) which match hash and app-code
  // self is determined by started time
  function findCompletedDupes (cb) {
    var query = {
    ...
    ContextVersion.find(query, null, opts, function (err, duplicates) {
      ...
      // if none found, no completed dups exist
      if (duplicates.length === 0) { return cb(null); }
  ...
  function replaceIfDupe(completed, pending, cb) {
    if (pending) {
      self.build.dupeFound = true;
      self.copyBuildFromContextVersion(pending, cb);
    } else if (completed) {
      self.build.dupeFound = true;
      self.copyBuildFromContextVersion(completed, cb);
    } else {
      cb(null, self);
    }
  }
...
function addAppCodeVeresionQuery(contextVersion, query) {
...
ContextVersionSchema.methods.dedupe = function (callback) {
  ...
  function dedupeSelf (cb) {
    ...
    query = addAppCodeVeresionQuery(contextVersion, query);
