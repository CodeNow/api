 ...
 */
ContextVersionSchema.statics.createShallowCopies = function (contextVersions, user, cb) {
  ...
  // shouldn't ever be the case.  So error.
    ...
    async.map(oldContextVersions, function(version, cb) {
      ContextVersion.createShallowCopy(version, user, cb);
    }, cb);
  }
};
 ...
 * @param cb callback
 */
ContextVersionSchema.statics.createShallowCopy = function (version, user, cb) {
  var createdBy = {
    github: user.accounts.github.id
  };
  this.newShallowCopy(version, createdBy, function (err, newVersion) {
    if (err) { return cb(err); }
    newVersion.save(cb);
  });
};

ContextVersionSchema.statics.newShallowCopy = function (version, createdBy, cb) {
 ...
 * @param cb callback
 */
ContextVersionSchema.methods.setBuildStarted = function (user, build, cb) {
