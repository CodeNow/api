...
ContextVersionSchema.methods.getAndUpdateHash = function (cb) {
  ...
  if (self.buildDockerFilePath) {
    log.trace('Context verison has `buildDockerfilePath`. Skipping...')
    return cb()
  }
...
ContextVersionSchema.methods.getUserContainerMemoryLimit = function () {
  ...
  return ContextVersion.getUserContainerMemoryLimit(this)
}
...
ContextVersionSchema.statics.getUserContainerMemoryLimit = function (contextVersion) {
  var log = logger.child({
    ...
    userContainerMemoryInBytes: contextVersion.userContainerMemoryInBytes
  })
  log.info('getUserContainerMemoryLimit called')

  if (contextVersion.userContainerMemoryInBytes) {
    return getUserContainerMemoryLimit.userContainerMemoryInBytes
  }
...
/**
 * Error thrown contextVersion could not create deploy keys
 * @param {ContextVersion} contextVersion - contextVersion object
 * @param {String}         repo           - repository name
 */
ContextVersion.DeployKeyError = class extends BaseSchema.CreateFailedError {
  constructor (contextVersionId, repo) {
    super(
      'Could not get github keys, do you have admin access? ' + repo,
      { contextVersionId, repo },
      { level: 'debug' }
    )
  }
}
