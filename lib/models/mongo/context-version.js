 ...
 */
ContextVersionSchema.statics.dedupeBuild = function (buildProps, cb) {
  ...
  ContextVersion.find(query, null, opts, function (err, duplicates) {
    ...
    // use first dup and set this one as started
    var latestDupe = duplicates[0];
    this.setBuildStarted(latestDupe.user, latestDupe.dockerHost, buildProps, cb);
  });
