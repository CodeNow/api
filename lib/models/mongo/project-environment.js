...
module.exports = function (ProjectSchema) {
  ...
  /**
   * Removes all of the builds, contexts, versions, and icvs of the given environment
   * @param envId Id of the environment
   * @param cb
   */
  ProjectSchema.methods.deleteEnvDependents = function (envId, cb) {
    var ids = {};
    // FIXME: make this an aggregation. lessen the load on mongo
    Build.find({ environment: envId }, function (err, builds) {
      if (err) { return cb(err); }
      ...
      async.each(builds, function (build, cb) {
        build.contexts.forEach(function (context) { ids.contexts.push(context); });
        build.contextVersions.forEach(function (version) { ids.versions.push(version); });
  ...
  ProjectSchema.methods.removeEnvironments = function (cb) {
    var self = this;
    async.each(this.environments, self.deleteEnvDependents, function (err) {
      cb(err, self);
    });
  };
};
