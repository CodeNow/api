...
module.exports = function (ProjectSchema) {
  ...
  ProjectSchema.methods.removeEnvironments = function (cb) {
    ...
    async.each(this.environments, function (env, cb) {
      ...
      if (err) { return cb(err); }
      async.parallel([
        Build.remove.bind(Build, { _id: { $in: ids.builds }}),
        Context.remove.bind(Context, { _id: { $in: ids.contexts }}),
        ContextVersion.remove.bind(ContextVersion, { _id: { $in: ids.versions }}),
        InfraCodeVersion.remove.bind(InfraCodeVersion, { _id: { $in: ids.icvs }}),
      ], function (err) {
        cb(err, self);
      });
    });
