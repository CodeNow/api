 ...
 * @return {Build}
 * @throws {Build.NotFoundError} if not found
 */
BuildSchema.statics.findBuildById = (buildId) => {
  return Build.findBuild({
    _id: buildId
  })
}
 ...
 * @throws {Build.NotFoundError} if not found
 */
BuildSchema.statics.findBuild = (query) => {
  return Build.findOneAsync(query)
  .tap(function checkBuild (build) {
    if (!build) {
      log.error({ query }, 'failed to find build')
      throw new Build.NotFoundError(query)
    }
  })
}
...
BuildSchema.methods.setInProgress = function (user, cb) {
  var build = this
  if (build.started) {
    return cb(Boom.conflict('Build is already in progress'))
  } else if (build.completed) {
    return cb(Boom.conflict('Build has already completed'))
  }
...
/**
 * Error thrown when an build is not found
 * @param {Object} query - query made for instance
 * @param {Object} level - error level
 */
Build.NotFoundError = class extends BaseSchema.NotFoundError {
  constructor (query, level) {
    super('Build', query, level || 'critical')
  }
}
