...
var githubUserDataSchema = joi.object().keys({
  ...
  name: joi.string().allow(null).required(), // Not everyone has a name in GH profile (string or null)
  ...
  github.getAuthorizedUser(function (err, githubUserData) {
    ...
    github.getUserEmails(githubUser.id, function (err, emails) {
      ...
      self._verify(req.body.accessToken, undefined, profile, function (err, user, info) {
        let githubId = keypather.get(user, 'accounts.github.id')
        if (err) {
          logger.log.warn({ profile, user, info, err }, 'Error verifying user')
          return self.error(err)
        }
        if (!user) {
          logger.log.warn({ profile, user, info }, 'No user found')
          return self.fail(info)
        }
        return UserService.getUser({ accounts: { github: githubId } })
          .asCallback(function (err, bigPoppaUser) {
            if (err) {
              logger.log.warn({ profile, user, bigPoppaUser, err }, 'Error getting bigpoppa user')
              return self.error(err)
            }
            user.bp = bigPoppaUser
