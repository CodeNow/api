...
exports.clusterCreated = joi.object({
  ...
  parsedCompose: joi.object({
    results: joi.array().items(joi.object({}).unknown()).required().min(0)
  }).required(),
  sessionUserBigPoppaId: joi.number().required(),
  orgBigPoppaId: joi.number().required(),
  triggeredAction: joi.string().required().valid('user', 'webhook'),
  repoFullName: joi.string().required()
}).unknown().required()

exports.clusterParentInstanceCreated =
exports.clusterSiblingInstanceCreated = joi.object({
  cluster: joi.object({
    id: joi.string().required()
  }).unknown().required(),
  instance: joi.object({
...
exports.clusterSiblingInstanceCreate = joi.object({
  cluster: joi.object({
    id: joi.string().required()
  }).unknown().required(),
  parsedComposeSiblingData: joi.object({}).unknown().required(),
  sessionUserBigPoppaId: joi.number().required(),
  orgBigPoppaId: joi.number().required(),
  triggeredAction: joi.string().required().valid('user', 'webhook'),
  repoFullName: joi.string().required()
