...
/**
 * create an org-whitelisted job and insert it into queue
 * @param {Object} data
 */
RabbitMQ.prototype.publishOrgWhitelisted = function (data) {
  log.info('RabbitMQ.prototype.publishOrgWhitelisted');
  var requiredKeys = ['orgId'];
  if (!hasKeypaths(data, requiredKeys)) {
    log.error({
      tx: true,
      data: data,
      requiredKeys: requiredKeys
    }, 'RabbitMQ.prototype.publishOrgWhitelisted missing required keys');
    throw Boom.badRequest('missing required keys');
  }
  this.hermesClient.publish('org-whitelisted', data);
};
