...
RabbitMQ.prototype.connect = function (cb) {
  ...
  this.hermesClient = require('hermes-private').hermesSingletonFactory(opts);
  this.hermesClient.on('error', function (err) {
    log.error({
      tx: true,
      err: err
...
RabbitMQ.prototype.deployInstance = function (data) {
  ...
  if (!data.instanceId && !data.buildId) {
    // Must have at least one of these
    log.error({
      tx: true,
      data: data,
      requiredKeys: 'buildId or instanceId'
    }, 'RabbitMQ.prototype.deployInstance missing buildId or instanceId');
    return;
  }
  var requiredKeys = [
    'sessionUserGithubId',
