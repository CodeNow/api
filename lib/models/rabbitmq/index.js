...
RabbitMQ.prototype.connect = function (cb) {
  var opts = {
    ...
    queues: [
      ...
      'isolation.kill',
      'isolation.redeploy',
      'isolation.match-commit-with-master',
      'isolation.match-updated-properties',
      'metis-github-event',
...
RabbitMQ.prototype.loadWorkers = function () {
  ...
  var ponosTasks = this.ponosTasks = {
    ...
    'isolation.match-commit-with-master': require('workers/isolation.match-commit-with-master'),
    'isolation.match-updated-properties': require('workers/isolation.match-updated-properties'),
...
/**
 * Create a 'isolation.match-commit-with-master' job
 *
 ...
 */
RabbitMQ.prototype.matchCommitWithIsolationMaster = function (data) {
  log.info({
    tx: true,
    data: data
  ...
  this.hermesClient.publish('isolation.match-commit-with-master', data)
}

/**
 * Create a 'isolation.match-updated-properties' job
 *
 * Given an isolation group, match the all the provided properties for all intances
 * in an isolation that have the same repo
 *
 * @param {Object}  data
 * @param {String}  data.isolationId         - Id of the isolation model
 * @param {String}  data.repo                - Name of repo
 * @param {Object}  data.updates             - Updates to be applied to all instances in isolation with same repo
 * @param {Number}  data.sessionUserGithubId - GitHub ID of the session user
 *
 */
RabbitMQ.prototype.matchUpdatedPropertiesForIsolation = function (data) {
  log.info({
    tx: true,
    data: data
  }, 'RabbitMQ.prototype.matchUpdatedPropertiesForIsolation')
  var requiredKeys = [
    'isolationId',
    'repo',
    'updates',
  ...
  this._validate(data, requiredKeys, 'matchUpdatedPropertiesForIsolation')
  this.hermesClient.publish('isolation.match-updated-properties', data)
}
