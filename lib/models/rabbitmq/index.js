...
RabbitMQ.prototype.connect = function (cb) {
  ...
  // hack: singleton connects in the constructor
  // the first time. second time we check if
  // _conn exists it has been previously connected
  // so call connect;
  if (this.hermesClient._connection) {
    this.hermesClient.connect(opts);
  }
  if (isFunction(cb)) {
    this.hermesClient.once('ready', cb);
  }
};
