...
Sauron.createWithAnyHost = function (cb) {
  log.info(this.logData, 'Sauron.createWithAnyHost');
  var self = this;
  var mavis = new Mavis();
  mavis.findDockForNetwork(function (err, dock) {
    if (err) {
      log.error(put({
        err: err
      }, self.logData), 'createWithAnyHost mavis.findDockerForMavis error');
      return cb(err);
    }
    var host = url.parse(dock).hostname; // no port
    log.trace(put({
      dock: dock
    }, self.logDat), 'createWithAnyHost mavis.findDockerForNetwork success');
...
Sauron.deleteHostFromContextVersion = function (cv, cb) {
  log.info(put({
    cv: cv
  }, this.logData), 'Sauron.deleteHostFromContextVersion');
...
Sauron.prototype.detachHostFromContainer = function (networkIp, hostIp, containerId, cb) {
  ...
  log.info(logData, 'Sauron.prototype.detachHostFromContainer');
  if (this.hostNotProvided) {
    return cb(new Error('Host must be provided (container host) it cannot be random'));
  }
