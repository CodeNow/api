...
Sauron.deleteHostFromContextVersion = function(cv, cb) {
  ...
  log.info(logData, 'Sauron.deleteHostFromContextVersion');
 ...
 * @param  {Function} cb          callback
 */
Sauron.prototype.attachHostToContainer = function(networkIp, hostIp, containerId, ignoreIpMappedToDiffErr, cb) {
  var logData = put({
    networkIp: networkIp,
    hostIp: hostIp,
    containerId: containerId,
...
Sauron.prototype.detachHostFromContainer = function(networkIp, hostIp, containerId, cb) {
  ...
  this.client.detachHostFromContainer(networkIp, hostIp, {
    ...
    if (err &&
      ...
      log.error(put({
        err: err
      }, logData), 'Sauron.prototype.detachHostFromContainer failed');
      return cb(err);
    }
    cb();
  }, 'Host detach failed', {
