...
Sauron.createWithAnyHost = function (cb) {
  log.info({tx: true}, 'Sauron.createWithAnyHost');
  var mavis = new Mavis();
  mavis.findDockForNetwork(function (err, dock) {
    if (err) {
      log.error({
        tx: true,
        err: err
      }, 'createWithAnyHost mavis.findDockerForMavis error');
      return cb(err);
    }
    ...
    log.trace({
      ...
      dock: dock,
      host: host
    }, 'createWithAnyHost mavis.findDockerForNetwork success');
...
Sauron.prototype.createNetwork = function (cb) {
  log.trace({tx: true}, 'Sauron.prototype.createNetwork');
...
Sauron.prototype.deleteNetwork = function (networkIp, cb) {
  log.trace({tx: true}, 'Sauron.prototype.deleteNetwork');
...
Sauron.prototype.createHost = function (networkIp, cb) {
  log.trace({tx: true}, 'Sauron.prototype.createHost');
...
Sauron.prototype.getContainerIp = function (containerId, cb) {
  ...
  }, 'Sauron.prototype.getContainerIp');
...
Sauron.prototype.detachHostFromContainer = function (networkIp, hostIp, containerId, cb) {
  ...
  log.trace(logData, 'Sauron.prototype.detachHostFromContainer');
  if (this.hostNotProvided) {
    return cb(new Error('Host must be provided (container host) it cannot be random'));
  }
  this.client.detachHostFromContainer(networkIp, hostIp, {
    ...
    if (err &&
      ...
      log.error(put({
        err: err }, logData), 'Sauron.prototype.detachHostFromContainer failed');
      return cb(err);
    }
    cb();
...
Sauron.prototype.unavailableErr = function (urlPath, message, err) {
  log.error({
    ...
    message: message,
    err: err
  }, 'Sauron.prototype.unavailableErr');
...
Sauron.prototype.findOrCreateHostForInstance = function (instance, cb) {
  log.trace({
    tx: true,
    instance: instance
  }, 'Sauron.prototype.findOrCreateHostForInstance');
  var self = this;
...
Sauron.prototype.findOrCreateHostForContext = function (context, cb) {
  log.trace({
    tx: true,
    context: context
  }, 'Sauron.prototype.findOrCreateHostForContext');
  var self = this;
...
Sauron.prototype.createOrFindNetwork = function (owner, cb) {
  log.trace({
    tx: true,
    owner: owner
  }, 'Sauron.prototype.createOrFindNetwork');
  var self = this;
