...
function parseCacheControl(str) {
  ...
  var params = str.split(',').map(function(v) {
    return v.trim();
  });
  var ret = {};
...
Github.prototype._listRepoHooks = function(shortRepo, cb) {
  ...
  this.repos.getHooks(query, function(err, hooks) {
    if (err) {
      ...
      err = (err.code === 404) ?
        Boom.notFound('Github repo ' + shortRepo + ' not found.', {
          err: err
        }) :
        Boom.create(502, 'Failed to get github repo hooks for ' + shortRepo, {
          err: err
        });
      return cb(err);
    }
  ...
  this.repos.createHook(query, function(err, hook) {
    ...
    if (err) {
      err = (err.code === 404) ?
        ...
        Boom.create(502, 'Failed to create github repo hook for ' + shortRepo, {
          err: err
        });
      cb(err);
    } else {
...
Github.prototype._updateRepoHook = function(hookId, shortRepo, cb) {
  ...
  this.repos.updateHook(query, function(err, hook) {
    if (err) {
      ...
      err = (err.code === 404) ?
        Boom.notFound('Github repo hook ' + hookId + ' not found.', {
          err: err
        }) :
        Boom.create(502, 'Failed to update github repo hook with id ' + hookId, {
          err: err
        });
      return cb(err);
    }
  ...
  this.repos.deleteHook(query, function(err, hook) {
    if (err) {
      err = (err.code === 404) ?
        ...
        Boom.create(502, 'Failed to delete github repo hook with id ' + hookId, {
          err: err
        });
      logger.log.error({
        tx: true,
...
Github.prototype.createRepoHookIfNotAlready = function(shortRepo, cb) {
  ...
  var hookUrl = process.env.FULL_API_DOMAIN + process.env.GITHUB_HOOK_PATH;
  async.waterfall([
    this._listRepoHooks.bind(this, shortRepo), function subscribeToHooksIfNotAlready(existingHooks, cb) {
...
Github.prototype.isOrgMember = function(orgName, cb) {
  ...
  var notFoundError = Boom.notFound('user is not a member of org', {
    org: orgName
  });
  /* jshint maxcomplexity:6 */
  this.getUserAuthorizedOrgs(function(err, orgs) {
    if (err) {
      ...
      return cb(Boom.create(502, 'failed to get user orgs', {
        err: err
      }));
    }
...
Github.prototype.addDeployKey = function(repo, cb) {
  ...
  var self = this;
  var split = repo.split('/');
  async.waterfall([function getKeypair(cb) {
...
Github.prototype.addDeployKeyIfNotAlready = function(repo, cb) {
  ...
  var self = this;
  async.waterfall([
    self.checkForDeployKey.bind(self, repo), function(key, cb) {
      if (!key) {
        self.addDeployKey(repo, cb);
      } else {
        cb(null);
      }
    },
