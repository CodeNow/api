...
Github.prototype.addComment = function(shortRepo, issueId, text, cb) {
  debug('addComment', formatArgs(arguments));
  var split = shortRepo.split('/');
  var query = {
    user: split[0],
    repo: split[1],
...
Github.prototype.updateComment = function(shortRepo, commentId, text, cb) {
  debug('updateComment', formatArgs(arguments));
  var split = shortRepo.split('/');
  var query = {
    user: split[0],
    repo: split[1],
  ...
  this.issues.editComment(query, function (err, comment) {
    debug('back from updating issue comment', err);
    if (err) {
      err = (err.code === 404) ?
        Boom.notFound('Github comment ' + commentId + ' not found.', { err: err }) :
        Boom.create(502, 'Failed to update github comment id' + commentId , { err: err });
      cb(err);
    }
    else {
      cb(null, comment);
...
Github.prototype.deleteComment = function(shortRepo, commentId, cb) {
  debug('deleteComment', formatArgs(arguments));
  var split = shortRepo.split('/');
  var query = {
    user: split[0],
    repo: split[1],
  ...
  this.issues.deleteComment(query, function (err) {
    debug('back from deleting issue comment', err);
    if (err) {
      err = (err.code === 404) ?
        Boom.notFound('Github comment ' + commentId + ' not found.', { err: err }) :
        Boom.create(502, 'Failed to delete github comment id' + commentId , { err: err });
      cb(err);
    }
    else {
      cb(null);
...
Github.prototype.listComments = function (shortRepo, issueId, cb) {
  debug('listComments', formatArgs(arguments));
  var split = shortRepo.split('/');
  var query = {
    user: split[0],
    repo: split[1],
  ...
  this.issues.getComments(query, function (err, comments) {
    debug('back from listing comments');
    if (err) {
      err = (err.code === 404) ?
...
Github.prototype.findCommentByUser = function (shortRepo, issueId, githubUsername, cb) {
  debug('findCommentByUser', formatArgs(arguments));
...
Github.prototype.getPullRequest = function (shortRepo, number, cb) {
  ...
  this.pullRequests.get(query, function (err, pr) {
    if (err) {
      err = (err.code === 404) ?
        Boom.notFound('Cannot find PRs for ' + shortRepo, { err: err }) :
        Boom.create(502, 'Failed to get PR for ' + shortRepo, { err: err });
      cb(err);
    }
    else {
      cb(null, pr);
...
Github.prototype.createDeployment = function (shortRepo, ref, payload, cb) {
  ...
  var query = {
    ...
    description: 'deploying code to the runnable.io sandbox'
  };
...
Github.prototype.createDeploymentStatus = function (shortRepo, deploymentId, opts, cb) {
  debug('createDeploymentStatus', formatArgs(arguments));
  var split = shortRepo.split('/');
  opts.user =  split[0];
  opts.repo = split[1];
  opts.id = deploymentId;
  opts.description = 'runnable is working';
  this.deployments.createStatus(opts, function (err, deployment) {
    if (err) {
      err = (err.code === 404) ?
...
Github.prototype.createBuildStatus = function (shortRepo, sha, opts, cb) {
  debug('createBuildStatus', formatArgs(arguments));
  var split = shortRepo.split('/');
  opts.user =  split[0];
  opts.repo = split[1];
  opts.sha = sha;
  this.statuses.create(opts, function (err, status) {
    if (err) {
      err = (err.code === 404) ?
...
Github.prototype.isCollaborator = function (shortRepo, username, cb) {
  ...
  this.repos.getCollaborator(query, function (err) {
    if (err) {
      if (err.code === 404) {
