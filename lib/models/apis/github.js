...
Github.prototype.getPullRequestHeadCommit = function (shortRepo, number, cb) {
  ...
  this.pullRequests.getCommits(query, function (err, commit) {
    if (err) {
      err = (err.code === 404) ?
        Boom.notFound('Cannot find PRs for ' + shortRepo + ' and #'  + number, { err: err }) :
        Boom.create(502, 'Failed to get PR for ' + shortRepo + ' and #'  + number, { err: err });
      cb(err);
    }
    else {
      cb(null, commit);
