...
var logger = require('middlewares/logger')('models:apis:github');
...
Github.prototype.isPublicRepo = function (repo, cb) {
  logger.log.trace({tid: true, repo: repo}, 'isPublicRepo');
...
Github.prototype.isOrgMember = function (orgName, cb) {
  ...
  this.user.getOrgMembership({ org: orgName }, function (err, membership) {
    if (err) {
      if (err.code === 404) {
        return cb(Boom.notFound('user is not a member of org', { org: orgName }));
      }
