 ...
 * @private
 */
Github.prototype._getAllOrgs = function (opts, block, cb) {
  logger.log.info('Github.prototype._getAllOrgs')
  ...
  const fetchPage = (page) => {
    logger.log.info('Github.prototype._getAllOrgs.fetchPage')
    return Promise.fromCallback((cb) => {
      logger.log.info('Github.prototype._getAllOrgs.fetchPage.Promise.fromCallback')
      let res = this.user.getOrgs(Object.assign({}, opts, {page}), block, cb)
      logger.log.info(res)
      logger.log.info("got res")
      return res
    })
      .then((response) => {
        logger.log.info('Github.prototype._getAllOrgs.fetchPage.then')
        allOrgs = allOrgs.concat(response.data)
        if (response.data.length === opts.per_page) {
          return fetchPage(page + 1)
        }

        logger.log.info('Github.prototype._getAllOrgs.fetchPage.then final')
        logger.log.info(allOrgs)
        return allOrgs
      })
      .catch((err) => {
        logger.log.info("there was an error")
        logger.log.info(err)
      })
  }
