...
Github.prototype.getRepoContent = function (repo, fullPath, commitRef, cb) {
  let cacheRef
  if (!cb && isFunction(commitRef)) {
    cb = commitRef
    commitRef = undefined
    cacheRef = ''
  } else {
    cacheRef = ':ref:' + commitRef
  }
  logger.log.info({
    repo,
    fullPath,
  ...
  var split = repo.split('/')
  var userKey = keyPrefix + (this.token ? this.tokenHash : 'runnable')
  var usernameKey = new redisTypes.String(userKey + ':getRepo:' + repo + ':content:' + fullPath + cacheRef)
  this._runQueryAgainstCache({
