...
Github.prototype._updateRepoHook = function (hookId, cb) {
  debug('_updateRepoHook');
  var split = shortRepo.split('/');
  var query = {
    id: hookId,
    events: ['*']
  ...
  this.repos.updateHook(query, function (err, hook) {
    ...
    if (err) {
      err = (err.code === 404) ?
        Boom.notFound('Github repo hook ' + shortRepo + ' not found.', { err: err }) :
    ...
    function subscribeToHooksIfNotAlready (existingHooks, cb) {
      ...
      if (hookExists) {
        if (hookExists.events[0] === '*') {
          ...
          self._updateRepoHook(hookExists.id, cb);
        }
      }
      else {
