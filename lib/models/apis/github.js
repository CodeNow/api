...
Github.prototype.updateCommentForUser = function (shortRepo, issueId, githubUserId, text, cb) {
  debug('listComments', formatArgs(arguments));
  var self = this;
  var split = shortRepo.split('/');
  var query = {
  ...
  this.issues.listComments(query, function (err, comments) {
    if (err) {
      return cb(err);
    }
    var comment = find(comments, hasProps({ 'user.id': githubUserId }));
    self.updateComment(shortRepo, comment.id, text, cb);
  });
};
