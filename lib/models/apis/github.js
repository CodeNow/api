...
Github.prototype.getRepo = function (repo, cb) {
  ...
  return this.repos.get(opts, this.errorHandler(cb, opts))
}
...
Github.prototype.getRepoContent = function (repo, fullPath, commitRef) {
  ...
  return Promise.fromCallback(cb => this.repos.getContent(opts, cb))
    .catch((err) => {
      if (err.isBoom && err.output.statusCode === 404) {
        throw Boom.notFound('Unable to find file on github: ' + repo + fullPath + ' for commit ' + commitRef, { err })
      }
      throw err
    })
}
...
Github.prototype.getDeployKeys = function (repo, cb) {
  ...
  this.repos.getKeys(opts, this.errorHandler(cb, opts))
}
...
Github.prototype.getCommit = function (repo, commit, cb) {
  ...
  return this.repos.getCommit(opts, this.errorHandler(cb, opts))
}
...
Github.prototype._listRepoHooks = function (shortRepo, cb) {
  ...
  this.repos.getHooks(query, (err, hooks) => {
    if (err) {
      ...
      err = (err.code === 404)
        ? Boom.notFound('Github repo ' + shortRepo + ' not found.', { err })
        : Boom.create(502, 'Failed to get github repo hooks for ' + shortRepo, { err })
      return cb(err)
    }
...
Github.prototype._createRepoHook = function (shortRepo, cb) {
  ...
  this.repos.createHook(query, (err, hook) => {
    if (err) {
      ...
      if (err.code === 404) {
        return cb(Boom.notFound('Github repo ' + shortRepo + ' not found.', { err }))
      }
...
Github.prototype._updateRepoHook = function (hookId, shortRepo, cb) {
  ...
  this.repos.updateHook(query, (err, hook) => {
    if (err) {
      ...
      err = (err.code === 404)
        ? Boom.notFound('Github repo hook ' + hookId + ' not found.', { err })
        : Boom.create(502, 'Failed to update github repo hook with id ' + hookId, { err })
      return cb(err)
    }
    this.logger.trace({
      hookId: hookId
...
Github.prototype._deleteRepoHook = function (hookId, shortRepo, cb) {
  ...
  this.repos.deleteHook(query, (err, hook) => {
    if (err) {
      err = (err.code === 404)
        ? Boom.notFound('Github repo hook ' + hookId + ' not found.', { err })
        : Boom.create(502, 'Failed to delete github repo hook with id ' + hookId, { err })
      this.logger.error({
        hookId: hookId,
        err
...
Github.prototype.isOrgMember = function (orgName, cb) {
  ...
  this.getUserAuthorizedOrgs((err, orgs) => {
    if (err) {
      ...
      return cb(Boom.create(502, 'failed to get user orgs', { err }))
    }
...
Github.prototype.errorHandler = function (cb, loggingData) {
  const log = this.logger.child({
    method: 'Github.prototype.errorHandler',
    loggingData
  })
  return (err, data) => {
    if (err) {
      log.trace({ err }, 'github error')
      if (err.code && err.message) {
        return cb(Boom.create(err.code, err.message, { err }))
      }
      return cb(err)
    }
    return cb(null, data)
