...
Github.prototype._createRepoHook = function (shortRepo, cb) {
  ...
  this.repos.createHook(query, function (err, hook) {
    ...
    if (err) {
      ...
      if (err.code === 422 && keypather.get(err, 'errors[0].message') ===
...
Github.prototype.createRepoHookIfNotAlready = function (shortRepo, cb) {
  ...
  async.waterfall([
    ...
    function subscribeToHooksIfNotAlready (existingHooks, hookCb) {
      ...
      if (hookExists) {
        return hookCb(null, hookExists);
      }
      self._createRepoHook(shortRepo, hookCb);
    }
  ], function (err) {
    logger.log.trace({
      tx: true,
