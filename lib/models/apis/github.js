...
Github.prototype._runQueryAgainstCache = function (options, cb) {
  ...
  async.waterfall([
    ...
    debug('got a 304 - not changed');debug('got a 304 - not changed');debug('got a 304 - not changed');debug('got a 304 - not changed');
  ...
  function updateCachedQueryData (queryData, cacheIsValid, cb) {
    ...
    if (cacheIsValid) {
      debug('got a 304 -11111111 not changed');
      // pass along the data if the cache was valid
      cb(null, queryData.data);
    } else {
      debug('got a 304 12121- not changed');
