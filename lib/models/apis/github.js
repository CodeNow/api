...
function Github (opts) {
  ...
  if (opts.token) {
    logger.log.info({
      tx: true
    }, 'Github authenticate with token')
    ...
    logger.log.info({
      tx: true
    }, 'Github authenticate without token')
    this.authenticate({
      type: 'oauth',
...
Github.prototype.getUserEmails = function (userId, cb) {
  logger.log.info({
    tx: true
  }, 'Github.prototype.getUserEmails')
  if (!this.token) {
...
Github.prototype.checkUserOrgMembership = function (githubOrgName, githubUserName, cb) {
  logger.log.info({
    tx: true
  }, 'Github.prototype.checkUserOrgMembership')
  if (!this.token) {
    var errorMsg = 'checkUserOrgMembership should only be called with a user token'
    return cb(Boom.badImplementation(errorMsg))
  }
  var userKey = keyPrefix + (this.token ? this.tokenHash : 'runnable')
  var userOrgsKey = new redisTypes.String(userKey + ':user:' + this.token + ':org')
  this._runQueryAgainstCache({
    query: this.orgs.getMember,
    debug: 'this.orgs.checkUserOrgMembership',
    stringKey: userOrgsKey,
    opts: {
      org: githubOrgName,
      user: githubUserName
