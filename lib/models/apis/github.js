...
Github.prototype._getAllOrgs = function (opts, block, cb) {
  const getOrgs = this.user.getOrgs

  opts = opts || {}
  opts.per_page = opts.per_page || 100
  // let allOrgs = []

  function fetchPage (page) {
    getOrgs(opts, block, cb)
  }
  ...
  //   options.page = page
  //
  //   return getOrgs(options, block, (err, response) => {
  //     if (err) {
  //       return cb(err)
  //     }
  //
