...
Github.prototype._createRepoHook = function (shortRepo, cb) {
  ...
  this.repos.createHook(query, function (err, hook) {
    ...
    if (err) {
      ...
      if (err.code === 422) {
        var message = keypather.get(err, 'errors[0].message');
        if (message === 'Hook already exists on this repository') {
          return cb(Boom.conflict('Github repo ' + shortRepo + ' already has a hook.', { err: err }));
        }
      }
      return cb(Boom.create(502, 'Failed to create github repo hook for ' + shortRepo , { err: err }));
    }
    cb(null, hook);
  });
};
