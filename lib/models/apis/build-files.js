...
BuildFiles.prototype.getObject = function (key, version, etag, cb) {
  ...
  this.s3.headObject(objectParams, function (err, data) {
    ...
    if (data.ContentLength > process.env.MAX_FILE_DOWNLOAD) {
      var boom = Boom.create(413, 'Requested file is too large.')
      boom.statusCode = 413
      return cb(boom)
    } else {
...
/**
 * Returns streaming version of object.
 * @param  {string}   key     Short key for the file
 * @param  {string}   version The versionId
 * @param  {string}   etag    Etag for the file
 * @return {Stream}           The file stream
 */
BuildFiles.prototype.getObjectStream = function (key, version, etag) {
  var data = {
    Bucket: this.bucket,
    Key: path.join(this.sourcePath, key)
  }
  ...
  return this.s3.getObject(data).createReadStream()
}
