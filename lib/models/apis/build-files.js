  ...
  upload
    ...
    .send(function (err, data) {
      if (err) {
        ...
        error.log(uploadErr);
        dogstatsd.increment('api.build-files.put-file-stream.err');
        return cb(uploadErr);
      }
      if (!data) {
        var noFileErr = Boom.create(500, 'Failed was not saved to s3', { key: key });
        return cb(noFileErr);
      }
      data.Key = fileKey;
      logger.log.trace({tx: true, key: key}, 'putFileStream:uploadFinished');
      dogstatsd.increment('api.build-files.put-file-stream.success');
      cb(null, data);
    });
};
