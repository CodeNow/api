...
BuildFiles.prototype.moveDir = function (sourceDirKey, sourceFsArr, destDirKey, cb) {
  ...
  async.map(sourceFsArr, function (fsModel, cb) {
    var destKey = path.join(fsModel.path, fsModel.name).replace(sourceDirKey, destDirKey);
    self.moveObject(sourceDirKey, fsModel.VersionId, destKey, function (err, newData) {
      if (err) { return cb(err); }
      fsModel.set(newData);
      cb(null, fsModel);
    });
