...
BuildFiles.prototype.copyObject = function (sourceKey, version, destKey, cb) {
  ...
  var self = this;
  var prefixLength = self.sourcePath.length;
  self.getObject(sourceKey.slice(prefixLength), version, null, function (err, data) {
    if (err) { return cb(err); }
    self.putFile(destKey.slice(prefixLength), data.Body.toString(), cb);
  });
};
...
BuildFiles.prototype.copyFileFrom = function (sourceKey, version, cb) {
  var destKey = path.join(this.sourcePath, sourceKey.slice(this.sourcePath.length));
  this.copyObject(
    sourceKey,
    version,
    destKey,
    error.wrapIfErr(cb, 502, 'Could not copy file: ' + sourceKey + '->' + destKey));
};
