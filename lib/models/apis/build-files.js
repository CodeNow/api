...
BuildFiles.prototype.putFileStream = function (key, stream, cb) {
  logger.log.trace({tx: true, key: key}, 'putFileStream');
  ...
  var params = {
    Body: stream,
    partSize: 5 * 1024 * 1024 // partSize is 5MB
  };
  s3.upload(params).
    on('httpUploadProgress', function (evt) {
      logger.log.trace({tx: true, key: key, evt: evt}, 'putFileStream:httpUploadProgress');
    }).
    send(function (err, data) {
      if (err) {
        var uploadErr = Boom.create(500, 'Failed to upload file to s3', { s3: err, key: key }));
        datadog.increment('api.build-files.put-file-stream.err');
        return cb(uploadErr);
      }
      if (!data) {
        var uploadErr = Boom.create(500, 'Failed wasnot saved to s3', { key: key }));
        return cb(uploadErr);
      }
      data.Key = fileKey;
      logger.log.trace({tx: true, key: key, evt: evt}, 'putFileStream:uploadFinished');
      datadog.increment('api.build-files.put-file-stream.success');
      cb(null, data);
    });
};
