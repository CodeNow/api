...
var Uploader = require('s3-stream-upload');
...
BuildFiles.prototype.putFileStream = function (key, stream, cb) {
  ...
  var u = stream.pipe(upload({ Key: fileKey }));
  u.on('error', cb);
  u._uploader.once('finish', function (res) {
    try {
...
BuildFiles.prototype.copyObject = function (sourceKey, version, destKey, cb) {
  ...
  var request = this.s3.getObject(data);
  request.send();
  this.putFileStream(destKey, request.createReadStream(), cb);

  // this.s3.getObject(data, function (err, data) {
  //   if (err) { return cb(err); }
  //   self.putFileStream(destKey, data.Body, cb);
  // });
};
