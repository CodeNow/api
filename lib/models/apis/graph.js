...
process.nextTick(function () {
  Instance = require('models/mongo/instance');
});
...
function Graph (dbType) {
  ...
  if (dbType === 'cayley') {
    var Cayley = require('models/graph/cayley');
    this.graph = new Cayley();
...
Graph.prototype.getInstanceDeps = function (instance, cb) {
...
Graph.prototype.graphInstanceDeps = function (instance, oldName, cb) {
  ...
  function getAllInstanceNamesRegex (data, cb) {
    ...
    data.nameRegex = new RegExp('([a-z0-9-_]+)\\.' + process.env.USER_CONTENT_DOMAIN);
    Instance.find(
      {
        owner: {
  ...
  function extendWriteAndDeleteArrays (data, cb) {
    ...
    self.graph.getNodes(
      instanceOwnerAndName,
      [
        { Out: 'dependsOn' }
      ],
      function (err, currentDepNames) {
        if (err) { return cb(err); }
  ...
  function writeChangesToCayley (data, cb) {
    ...
    async.series([
      self.graph.deleteConnections.bind(self.graph, deletes),
      self.graph.writeConnections.bind(self.graph, writes)
    ], function (err) {
      cb(err, data);
    });
  }
  ...
  function resetMongoCacheForInstance (data, cb) {
    ...
    function updateThisInstance (cb) {
      self.getInstanceDeps(instance, function (err, deps) {
        Instance.update({
          _id: instance._id
        }, {
          $set: {
    ...
    function removeCachedDepsOfConnectedInstances (cb) {
      if (!data.decache || data.decache.length === 0) {
        process.nextTick(cb, 0); // just to ensure this is 'async'
      } else {
