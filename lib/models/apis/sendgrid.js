 ...
 *
 * @param   {Number} orgGithubId - Org who just got their docks spun up
 *
 * @returns {Promise} resolves when email request is done
 */
SendGridModel.prototype.dockCreated = function (orgGithubId) {
  var log = this.log.child({
    method: 'SendGridModel.dockCreated',
    org: orgGithubId
  })
  log.info('SendGridModel.dockCreated called')
  return UserWhitelist.findOneAsync({ githubId: orgGithubId })
    .bind(this)
    .then(function (organization) {
      return this.sendEmail({
        email: 'praful@runnable.com',
        from: 'support@runnable.com',
        fromname: 'Runnable Support',
