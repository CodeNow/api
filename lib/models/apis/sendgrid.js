...
SendGridModel.prototype.inviteAdmin = function (recipient, sessionUser, emailMessage) {
  ...
  return this.sendEmail({
    ...
    .catch(function (err) {
      log.error(put({
        err: err
      }, logData), 'SendGridModel.prototype.inviteAdmin failure')
      throw err
    })
}
...
SendGridModel.prototype.inviteUser = function (recipient, sessionUser, organizationId) {
  ...
  return findGithubOrgByGithubIdAsync(organizationId)
    ...
    .catch(function (err) {
      log.error(put({
        err: err
      }, logData), 'SendGridModel.prototype.inviteUser failure')
      throw err
    })
}
...
SendGridModel.prototype.sendEmail = function (emailOptions) {
  ...
  if (!this._sendgrid) {
    ...
    }, logData), 'SendGridModel.prototype.sendEmail missing api key')
  ...
  return this._sendgrid.sendAsync(email)
    ...
    .catch(function (err) {
      log.error(put({
        err: err
      }, logData), 'SendGridModel.prototype.sendEmail failure')
