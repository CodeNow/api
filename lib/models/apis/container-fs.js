...
module.exports  = {
  list: function (container, pathToDir, cb, opts) {
    // path must have trailing slash to ensure this is a file
    var start = new Date();
    request(
      createRequest('GET', container, pathModule.join(pathToDir,'/'), opts),
      function (err, res) {
  ...
  get: function (container, path, cb, opts) {
    var start = new Date();
    request(
      createRequest('GET', container, path, opts),
      function (err, res) {
  ...
  patch: function (container, path, newObject, cb) {
    var start = new Date();
    request(
      createRequest('POST', container, path, {
        body: newObject
      }),
      function (err, res) {
  ...
  del: function (container, path, cb) {
    var start = new Date();
    request(
      createRequest('DELETE', container, path),
      function (err, res) {
