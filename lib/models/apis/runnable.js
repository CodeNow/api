...
/**
 * Deploy build to the instance but check that autodeploy should be perfomed.
 * RIght now just check that instance configuration didn't change.
 */
Runnable.prototype.autoDeployBuildToInstance = function (instance, buildId, cb) {
  debug('autoDeployBuildToInstance', formatArgs(arguments));
  Instance.findById(instance._id, function (err, latestCopy) {
    if (err) { return cb(err); }
    instance.isConfigDrifted(latestCopy, function (err, drifted) {
      if (err) { return cb(err); }
      if (drifted) {
        // do nothing
        return cb(null);
      ...
      this.updateInstance(instance.shortHash, payload, cb);
    });
  }.bind(this));
};
