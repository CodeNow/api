...
Runnable.prototype.copyInstance = function (build, parentInstance, newProps, cb) {
  ...
  if (parentInstance.public) {
    body.publicParent = true;
    body.owner = {
      github: this.sessionUser.accounts.github.id
    };
  }
...
Runnable.prototype.deepCopyPublicBuild = function (build, cb) {
  ...
  // Calling out to the API to fetch the project and env, then create a new Build
  var newBuild = buildModel.copy(body, function (err) {
    cb(err, newBuild);
  });
};
