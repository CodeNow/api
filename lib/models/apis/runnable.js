 ...
 * @return null
 */
Runnable.prototype.workerOnInstanceContainerCreate = function (body, cb) {
  log.trace({
    tx: true
  }, 'workerOnInstanceContainerCreate');
  this.client.post('/workers/on-instance-container-create', body, cb);
};
...
Runnable.prototype.deployInstance = function (instance, opts, cb) {
  log.debug({
    ...
    instance: instance,
    opts: opts
  }, 'Runnable.prototype.deployInstance');
  if (isFunction(opts)) {
    cb = opts;
    opts = {};
  }
  var instanceModel = this.newInstance(instance.shortHash);
  instanceModel.deploy(opts, function (err) {
    if (err) {
      log.warn({
        tx: true,
        instance: instance,
        err: err
      }, 'Runnable.prototype.deployInstance deploy error');
    }
    else {
      log.trace({
        tx: true,
        instance: instance,
        err: err
      }, 'Runnable.prototype.deployInstance deploy success');
    }
