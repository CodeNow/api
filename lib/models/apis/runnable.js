...
Runnable.prototype.createAndBuildBuild = function(cvId, ownerGithubId, repo, commit, cb) {
  ...
  this.createBuild({
    json: newBuildPayload
  }, function(err, newBuild) {
    if (err) {
      return cb(err);
    }
    this.buildBuild(newBuild, {
      json: buildBuildPayload
    }, cb);
  }.bind(this));
};
...
Runnable.prototype.deepCopyContextVersionAndPatch = function(contextVersion, repoData, cb) {
  ...
  this.deepCopyContextVersion(contextVersion.context, contextVersion._id, function(err, newCvModel) {
    if (err) {
      return cb(err);
    }
