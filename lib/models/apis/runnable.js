...
var log = require('logger').child({ module: 'models:runnable'}, true);
...
Runnable.prototype.workerContainerCreate = function (body, cb) {
  debug('workerContainerCreate', formatArgs(arguments));
...
Runnable.prototype.updateInstance = function (id, opts, cb) {
  ...
  origUpdateInstance.call(self, id, opts, function (/* err, args... */) {
...
Runnable.prototype.startInstance = function (instanceShortHash, cb) {
  ...
  var namespace = getNamespace('runnable');
  //console.log('hello from startInstance', namespace.get('tid'));

  debug('startInstance', formatArgs(arguments));
  var data = {};
  var start = new Date();
