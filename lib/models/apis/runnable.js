...
Runnable.prototype.deepCopyContextVersion = function (contextId, contextVersionId, body, cb) {
  log.info({
    tx: true,
    contextId: contextId,
  ...
  var newCV = this
    ...
    .deepCopy({ json: body }, function (err) {
      cb(err, newCV);
    });
};
...
Runnable.prototype.deepCopyContextVersions = function (contextIds, contextVersionIds, body, cb) {
  log.info({
    tx: true,
  ...
  async.map(idsArr, function (ids, cb) {
    ...
    newSelf.deepCopyContextVersion(ids.contextId, ids.versionId, body, cb);
  }, cb);
};
