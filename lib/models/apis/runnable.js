...
/**
 * This calls the deploy route for a given instance
 * @param {Object} instance  instance to deploy
 * @param {Object} opts      optional options to pass for deploy
...
Runnable.prototype.updateInstance = function (shortHash, opts, cb) {
  ...
  update();
  function update () {
    origUpdateInstance.call(self, shortHash, opts, function (err) {
      if (err) {
        if (err.output.statusCode === 409 &&
...
/**
 * Deploy build to the instance but check that autodeploy is enabled.
 */
Runnable.prototype.autoDeployBuildToInstance = function (instanceId, buildId, cb) {
  debug('autoDeployBuildToInstance', formatArgs(arguments));
  // NOTE: we pass `inst` as 3d parameter because we need `inst.shortHash`
  Instance.isAutoDeployOn(instanceId, function (err, on, inst) {
    if (err) { return cb(err); }
    if (!on || !inst) { return cb(null); }
    var payload = {
      json: {
        build: buildId
      }
    };
    this.updateInstance(inst.shortHash, payload, cb);
  }.bind(this));
};
...
Runnable.prototype.destroyForkedInstances = function (masterInstance, cb) {
  debug('destroyForkedInstances', formatArgs(arguments));
...
Runnable.prototype.deepCopyContextVersionAndPatch = function (contextVersion, repoData, cb) {
  debug('deepCopyContextVersionAndPatch', formatArgs(arguments));
