...
/**
 * Check if auto-deploy is enabled for the instance.
 * @param {String} instanceHash  short hash identifier for the instance
 * @param {Function} cb      standard callback. second parameter is boolean
 */
Runnable.prototype.isAutoDeployEnabled = function (instanceHash, cb) {
  debug('isAutoDeployEnabled', formatArgs(arguments));
  this.newInstance(instanceHash).fetch(function(err, instance) {
    if (err) { return cb(err); }
    if (!instance) {
      var notFoundErr = Boom.notFound('Cannot find instance', {
        shortHash: instanceHash
      });
      return cb(notFoundErr);
    }
    cb(null, !instance.locked);
  });
};
  ...
  this.createInstance(body, function (err, instance) {
    if (err) {
      ...
      if (err.output.statusCode === 409 && errMsg === 'instance with lowerName already exists') {
        // retry and append suffix to the name branch
        var newBranchName = branch + '-copy';
        this.forkMasterInstance(masterInst, buildId, newBranchName, cb);
      }
      else {
        cb(err);
      }
