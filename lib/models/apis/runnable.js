...
Runnable.prototype.redeployInstance = function (instance, cb) {
  this.newInstance(instance.shortHash).redeploy(cb);
};
...
Runnable.prototype.deepCopyContextVersion = function (contextId, contextVersionId, cb) {
  var newCV = this
    ...
    .newVersion(contextVersionId.toString())
...
Runnable.prototype.updateCVCommitForRepoAndBranch = function (contextVersion, repo, branch, commit, cb) {
  var cv = this
  ...
  cv.fetch(function (err) {
      ...
      var acvIndex = findIndex(cv.json().appCodeVersions, hasProps({
        lowerRepo: repo.toLowerCase(),
        lowerBranch: branch.toLowerCase()
      }));
      if (acvIndex !== -1) {
        cv.appCodeVersions.models[acvIndex].update({
          commit: commit
        }, cb);
      } else {
        cb(null, cv);
