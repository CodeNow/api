 ...
 * @param cb
 */
Runnable.prototype.redeployInstances = function (instances, cb) {
  var self = this;
  async.each(instances, function (instance, cb) {
    console.log('Deploy', instance._id);
    self.newInstance(instance.shortHash).redeploy(function (err, req, body) {
      console.log(instance._id, instance.shortHash, 'DONE', err);
      cb(err);
    });
  }, cb);
};
