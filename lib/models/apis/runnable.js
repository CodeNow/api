...
Runnable.prototype.patchInstance = function (instanceShortHash, buildId, cb) {
  var self = new Runnable(this.headers, this.sessionUser);
  self.updateInstance(instanceShortHash.toString(), {json: {build: buildId}}, cb);
};
...
Runnable.prototype.createNewBuild = function (contextVersions, repoOwner, cb) {
  debug('createNewBuild', formatArgs(arguments));
  var body = {
    contextVersions: contextVersions,
    owner: repoOwner
  };
  debug('new build should be created with contextVersions', contextVersions, repoOwner);
