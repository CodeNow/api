 ...
 * @param  {String}   serverName    name of the Runnable server to use in context
 * @param  {String}   targetUrl     url pointing to the runnable server
 */
PullRequest.prototype.buildStarted = function (gitInfo, serverName, targetUrl) {
  debug('buildStarted', formatArgs(arguments));
  var description = 'This commit is building on Runnable.';
  this._buildStatus(gitInfo, 'pending', description, serverName, targetUrl);
};
 ...
 * @param  {String}   targetUrl     url pointing to the runnable server
 */
PullRequest.prototype.buildSucceeded = function (gitInfo, serverName, targetUrl) {
  debug('buildSucceeded', formatArgs(arguments));
  var description = 'This commit is ready to run on Runnable.';
  this._buildStatus(gitInfo, 'success', description, serverName, targetUrl);
};
 ...
 * @param  {String}   targetUrl     url pointing to the runnable server
 */
PullRequest.prototype.buildErrored = function (gitInfo, serverName, targetUrl) {
  debug('buildErrored', formatArgs(arguments));
  var description = 'This commit has failed to build on Runnable.';
  this._buildStatus(gitInfo, 'error', description, serverName, targetUrl);
};
 ...
 */
 /*jshint maxparams: 6 */
PullRequest.prototype._buildStatus =
  function (gitInfo, state, description, serverName, targetUrl, cb) {
    ...
    var context = 'runnable/' + serverName;
    var payload = {
      state: state,
 ...
 * @param  {Object}   gitInfo       gitInfo with `repo` and `commit`
 * @param  {String}   serverName    name of the Runnable server for deployment
 * @param  {String}   targetUrl     url pointing to the runnable server
 * @param  {Function} cb            standard callback
 */
PullRequest.prototype.createAndStartDeployment =
  function (gitInfo, serverName, targetUrl, cb) {
    debug('createAndStartDeployment', formatArgs(arguments));
    this.createDeployment(gitInfo, serverName, function (err, deployment) {
 ...
 * @param  {Object}   gitInfo       gitInfo with `repo` and `commit`
 * @param  {String}   serverName    name of the Runnable server for deployment
 * @param  {Object}   payload       additional payload that would be send to GitHub (optional)
 * @param  {Function} cb            standard callback
 */
...
PullRequest.prototype.deploymentSucceeded =
  function (gitInfo, deploymentId, serverName, targetUrl) {
    debug('deploymentSucceeded', formatArgs(arguments));
    var description = 'Deployed to ' + serverName + ' on Runnable.';
    this._deploymentStatus(gitInfo, deploymentId, 'success', description, targetUrl);
  };
...
PullRequest.prototype.deploymentErrored =
  function (gitInfo, deploymentId, serverName, targetUrl) {
    ...
    this._deploymentStatus(gitInfo, deploymentId, 'error', description, targetUrl);
  };
