...
Timers.prototype.sendToDatadog = function (name, timer, opts) {
  var ms = (timer[0] * 1000) + (timer[1] / 1000000);
  // ms = seconds * 1000 + nanoseconds / 1000000
  dogstatsd.timing('api.timers.' + name, ms, opts);
};
...
Timers.prototype.stopTimer = function (name, opts, cb) {
  if (isFunction(name)) {
    cb = name;
    return cb(new Error('timers require a name'));
  }
  ...
  this.sendToDatadog(name, stop, opts);
