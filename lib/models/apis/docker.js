...
Docker.prototype.buildVersion = function (version, sessionUser, cb) {
  if (version.build.completed) {
    ...
    version.populate('infraCodeVersion', function (err) {
      ...
      findDock('container_build', function (err, dockerHost) {
        if (err) {
          return cb(err);
        }
        self.loadDocker(dockerHost);
        // no RUNNABLE_REPO
        var dockerErrInfo = {
 ...
 * @param cb Callback to send the log stream back to
 */
Docker.prototype.getLogs = function(dockerHost, containerId, cb) {
  var self = this;
  self.loadDocker(dockerHost);
...
Docker.prototype.inspectContainer = function (container, cb) {
  var self = this;
  self.loadDocker(container.dockerHost);
  self.docker
 ...
 * @param cb: Callback
 */
Docker.prototype.stopContainers = function (containers, cb) {
 ...
 * @param taskType: ['container_build', 'container_start']
 * @param prevDock: previous dock this image was run on
 * @param cb: Callback
 */
function findDock (taskType, prevDock, cb) {
