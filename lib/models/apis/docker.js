...
Docker.prototype.createUserContainer = function (opts, cb) {
  ...
  joi.validateOrBoom(map(opts, toJSON), joi.object({
    ...
    // container envs
    var elasticUrl = instance.getElasticHostname(instance.owner.username)
    var env = Docker._evalEnvVars([
      'RUNNABLE_CONTAINER_ID=' + instance.shortHash,
      'RUNNABLE_HOSTNAME=' + instance.shortHash + '-' + elasticUrl,
...
/**
 * update memory limits on a container
 * @param {String} containerId   id of target container
 * @param {Number} memoryInBytes memory limit to update container with
 * @param {Function} cb
 */
Docker.prototype.updateContainerMemory = function (containerId, memoryInBytes, cb) {
  var logData = {
    tx: true,
    containerId: containerId,
    memoryInBytes: memoryInBytes
  }
  log.info(logData, 'Docker.prototype.updateContainerMemory')
...
Docker.prototype.killContainer = function (containerId, cb) {
  log = log.child(put({
    containerId: containerId,
    method: 'Docker.prototype.stopContainer'
  }, this.logData))
  log.info('called')
