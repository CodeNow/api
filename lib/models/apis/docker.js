...
function Docker (host) {
  ...
  this.port = 2375;//parsed.port || 4243;
...
Docker.prototype.buildVersion = function (version, cb) {
  if (version.build.dockerImage) {
    ...
    indexedVersions[version.dockerfile.Key] = version.dockerfile.VersionId;
    ...
    var env = [
      ...
      'RUNNABLE_FILES=' + '{ "53977c5d382ce69d459c013b/source/Dockerfile": "CgnTassErdFOwHgnXlEiDlEkWbb14sS1" }',//JSON.stringify(indexedVersions),
      'RUNNABLE_REPO=https://github.com/visionmedia/express',
      'RUNNABLE_DOCKER=' + 'tcp://192.168.59.103:2375', //this.host.replace(/https?/, 'tcp') + ':' + this.port,
      'RUNNABLE_DOCKERTAG=' + (version.name ? version.name : version._id)
    ];
    console.log(env);
    var build = this.docker.run(
