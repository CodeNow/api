...
Docker.prototype.createUserContainer = function (opts, cb) {
  ...
  joi.validateOrBoom(map(opts, toJSON), joi.object({
    ...
    self._createUserContainerLabels(opts, function (err, labels) {
      ...
      var isRepo = ContextVersion.getMainAppCodeVersion(opts.contextVersion.appCodeVersions)
      var memoryLimit = isRepo ? process.env.CONTAINER_REPO_MEMORY_LIMIT_BYTES : process.env.CONTAINER_NON_REPO_MEMORY_LIMIT_BYTES
...
Docker.prototype.startUserContainer = function (containerId, cv, cb) {
  ...
  var isRepo = ContextVersion.getMainAppCodeVersion(cv.appCodeVersions)
  var memoryLimit = isRepo ? process.env.CONTAINER_REPO_MEMORY_LIMIT_BYTES : process.env.CONTAINER_NON_REPO_MEMORY_LIMIT_BYTES
