    ...
    version.populate('infraCodeVersion', function (err) {
      ...
      self.docker.createContainer({
        ...
        container.attach({stream: true, stdout: true, stderr: true}, function (err, stream) {
          ...
          if (err) { return cb(dockerErr('docker builder attach failed', err)); }
            ...
            container.wait(function (err, waitData) {
              ...
              buildStream.getBuildLog(version._id, function(err, buildLogData) {
                ...
                if (!(result && result[1])) {
                  ...
                  cb(Boom.badGateway('docker builder run failed to build', {
                    res: {
                  ...
                  self.pushImageToRegistry(runnableDockerTag.split(':')[0], version._id);
                }
              });
            });
          });
...
Docker.prototype.pushImageToRegistry = function (imageId, tag, cb) {
  if (!cb) {
    cb = error.logIfErr;
  }
  var i = this.docker.getImage(imageId);
  i.push({tag: tag}, cb);
};
