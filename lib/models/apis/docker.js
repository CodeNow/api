...
Docker.prototype._createImageBuilderLabels = function (opts) {
  ...
  var flatContextVersion = keypather.flatten(
...
Docker.prototype.getBuildInfo = function(containerId, cb) {
  ...
  pipeDomain.runnableData = keypather.get(process, 'domain.runnableData');
  // For logging tx.tid & user data from parent domain
  pipeDomain.on('error', function (err) {
    log.fatal(put({err: err}, logData), 'Docker.prototype.getBuildInfo: domain err');
    error.log(err);
    self.handleErr(cb, 'runtime error')(err);
  });
  // run domain
  pipeDomain.run(function () {
    ...
    container.logs({
      ...
      var logArr = [];
      // handle errors
      stream.on('error',         handleErr(cb, 'docker logs stream failed'));
      streamCleanser.on('error', handleErr(cb, 'docker stream cleanser failed'));
      jsonParser.on('error',     handleErr(cb, 'json parse failed to read build logs'));
      // jsonParser
      // pipe
      stream
        .pipe(streamCleanser)
      ...
      // error handler
      function handleErr (cb, msg) {
        return function (err) {
          stream.removeAllListeners();
          streamCleanser.removeAllListeners();
          jsonParser.removeAllListeners();
          // streamCleanser.unpipe(jsonParser)
          self.handleErr(cb, msg, errDebug)(err);
        };
      }
    });
  });
