...
/*
 * Takes `opts` for additional docker options
 */
function Docker (dockerHost, opts) {
  opts = opts || {};
  log.info({tx: true, dockerHost: dockerHost}, 'new Docker');
  if (!dockerHost) {
...
/**
 * @param {Object} opts
 *   opts.manualBuild - Boolean indicating automatic or manual build
 *   opts.sessionUser - Currently authenticated user
 *   opts.contextVersion - contextVersion to be built
 *   opts.dockerTag - Tag to give image created by image-builder
 *   opts.network - Object containing container network information
 *   opts.noCache - 
 *   opts.tid - TID value to place on labels
 */
...
Docker.prototype._createImageBuilderValidateCV = function (contextVersion) {
  ...
  if (contextVersion.build.completed) {
    log.warn(logData, '_createImageBuilderValidateCV build completed');
    return Boom.conflict('Version already built', contextVersion);
  }
  if (!contextVersion.infraCodeVersion) {
    log.warn(logData, '_createImageBuilderValidateCV no icv');
  ...
  if (utils.isObjectId(contextVersion.infraCodeVersion)) {
    log.warn(logData, '_createImageBuilderValidateCV not populated icv');
...
Docker.prototype._createImageBuilderLabels = function (opts) {
  var logData = {tx: true};
  log.info(logData, 'Docker.prototype._createImageBuilderLabels');
  var buildContainerLabels = put({
    dockerTag: opts.dockerTag,
    hostIp: opts.network.hostIp,
    manualBuild: opts.manualBuild,
    networkIp: opts.network.networkIp,
    ...
    sessionUserId: opts.sessionUser.accounts.github.id,
    sessionUserUsername: opts.sessionUser.accounts.github.username,
    tid: opts.tid,
    type: 'image-builder-container'
  }, keypather.flatten(opts.contextVersion.toJSON(), '.', 'contextVersion'));
  ...
  }, logData), '_createImageBuilderLabels buildContainerLabels');
...
Docker.prototype._createImageBuilderEnv = function (opts) {
  ...
  log.info(logData, 'Docker.prototype._createImageBuilderEnv');
...
Docker.prototype.startUserContainer = function (container, ownerId, opts, cb) {
  ...
  if (!container) {
    log.error(logData, 'startUserContainer: !container');
...
Docker.prototype.inspectUserContainer = function (container, cb) {
  log.info({
    tx: true,
    container: container
  }, 'Docker.prototype.inspectUserContainer');
  if (!container) {
    return cb(Boom.badRequest('Container must be provided to start'));
  }
  var self = this;
 ...
 * @param container docker container id string or
 *   container or mongo container object
