  ...
  self.startContainer(container, startContainerData, function (err) {
    ...
    container.logs({
      ...
      streamCleanser.async(stream, function (result) {
        ...
        var dockerTag = getDockerTag(sessionUser, version);
        self.inspectContainer(container, function(err, info) {
          if (err) {
            return self.handleErr(cb, 'docker build container inspect failed',
              { containerId: container.Id })(err);
          }
          if (buildFailed || info.State.ExitCode) {
            debug('build failed - ', info, buildLogData);
            cb(Boom.badRequest('Build Failed: Dockerfile error code: ' +
              info.State.ExitCode +
              ' message ' +
              info.Stage.Error,
              { docker: {
...
Docker.prototype.pushImageToRegistry = function (imageName, tag, cb) {
  ...
  this.docker
    .getImage(imageName)
    .push({ tag: tag },
      this.handleErr(cb, 'Push image failed', {
...
Docker.prototype.pullImage = function (version, cb) {
  ...
  var self = this;
  var dockerTag = version.build.dockerTag;
  var dockerImage = version.build.dockerImage;
  self.docker.pull(version.build.dockerTag,
    self.handleErr(cb, 'Pull image failed', {
      dockerTag: dockerTag,
