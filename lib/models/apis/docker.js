 ...
 */
Docker.prototype.exportImage = function (imageId, cb) {
  logger.log.trace({
    tx: true,
    imageId: imageId
  }, 'exportImage');
  this.getImage(imageId).get(cb);
};
/**
 ...
 */
Docker.prototype.importImage = function (imageStream, cb) {
  logger.log.trace({
    tx: true
  }, 'importImage');
  this.loadImage(imageStream, cb);
};
/**
...
Docker.prototype.transferImage = function (image, targetDockUrl, cb) {
  ...
  targetDock.exportImage(function(err, stream) {
    if (err) { return cb(err); }
    self.importImage(stream, self.handleErr(cb, 'docker transfer failed', {
      image: image,
      targetDockUrl: targetDockUrl
    }));
  });
