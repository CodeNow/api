...
Docker.prototype.buildVersion = function (version, cb) {
  if (version.build.dockerImage) {
    ...
    var builder = this.docker.run(
      'docker-image-builder', // FIXME: this will probably be runnable/docker-image-builder
      null,
      null,
      { Env: env },
      function (err, data, container) {
        if (err) { return cb(err); }
        debug('callback', data, container);
        if (data.StatusCode !== 0) {
          return cb(Boom.badImplementation('docker build failed (' + data.StatusCode + ')'));
        }
        var imageId;
        ...
        if (result && result[0]) {
          ...
          return cb(Boom.badImplementation('docker build failed'));
        }
        cb(null, {
          dockerImage: imageId,
