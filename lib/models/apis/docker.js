      ...
      function buildContainer() {
        self.docker.createContainer({
          ...
          if (err && err.statusCode === 404) {
            error.logIfErr(new Error('image ' + process.env.DOCKER_IMAGE_BUILDER_VERSION +
              ' not found on dock '+ self.dockerHost));

            return self.docker.pull('runnable/image-builder:' + process.env.DOCKER_IMAGE_BUILDER_VERSION,
              function(err, stream) {
                if(err) { return cb(dockerErr('pulling builder failed', err)); }
                stream.on('error', cb);
                stream.on('end', buildContainer);
            });
          }
