...
var log = require('logger').child({ module: 'models:apis:docker' }, true);
...
Docker.prototype.startContainer = function (container, opts, cb) {
  ...
  log.info(startInfo, 'startContainer');
  self.docker
    .getContainer(containerId)
    .start(opts,
      function (err) {
        if (err) {
          log.error(startInfo, 'startContainer error');
        }
        else {
          log.info(startInfo, 'startContainer success');
        }
        self.handleErr(cb, 'Start container failed', { containerId: containerId, opts: opts })(err);
      });
};
