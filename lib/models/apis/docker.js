...
Docker.isImageNotFoundForPullErr = function (err) {
  if (!err) { return false }
  // Handle error fired by Docker.pullImage
  if (
    err.isBoom &&
    err.output.statusCode === 404 &&
    // NOTE: There should be a more accurate way of testing this
    /image.*not found/.test(err.message) &&
    /pull image failed/i.test(err.message)
  ) {
    return true
  }
  // Handle error fired by `createContainer` from Swarm
  // Error might be 502: https://github.com/CodeNow/api/blob/add-swarm-to-build/lib/models/apis/docker.js#L1175-L1176
  var originalMessage = keypather.get(err, 'data.err.json')
  if (
    err.isBoom &&
    (err.output.statusCode === 500 || err.output.statusCode === 502) &&
    (/image.*not found/.test(err.message) || /image.*not found/.test(originalMessage))
  ) {
    return true
  }
  // Vanilla JS object for error
  if (
    err.statusCode === 500 &&
    err.reason === 'server error' &&
    /image.*not found/.test(err.json)
  ) {
    return true
  }
  return false
...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  self.createContainer(builderContainerData, function (err, container) {
    if (err) {
      log.error(put({err: err}, logData), 'Docker.prototype.createImageBuilder createContainer failed')
      return self._handleCreateContainerError(err, builderContainerData, cb)
    }
    log.trace(put({container: container}, logData), 'Docker.prototype.createImageBuilder createContainer success')
 ...
 * if constrains are not met then schedule on default dock
...
Docker.prototype._handleCreateContainerError = function (err, opts, cb) {
  log.error({
    err: err,
    opts: opts
  }, '_handleCreateContainerError error')

  if (Docker._isConstraintFailure(err)) {
    // FIXME: remove once we do not allow default docks
    log.warn({ org: opts.Labels['com.docker.swarm.constraints'] }, 'unable to find dock for org')
    opts.Labels['com.docker.swarm.constraints'] = Docker.createSwarmConstraints([{
      name: 'org',
      value: 'default',
      type: 'hard'
    }])

    return this.createContainer(opts, cb)
  }

  if (Docker._isOutOfResources(err)) {
    log.warn({ Memory: opts.Memory }, 'unable to find dock with required resources')
    delete opts.Memory
  ...
  joi.validateOrBoom(map(opts, toJSON), joi.object({
    ...
    self._createUserContainerLabels(opts, function (err, labels) {
      ...
      self.createContainer(userContainerData, function (err, container) {
        if (err) {
          log.error(put({err: err}, userContainerData), 'Docker.prototype.createUserContainer createContainer failed')
          return self._handleCreateContainerError(err, userContainerData, cb)
        }

        log.trace(put({container: container}, userContainerData), 'Docker.prototype.createUserContainer createContainer success')
        cb(null, container)
      })
    })
  })
...
Docker.prototype.pullImage = function (image, cb) {
  ...
  self.docker.pull(image, function (err, pullStream) {
    ...
    self.docker.modem.followProgress(pullStream, function onFinished (err, output) {
      var msg = 'Follow pull image failed: '
      if (err) {
        log.error(put({
          err: err
        }, logData), 'pullImage docker.pull followProgress onFinished error')
        // Error can be a string or error
        // https://github.com/apocas/docker-modem/blob/f5c47890e7f42bd933074299d41481649d1b3641/lib/modem.js#L288
        if (isString(err) && /image.*not found/i.test(err)) {
          // Ex: "Error: image <dockerTag>: not found"
          err = Boom.notFound(msg + err, { err: err, image: image })
        } else {
          var errMessage = err.message || err
          err = Boom.badGateway(msg + errMessage, { err: err, image: image })
        }

        return cb(err)
      }
      if (!Array.isArray(output)) {
        return cb(Boom.badGateway('Invalid pull output', { image: image, output: output }))
      }
      // success messages:
      // { status: 'Status: Image is up to date for ruby:latest' }
      // { status: 'Status: Downloaded newer image for centos:latest' }
      var successRE = /^(Status: Image is up to date)|(Status: Downloaded newer image for).*/
      var isSuccessful = successRE.test(last(output).status)
      if (!isSuccessful) {
        return cb(Boom.badGateway('Pull unsuccessful', { image: image, output: output }))
      }
      // success
