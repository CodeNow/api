...
Docker.prototype.createImageBuilderAndAttach = function (sessionUser, version, cb) {
  if (version.build.completed) {
    ...
    self.docker.createContainer({
      ...
      debug('container created - ', 'err:', err, 'container:', container && container.Id);
      ...
      container.attach({
        ...
        debug('container attached - ', 'err:', err, 'container:', container && container.Id);
        cb(err, container, stream);
      });
    });
  }
};
...
Docker.prototype.startImageBuilderAndWait = function (sessionUser, version, container, cb) {
  ...
  container.start({ Binds: binds }, function (err) {
    ...
    container.wait(function (err, res) {
      ...
      buildStream.getBuildLog(version._id, function(err, logData) {
        debug('build logs recieved - ', 'err:', err);
        if (err) { return cb(err); }

        var match = successRe.exec(logData);
        var buildFailed = !match || !match[1];
        if (buildFailed) {
          debug('build failed - ', buildFailed);
          cb(self.handleResError(cb, res, 'Image-builder build failed', {
            cmd: 'wait',
