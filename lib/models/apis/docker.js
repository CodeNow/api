...
Docker.prototype.createUserContainer = function (version, opts, cb) {
  ...
  if (req) {
    debug('createUserContainer', 'req!', req.method, req.url, req.body);
  }
  else {
...
Docker.prototype.startContainer = function (container, opts, cb) {
  ...
  console.log('L100 starting container', containerId);
  self.docker
    .getContainer(containerId)
    .start(opts,
      self.handleErr(callback, 'Start container failed', { containerId: containerId, opts: opts }));
  function callback (err) {
    console.log('L100 starting container finished', containerId, err);
