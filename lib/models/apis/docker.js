    ...
    self.docker.modem.followProgress(pullStream, function onFinished (err, output) {
      ...
      if (err) {
        ...
        if (isString(err) && /image.*not found/i.test(err)) {
          ...
          err = Boom.notFound(msg + err, { err: err })
        } else {
        ...
        // note: remove conditional after Docker 1.9+
        var re = /already being pulled/
        var isAlreadyPulling = find(output, function (item) {
          return re.test(item && item.status)
        })
        if (isAlreadyPulling) {
          err = Boom.conflict(msg + isAlreadyPulling.status)
          return cb(err)
        }
      }
      // success
