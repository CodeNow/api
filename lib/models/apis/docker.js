...
function getEnvForImageBuilder (opts) {
  ...
  // push only for build cluster orgs
  if (version.owner.github === process.env.CODENOW_GITHUB_ID) {
...
Docker.prototype.createImageBuilder = function (
  ...
  log.info({
    ...
    tx: true,
    version: version
  }, 'Docker.prototype.createImageBuilder');
...
/**
  ...
  First option argument mayb have 3 keys:
    ...
    - ignoreStatusCode - original error status code
       that can be counted as success (no retries would be performed)
  Following methods are available:
    - pullImageWithRetry
    - inspectContainerWithRetry
    - startImageBuilderContainerWithRetry
    - startUserContainerWithRetry
*/
methodNames.forEach(function (method) {
  ...
  Docker.prototype[newMethodName] = function () {
    ...
    // first argument should be max retries number
    var opts = args.shift();
    ...
    async.retry(retryOpts, function (cb) {
      ...
      var retryCb = function (err) {
        ...
        if (err) {
          ...
          if (opts.ignoreStatusCode && err.statusCode === opts.ignoreStatusCode) {
            log.info({
              tx: true,
              err: err,
              attemptCount: attemptCount
            }, method + ' retry call:' + err.reason);
            return cb(null);
          }
        }
        cb.apply(this, arguments);
