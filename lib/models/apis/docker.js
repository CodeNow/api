...
function getEnvForImageBuilder (version, dockerTag) {
  ...
  if (process.env.DOCKER_IMAGE_BUILDER_CACHE) {
    env.push('RUNNABLE_DOCKER_LAYER_CACHE=' + process.env.DOCKER_IMAGE_BUILDER_CACHE);
  }
  if (process.env.DOCKER_IMAGE_BUILDER_LAYER_CACHE) {
    env.push('RUNNABLE_DOCKER_LAYER_CACHE=' + process.env.DOCKER_IMAGE_BUILDER_LAYER_CACHE);
  }
  ...
  self.startContainer(container, startContainerData, function (err) {
    ...
    container.logs({
      ...
      streamCleanser.async(stream, function (result) {
        ...
        if (buildFailed) {
          debug('build failed');
          cb(Boom.badRequest('Building dockerfile failed due to non zero exit code', {
            docker: {
              containerId: container.Id,
              versionId: version._id,
              dockerTag: dockerTag,
