...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  organization.sshKeys = self.addSshKeyEnvs(organization.sshKeys)
  ...
  if (organization && ((organization.privateRegistryUrl && organization.privateRegistryUsername) || organization.sshKeys)) {
    const volumes = {}
  ...
  if (process.env.DOCKER_IMAGE_BUILDER_HOST_CONFIG_PATH) {
    ...
    builderContainerData.HostConfig.Binds.push(`${configPath}:${configPath}:ro`)
...
Docker.prototype._createImageBuilderEnv = function (opts) {
  ...
  const env = [
    ...
    'RUNNABLE_ORG_ID=' + organization.id
  ]

  if (organization.sshKeys) {
    env.push('RUNNABLE_SSH_KEY_IDS=' + organization.sshKeys)
  }
...
Docker.prototype.addSshKeyEnvs = (orgSshKeys) => {
  let sshKeys
  let keyUserIds

  if (keypather.get(orgSshKeys, 'length')) {
    sshKeys = orgSshKeys.reduce((acc, key) => {
      acc += key.userId + ','
      return acc
    }, '')
    // remove the last comma from the string
    keyUserIds = sshKeys.slice(0, -1)
  }
  return keyUserIds
}
