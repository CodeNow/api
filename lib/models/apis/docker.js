...
Docker.prototype.getBuildInfo = function(containerId, cb) {
  ...
  container.logs({
    ...
    log.trace(put({
      err: err
    }, logData), 'getBuildInfo build logs received');
    var errDebug = {
      containerId: containerId
    };
    if (err) {
    ...
    pipeDomain.on('error', function(err) {
      log.fatal(put({
        err: err
      }, logData), 'Docker.prototype.getBuildInfo: domain err');
      error.log(err);
...
Docker.prototype.createContainer = function(opts, cb) {
  ...
  var start = new Date();
  self.docker.createContainer(opts, function(err, response) {
    if (err) {
      log.error({
        tx: true,
...
Docker.prototype.startContainer = function(containerId, opts, cb) {
  ...
  self.docker
    .getContainer(containerId)
    .start(opts, function(err, response) {
      if (err) {
        log.error({
          tx: true,
          startInfo: startInfo,
...
Docker.prototype.stopContainer = function(containerId, force, cb) {
  ...
  self.docker
    .getContainer(containerId)
    .stop(opts, function(err, response) {
      if (err) {
        log.error(put({
          elapsedTimeSeconds: start,
          err: err
...
[
  ...
  Docker.prototype[newMethodName] = function() {
    ...
    log.info({
      tx: true,
      args: args
    }, 'Docker.prototype.' + newMethodName);
  ...
  return function(err) {
    if (err) {
      ...
      if (code >= 400) {
        ...
        // other than boom..
        var boomErr = Boom.create(400, message, {
          docker: errDocker,
          err: err
        });
