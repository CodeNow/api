...
/**
 * attempts to delete an instance volume
 * @param {Array} volumes
 */
Docker.prototype.deleteInstanceVolumes = function (volumes) {
  ...
  const volumeDeletePromises = volumes.map((volume) => {
    return this._volumeAction(volume.Name, 'remove')
  })
  return Promise.all(volumeDeletePromises)
}
...
/**
 * Function to perform docker action on a volume
 * It also reports data to the datadog (count events) and logs response.
 * @param {String} volumeId - volume ID
 * @param {String} action - Docker operation like `remove` etc
 */
Docker.prototype._volumeAction = function (volumeId, action) {
  ...
  .catch((err) => {
    ...
    self.handleErr(cb, 'Volume action ' + action + ' failed',
      {opts, volume}).apply(self, arguments)
  })
}
