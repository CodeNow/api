...
Docker.prototype._createImageBuilderLabels = function (opts) {
  ...
  const sessionUserJSON = toJSON(opts.sessionUser)
  let orgId
  if (sessionUserJSON.accounts.github.id === cvJSON.owner.github) {
    orgId = 1
  } else {
    orgId = cvJSON.owner.github
  }
  const constraints = [{
    name: 'org',
    value: orgId,
...
Docker.prototype._createUserContainerLabels = function (opts, cb) {
  ...
  joi.validateOrBoom(map(opts, toJSON), joi.object({
    ...
    let orgId
    if (opts.sessionUserGithubId == cv.owner.github) {
      orgId = 1
    } else {
      orgId = cv.owner.github
    }
    const constraints = [{
      name: 'org',
      value: orgId,
