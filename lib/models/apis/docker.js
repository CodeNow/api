...
Docker._evalEnvVars = function (envVars) {
  ...
  return envVars.map(function (env) {
    console.log('env', env, envVarHash)
    ...
    // After Add values to hash sequentially in order to keep ENV order
    envVarHash[envKey] = result.split(/=(.*)/)[1]
    return result
  })
}
...
Docker.prototype.createUserContainer = function (opts, cb) {
  ...
  joi.validateOrBoom(map(opts, toJSON), joi.object({
    ...
    // container envs
    var env = Docker._evalEnvVars(instance.env.concat([
