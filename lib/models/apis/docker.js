...
Docker.prototype.startImageBuilderAndWait = function (sessionUser, version, container, cb) {
  ...
  self.startContainer(container, startContainerData, function(err) {
    debug('startContainer recieved - ', 'err:', err);
...
Docker.prototype.getBuildInfo = function(containerId, cb) {
  ...
  container.logs({
    ...
    if (err) {
      return self.handleErr(cb, 'docker logs failed', { containerId: containerId })(err);
    }
    streamCleanser.async(stream, function (result) {
      debug('build logs cleansed', 'result:', !!result);
      if (!result) {
