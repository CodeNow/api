...
function getEnvForImageBuilder (opts) {
  ...
  version.appCodeVersions.forEach(function (acv) {
    ...
    if (acv.transformRules) {
      var exclude = acv.transformRules.exclude;
      var replace = acv.transformRules.replace;
      var rename = acv.transformRules.rename;
      var rules = [];
      if (exclude.length > 0) {
        rules.push({ action: 'exclude', files: exclude });
      }
      rules = rules.concat(replace, rename);
      searchAndReplaceRules.push(JSON.stringify(acv.transformRules));
    }
    else {
  ...
  env.push('SEARCH_AND_REPLACE_RULES=' + searchAndReplaceRules.join(';'));
 ...
 * @param  {Function} cb      (err, stream)
 */
Docker.prototype.pushImage = function (imageId, cb) {
  logger.log.trace({
    tx: true,
    imageId: imageId
  }, 'pushImage');
  ...
  this.docker.getImage(imageName).push({
    tag: tag
  }, cb);
};

/**
 * runs docker save on an image and returns the stream of the image data
 * @param  {string}   imageId id of image to get stream from
 *                            format: registry.runnable.com/123/456:<tag>
 * @param  {Function} cb      (err, stream)
 */
Docker.prototype.saveImage = function (imageId, cb) {
  logger.log.trace({
    tx: true,
    imageId: imageId
  }, 'saveImage');
  this.docker.getImage(imageId).get(cb);
};

/**
 * runs docker load on an image which moves the image onto the dock
 * cb returns when transfer is done.
 * @param  {object}   imageStram tar stream from export image
 * @param  {Function} cb         (err)
 */
Docker.prototype.loadImage = function (imageStream, cb) {
  logger.log.trace({
    tx: true
  }, 'loadImage');
  this.docker.loadImage(imageStream, cb);
};

/**
 * in charge of moving docker image from on server to the next
 * try direct transfer, if that fails use registry
 * @param  {string} image         image to transfer
 *                                format: registry.runnable.com/123/456:<tag>
 * @param  {string} targetDockUrl url dock to transfer too
 * @param  {Function} cb          (err)
 */
Docker.prototype.transferImage = function (image, targetDockUrl, cb) {
  logger.log.trace({
    tx: true,
    image: image,
    targetDockUrl: targetDockUrl
  }, 'transferImage');
  var self = this;
  // TODO: remove after org isolation
  if (self.dockerHost === targetDockUrl) {
    return cb();
  }
  self.directTransferImage(image, targetDockUrl, function (err) {
    // fall back on registry push/pull if direct transfer error
    if (err) {
      return self.registryTransferImage(image, targetDockUrl, cb);
    }
    cb();
  });
};
...
/**
 * uses streaming save/load to get images from one dock to another
 * @param  {string} image         image to transfer
 *                                format: registry.runnable.com/123/456:<tag>
 * @param  {string} targetDockUrl url dock to transfer too
 * @param  {Function} cb          (err)
 */
Docker.prototype.directTransferImage = function (image, targetDockUrl, cb) {
  logger.log.trace({
    tx: true,
    image: image,
    targetDockUrl: targetDockUrl
  ...
  var self = this;
  var targetDock = new Docker(targetDockUrl);
  targetDock.saveImage(image, function(err, stream) {
    if (err) { return cb(err); }
    self.loadImage(stream, self.handleErr(cb, 'docker direct transfer failed', {
      image: image,
      targetDockUrl: targetDockUrl
    }));
  });
...
/**
 * uses registry to transfer image from one dock to another
 * @param  {string} image         image to transfer
 *                                format: registry.runnable.com/123/456:<tag>
 * @param  {string} targetDockUrl url dock to transfer too
 * @param  {Function} cb          (err)
 */
Docker.prototype.registryTransferImage = function (image, targetDockUrl, cb) {
  logger.log.trace({
    tx: true,
    image: image,
    targetDockUrl: targetDockUrl
  }, 'registryTransferImage');
  var self = this;
  var targetDock = new Docker(targetDockUrl);
  targetDock.pushImage(image, function(err, pushStream) {
    if (err) { return cb(err); }
    self.docker.modem.followProgress(pushStream, function (err) {
      if (err) { return cb(err); }
      self.pullImage(image, function (err, pullStream) {
        if (err) { return cb(err); }
