...
Docker.prototype.getBuildInfo = function(containerId, cb) {
  ...
  pipeDomain.run(function () {
    ...
    container.logs({
      ...
      var logArr = [];
      // handle errors
      stream.on('error',         handleStreamErr(cb, 'docker logs stream failed'));
      streamCleanser.on('error', handleStreamErr(cb, 'docker stream cleanser failed'));
      jsonParser.on('error',     handleStreamErr(cb, 'json parse failed to read build logs'));
      // pipe
      stream
        .pipe(streamCleanser)
        .pipe(jsonParser)
      ...
      // error handler
      function handleStreamErr (cb, msg) {
        return function (err) {
