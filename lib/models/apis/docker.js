...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  if (organization.sshKeys.length) {
    sshKeys = organization.sshKeys.reduce((acc, key) => {
      acc.names += key.keyName + ','
      acc.ids += key.userId + ','
      return acc
    }, {names: '', ids: ''})

    keyNames = sshKeys.names.slice(0, -1)
    keyUserIds = sshKeys.ids.slice(0, -1)
  }
  ...
  if (organization && ((organization.privateRegistryUrl && organization.privateRegistryUsername) || keyUserIds)) {
    const volumes = {}
...
Docker.prototype._createImageBuilderEnv = function (opts) {
  ...
  const sshKeyNames = opts.keyNames
  const sshKeyIds = opts.keyUserIds
  ...
  if (sshKeyNames && sshKeyIds) {
    env.push('SSH_KEY_NAMES=' + sshKeyNames)
    env.push('SSH_KEY_IDS=' + sshKeyIds)
  }
