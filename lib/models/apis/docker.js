...
Docker.prototype.buildVersion = function (version, sessionUser, cb) {
  if (version.build.completed) {
    ...
    version.populate('infraCodeVersion', function (err) {
      ...
      // multiple repos
      if (!version.appCodeVersion || version.appCodeVersion.length === 0) {
        cb(Boom.badRequest('Cannot build a version without any source code', version));
      }
      ...
      var editedRepoUrl = version.appCodeVersion[0].repo.remove(/https?:\/\//);
