...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  self.createContainer(builderContainerData, function (err, container) {
    if (err) {
      log.error(err, 'createContainer error')
      builderContainerData.Labels['com.docker.swarm.constraints'] =
        Docker.createSwarmConstraint('org', 'default')
      return self.createContainer(builderContainerData, cb)
    }
