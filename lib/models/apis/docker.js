...
Docker.prototype._handleCreateContainerError = function (err, opts, cb) {
  log.info({
    err: err,
    opts: opts
  }, 'Docker.prototype._handleCreateContainerError')

  if (Docker._isConstraintFailure(err)) {
    // FIXME: remove once we do not allow default docks
    log.warn({ org: opts.Labels['com.docker.swarm.constraints'] }, '_handleCreateContainerError unable to find dock for org')
  ...
  if (Docker._isOutOfResources(err)) {
    log.warn({ Memory: opts.Memory }, '_handleCreateContainerError unable to find dock with required resources')
    delete opts.Memory
    ...
    .inspect(function (err) {
      if (err) {
        log.error({
          ...
          elapsedTimeSeconds: start,
          containerId: containerId
        }, 'inspectContainer inspect error')
      } else {
        log.trace({
          tx: true,
          elapsedTimeSeconds: start,
          containerId: containerId
        }, 'inspectContainer inspect success')
      }
...
Docker.prototype.startContainer = function (containerId, opts, cb) {
  ...
  log.trace(startInfo, 'startContainer startContainer startInfo')
  var start = new Date()
  self.docker
    .getContainer(containerId)
    .start(opts,
