...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  this.createContainer(builderContainerData, function (err, container) {
    if (err && err.message === 'unable to find a node that satisfies') {
      builderContainerData.Labels['com.docker.swarm.constraints'] = ['org==default'];
...
Docker.prototype._createImageBuilderLabels = function (opts) {
  ...
  var labels = extend(flatContextVersion, {
    ...
    ownerUsername: opts.ownerUsername,
    // 'com.docker.swarm.constraints': ['org==' + opts.ownerUsername],
    tid: opts.tid,
...
Docker.prototype.pullImage = function (imageId, cb) {
  ...
  self.docker.pull(imageId, function (err, pullStream) {
    ...
    self.docker.modem.followProgress(pullStream, function onFinished (err) {
      ...
      log.trace(put({
        event: event
      }, logData), 'pullImage docker.pull followProgress onProgress');
    });
  });
};
