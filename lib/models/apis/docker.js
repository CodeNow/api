 ...
 */
Docker.prototype.buildVersion = function (version, sessionUser, afterCreateCb, cb) {
  if (version.build.completed) {
    ...
    version.populate('infraCodeVersion', function (err) {
      ...
      var imageName = join('registry.runnable.com/',
        '' + sessionUser.accounts.github.id,
        '' + version.context);
      var imageTag = version._id;
      var runnableDockerTag = imageName + ':' + imageTag;
      ...
      self.docker.createContainer({
        ...
        container.start({ Binds: binds }, function (err, data) {
          ...
          if (err) { return cb(dockerErr('docker builder start failed', err)); }

          container.wait(function (err, waitData) {
            debug('wait data', err, waitData);
            if (err) { return cb(dockerErr('docker builder wait failed', err)); }
            ...
            container.logs({
              ...
              stream.on('end', function() {
                ...
                if (!(result && result[1])) {
                  ...
                  return cb(Boom.badGateway('docker builder run failed to build', {
                    res: {
                ...
                console.log('anande end build stream docker');
                cb(null, {
                  dockerImage: imageId,
...
Docker.prototype.createAndInspectContainer = function (version, cb) {
  ...
  this.createContainer(version.build.dockerImage, function (err, container) {
    ...
    container.start({ PublishAllPorts: true, Privileged: sudo}, function (err) {
      if (err) { return cb(err); }
...
Docker.prototype.restartContainer = function (container, cb) {
  ...
  self.docker
    ...
    .restart({}, function (err) {
      if (err) {
        cb(Boom.badGateway('Error restarting container', {
          err: err,
          debug: {
            dockerHost: container.dockerHost,
            containerId: container.dockerContainer
