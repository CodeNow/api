...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  var minMemoryLimit = opts.contextVersion.getUserContainerMemoryLimit()
  var maxMemoryLimit = Math.max(minMemoryLimit, process.env.CONTAINER_HARD_MEMORY_LIMIT_BYTES)
...
/**
 * Checks container create errors to determine if they correspond to a cluster
...
Docker.prototype.createContainer = function (opts, cb) {
  ...
  keypather.set(opts, 'HostConfig.Dns', [
    process.env.DNS_CHARON_RESOLVER,
    process.env.DNS_AWS_RESOLVER,
    '8.8.8.8'
  ])
