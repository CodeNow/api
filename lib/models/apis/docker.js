...
Docker.prototype._handleCreateContainerError = function (err, opts, cb) {
  ...
  if (Docker._isConstraintFailure(err)) {
    ...
    log.warn(put(logData, { org: opts.Labels['com.docker.swarm.constraints'] }), '_handleCreateContainerError unable to find dock for org')
  ...
  if (Docker._isOutOfResources(err)) {
    log.warn(put(logData, { Memory: opts.Memory }), '_handleCreateContainerError unable to find dock with required resources')
    delete opts.Memory
...
/**
 * explanation of reason for wanting this
 * runnable.atlassian.net/browse/SAN-3081
 * Return true if error is image not found
 */
Docker._isImageNotFoundErr = function (err) {
  // matches
  // "image 157693/558dae5e7562460d0024f5a8:5668ccbacdab6c1e0054a780 not found"
  return err.statusCode === 500 && err.message.indexOf(/image \S+\:\S+ not found/)
}
