...
Docker.prototype.createContainer = function (opts, cb) {
  ...
  self.docker.createContainer(opts,
    self.handleError(callback, 'Create container failed', { opts: opts }));
...
Docker.prototype.inspectContainer = function (container, cb) {
  ...
  this.docker
    ...
    .inspect(this.handleError(cb, 'Inspect container failed', { containerId: containerId }));
};
...
Docker.prototype.startContainer = function (container, opts, cb) {
  ...
  self.docker
    .getContainer(containerId)
    .start(opts,
      self.handleError(cb, 'Start container failed', { containerId: containerId, opts: opts }));
};
...
Docker.prototype.restartContainer = function (container, cb) {
  ...
  self.docker
    .getContainer(containerId)
    .restart({},
      self.handleError(cb, 'Restart container failed', { containerId: containerId }));
};

/**
 * pulls image and returns stream
 * @param image: Image to pull: format 'myrepo/myname:tag'
 * @param cb: Callback
 */
Docker.prototype.pullImage = function (image, cb) {
  debug('pullImage', formatArgs(arguments));
  var self = this;
  self.docker.pull(image, cb);
};
...
Docker.prototype.stopContainer = function (container, force, cb) {
  ...
  self.docker
    .getContainer(containerId)
    .stop(opts,
      self.handleError(callback, 'Stop container failed',
...
Docker.prototype.removeContainer = function (container, cb) {
  ...
  self.docker
    .getContainer(containerId)
    .remove({},
      self.handleError(cb, 'Remove container failed', { containerId: containerId }));
};
