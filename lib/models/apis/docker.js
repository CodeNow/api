...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  const orgGithubId = keypather.get(opts.contextVersion, 'owner.github')
  const org = UserService.getOrgFromUserByGithubId(opts.sessionUser.bigPoppaUser, orgGithubId)
 ...
 * @param {Organization} opts.org
 * @param {String}       opts.org.privateRegistryUrl
 * @param {String}       opts.org.privateRegistryUsername
 *
...
Docker.prototype._createImageBuilderEnv = function (opts) {
  ...
  if (keypather.get(org, 'privateRegistryUrl')) {
    env.push('RUNNABLE_REGISTRY_URL=' + org.privateRegistryUrl)
    env.push('RUNNABLE_REGISTRY_USERNAME=' + org.privateRegistryUsername)
  }
