...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  self.createContainer(builderContainerData, function (err, container) {
    if (err) {
      return self._handleImageBuilderError(err, builderContainerData, cb)
    }
...
/**
 * handles errors with create image builder
 * if constrains are not met then scedule on default dock
 ...
 */
Docker.prototype._handleImageBuilderError = function (err, opts, cb) {
  log.error({
    err: err,
    opts: opts
  }, '_handleImageBuilderError error')

  if (Docker._isConstraintFailure(err)) {
    log.warn({ org: opts.Labels['com.docker.swarm.constraints'] }, 'unable to find dock for org')
    opts.Labels['com.docker.swarm.constraints'] = Docker.createSwarmConstraint('org', 'default')
 ...
 * should be string of form: '["name==value"]'
 * @param  {String} constraintsName  name of constraint
 * @param  {String} value           value of constraint
 * @return {String} properly formated label
 */
Docker.createSwarmConstraint = function (constraintsName, value) {
  return '["' + constraintsName + '==' + value + '"]'
}
...
Docker.prototype._createImageBuilderLabels = function (opts) {
  ...
  var labels = extend(flatContextVersion, {
    ...
    'com.docker.swarm.constraints': Docker.createSwarmConstraint('org', cvJSON.owner.github),
