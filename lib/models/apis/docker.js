...
Docker.prototype.deleteInstanceVolumes = function (volumes, cb) {
  const logData = {
    volumes
  }
  log.info(logData, 'Docker.prototype.deleteInstanceVolumes')
  volumes.forEach((volume) => {
    this._volumeAction(volume.Name, 'remove', {}, cb)
  })
}
 ...
 * @param {Function} cb standard callback
 */
Docker.prototype._volumeAction = function (volumeId, action, opts, cb) {
  const self = this
  const logData = {
    volumeId,
    action,
  ...
  volume[action](opts, function (err, response) {
    if (err) {
      monitor.increment('api.docker.call.failure.' + action, 1, [
        'code:' + err.statusCode
      ])
