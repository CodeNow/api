...
var socketStreams = require('socket/build-stream.js');
    ...
    version.populate('infraCodeVersion', function (err) {
      ...
      if (version.appCodeVersions && version.appCodeVersions.length > 0) {
        var repoUrl = url.format({
          protocol: 'https',
          slashes: true,
          auth: sessionUser.accounts.github.accessToken + ':x-oauth-basic',
          host: 'github.com',
          pathname: version.appCodeVersions[0].repo
        }, false, false);
      ...
      self.docker.createContainer({
        ...
        container.attach({stream: true, stdout: true, stderr: true}, function (err, stream) {
          ...
          socketStreams.sendStream(version._id, stream);
          ...
          container.start({ Binds: binds }, function (err, data) {
            ...
            container.wait(function (err, waitData) {
              ...
              var imageId;
              socketStreams.getBufferLog(version._id, function(err, buildLogData) {
                  if (err) {
                    return cb(err);
                  }
...
Docker.prototype.createAndInspectContainer = function (imageId, dockerHost, cb) {
  this.createContainer(imageId, dockerHost, function (err, container) {
    ...
    container
      .attach({stream: true, stdout: true, stdin: true, stderr: true}, function (err, stream) {
      if (err || !stream) {
        return cb(Boom.badGateway('docker attach failed', { err: err, docker: dockerHost }));
      }
      debug('attach container');

      // Since everything we need is now attached to the container's stream, let's send it out.
      socketStreams.sendStream(containerId, stream);
...
Docker.prototype.destroyContainerOnHost = function (container, cb) {
  if (!container.dockerContainer || !container.dockerHost) {
    cb(Boom.badRequest('Container does not have docker information'));
  } else {
    this.loadDocker(container.dockerHost);
    var dockerContainer = this.docker.getContainer(container.dockerContainer);
    dockerContainer.stop({}, wrapIfErr(cb, 502, 'Error inspecting container', {
      debug: { containerId: container.dockerContainer, dockerHost: container.dockerHost }
    }));
  }
};

Docker.prototype.destroyContainers = function (containers, cb) {
  if (!Array.isArray(containers)) {
    containers = [containers];
  }
  async.map(
    containers,
    this.destroyContainerOnHost.bind(this),
    cb);
};
...
Docker.prototype.stopContainer = function (container, cb) {
  ...
  this.docker
    ...
    .stop({}, function (err) {
      if (err) {
        cb(Boom.badGateway('Error stopping container', {
          err: err,
          debug: {
