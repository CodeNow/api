...
Docker.prototype.getLogs = function (containerId, tail, cb) {
  ...
  var timer = monitor.timer('api.docker.get_logs', true)
...
Docker.prototype.removeContainer = function (containerId, cb) {
  ...
  var self = this
  var timer = monitor.timer('api.docker.remove_container', true)
  self.docker
    .getContainer(containerId)
    .remove({},
      function (err, response) {
...
Docker.prototype.execContainer = function (containerId, cb) {
  ...
  var self = this
  var execTimer = monitor.timer('api.docker.exec', true)
  var execStarTimer = monitor.timer('api.docker.exec_start', true)
  self.docker
    .getContainer(containerId)
