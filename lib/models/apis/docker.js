 ...
 * @param {Object} opts.organizatiin - Organization object with `id` and `github` props
...
Docker.prototype._createImageBuilderEnv = function (opts) {
  ...
  // registry envs
  if (organization.private_registry_url && organization.private_registry_username) {
    env.push('RUNNABLE_VAULT_TOKEN_FILE_PATH=' + process.env.RUNNABLE_VAULT_TOKEN_FILE_PATH)
    env.push('RUNNABLE_VAULT_ENDPOINT=' + process.env.RUNNABLE_USER_VAULT_ENDPOINT)
    env.push('RUNNABLE_DOCKER_REGISTRY_URL=' + organization.private_registry_url)
    env.push('RUNNABLE_DOCKER_REGISTRY_USERNAME=' + organization.private_registry_username)
