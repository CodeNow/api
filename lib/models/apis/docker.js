...
var logger = require('middlewares/logger')('models:apis:docker');
...
function Docker (dockerHost) {
  logger.log.trace({tid: true, dockerHost: dockerHost}, 'new Docker');
  if (!dockerHost) {
...
Docker.prototype.startImageBuilderContainer = function (container, cb) {
  logger.log.trace({tid: true, container: container}, 'startImageBuilderContainer');
  var self = this;
  var binds = [
...
Docker.prototype.getBuildInfo = function(containerId, cb) {
  logger.log.trace({tid: true, containerId: containerId}, 'getBuildInfo');
  var self = this;
  ...
  container.logs({
    ...
    logger.log.trace({tid: true, err: err}, 'getBuildInfo build logs received');
...
Docker.prototype.startContainer = function (container, opts, cb) {
  ...
  logger.log.info(startInfo, 'startContainer');
  self.docker
    .getContainer(containerId)
    .start(opts,
      function (err) {
        if (err) {
          logger.log.error(startInfo, 'startContainer error');
        }
        else {
          logger.log.info(startInfo, 'startContainer success');
        }
