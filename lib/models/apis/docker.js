...
/**
 * @param {String} dockerHost host to connect to
...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  if (process.env.DOCKER_IMAGE_BUILDER_WEAVE_PATH) {
    builderContainerData.Binds.push(
 ...
 * @param {Object} contextVersion cv to validate
 * @return {Object} || {{null}}
 */
 ...
 * @param {Object} opts options
 * @param {Object} opts.contextVersion cv
 * @param {String} opts.dockerTag format: registry.runnable.com/<user>/<id>
 * @param {Object} opts.network format: 10.0.0.0
 * @param {Boolean} opts.manualBuild is this build manual
 * @param {Boolean} opts.noCache true if we want to not use cache
 * @param {Object} opts.sessionUser session user
 * @param {String} opts.tid transaction id
...
Docker.prototype._createImageBuilderEnv = function (opts) {
  ...
  var indexedVersions = indexBy(infraCodeVersion.files, 'Key');
 ...
 * @param {Function} cb (err, inspectData)
 */
 ...
 * @returns {Function} accepts err object
 */
