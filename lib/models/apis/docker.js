 ...
 */
Docker.prototype._evalEnvVars = function (envVars) {
  var envVarHash = {}
  ...
  return envVars.map(function (env) {
    ...
    // Add values to hash sequentially in order to keep ENV order
    envVarHash[envKey] = envContent

    var envMatches = envContent.match(globalEnvVarRegex)
    if (envMatches !== null) {
      return env.replace(globalEnvVarRegex, function (text) {
