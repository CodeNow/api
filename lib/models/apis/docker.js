...
Docker.prototype.createImageBuilder = function (opts, cb) {
  ...
  // }
  rabbitMQ.buildContainerRequested(builderContainerData)
...
Docker.prototype.createUserContainer = function (opts, cb) {
  ...
  joi.validateOrBoom(map(opts, toJSON), joi.object({
    ...
    self._createUserContainerLabels(opts, function (err, labels) {
      ...
      if (instance.owner.github === 2335750) {
        rabbitMQ.applicationContainerRequested(userContainerData)
        return cb()
      } else {
...
Docker.prototype._createUserContainerLabels = function (opts, cb) {
  ...
  joi.validateOrBoom(map(opts, toJSON), joi.object({
    ...
    Docker._addIfExist(instance.isTesting, 'isTesting', labels)
    Docker._addIfExist(instance.isTestReporter, 'isTestReporter', labels)
    Docker._addIfExist(instance.isolated, 'isolated', labels)
    Docker._addIfExist(instance.masterPod, 'masterPod', labels)
...
Docker._addIfExist = function (item, key, target) {
  if (!!item) {
    target[key] = item
  }
}
