...
Docker.prototype.startImageBuilderAndWait = function (sessionUser, version, container, cb) {
  ...
  self.startContainer(container, { Binds: binds }, function (err) {
    ...
    container.wait(function (err, res) {
      ...
      if (err) {
        return self.handleError(cb, 'docker wait failed', { containerId: container.Id })(err);
      }
      ...
      container.logs({
        ...
        var buildLogData = '';
        // Since the encoding is not set on the stream, data should just be a buffer
        stream.on('data', function(data) {
          // Write it to hex so we don't corrupt anything
          buildLogData += data.toString('hex');
        });
        stream.on('end', function() {
          // buildLogData should have the whole log, so now let's filter it
          filterLog(buildLogData, function(buildLogData) {
            ...
            if (buildFailed) {
              ...
              if (res.StatusCode) {
                cb(Boom.badRequest('Build Failed: Dockerfile error code: ' + res.StatusCode,
                  { docker: { containerId: container.Id, log: buildLogData }}));
              } else {
                return self.handleError(cb, 'build failed to build', { containerId: container.Id })(
                  new Error('build failed to build: ' + buildLogData)
                );
              }
            } else {
              debug('build success');
...
function filterLog(buildLogData, cb) {
  // First, split it with (headerByte)000000 (the three bytes in between each message u/
  var messageArray = buildLogData.split(/(01|02|03)000000/);

  // messageArray should now be the messages, with only the 4 size byte at the beginning.
  async.map(messageArray, function(message, cb) {
    if (message && message !== '') {
      // Slice off the 8 characters from the front (4 bytes), then convert it to utf8
      cb(null, new Buffer(message.slice(8), 'hex').toString('utf8'));
    } else {
      cb();
    }
...
Docker.prototype.inspectContainer = function (container, cb) {
  ...
  var dockerContainer = this.docker.getContainer(containerId);

  var inspectOp = retry.operation();
  cb = this.handleError(cb, 'Error inspecting container', { containerId: containerId });
  inspectOp.attempt(function (attemptNumber) {
    debug('attempt number', attemptNumber);
    dockerContainer.inspect(function () {
      var args = Array.prototype.slice(arguments);
      var err = args[0];
      // if we are to retry, retry
      if (inspectOp.retry(err)) { return; }
      // else pass the error along
      cb.apply(err ? inspectOp.mainError() : null, arguments);
    });
  });
};
