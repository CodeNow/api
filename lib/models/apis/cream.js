...
module.exports = class CreamAPI {
  ...
  static getPaymentMethodForOrganization (organizationId) {
    ...
    return CreamAPI._makeReuest('payment-method', organizationId)
  }
  ...
  static _makeReuest (path, organizationId, body) {
    log.info({ path: path, organizationId: organizationId, body: body }, '_makeReuest')
    let opts = {
      method: 'GET',
    ...
    log.trace({ opts: opts }, '_makeReuest opts')
    return Promise.fromCallback(cb => {
      ...
      .then(function parseResponse (res) {
        ...
        if (res.statusCode >= 500) {
          log.trace({ err: res.body }, '_makeReuest error')
        ...
        let json
        log.trace('_makeReuest parse body')
        try {
        ...
        log.trace({ json: json }, '_makeReuest response')
        return json
      })
  }
