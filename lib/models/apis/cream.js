...
module.exports = class CreamAPI {
   ...
   * @param {String}   ownerEmail      - Email to associate with paymentMethod owner
   * @returns {Promise}
   */
  static postPaymentMethodForOrganization (organizationId, stripeToken, ownerBigPoppaId, ownerEmail) {
    log.info({
      organizationId,
      stripeToken,
      ownerBigPoppaId,
    ...
    return Promise.fromCallback(function (cb) {
      ...
      .then(function parseResponse (res) {
        ...
        if (res.statusCode === 404) {
          log.trace({ err: res.body }, '_makeRequest 404 error')
          let message = keypather.get(res, 'body.message') || 'No Payment Method Found'
          throw Boom.notFound(`Cream Bad Request: No payment method found: ${message}`, { err: res.body, opts: opts })
        }
        if (res.statusCode >= 400) {
          log.trace({ err: res.body }, '_makeRequest error')
          let message = keypather.get(res, 'body.message') || 'Unknown error'
          throw Boom.badRequest(`Cream Bad Request: ${message}`, { err: res.body, opts: opts })
        }
        let json
