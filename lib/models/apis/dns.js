...
Dns.prototype.putEntryForInstance = function (branch, instanceName, ownerUsername, hostIp, cb) {
  debug('putEntryForInstance', formatArgs(arguments));
  var url = Dns.generateUrl(instanceName, ownerUsername);
  var urlMP = Dns.generateUrl(instanceName + '-staging', ownerUsername);
  var urlDirect = Dns.generateUrl(branch + '-' + instanceName + '-staging', ownerUsername);
  var count = createCount(3, cb);
  this.putEntry(url, hostIp, count.next);
  this.putEntry(urlMP, hostIp, count.next);
  this.putEntry(urlDirect, hostIp, count.next);
};

Dns.prototype.deleteEntryForInstance = function (branch, instanceName, ownerUsername, hostIp, cb) {
  debug('deleteEntryForInstance', formatArgs(arguments));
  var url = Dns.generateUrl(instanceName, ownerUsername);
  var urlMP = Dns.generateUrl(instanceName + '-staging', ownerUsername);
  var urlDirect = Dns.generateUrl(branch + '-' + instanceName + '-staging', ownerUsername);
  var count = createCount(3, cb);
  this.deleteEntry(url, hostIp, count.next);
  this.deleteEntry(urlMP, hostIp, count.next);
  this.deleteEntry(urlDirect, hostIp, count.next);
};
