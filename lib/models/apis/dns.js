...
var redisList = new RedisList(process.env.REDIS_NAMESPACE+'dns-changes');
...
function batchChangeRequestHandler () {
  redisList.lock(function (err, hasLock) {
    if (err) {
      debug('redisList.lock', err);
      error.log('redisList.lock', err);
    }
    if (!hasLock) { return; }
    ...
    activeApi.isMe(function (err, isMe) {
      ...
      redisList.llen(function (err, listLength) {
        if (err) {
          ...
          error.log('llen', redisList.key, err);
        }
          ...
          redisList.lrangepop(0, Math.min(99, listLength-1), function (err, apiDataList) {
            if (err) {
              ...
              body.ChangeBatch.Changes.push(apiDataList.map(function (listItem) {
