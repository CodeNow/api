...
Dns.generateUrls = function (instanceName, ownerUsername, instance) {
  var urls = [];

  var opts = {
    shortHash: instance.shortHash,
    instanceName: instanceName, // don't use instance.name!
    branch: Instance.getMainBranchName(instance),
    ownerUsername: ownerUsername,
    masterPod: instance.masterPod,
    userContentDomain: process.env.USER_CONTENT_DOMAIN
  };
  if (opts.masterPod) { // master w/ repo, ex: api master
    if (opts.branch) {
      urls.push(runnableHostname.elastic(opts));
      urls.push(runnableHostname.direct(opts));
    }
...
Dns.prototype.putEntryForInstance = function (instanceName, ownerUsername, instance, cb) {
  ...
  var urls = Dns.generateUrls(instanceName, ownerUsername, instance);
