...
var RedisList = require('models/redis/list')
var redisList = new RedisList('dnsChanges');
...
setInterval(batchChangeRequestHandler, 3000);
function batchChangeRequestHandler () {
  ...
  redisList.llen(function (err, result) {
    if (err) {
      debug('llen', redisList.key, err);
    }
    else if (result === 0) {
      //debug('llen empty');
    }
    else {
      redisList.lrangepop(0, Math.min(99, result-1), function (err, result) {
        if (err) {
          ...
          body.ChangeBatch.Changes.push(result);
