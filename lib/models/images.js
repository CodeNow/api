...
imageSchema.statics.findFirstImageInChannel = function (channelId) {
  ...
  Image.find({}, fields, opts, function (err, images) {
      ...
      if (err) {
        ...
        fs.exists('' + runnablePath + '/Dockerfile', function (exists) {
          if (!exists) {
            ...
            fs.readFile('' + runnablePath + '/Dockerfile', 'utf8', domain.intercept(function (dockerfile) {
...
imageSchema.statics.relatedChannelIds = function (domain, channelIds, cb) {
  this.distinct('tags.channel', { 'tags.channel': { $in: channelIds } }, domain.intercept(function (channelIds) {
    cb(null, channelIds);
  }));
};
