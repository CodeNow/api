var ObjectId, Schema, async, buildDockerImage, configs, copyPublishProperties, cp, crypto, domain, encodeId, error, fs, imageSchema, minus, mongoose, mu, path, plus, request, slash, sync, syncDockerImage, textSearch, underscore, uuid, _, self = this;
async = require('async');
                ...
                return mu.compileText('Dockerfile', dockerfile, function (err, compiled) {
                  ...
                  if (err) {
                    ...
                    writestream.on('close', function () {
                      return self.findOne({ name: runnable.name }, domain.intercept(function (existing) {
