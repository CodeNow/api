...
(function() {
  var ObjectId, Schema, async, buildDockerImage, configs, copyPublishProperties, cp, crypto, domain, encodeId, error, fs, imageSchema, minus, mongoose, mu, path, plus, request, slash, sync, syncDockerImage, textSearch, underscore, uuid, _,
    _this = this;
  ...
  imageSchema.statics.incVote = function(domain, runnableId, cb) {
    ...
    }, domain.intercept(function(success) {
  ...
  imageSchema.statics.relatedChannelIds = function(domain, channelIds, cb) {
    return this.distinct('tags.channel', {
      'tags.channel': {
        $in: channelIds
      }
  ...
  copyPublishProperties = function(image, container, noOwner) {
    ...
    properties = ['name', 'description', 'tags', 'files', 'image', 'dockerfile', 'file_root', 'file_root_host', 'cmd', 'build_cmd', 'start_cmd', 'service_cmds', 'port', 'output_format'];
    objectIdProperties = ['parent', 'specification'];
    if (!noOwner) {
      objectIdProperties.push('owner');
    }
    properties.forEach(function(property) {
      return image[property] = _.clone(container[property]);
    });
    return objectIdProperties.forEach(function(property) {
      return image[property] = container[property] && container[property].toString();
    });
  };
