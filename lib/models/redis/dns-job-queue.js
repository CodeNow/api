...
DnsJobQueue.prototype.execJob = function (params, cb) {
  ...
  self.on(params.id, function (errStr) {
    debug('execJob error', params.id, formatArgs(arguments));
    // if err, cast to err..
...
DnsJobQueue.prototype.getJobs = function (first, last, cb) {
  ...
  this.redis.multi()
    ...
    .ltrim(this.key, last, -1)
    .exec(function (err, results) {
      if (err) { return cb(err); }
      debug(results[0]);
      debug(typeof results[0]);
      debug(Array.isArray(results[0]));
      debug(results[1]);
      debug(typeof results[1]);
      debug(Array.isArray(results[1]));
