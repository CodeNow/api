...
HipacheHosts.prototype.createRoutesForContainer =
  function (ownerUsername, instance, container, cb) {
    ...
    if (!container.dockerHost) {
      return cb(Boom.badRequest('Container is missing dockerHost',
        { debug: container.toJSON() }));
    }
    if (!container.dockerContainer) {
      return cb(Boom.badRequest('Container is missing dockerContainer',
        { debug: container.toJSON() }));
    }
    dns.putEntryForInstance(instance, ownerUsername, function(err) {
      if (err) { return cb(err); }
      if (container.ports) {
        return async.each(Object.keys(container.ports), function (containerPort, cb) {
...
HipacheHosts.prototype.removeRouteForContainerPort =
  function (ownerUsername, instance, container, containerPort, cb) {
    var hipacheHost = new HipacheEntry(containerPort, instance, ownerUsername);
    dns.deleteEntryForInstance(instance, ownerUsername, function(err) {
      if (err) { return cb(err); }
      hipacheHost.del(cb);
    });
  };
