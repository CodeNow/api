...
Hosts.prototype.parseHostname = function (hostname, cb) {
  ...
  this.validateHostname(hostname, function (err) {
    if (err) {
      return cb(err)
    }
...
Hosts.prototype.validateHostname = function (hostname, cb) {
  log.info({
    tx: true,
    hostname: hostname
  }, 'Hosts.prototype.validateHostname')
 ...
 */
Hosts.prototype.upsertHostsForInstance = function (ownerUsername, instance, instanceName, container, cb) {
  var logData = {
    tx: true,
...
Hosts.prototype.removeHostsForInstance = function (lightEntry, container, cb) {
  ...
  if (/^[A-z0-9]*--/.test(lightEntry.instanceName) && !lightEntry.isolated) {
    var err = new Error('NaviEntry missing isolation opts')
    err.data = lightEntry
    // We can still push this through for now
    error.report(err)
    // We know it's not an isolatedMaster since it matched our naming scheme
    lightEntry.isolated = true
  }
  if (!container || !container.ports || !Object.keys(container.ports).length) {
    log.warn('missing container or container ports')
    return cb()
  }
  ...
  Object.keys(container.ports).forEach(function (containerPort) {
    ...
    }, 'container.ports forEach removing entry')
    var entry = put({
      exposedPort: containerPort,
      userContentDomain: process.env.USER_CONTENT_DOMAIN
    }, lightEntry)
