...
Hosts.prototype.upsertHostsForInstance =
  function (ownerUsername, instance, instanceName, container, cb) {
    ...
    cb = args.cb;
    var branchName = keypather.get(instance, 'contextVersion.appCodeVersions[0].lowerBranch');
    var hosts = this;
    if (!container || !container.dockerContainer || !container.ports) {
      ...
      dns.putEntryForInstance(
        branchName, instanceName, ownerUsername, instance.network.hostIp, count.next);
      ...
      Object.keys(container.ports).forEach(function (containerPort) {
        hosts.upsertHostForContainerPort(
          containerPort, branchName, ownerUsername, instance, instanceName, count.next);
      });
    }
  };
...
Hosts.prototype.upsertHostForContainerPort =
  function (containerPort, branch, ownerUsername, instance, instanceName, cb) {
    var hipacheHost = new HipacheEntry(containerPort, instanceName, ownerUsername);
    var hipacheHostMP = new HipacheEntry(containerPort, instanceName, 'staging-' + ownerUsername);
    var hipacheHostDirect =
    ...
    async.series([
      ...
      hipacheHostMP.del.bind(hipacheHostMP),
      hipacheHostMP.rpush.bind(hipacheHostMP, instanceName, backendUrl),
      hipacheHostDirect.del.bind(hipacheHostDirect),
      hipacheHostDirect.rpush.bind(hipacheHostDirect, instanceName, backendUrl)
    ], cb);
  };
