 ...
 * @param  {string}   username  name of the instance owner
 * @param  {Function} cb   callback(err, instanceName)
 */
Hosts.prototype.parseInstanceNameFromHostname = function (hostname, username, cb) {
  parseSubdomainFromHostname(hostname, function (err, subdomain) {
    ...
    var instanceName = subdomain.replace(regex, ''); // name-username
...
function invalidHostname (cb) {
  cb(new Boom.badRequest('Invalid hostname (ex: name-org.'+process.env.USER_CONTENT_DOMAIN+')'));
}
