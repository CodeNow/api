 ...
 * upsert hosts (hipache) for   instance
 * @param  {String}     ownerUsername   instance owner's username
 * @param  {String}     mainBranch      lowerBranch name of the main appCodeVersion
 * @param  {String}     shortHash       instance shortHash
 * @param  {String}     ownerGithub     instance owner github id
 * @param  {Boolean}    masterPod       instance masterPod flag
 * @param  {String}     [instanceName]  instanceName (could be diff from current name - old or new)
  ...
  function (ownerUsername, branch, shortHash, ownerGithub, masterPod, instanceName, container, cb) {
    ...
    if (!container || !container.ports || !Object.keys(container.ports).length) {
      log.warn({
        tx: true,
        container: container
      }, 'removeHostsForInstance: missing container or container ports');
      cb();
    }
    else {
      var count = createCount(Object.keys(container.ports).length, function (err) {
        if (err) {
          log.trace({
            ...
            container: container,
            err: err
          }, 'removeHostsForInstance: failure');
        }
        else {
          log.trace({
            tx: true,
            container: container
          }, 'removeHostsForInstance: success');
        }
      ...
      Object.keys(container.ports).forEach(function (containerPort) {
        log.trace({
          ...
          instanceName: instanceName,
          USER_CONTENT_DOMAIN: process.env.USER_CONTENT_DOMAIN
        }, 'removeHostsForInstance container.ports forEach removing entry');
        new NaviEntry({
          shortHash: shortHash,
          exposedPort: containerPort,
          branch: branch,
