 ...
 * @param  {Object}     lightEntry      light object with following properties:
                                          ...
                                          - masterPod
                                          - instanceName
 * @param  {Object}  ports              container ports
 * @param  {Function}   cb              callback
 */
Hosts.prototype.removeHostsForInstance = function (lightEntry, ports, cb) {
  var logData = put({
    tx: true,
    ports: ports
  }, lightEntry)
  ...
  var count = createCount(Object.keys(ports).length, function (err) {
    if (err) {
      log.trace(put({
        err: err
  ...
  Object.keys(ports).forEach(function (containerPort) {
    ...
    log.trace(put(entry, logData), 'removeHostsForInstance container.ports forEach removing entry')
