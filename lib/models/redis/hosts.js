...
/**
 * parse instance url into name and username
 * @param  {string}   url  url of an instance
 * @param  {string}   name name of the instance
 * @param  {Function} cb   [description]
 * @return {[type]}        [description]
 */
Hosts.prototype.parseUsernameFromUrl = function (url, name, cb) {
  parseSubdomainFromUrl(url, name, function (err, subdomain) {
    if (err) { return cb(err); }
    var username = subdomain.replace(name+'-', ''); // name-orgname split
    if (username.length === 0) {
      return invalidUrl(cb);
    }
    cb(null, {
      username: username, // must be first
      name: name
    });
  });
};
function parseSubdomainFromUrl (url, name, cb) {
  var parsed = parse(url);
  if (!parsed.hostname ||
      !~parsed.hostname.indexOf(process.env.USER_CONTENT_DOMAIN)) {
    return invalidUrl(cb);
  }
  var split = parsed.hostname.split('.'); // url dot split (subdomains)
  if (split.length !== 3) {
    return invalidUrl(cb);
  }
  var subdomain = split[0];
  if (subdomain.indexOf(name) !== 0) {
    return invalidUrl(cb);
  }
  cb(null, subdomain);
}
function invalidUrl (cb) {
  cb(new Boom.badRequest('Invalid url (ex: http://name-org.'+process.env.USER_CONTENT_DOMAIN+')'));
}
