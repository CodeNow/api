...
ContextService.handleVersionDeepCopy = function (context, contextVersion, user, opts, cb) {
  ...
  var newOwner  = opts.owner || {
    github: user.accounts.github.id
  };
  var ownerUnchanged = contextVersion.owner.github === newOwner.github;
  if (ownerUnchanged) {
    // no need to copy context
    // deep copy context version only!
    return ContextVersion.createDeepCopy(user, contextVersion, newOwner, cb);
  }
  // 1. deep copy contextVersion
  ContextVersion.createDeepCopy(user, contextVersion, newOwner, function (err, newContextVersion) {
    ...
    newContextVersion.owner = newOwner;
