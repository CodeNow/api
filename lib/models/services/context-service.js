...
ContextService.createNew = function (sessionUser, payload) {
  ...
  return joi.validateOrBoomAsync(data, newContextSchema)
    ...
    .then(() => {
      return ContextService._addBpInfoToDataFromSessionUser(sessionUser, data.owner.github, data)
...
ContextService._addBpInfoToDataFromSessionUser = function (sessionUser, githubOrgId, data) {
  ...
  const bpOrgId = UserService.getBpOrgInfoFromGithubOrgId(sessionUser, githubOrgId)
  keypather.set(data, 'owner.bigPoppa', bpOrgId)
  log.info({ bpUserId, bpOrgId }, 'add BP data to context version')
}
