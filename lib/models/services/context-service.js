...
ContextService.handleVersionDeepCopy = function (context, contextVersion, user, opts, cb) {
  ...
  if (contextOwnerId === process.env.HELLO_RUNNABLE_GITHUB_ID && userGithubId !== contextOwnerId) {
    ...
    ContextVersion.createDeepCopy(user, contextVersion, function (err, newContextVersion) {
      ...
      // the owner should be either opts' owner (req.body) or the user who created it
      ...
      var newContextVersionUpdate = {
        $set: {
          // 3. 'move' new contextVerion -> new context
          context: newContext._id,
          // 4. update the owner of the contextVersion
          // the new context version should be owned by the same person who owns the context
          'owner.github': ownerGithubId
        }
      }
      ...
      async.series([
        ...
        function updateContextVersion (cb) {
          ContextVersion.findOneAndUpdate(
            { _id: newContextVersion._id },
            newContextVersionUpdate,
            function (err) {
              if (err) { cb(err) }
              ContextVersion.findOne({ _id: newContextVersion._id }, cb)
            }
          )
        },
        ...
        // [1]: newContextVersion.findOne result
        cb(err, keypather.get(results, '[1]'))
      })
    })
  } else {
