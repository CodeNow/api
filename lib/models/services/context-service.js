...
ContextService.createNew = function (sessionUser, payload) {
  ...
  log.info('called')
  if (!keypather.get(data, 'owner.github')) {
  ...
  return joi.validateOrBoomAsync(data, newContextSchema)
    ...
    .then(() => {
      return Promise.try(() => {
        ContextService._addBpInfoToDataFromSessionUser(sessionUser, data.owner.github, data)
      })
      // we do not want error form above to stop this
      .catch(noop)
    })
    .then(function () {
...
/**
 * Adds createdBy.bigPoppa, and owner.bigPoppa info to `data` argument
 * @param {SessionUser} sessionUser
 * @param {Number} githubOrgId
 * @param {Object} data
 */
ContextService._addBpInfoToDataFromSessionUser = function (sessionUser, githubOrgId, data) {
  const log = this.logger.child({
    method: '_addBpInfoToDataFromSessionUser',
    sessionUser, githubOrgId, data
  })
  const bpUserId = keypather.get(sessionUser, 'bigPoppaUser.id')
  keypather.set(data, 'createdBy.bigPoppa', bpUserId)

  const bpOrgInfo = UserService.getBpOrgInfoFromGithubOrgId(sessionUser, githubOrgId)
  keypather.set(data, 'owner.bigPoppa', bpOrgInfo.id)
  log.info({ bpUserId, bpOrgInfo }, 'add BP data to context version')
}
