...
module.exports = class ClusterDataService {
  ...
  static populateInstanceWithClusterInfo (instance) {
    ...
    return Promise.try(() => {
      ...
      return Instance.findOneByShortHashAsync(instanceParentShortHash)
        .then(function (parentInstance) {
          return keypather.get(parentInstance, '_id.toString()')
        })
    })
      ...
      .then((aiConfigs) => {
        ...
        .then((clusters) => {
          ...
          instances.forEach(function (instance) {
            // if instance isolated take parent
            const instanceParentShortHash = keypather.get(instance, 'parent.toString()')
            const instanceLookupId = instance.isolated ? 
              instancesShortHashesToIds[instanceParentShortHash] : keypather.get(instance, '_id.toString()')
            const aig = instancesByAig[instanceLookupId]
            log.trace({
              aig,
              instanceId: instanceLookupId
