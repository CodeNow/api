  ...
  static populateInstancesWithClusterInfo (instances, sessionUser) {
    ...
    return Promise.try(() => {
      ...
      return AutoIsolationConfig.findAllActive({
        ...
        .then((aiConfigs) => {
          log.trace({ ids: aiConfigs.map(pluck('_id')) }, 'found AutoIsolationConfigs')
          ...
          return InputClusterConfig.findAllActive({
            ...
            .then((clusters) => {
              ...
              return Promise.all(instances.map(function (instance) {
                ...
                let instanceLookupIdPromise = Promise.resolve(instanceLookupId)
                if (!instanceLookupId) {
                  // if parent not found - fetch it
                  instanceLookupIdPromise = Instance.findInstanceIdByShortHash(instanceParentShortHash)
                }
                // if instance not isolated take instanceId
                if (!instance.isolated) {
                  instanceLookupIdPromise = Promise.resolve(instanceId)
                }
                ...
                return instanceLookupIdPromise
                  .then((instanceLookupId) => {
                    const aig = instancesByAig[instanceLookupId]
                    log.trace({ aig, instanceLookupId }, 'found aig for instance')
                    if (aig) {
                      const cluster = find(clusters, hasProps({ autoIsolationConfigId: aig._id }))
                      if (cluster) {
                        const clusterConfigData = ClusterDataService.makeClusterData(cluster, aig)
                        log.trace({ instanceId, clusterConfigData }, 'adding cluster config data to the instance')
                        instance.inputClusterConfig = clusterConfigData
                        if (instance._doc) {
                          instance._doc.inputClusterConfig = clusterConfigData
                        }
                        log.trace({ instanceId, clusterConfigData }, 'added cluster config data to the instance')
                      } else {
                        log.trace({ aig, clusters }, 'not found cluster by autoIsolationConfig')
                      }
                    }
                  })
              })).return(instances)
    ...
    .finally((instances) => {
      log.trace({ instances }, 'result of attaching cluster data to instances')
      return instances
    })
  }
}
