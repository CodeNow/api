...
module.exports = class ClusterDataService {
   ...
   * @param {Instance[]}                 instances.parent - Parent shortHash
   *
   * @returns {Object<InputClusterConfig>} iccsByInstanceId
   * @private
   */
  static _setClustersOnAllInstances (iccsByInstanceId, instances) {
  ...
  static populateInstancesWithClusterInfo (instances) {
    const instanceIds = (instances || []).map(pluck('_id'))
    const log = ClusterDataService.log.child({
    ...
    return Promise.try(() => {
      ...
      .then(iccsByInstanceId => this._setClustersOnAllInstances(iccsByInstanceId, instances))
      .catch(err => {
        // We don't want to rethrow error at least for now. UI will degrade but still could work
