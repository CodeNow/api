...
module.exports = class ClusterDataService {
  ...
  static _fetchParentsAndAddToArray (instances) {
    const allInstances = [].concat(instances) // make a new array so we don't change the list going to the user
    return Instance.fetchParentInstances(instances)
      .reduce((allInstances, parentInstance) => {
        allInstances.push(parentInstance)
        return allInstances
      }, allInstances)
  }
