...
AutoIsolationService.fetchAutoIsolationForInstance = function (instanceId) {
  return AutoIsolationConfig.findActiveByInstanceId(instanceId)
    .catch(AutoIsolationConfig.NotFoundError, err => {
      // To disable branch compose, we just need to turn off the ability to fetch it's inherited one
      // This means all branches will fail fetching this, and just autodeploy
      if (!process.env.ENABLE_BRANCH_COMPOSE) { throw err }
...
AutoIsolationService.fetchDependentInstances = function (mainInstance, autoIsolationConfig) {
  ...
  return Promise
    .map(cleanDeps, dep => AutoIsolationService._fetchIsolationInstanceModel(dep.instance))
    .filter(dep => !!dep) // filter out any that are missing
}

/**
 * Fetches the instance that belongs to the isolation owned by the mainInstance.
 *
 * @param {ObjectId}            instanceId          - Id of the 'requested dependency' for the cluster
 *
 * @resolves {Instance}  instance - The requested instance
 */
AutoIsolationService._fetchIsolationInstanceModel = function (instanceId) {
