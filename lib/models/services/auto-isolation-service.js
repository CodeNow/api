 ...
 *
 * @param {ObjectId} mainInstanceId - Instance Id for the main instance of the cluster
 * @returns {Promise}
 * @resolve {Instance[]} All the dependent instances of the Isolation Config of the given instance
 */
AutoIsolationService.fetchAutoIsolationDependentInstances = function (mainInstanceId) {
  const log = AutoIsolationService.logger.child({
    method: 'fetchAutoIsolationDependentInstances',
    mainInstanceId
  })
  log.trace('called')
  return AutoIsolationConfig.findActiveByInstanceId(mainInstanceId)
    .get('requestedDependencies')
    .filter(hasKeypaths(['instance']))
    .map(dep => {
      return InstanceService.findInstanceById(dep.instance)
        .catchReturn(Instance.NotFoundError, undefined) // If not found, return undefined
    })
    .filter(exists) // and filter them out
}
