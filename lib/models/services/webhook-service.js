...
WebhookService.autoFork = function (contextIds, githubPushInfo) {
  ...
  return this.checkCommitPusherIsRunnableUser(githubPushInfo)
    .then(function () {
      return Instance.findForkableMasterInstancesAsync(githubPushInfo.repo, githubPushInfo.branch, contextIds)
    })
    .catch(function (err) {
  ...
  return Instance.findInstancesLinkedToBranchAsync(githubPushInfo.repo, githubPushInfo.branch)
    ...
    .then(function (instances) {
      ...
      if (process.env.ENABLE_AUTOFORK_ON_BRANCH_PUSH) {
        ...
        var uniqueContextIds = unique(instances
          .filter(function (instance) {
            // If a masterPod has isolated children, but not pod children, we may still want to
            // autoFork it
            return !instance.isolated
          })
          .map(pluck('contextVersion.context.toString()'))
        )

        if (uniqueContextIds.length) {
          promises.push(WebhookService.autoFork(uniqueContextIds, githubPushInfo))
        }
      }
