 ...
 * @resolves {[String]}           Array of Instance Ids which were deleted
 ...
 * do not count
 *
 * @param {[String]} contextIds            - ContextIds of Pods that already have an instance with
 *                                             this branch
 * @param {Object}   githubPushInfo        - githook data
 * @param {Object}   githubPushInfo.repo   - name of the repository that was updated
 * @param {Object}   githubPushInfo.branch - name of the branch that was updated
 *
 * @returns  {Promise}    When all of the instances have been created, and auto-isolated (if applicable)
 * @resolves {[Instance]} Instances will
 */
WebhookService.autoFork = function (contextIds, githubPushInfo) {
  ...
  return this.checkCommitterIsRunnableUser(githubPushInfo)
    ...
    .then(function () {
      Instance.findForkableMasterInstancesAsync(githubPushInfo.repo, githubPushInfo.branch, contextIds)
    })
    .tap(function (instances) {
      if (instances.length === 0) {
