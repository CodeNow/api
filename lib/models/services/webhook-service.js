...
WebhookService.autoDelete = function (githubPushInfo) {
  ...
  return Instance.findNonIsolatedForkedInstances(githubPushInfo.repo, githubPushInfo.branch)
    ...
    .tap(function (instancesIds) {
      log.info({ instancesIds }, 'autoDelete deleted instances')
    })
}
...
WebhookService.autoDeploy = function (instances, githubPushInfo) {
  ...
  function checkCompose (instance) {
    return ClusterConfigService.checkIfComposeFileHasChanged(instance._id, githubPushInfo)
      .then(() => {
        return rabbitMQ.updateCluster({
  ...
  return Promise.map(instances, checkCompose)
}
...
WebhookService.checkCommitPusherIsRunnableUser = function (githubPushInfo) {
  ...
  return Promise.try(function () {
    ...
    .tap(function (record) {
      if (!record) {
        log.error({ username: committerUsername }, 'checkCommitPusherIsRunnableUser Committer is not a Runnable user')
...
WebhookService.parseGitHubPushData = function (payload) {
  ...
  log.info('parseGitHubPushData called')
