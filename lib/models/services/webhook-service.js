 ...
 * @resolves {[Object]} Array of maps containing the new builds, cvs, and user model for
 *                        each instance
...
WebhookService.parseGitHubPushData = function (payload) {
  ...
  return Promise.try(function () {
    if (!repository) {
      var repErr = Boom.badRequest('Unexpected commit hook format. Repository is required')
      log.error({
        error: repErr
      }, 'doAutoDeployAndAutoFork Repository is required')
      throw repErr
    }
    ...
    var ref = payload.ref
    if (!ref) {
      var refErr = Boom.badRequest('Unexpected commit hook format. Ref is required', { payload: payload })
      log.error({
        error: refErr
      }, 'doAutoDeployAndAutoFork Ref is required')
      throw refErr
    }
