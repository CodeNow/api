 ...
 * it. Rabbit job 'application.container.delete' will be created for each instance found
 *
...
WebhookService.checkCommitPusherIsRunnableUser = function (githubPushInfo) {
  ...
  return Promise.try(function () {
    ...
    .tap(function (record) {
      if (!record) {
        const committerNotRunnableUserError = Boom.forbidden(
...
WebhookService.checkRepoOrganizationAgainstWhitelist = function (githubPushInfo) {
  ...
  return OrganizationService.getByGithubUsername(orgGithubName)
    .catch(errors.OrganizationNotFoundError, function (err) {
      ...
      const error = Boom.forbidden('Repo owner is not registered on Runnable', {
        originalError: err
      })
      error.data = { level: 'info' }
      throw error
    })
    .tap(function (record) {
      if (!record.allowed) {
  ...
  return WebhookService.parseGitHubPushData(payload)
    ...
    .tap(function (githubPushInfo) {
      ...
      if (ref && ref.indexOf('refs/tags/') === 0) {
        const err = new NotImplementedException('processGithookEvent', 'Cannot handle tags\' related events')
        throw err
      }
    })
