...
const UserService = module.exports = {
   ...
   * @return {Promise}
   */
  getMongoUserByIdWithBigPoppaUser: function (id) {
    const log = UserService.log.child({
      method: 'UserService.getMongoUserByIdWithBigPoppaUser',
      id
    })
    log.info('getMongoUserByIdWithBigPoppaUser called')
   ...
   * @return {Promise}
   */
  getMongoUserByGithubIdWithBigPoppaUser: function (githubId) {
    const log = UserService.log.child({
    ...
    return Promise.all([
      Users.findByGithubIdAsync(githubId),
    ...
    .catch(function (err) {
      log.warn({ err }, 'Error getting bigpoppa user')
      throw err
    })
  }
}
