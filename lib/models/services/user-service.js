...
const UserService = module.exports = {
   ...
   */
  assertFound: function (githubId) {
    return function (user) {
      if (!user) {
        throw new Users.NotFoundError({
          githubId: githubId
        })
      }
    }
  ...
  getUser: function (accounts) {
    ...
    return bigPoppaClient.getUsers(opts)
      .get('0')
      .tap(UserService.assertFound(opts.githubId))
      .catch(function (err) {
   ...
   *
   * @returns {Organization|Undefined} Org model if the user is part of it, null if not
   */
  getOrgFromUserByGithubId: function (user, orgGithubId) {
  ...
  isUserPartOfOrgByGithubId: function (user, orgGithubId) {
    return !!this.getOrgFromUserByGithubId(user, orgGithubId)
  },
  ...
  getByGithubId: function (githubId) {
    ...
    return bigPoppaClient.getUsers({ githubId: githubId })
      .get('0')
      .tap(UserService.assertFound(githubId))
      .catch(function (err) {
        log.error(err, 'UserService.getByGithubId failed to fetch the user')
        throw err
      })
  ...
  getCompleteUserById: function (id) {
    ...
    return Users.findByIdAsync(id)
      .tap(UserService.assertFound(id))
      .tap(function (user) {
