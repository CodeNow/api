...
const UserService = module.exports = {
  ...
  getUsersOrganizationsWithGithubModel: function (sessionUser) {
    var log = UserService.log.child({ method: 'UserService.getUsersOrganizationsWithGithubModel' })
    log.trace({ sessionUser, sessionUserOrgs: sessionUser.bigPoppaUser.organizations }, '#1 jorge')
    return UserService.getUsersOrganizations(sessionUser)
      .then(function saveOrgsByName (orgs) {
        if (!orgs) {
          return []
        }
        log.trace({ orgs }, '#2 jorge')
        var orgsByGithubId = {}
