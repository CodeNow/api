...
module.exports = class ClusterConfigService {
  ...
  static create (sessionUser, data) {
    ...
    return ClusterConfigService.fetchFileFromGithub(bigPoppaUser, repoFullName, filePath, branchName)
      .then(parseInput => {
   ...
   * @param {String=} branchOrCommit           - Name of the commit/branch/tag (leave blank for default)
   *
   ...
   * @resolves {String=} model.branch          - Name of commit/branch/tag this file is in
   *
   */
  static fetchFileFromGithub (bigPoppaUser, repoFullName, filePath, branchOrCommit) {
    const log = ClusterConfigService.log.child({
      method: 'fetchFileFromGithub',
      bigPoppaUser, repoFullName, filePath, branchOrCommit
    })
    log.trace('called')
    const token = keypather.get(bigPoppaUser, 'accessToken')
    const github = new GitHub({ token })
    return github.getRepoContent(repoFullName, filePath, branchOrCommit)
      .then(fileContent => {
        ...
        return UserService.getByGithubId(githubPushInfo.user.id)
          .then(bpUser => {
            return ClusterConfigService.fetchFileFromGithub(
              bpUser,
              clusterConfig.repo || githubPushInfo.repo,
              clusterConfig.filePath,
              clusterConfig.branch || githubPushInfo.commit
            )
          })
  ...
  static checkFileChangeAndCreateUpdateJob (instance, githubPushInfo) {
    ...
    log.info('called')
    return ClusterConfigService.checkIfComposeFileHasChanged(instance, githubPushInfo)
