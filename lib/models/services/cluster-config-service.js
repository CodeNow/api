...
module.exports = class ClusterConfigService {
   ...
   * @return {Promise}    with object that has AutoIsolationConfig
   */
  static createFromRunnableConfig (sessionUser, runnableConfig, triggeredAction, repoFullName, filePath, fileSha, repositoryName, isTesting) {
    const log = ClusterConfigService.log.child({
      method: 'create',
      sessionUser,
      triggeredAction,
  ...
  /**
   * Returns a function that takes each individual instanceObj, and sorts them by what needs to be
   * done: update, create, and delete.
   *
   * Updates are added to the updateInstances array.
   * Creates get their Context model created (and attached to the config)
   * Deletes get a job scheduled to delete the instance
   *
   * @param {SessionUser} sessionUser           - User model for the an owner with permission
   * @param {Object}      orgInfo
   * @param {String}      orgInfo.githubOrgId
   * @param {String}      orgInfo.bigPoppaOrgId
   * @param {Object[]}    updateInstances       - empty array to fill with instanceObjs to update
   * @param {Object[]}    createContextPromises - empty array to fill with promises resolving with
   *                                                configs with contextIds attached
   * @returns {Function(instanceObj)}
   * @private
   */
  static _sortConfigsForClusterUpdate (sessionUser, orgInfo, updateInstances, createContextPromises) {
    const log = ClusterConfigService.log.child({
      method: '_sortConfigsForClusterUpdate',
      sessionUser, orgInfo
    })
    return (instanceObj) => {
      if (instanceObj.config) {
        ...
        return createContextPromises.push(ClusterConfigService.createClusterContext(
          sessionUser,
          instanceObj.config,
          orgInfo
        ))
  ...
  static _createUpdateAndDeleteInstancesForClusterUpdate (sessionUser, instanceObjs, mainInstance, githubPushInfo) {
    ...
    instanceObjs.forEach(ClusterConfigService._sortConfigsForClusterUpdate(
      sessionUser,
      orgInfo,
      updateInstances,
    ...
    return Promise
      .all(createContextPromises) // Make sure creates happen first
      .tap(contextIdFilledConfigs => {
        // Now we need to put all of the matching contextIds in all of the configs
        // So first combine them
        const allConfigs = contextIdFilledConfigs.concat(updateInstances.map(model => model.config))
        // Then map the aliases to the context ids.  This should give us
  ...
  static _addConfigToInstances (configs, instances) {
    return instances.map((instance) => {
      ...
      if (config) {
        config.contextId = keypather.get(instance, 'contextVersion.context._id')
      }
      return {
        instance,
        config
