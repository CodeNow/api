...
module.exports = class ClusterConfigService {
  ...
  static create (sessionUser, data) {
    ...
    return ClusterConfigService.fetchFileFromGithub(bigPoppaUser, repoFullName, filePath)
      .then(parseInput => {
        ...
        return ClusterConfigService.parseComposeFileAndPopulateENVs(parseInput, repoFullName, instanceName)
          .then(parsedCompose => {
   ...
   */
  static parseComposeFileAndPopulateENVs (composeFileData, repoFullName, mainInstanceName) {
    return ClusterConfigService.parseComposeFile(composeFileData, repoFullName, mainInstanceName)
    .then((res) => {
      return populateENVsFromFiles()
    })
    .then((res) => {
      return populateENVsFromFiles()
    })
