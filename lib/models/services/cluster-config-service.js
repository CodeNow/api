...
module.exports = class ClusterConfigService {
  ...
  static create (sessionUser, triggeredAction, repoFullName, branchName, filePath, newInstanceName) {
    ...
    return github.getRepoContentAsync(repoFullName, filePath)
      ...
      .then(function (parsedCompose) {
        ...
        return ClusterConfigService.createFromRunnableConfig(sessionUser, parsedCompose, triggeredAction, repoFullName, filePath)
      })
  }
   ...
   * @return {Promise} with object that has AutoIsolationConfig
   */
  static createFromRunnableConfig (sessionUser, runnableConfig, triggeredAction, repoFullName, filePath) {
    const log = ClusterConfigService.log.child({
      method: 'create',
      sessionUser,
      triggeredAction,
    ...
    return Promise.map(parsedInstancesDef, (instanceDef) => {
      return ClusterConfigService.createClusterInstance(sessionUser, instanceDef, repoFullName, triggeredAction)
        .then((instance) => {
          return {
            instanceDef,
            instance
   ...
   * @param  {Object} parsedInstaceData
   * @param  {String} parsedInstaceData.contextVersion.buildDockerfilePath
   * @param  {String} parsedInstaceData.instance.name
   * @param  {Array<String>}  parsedInstaceData.instance.env
   * @param  {String}  parsedInstaceData.instance.containerStartCommand
   * @param  {String} repoFullName (org/repo)
   * @param  {String} triggeredAction
   * @return {Instance}
   */
  static createClusterInstance (sessionUser, parsedInstaceData, repoFullName, triggeredAction) {
    const log = ClusterConfigService.log.child({
      method: 'createClusterInstance',
      sessionUser, parsedInstaceData, repoFullName, triggeredAction
    })
    log.info('called')
    ...
    .then((context) => {
      ...
      return ClusterConfigService._createContextVersion(sessionUser, context._id, orgInfo, repoFullName, parsedInstaceData)
    })
    .then((contextVersion) => {
    ...
    .then((build) => {
      ...
      return ClusterConfigService._createInstance(sessionUser, parsedInstaceData.instance, buildId)
    })
  }
   ...
   */
  static _createInstance (sessionUser, configData, parentBuildId) {
