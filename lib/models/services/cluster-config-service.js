...
module.exports = class ClusterConfigService {
  ...
  /**
   * Returns the instance that matches the mainInstanceKey with it's shortName (metadata name)
   *
   ...
   * @param {String}   mainInstanceKey                     - Instance name to search with
   *
   * @returns {Instance} Instance that matched the mainInstanceKey
   * @private
   */
  static _getInstanceFromServicesByShortName (configObjects, mainInstanceKey) {
    const log = ClusterConfigService.log.child({
    ...
    const main = configObjects.find(instanceModels => {
      return instanceModels.config.metadata.name === mainInstanceKey
    })
   ...
   * @param {String}      data.mainInstanceKey            - main instance for this cluster
  ...
  static create (sessionUser, data) {
    ...
    let mainInstanceKey = data.mainInstanceKey
    ...
    return ClusterConfigService._parseComposeInfoForConfig(
      ...
      .then(results => {
        if (!mainInstanceKey) {
          mainInstanceKey = ClusterConfigService._getMainKeyFromOctobearMains(results.mains)
        }
        const buildOpts = {
          clusterCreateId,
          repoFullName,
   ...
   * @param {String}      buildOpts.mainInstanceKey                - Main instance service name
   ...
   * @param {String}      mainInstanceKey                          - Main instance service name
   * @return {Promise}    with object that has AutoIsolationConfig
   */
  ...
  static deleteAllICC (clusterId) {
    ...
    return InputClusterConfig.findActiveByAutoIsolationId(clusterId)
      ...
      .each((cluster) => rabbitMQ.deleteCluster({ cluster: { id: cluster.autoIsolationConfigId } }))
  }
  ...
  static _createInstance (sessionUser, parsedInstanceData, parentBuildId, testingOpts, buildOpts) {
    ...
    const shouldNotAutofork = keypather.get(parsedInstanceData, 'metadata.name') !== buildOpts.mainInstanceKey
  ...
  static updateCluster (sessionUser, mainInstance, githubPushInfo, octobearInfo, clusterOpts) {
    const log = ClusterConfigService.log.child({
      ...
      sessionUser, mainInstance, githubPushInfo, octobearInfo, clusterOpts
    })
    log.info('called')
    const buildOpts = {
      ...
      mainInstanceKey: mainInstance.shortName
    }
    const ownerInfo = ClusterConfigService._getOwnerInfo(sessionUser, buildOpts.repoFullName)
  ...
  static updateIsolationConfig (ownerInfo, instanceObjects, clusterOpts, mainInstance) {
    const log = ClusterConfigService.log.child({
      method: 'updateIsolationConfig',
      ownerInfo, instanceObjects, clusterOpts, mainInstance
    })
    log.info('called')
  ...
  static checkFileChangeAndCreateUpdateJob (instance, githubPushInfo) {
    ...
    return ClusterConfigService.checkIfComposeFilesChanged(instance, githubPushInfo)
      .then(() => {
        const organizationId = githubPushInfo.bpOrganizationId
        const triggeredInfo = {
          action: 'webhook',
        ...
        return ClusterBuildService.create(githubPushInfo.bpUserId, organizationId, triggeredInfo)
      })
      .then((clusterBuild) => {
        log.info({ clusterBuild }, 'clusterBuild')
        rabbitMQ.updateCluster({
          clusterBuildId: clusterBuild._id.toString(),
          instanceId: instance._id.toString()
        })
      })
      .catch(err => {
