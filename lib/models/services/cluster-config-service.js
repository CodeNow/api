...
module.exports = class ClusterConfigService {
  ...
  static create (sessionUser, data) {
    ...
    return ClusterConfigService.fetchComposeFileFromGithub(bigPoppaUser, repoFullName, filePath)
      .then(function (parseInput) {
        ...
        parseInput.repositoryName = data.newInstanceName || repoName
        parseInput.ownerUsername = GitHub.getOrgFromFullRepoName(repoFullName)
        parseInput.userContentDomain = process.env.USER_CONTENT_DOMAIN
   ...
   *
   * @param {Instance[]} instancesWithConfigs                        - instances with OctoBear configurations
   ...
   * @returns {ObjectId} model.requestedDependencies.matchBranch - True if the copies should match
   *                                                                  the branch of the master
   * @private
   */
  static _createAutoIsolationModelsFromClusterInstances (instancesWithConfigs) {
    const mainInstance = instancesWithConfigs.find((instanceObj) => {
      return instanceObj.config.metadata.isMain
    })
    const dependencies = instancesWithConfigs.filter((instanceObj) => {
      return !instanceObj.config.metadata.isMain
    })
    const mainInstanceId = mainInstance._id
    const requestedDependencies = dependencies.map((instance) => {
      const dep = {
        instance: instance._id
      }
      // if files provided -> it is not repo instance
      const isRepoInstance = !keypather.get(instance, 'config.files')
      if (isRepoInstance) {
   ...
   * @param {Instance[]} instances               - Dependent Instance (or empty model with config)
   * @param {Object}     instances.config        - Octobear config info (should delete instance if missing)
   * @param {String}     instances.name          - Instance's name (should create instance if missing)
   ...
   * @private
   */
  static _createUpdateAndDeleteInstancesForClusterUpdate (sessionUser, instances, mainInstance, githubPushInfo) {
    const log = ClusterConfigService.log.child({
      method: '_createUpdateAndDeleteInstancesForClusterUpdate',
      sessionUser, instances, mainInstance, githubPushInfo
    })
    return Promise.map(instances, instance => {
      if (keypather.get(instance, 'config')) {
        // if the config exists, then we know we need to create or update
        if (instance.name) {
          return ClusterConfigService._updateInstancesWithConfigs(sessionUser, instance)
        }
        // Without the name, we know the 'instance' is just an empty object with a config, so we
        // need to make the instance
      ...
      // With no config, we delete the instance, and return undefined
   ...
   *
   * @param {User}     sessionUser                                - User model for the an owner with permission
   * @param {Instance} instanceWithConfig                         - Instance model which also contains a config
   * @param {ObjectId} instanceWithConfig.config                  - Octobear config model
   * @param {String}   instanceWithConfig.config.instance.containerStartCommand  - Container's start command
   * @param {Number[]} instanceWithConfig.config.instance.ports   - Array of ports to open on the instance
   * @param {String[]} instanceWithConfig.config.instance.env     - Array of envs for the instance (env=a)
   *
   * @returns {Promise}
   ...
   * @private
   */
  static _updateInstancesWithConfigs (sessionUser, instanceWithConfig) {
    return InstanceService.updateInstance(
      instanceWithConfig, {
        env: instanceWithConfig.config.instance.env,
        ports: instanceWithConfig.config.instance.ports,
        containerStartCommand: instanceWithConfig.config.instance.containerStartCommand
      },
      sessionUser
    )
      .tap(instance => {
        instance.config = instanceWithConfig.config
      })
  }
