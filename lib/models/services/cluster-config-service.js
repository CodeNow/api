...
module.exports = class ClusterConfigService {
   ...
   * @param {Boolean} isTesting - is this testing cluster
   * @param {String} newInstanceName - optional new instance name
   * @return {Promise} with object that has AutoIsolationConfig
   */
  static create (sessionUser, triggeredAction, repoFullName, branchName, filePath, isTesting, newInstanceName) {
    const log = ClusterConfigService.log.child({
      method: 'create',
      sessionUser,
      triggeredAction,
      repoFullName, branchName, filePath, isTesting, newInstanceName
    })
    ...
    return github.getRepoContentAsync(repoFullName, filePath)
      ...
      .then(function (parsedCompose) {
        ...
        return ClusterConfigService.createFromRunnableConfig(sessionUser, parsedCompose, triggeredAction, repoFullName, filePath, isTesting)
      })
  }
   ...
   * @return {Promise} with object that has AutoIsolationConfig
   */
  static createFromRunnableConfig (sessionUser, runnableConfig, triggeredAction, repoFullName, filePath, isTesting) {
    const log = ClusterConfigService.log.child({
      method: 'create',
      sessionUser,
      triggeredAction,
