...
module.exports = class ClusterConfigService {
  ...
  static create (sessionUser, data) {
    ...
    return ClusterConfigService.fetchAndParseComposeFile(bigPoppaUser, repoFullName, null, filePath, clusterName)
      .then((parsedCompose) => {
  ...
  /**
   * Fetch and parse a docker compose file
   * @param bigPoppaUser
   * @param repoFullName
   * @param clusterFilePath
   * @param clusterName
   * @returns {Promise.<Object>} Cluster configurations
   */
  static fetchAndParseComposeFile (bigPoppaUser, repoFullName, commit, clusterFilePath, clusterName) {
    const log = ClusterConfigService.log.child({
      method: 'fetchAndParseComposeFile',
      repoFullName,
      commit,
      clusterFilePath,
      clusterName
    ...
    log.info('called')
    return ClusterConfigService.fetchFileFromGithub(bigPoppaUser, repoFullName, clusterFilePath, commit)
      .tap(composeFileData => log.info({ composeFileData }, 'Compose file data'))
      .then(parseInput => {
        return ClusterConfigService.parseComposeFileAndPopulateENVs(parseInput, repoFullName, clusterName, bigPoppaUser)
      })
  }
  ...
  static _updateInstancesWithConfigs (sessionUser, instanceObj) {
    // Determine if we need to trigger a new build.
