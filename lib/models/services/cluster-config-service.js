...
module.exports = class ClusterConfigService {
   ...
   * @resolves {Instance}
   */
  static createClusterInstance (sessionUser, parsedInstanceData, repoFullName, testingOpts, masterInstance, triggeredAction) {
    const isTesting = testingOpts.isTesting
    const isTestReporter = testingOpts.isTestReporter
    const log = ClusterConfigService.log.child({
    ...
    return Promise.try(() => {
      ...
      .then((build) => {
        ...
        return ClusterConfigService._createInstance(sessionUser, parsedInstanceData.instance, buildId, testingOpts, masterInstance)
      })
  }
   ...
   */
  static _createInstance (sessionUser, configData, parentBuildId, testingOpts, masterInstance) {
    ...
    const defaultInstanceOpts = {
      ...
      isolated: masterInstance.isolated,
  ...
  static updateCluster (sessionUser, mainInstance, githubPushInfo, octobearInfo) {
    ...
    return AutoIsolationService.fetchAutoIsolationDependentInstances(mainInstance._id)
      .tap(children => children.push({ instance: mainInstance }))
  ...
  static _compareConfigToInstanceObject (config, model) {
    const configName = model.parent ? model.parent.name : model.instance.name
   ...
   * @param {Object[]} instanceObjects               - Array
   * @param {Object}   instanceObjects.parent
   * @param {String}   instanceObjects.parent.name
   * @param {Object}   instanceObjects.instance
   * @param {String}   instanceObjects.instance.name
   * @returns {Object[]} super objects containing both the instance and the config
   */
  static _addConfigToInstances (configs, instanceObjects) {
    instanceObjects.forEach(model => {
      const config = configs.find(config => this._compareConfigToInstanceObject(config, model))
      if (config) {
        config.contextId = keypather.get(model.instance, 'contextVersion.context')
        model.config = config
      }
    })
  ...
  static _createNewInstancesForNewConfigs (sessionUser, octobearInfo, repoFullName, testingOpts, masterInstance, triggeredAction) {
    const testReporters = testingOpts.testReporters
    ...
    return ClusterConfigService.parseComposeFile(composeFileData, repoFullName, clusterName)
      .then(parsedCompose => {
        ...
        const fileFetches = parsedCompose.envFiles.reduce((obj, fileName) => {
          obj[fileName] = ClusterConfigService.fetchFileFromGithub(bigPoppaUser, repoFullName, fileName, commitRef)
            .then(f => f.fileString)
          return obj
        }, {})
