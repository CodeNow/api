  ...
  static _createAutoIsolationModelsFromClusterInstances (instancesWithConfigs, mainInstance) {
    ...
    const requestedDependencies = instancesWithConfigs.filter(instanceObj => {
      ...
      .map(instanceObj => {
        ...
        const config = instanceObj.config
        const hasBuildDockerfilePath = !!keypather.get(config, 'build.dockerFilePath')
        const isGithubRepo = !!keypather.get(config, 'code.repo')
        const matchBranch = (hasBuildDockerfilePath && !isGithubRepo) || undefined
        log.trace({ config, hasBuildDockerfilePath, isGithubRepo, matchBranch }, 'Check for matched branch')
