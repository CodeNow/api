...
module.exports = class ClusterConfigService {
  ...
  static createFromRunnableConfig (sessionUser, runnableConfig, triggeredAction, repoFullName, filePath, fileSha, repositoryName, isTesting) {
    ...
    return Promise
      ...
      .tap(ClusterConfigService.mapAliasesToContexts)
      .map(instanceDef => {
   ...
   * @param {String}   instanceConfigs.metadata.name
   *
   * @returns {undefined}
   */
  static mapAliasesToContexts (instanceConfigs) {
    const log = ClusterConfigService.log.child({
      method: 'mapAliasesToContexts'
    })
    log.info('called')
    if (!instanceConfigs) {
      return
    ...
    const dict = instanceConfigs
      .reduce((map, instanceConfig) => Object.assign(map, {[instanceConfig.metadata.name]: instanceConfig}), {})
    ...
    instanceConfigs.forEach(instanceConfig => {
      ...
      Object.keys(aliases).forEach(key => {
        if (dict[aliases[key].instanceName]) {
          aliases[key].contextId = dict[aliases[key].instanceName].contextId
        }
      })
    })
  }
   ...
   * @param {Object}      parsedInstanceData.contextId
