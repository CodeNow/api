...
module.exports = class ClusterConfigService {
      ...
      .then(function (fileContent) {
        ...
        return octobear.parse(parseInput)
          .then(function (parsedCompose) {
            ...
            return ClusterConfigService.createFromRunnableConfig(sessionUser, parsedCompose, triggeredAction, repoFullName, filePath, fileContent.sha)
          })
      })
  }
   ...
   * @return {Promise} with object that has AutoIsolationConfig
   */
  static createFromRunnableConfig (sessionUser, runnableConfig, triggeredAction, repoFullName, filePath, fileSha) {
    const log = ClusterConfigService.log.child({
      method: 'create',
      sessionUser,
      triggeredAction,
   ...
   * @return {Instance}
   */
  static createClusterInstance (sessionUser, parsedInstanceData, repoFullName, triggeredAction) {
    const log = ClusterConfigService.log.child({
      method: 'createClusterInstance',
      sessionUser, parsedInstanceData, repoFullName, triggeredAction
    })
    log.info('called')
    ...
    .then((build) => {
      ...
      return ClusterConfigService._createInstance(sessionUser, parsedInstanceData.instance, buildId)
    })
  }
