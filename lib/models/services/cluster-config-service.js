...
module.exports = class ClusterConfigService {
  ...
  static _createUpdateAndDeleteInstancesForClusterUpdate (sessionUser, instanceObjs, clusterOpts, buildOpts, ownerInfo) {
    ...
    const deleteInstancePromises = ClusterConfigService.deleteInstancesFromClustersAndEmitUpdate(
  ...
  /**
   * Creates delete jobs for each instance in the deletedConfigs, then emits an update
   *
   * @param {Number}   githubOrgId                       - Used to send the message to the right connections
   * @param {String}   clusterName                       - ClusterName of the cluster
   * @param {Object[]} deletedConfigs                    - InstanceModels that should be deleted
   * @param {Instance} deletedConfigs.instance           - Instance model
   * @param {String}   deletedConfigs.instance.shortName - Service Name
   */
  static deleteInstancesFromClustersAndEmitUpdate (githubOrgId, clusterName, deletedConfigs) {
    return Promise.map(deletedConfigs, instanceObj => {
      return rabbitMQ.deleteInstance({instanceId: keypather.get(instanceObj, 'instance._id.toString()')})
    })
      .tap(() => {
        return ClusterConfigService.sendClusterSocketUpdate(githubOrgId, {
          task: 'compose-cluster-updated',
          clusterName: clusterName,
          deleted: deletedConfigs.map((config) => config.instance.shortName)
        })
      })
  }
