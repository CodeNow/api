...
module.exports = class ClusterConfigService {
  ...
  static _createAutoIsolationModelsFromClusterInstances (instancesWithConfigs, mainInstance) {
    ...
    log.info({ instancesWithConfigs }, 'requested dependencies embiid 2')
    const requestedDependencies = instancesWithConfigs.filter(instanceObj => {
      ...
      .map(instanceObj => {
        const matchBranch = !!keypather.get(instanceObj, 'instance.contextVersion.buildDockerfilePath')
        const contextVersion = keypather.get(instanceObj, 'instance.contextVersion')
        const keys = Object.keys(instanceObj)
        log.info({ matchBranch, keys, contextVersion, buildDockerfilePath: keypather.get(instanceObj, 'contextVersion.buildDockerfilePath') }, 'instance obj - embiid 2')
