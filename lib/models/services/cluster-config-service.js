...
module.exports = class ClusterConfigService {
  ...
  static checkFileChangeAndCreateUpdateJob (instance, githubPushInfo) {
    ...
    return ClusterConfigService.checkIfComposeFilesChanged(instance, githubPushInfo)
      .then(() => {
        const userGitHubId = githubPushInfo.user.id
        const organizationId = githubPushInfo.organizationId
        return UserService.getCompleteUserByGithubId(userGitHubId)
          .then((user) => {
            return ClusterBuildService.create(user.id, organizationId,
              {
                action: 'webhook',
