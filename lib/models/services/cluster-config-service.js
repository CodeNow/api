...
module.exports = class ClusterConfigService {
   ...
   * @param {Instance[]} instanceObjs               - Dependent Instance (or empty model with config)
   * @param {Object}     instanceObjs.config        - Octobear config info (should delete instance if missing)
   * @param {String}     instanceObjs.name          - Instance's name (should create instance if missing)
  ...
  static _createUpdateAndDeleteInstancesForClusterUpdate (sessionUser, instanceObjs, mainInstance, githubPushInfo) {
    ...
    return Promise
      .map(instanceObjs, instanceObj => {
        if (keypather.get(instanceObj, 'config')) {
          ...
          // Without the instance, we know the 'instanceObj' is just an empty object with a config, so we
        ...
        const deletedId = keypather.get(instanceObj, 'instance._id')
   ...
   *
   * @param {User}     sessionUser                                - User model for the an owner with permission
   * @param {Instance} instanceObj                         - Instance model which also contains a config
