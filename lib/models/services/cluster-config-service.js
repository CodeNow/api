...
module.exports = class ClusterConfigService {
  ...
  static updateBuildContextForEachService (composeFilePath, services) {
    ...
    services.forEach((service) => {
      if (service.build.buildDockerContext) {
        const newContext = path.resolve(composeFileDirname, service.build.buildDockerContext)
        log.info({
          newContext,
          composeFileDirname,
          oldContext: service.build.buildDockerContext
        }, 'new context')
        service.build.buildDockerContext = newContext
      }
    })
  }
    ...
    return InfraCodeVersionService.findBlankInfraCodeVersion()
      .then((parentInfaCodeVersion) => {
        ...
        .then((appCodeVersion) => {
          ...
          const buildDockerfilePath = parsedConfigData.build.dockerFilePath
          if (buildDockerfilePath) {
            cvOpts.buildDockerfilePath = buildDockerfilePath
          }
          // TODO: change context if compose file not in the root
          const dockerBuildContext = parsedConfigData.build.dockerBuildContext
