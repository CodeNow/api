...
module.exports = class ClusterConfigService {
  ...
  static createClusterInstance (sessionUser, parsedInstanceData, testingOpts, buildOpts, ownerInfo) {
    ...
    return Promise.try(() => {
      ...
      .then(() => ClusterConfigService._createContextVersion(
        sessionUser,
        ownerInfo,
        buildOpts,
        parsedInstanceData
  ...
  static _createInstance (sessionUser, parsedInstanceData, parentBuildId, testingOpts, buildOpts) {
    ...
    if (buildOpts.isolated) {
      let newName = inputInstanceOpts.name
      if (buildOpts.branch) {
        var sanitizedBranch = buildOpts.branch.replace(/[^a-zA-Z0-9]/g, '-')
        newName = sanitizedBranch + '-' + newName
      }
      inputInstanceOpts.name = buildOpts.masterShorthash + '--' + newName
    }
