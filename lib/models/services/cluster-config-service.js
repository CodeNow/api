...
module.exports = class ClusterConfigService {
  ...
  static createClusterInstance (sessionUser, parsedInstanceData, repoFullName, testingOpts, masterInstance, triggeredAction) {
    const log = ClusterConfigService.log.child({
      ...
      sessionUser, parsedInstanceData, repoFullName, testingOpts, triggeredAction
    })
    log.info('called')
    const bigPoppaOwnerObject = UserService.getBpOrgInfoFromRepoName(sessionUser, repoFullName)
    const orgInfo = {
    ...
    return Promise.try(() => {
      ...
      .then((build) => {
        ...
        return ClusterConfigService._createInstance(sessionUser, parsedInstanceData, buildId, testingOpts, masterInstance)
      })
  }
   ...
   */
  static _createInstance (sessionUser, parsedInstanceData, parentBuildId, testingOpts, masterInstance) {
  ...
  static checkIfComposeFileHasChanged (instance, githubPushInfo) {
    return ClusterConfigService.fetchConfigByInstanceId(instance)
      .then(clusterConfig => {
