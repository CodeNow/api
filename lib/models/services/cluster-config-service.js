...
module.exports = class ClusterConfigService {
  ...
  static _createAutoIsolationModelsFromClusterInstances (instancesWithConfigs, mainInstance) {
    ...
    const requestedDependencies = instancesWithConfigs.filter(instanceObj => {
      ...
      .map(instanceObj => {
        // If there's a build property and it's not main, match the branch
        // since we know it's the same repo
        const superconfig = keypather.get(instanceObj, 'config')
        ClusterConfigService.log.info({ superconfig }, 'super config - embiid 3')
        const supercv = keypather.get(instanceObj, 'instance.contextVersion')
        ClusterConfigService.log.info({ supercv }, 'context version - embiid 3')
        const superinstance = keypather.get(instanceObj, 'instance')
        ClusterConfigService.log.info({ superinstance }, 'superInstance - embiid 3')
        const matchBranch = !!keypather.get(instanceObj, 'instance.contextVersion.buildDockerfilePath') || undefined
        ClusterConfigService.log.info({ matchBranch }, 'matchBranch - embiid 3')
        return { instance: instanceObj.instance._id, matchBranch }
      })
