...
module.exports = class ClusterConfigService {
      ...
      .then((newInstanceObjs) => {
        return Promise
          .map(updateConfigs, (instanceObj) => {
            ...
            return ClusterConfigService._updateInstancesWithConfigs(sessionUser, instanceObj, orgInfo)
          })
          .then((instanceObjs) => {
   ...
   * @private
   */
  static _updateInstancesWithConfigs (sessionUser, instanceObj, orgInfo) {
    // We need to determine if the instance needs to be re-built
    const commitish = keypather.get(instanceObj.config, 'code.commitish')
    const branch = instanceObj.instance.getMainBranchName()
    const hasCommitChanged = commitish && branch !== commitish.toLowerCase()

    const mainACV = ContextVersion.getMainAppCodeVersion(instanceObj.instance.appCodeVersions)
    const currentDockerfilePath = keypather.get(mainACV, 'buildDockerfilePath')
    const newDockerfilePath = instanceObj.config.contextVersion.buildDockerfilePath
    ...
    if (hasCommitChanged || hasDockerfilePathChanged) {
      ...
      buildPromise = ClusterConfigService._createCVAndBuildBuild(sessionUser, contextId, orgInfo, repoFullName, instanceObj.config, 'automatic')
    }
