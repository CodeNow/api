...
module.exports = class ClusterConfigService {
   ...
   * @param {String} clsuterName                - Name that the cluster
   * @param {User}   bigPoppaUser               - The bigPoppaUser model for the owner of this repo
   *
   ...
   */
  static parseComposeFileAndPopulateENVs (composeFileData, repoFullName, clsuterName, bigPoppaUser) {
    const log = ClusterConfigService.log.child({
      method: 'parseComposeFileAndPopulateENVs',
      composeFileData, repoFullName, clsuterName
    })
    log.info('called')
    return ClusterConfigService.parseComposeFile(composeFileData, repoFullName, clsuterName)
      .then(parsedCompose => {
   ...
   * @resolves {String[]} octobearInfo.instance.env     - Array of envs for the instance (env=a)
   */
  static parseComposeFile (composeFileData, repoFullName, clsuterName) {
    const log = ClusterConfigService.log.child({
