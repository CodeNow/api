...
module.exports = class ClusterConfigService {
      ...
      .then(autoIsolationOpts => {
        ...
        return AutoIsolationService.createOrUpdateAndEmit(autoIsolationOpts)
          .then((autoIsolationConfig) => {
            log.trace({ autoIsolationConfig }, 'autoisolation config was created was created for cluster')
            const files = [
              {
                path: clusterOpts.filePath,
                sha: clusterOpts.fileSha
              }
            ]
            const clusterConfigOpts = {
            ...
            log.trace({ clusterConfigOpts }, 'cluster config opts to be saved')
            return InputClusterConfig.createAsync(clusterConfigOpts)
            .tap((clusterConfig) => {
              log.trace({ clusterConfig }, 'cluster config was created')
            })
          })
      })
  }
  ...
  /**
   * Given a repo and array of filepathes, fetch all Docker Compose files.
   *
   * @param {User}    bigPoppaUser             - The bigPoppaUser model for the owner of this repo
   * @param {String}  bigPoppaUser.accessToken - The user's access token
   * @param {String}  repoFullName             - Org/Repo for the repository we want to fetch from
   * @param {Array[String]}  filesPathes       - Array of pathes to the Docker Compose files
   * @param {String=} commitRef                - Name of the commit/branch/tag (leave blank for default)
   *
   * @resolves {Array[Object]}  array of file objects - each object should have `fileString`, `fileSha`, `filePath`, `commitRef`
   *
   */
  static fetchFilesFromGithub (bigPoppaUser, repoFullName, filesPathes, commitRef) {
    const log = ClusterConfigService.log.child({
      method: 'fetchFilesFromGithub',
      bigPoppaUser, repoFullName, filesPathes
    })
    log.trace('called')
    return Promise.map(filesPathes)
      .each((filePath) => {
   ...
   */
  static checkIfComposeFilesHasChanged (instanceId, githubPushInfo) {
