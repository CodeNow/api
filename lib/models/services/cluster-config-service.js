  ...
  static parseComposeFileAndPopulateENVs (composeFileData, repoFullName, clusterName, bigPoppaUser, filePath) {
    ...
    return octobear.findExtendedFiles(composeFileData.fileString)
      ...
      .then((allComposeFiles) => {
        ...
        return ClusterConfigService.parseComposeFilesIntoServices(filesContents, repoFullName, clusterName,
          bigPoppaUser, composeFileData.commitRef, filePath)
      })
  }

  static parseComposeFilesIntoServices (composeFilesContents, repoFullName, clusterName, bigPoppaUser, commitRef, rootFilePath) {
    const log = ClusterConfigService.log.child({
      method: 'parseComposeFilesIntoServices',
      composeFilesContents, repoFullName, clusterName, bigPoppaUser, commitRef, rootFilePath
    })
    log.info('called')
    const opts = {
    ...
    return octobear.parseAndMergeMultiple(opts, composeFilesContents)
      .then(parsedResult => {
        ...
        return [parsedResult.results, Promise.props(fileFetches)]
      })
      .spread((services, fileFetches) => {
