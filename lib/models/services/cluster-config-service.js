...
module.exports = class ClusterConfigService {
   ...
   *
   * @throws BaseSchema.NotFoundError           - When the config's sha's match, so this won't be done
   * @throws ClusterConfigService.NotFoundError - When no config is found to match the given instance
   */
        ...
        return UserService.getByGithubId(githubPushInfo.user.id)
          ...
          .then(function (newComposeFileData) {
            if (newComposeFileData.fileSha === clusterConfig.fileSha) {
              throw new BaseSchema.NotFoundError('Compose Config', {
