  ...
  static parseComposeFileAndPopulateENVs (composeFileData, repoFullName, clusterName, bigPoppaUser, filePath) {
    ...
    return octobear.findExtendedFiles(composeFileData.fileString)
      ...
      .then((allComposeFiles) => {
        const filesContents = allComposeFiles.map((file) => {
          return {
        ...
        return octobear.parseAndMergeMultiple(opts, filesContents)
      })
      .then(parsedResult => {
        log.trace({ parsedResult }, 'Parsed docker compose file')
        ...
        return [parsedResult.result, Promise.props(fileFetches)]
      })
