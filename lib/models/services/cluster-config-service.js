...
module.exports = class ClusterConfigService {
   ...
   */
  static getOwnerInfo (sessionUser, repoFullName) {
  ...
  static create (sessionUser, data) {
    ...
    return ClusterConfigService.fetchFileFromGithub(bigPoppaUser, repoFullName, filePath)
      .then(parseInput => {
        return ClusterConfigService.parseComposeFileAndPopulateENVs(parseInput, repoFullName, clusterName, sessionUser.bigPoppaUser)
          .then(parsedCompose => {
  ...
  static createFromRunnableConfig (sessionUser, runnableConfig, triggeredAction, repoFullName, clusterOpts) {
    ...
    const ownerInfo = ClusterConfigService.getOwnerInfo(sessionUser, repoFullName)
    const buildOpts = {
      triggeredAction,
      repoFullName
    }
    return Promise
      .each(parsedInstancesDef, instanceDef => {
        return ClusterConfigService.createClusterContext(sessionUser, instanceDef, ownerInfo)
  ...
  static createClusterInstance (sessionUser, parsedInstanceData, testingOpts, buildOpts, ownerInfo) {
    ...
    return Promise.try(() => {
      ...
      .then(function () {
        return ClusterConfigService._createContextVersion(sessionUser, ownerInfo, buildOpts.repoFullName, parsedInstanceData)
      })
      .then((contextVersion) => {
        log.trace({ contextVersion }, 'cv created')
        return ClusterConfigService._createBuild(sessionUser, contextVersion._id, ownerInfo)
      })
      .then(ClusterConfigService._buildBuild(sessionUser, buildOpts.triggeredAction))
      .then((build) => {
        log.trace({ build }, 'build created')
        const buildId = keypather.get(build, '_id.toString()')
        return ClusterConfigService._createInstance(sessionUser, parsedInstanceData, buildId, testingOpts, buildOpts)
      })
  }
   ...
   * @param {Object}      parsedConfigData.contextId        - contextId to version off of
   ...
   */
  static _createContextVersion (sessionUser, ownerInfo, repoName, parsedConfigData) {
    const log = ClusterConfigService.log.child({
      method: '_createContextVersion',
      sessionUser, ownerInfo, repoName, parsedConfigData
    })
    log.info('called')
  ...
  static _createUpdateAndDeleteInstancesForClusterUpdate (sessionUser, instanceObjs, githubPushInfo, clusterOpts, buildOpts, ownerInfo) {
    const log = ClusterConfigService.log.child({
      ...
      sessionUser, instanceObjs, githubPushInfo, buildOpts
    })
    log.info('called')
  ...
  static updateCluster (sessionUser, mainInstance, githubPushInfo, octobearInfo, clusterOpts) {
    ...
    const ownerInfo = ClusterConfigService.getOwnerInfo(sessionUser, buildOpts.repoFullName)
   ...
   * @param {Object}      clusterOpts.filePath       - Filepath for the compose file
   * @param {Object}      clusterOpts.fileSha        - Sha of the compose file
   ...
   * @private
   */
  static _compareConfigToInstanceObject (config, model) {
    if (!model.instance) {
      return false
    }
    const configName = model.master ? model.master.name : model.instance.name
    return configName === config.instance.name
  }
   ...
   * @param {Object}   instanceModels.master
   * @param {String}   instanceModels.master.name
   ...
   * @private
   */
  static _createNewInstancesForNewConfigs (sessionUser, octobearInfo, clusterOpts, buildOpts, ownerInfo) {
    const testingOpts = {
   ...
   */
  static checkIfComposeFileHasChanged (instance, githubPushInfo) {
