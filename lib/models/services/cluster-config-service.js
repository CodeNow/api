...
module.exports = class ClusterConfigService {
  ...
  static _createAutoIsolationModelsFromClusterInstances (instancesWithConfigs, mainInstance) {
    ...
    log.info({ instancesWithConfigs }, 'requested dependencies embiid')
    const requestedDependencies = instancesWithConfigs.filter(instanceObj => {
      ...
      .map(instanceObj => {
        const matchBranch = !!keypather.get(instanceObj, 'contextVersion.buildDockerfilePath')
        log.info({ matchBranch, instanceObj, buildDockerfilePath: keypather.get(instanceObj, 'contextVersion.buildDockerfilePath') }, 'instance obj - embiid')
