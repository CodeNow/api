...
module.exports = class ClusterConfigService {
  ...
  static create (sessionUser, data) {
    ...
    return Promise.fromCallback(function (cb) {
      github.getBranch(repoFullName, branch, cb)
    })
      .then(branchInfo => {
        const commit = keypather.get(branchInfo, 'commit.sha')
  ...
  static createFromRunnableConfig (sessionUser, runnableConfig, buildOpts, clusterOpts) {
    ...
    return Promise
      ...
      .tap(instances => {
        log.trace({ instances }, 'print instance definitions embiid')
      })
  ...
  static _addBranchName (instanceDef, clusterOpts) {
    if (instanceDef.build && clusterOpts.branch) {
  ...
  static _createContextVersion (sessionUser, ownerInfo, buildOpts, parsedConfigData) {
    ...
    return InfraCodeVersionService.findBlankInfraCodeVersion()
      .then((parentInfaCodeVersion) => {
        ...
        let instanceBranch
        log.trace({
          odlInstanceName: keypather.get(parsedConfigData, 'code.repo') || buildOpts.repoFullName,
          instanceRepoName,
          buildOptsWOW: buildOpts,
          hasRepo: !!codeConfig.repo,
          metadataBranchName
        }, 'service repo name 1 embiid')
        if (parsedConfigData.build && metadataBranchName) {
          instanceBranch = metadataBranchName
        }
        if (codeConfig.repo && codeConfig.commitish) {
          instanceRepoName = codeConfig.repo
          instanceBranch = codeConfig.commitish
        }
        log.trace({
          instanceRepoName,
          instanceBranch
