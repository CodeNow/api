...
class BillingService {
  ...
  static getPlanForOrganization (organizationId, sessionUserGithubId, sessionUserAccessToken) {
    log.info({ organizationId: organizationId, sessionUserGithubId: sessionUserGithubId }, 'getPlanForOrganization')
  ...
  static getInvoicesForOrganization (organizationId, sessionUserGithubId, sessionUserAccessToken) {
    ...
    return joi.validateOrBoomAsync({
      ...
      .then(function addGithubUserToInvoices (response) {
        ...
        return Promise.mapSeries(response.invoices, function (invoice) {
          let githubId = keypather.get(invoice, 'paidBy.githubId')
          if (!githubId) {
            log.trace({ githubId }, 'No githubId found for this invoice')
            return invoice
          }
          log.trace({ githubId, type: typeof githubId }, 'Fetching github user')
          return github.getUserByIdAsync(githubId)
            .then(function (githubUser) {
              log.trace({ githubId }, 'Github user found')
              invoice.paidBy.githubUser = githubUser
              return invoice
            })
            .catch(err => {
              log.trace({ err, githubId }, 'Error finding github user')
              return invoice
            })
        })
      })
   ...
   * @returns {Promise}
   */
  static postPaymentMethodForOrganization (organizationId, githubId, stripeToken, userEmail) {
    log.info({ organizationId, githubId, stripeToken, userEmail }, 'postPaymentMethodForOrganization')
    return joi.validateOrBoomAsync({
      organizationId: organizationId,
      githubId: githubId,
