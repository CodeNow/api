...
const BillingService = {
   ...
   * @returns {Promise}
   */
  getBigPoppaUserIdAndAssertUserIsPartOfOrg: function (githubId, organizationId) {
    ...
    return BillingService.getBigPoppaUserByGithubId(githubId)
      .then(function (user) {
        var foundOrg = user.organizations.find(function (org) {
   ...
   * @returns {Promise}
   */
  getBigPoppaUserByGithubId: function (githubId) {
   ...
   * @returns {Promise}
   */
  getPlanForOrganization: function  (organizationId, sessionUserGithubId, sessionUserAccessToken) {
   ...
   * @returns {Promise}
   */
  getInvoicesForOrganization: function (organizationId, sessionUserGithubId, sessionUserAccessToken) {
    log.info({ organizationId: organizationId, sessionUserGithubId: sessionUserGithubId }, 'getInvoicesForOrganization')
    return joi.validateOrBoomAsync({
      organizationId: organizationId,
      githubId: sessionUserGithubId,
      ...
      .then(function addGithubUserToInvoices (response) {
        ...
        return Promise.map(response.invoices, function (invoice) {
          var githubId = keypather.get(invoice, 'paidBy.githubId')
   ...
   * @returns {Promise}
   */
  getPaymentMethodForOrganization: function (organizationId, sessionUserGithubId, sessionUserAccessToken) {
    ...
    return joi.validateOrBoomAsync({
      ...
      .then(function addGithubUserToPlan (plan) {
        const github = new Github({ token: sessionUserAccessToken })
        var githubId = keypather.get(plan, 'owner.githubId')
        log.trace({ githubId: githubId }, 'Fetching github user')
        if (!githubId) {
          return plan
        }
   ...
   * @returns {Promise}
   */
  postPaymentMethodForOrganization: function (organizationId, githubId, stripeToken) {
    ...
    return joi.validateOrBoomAsync({
      ...
      .then(function () {
        return BillingService.getBigPoppaUserIdAndAssertUserIsPartOfOrg(githubId, organizationId)
      })
      .then(function (user) {
