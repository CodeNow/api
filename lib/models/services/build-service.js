...
BuildService.buildBuild = function (buildId, data, sessionUser) {
  ...
  return BuildService.findBuildAndAssertAccess(buildId, sessionUser)
    .tap(BuildService._validateBuildState)
    .tap(() => {
      BuildService._emitBuildRequestedEvent(buildId, data.message)
    })
    .tap(() => {
      BuildService._addTriggeredActionToData(data)
    })
    .tap(BuildService._validateContextVersionOnBuild)
    .then((build) => {
      BuildService._buildBuild(build, data, sessionUser)
    })
}

BuildService._emitBuildRequestedEvent = (buildId, buildMessage) => {
  return publisher.publishBuildRequested({
    reasonTriggered: buildMessage,
...
BuildService._addTriggeredActionToData = (buildData) => {
  var triggeredAction = buildData.triggeredAction
  if (triggeredAction) {
    if (!triggeredAction.rebuild && !triggeredAction.appCodeVersion) {
      buildData.triggeredAction.manual = true
    }
  } else {
    buildData.triggeredAction = {
      manual: true
    }
  }
}

BuildService._validateContextVersionOnBuild = (build) => {
  var log = BuildService.logger.child({
    method: '_validateContextVersionOnBuild',
    buildId: build.id.toString(),
    contextVersions: build.contextVersions
  })
  log.info('_validateContextVersionOnBuild: called')
...
BuildService._buildBuild = function (build, data, sessionUser) {
  var log = BuildService.logger.child({
    method: '_buildBuild',
    buildId: build.id.toString(),
    data: pick(data, ['triggeredAction']),
