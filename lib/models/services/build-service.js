...
BuildService.handleBuildComplete = function (buildId, buildInfo) {
  var log = logger.child({ method: 'BuildService.handleBuildComplete' })
  log.info('BuildService.handleBuildComplete called')
  return ContextVersion.updateBuildCompletedByBuildIdAsync(buildId, buildInfo)
    .then(function updateInstances (versions) {
      log.trace('contextVersion.updateBuildCompletedByBuildId success')
        ...
        .then(function (newContextVersion) {
          ...
          return BuildService.createBuild(createPayload, activeUser)
            .then(function (build) {
              ...
              return BuildService.buildBuild(build._id, buildBuildPayload, activeUser, process.domain)
            })
            .then(function (build) {
              return {
                user: activeUser,
