...
BuildService.handleSuccessfulBuild = (buildId, dockerTag) => {

}

BuildService.handleFailedBuild = (buildId, errorMessage) => {

}

/**
 * Handle successful & unsuccessful builds
 * Update ContextVersions and Build objects
 * Refresh ContextVersions on Instances affected
 * @param    {String}  buildId    buildId
 * @param    {Object}  buildInfo
 * @param    {boolean} buildInfo.failed - true if build failed
 * @resolves {ContextVersion[]}     versions that where updated
 * @return   {Promise}
 */
BuildService.handleBuildComplete = function (buildId, buildInfo) {
  var log = logger.child({ method: 'handleBuildComplete' })
  log.info('handleBuildComplete called')
  var p = buildInfo.failed
    ? ContextVersion.updateAndGetFailedBuild(buildId, buildInfo.error && buildInfo.error.message)
    : ContextVersion.updateAndGetSuccessfulBuild(buildId, buildInfo.dockerTag)
  return p
    .tap(BuildService._updateInstancesAndCvs.bind(null, buildInfo))
}

BuildService._updateInstancesAndCvs = (buildInfo, versions) => {
  var log = logger.child({ method: 'handleBuildComplete' })
  var versionIds = versions.map(pluck('_id'))
  log.trace({ versionIds }, 'finding instances by CV ids and updating builds')

  var updateInstancesPromise = BuildService._updateInstances(versionIds)
...
BuildService._updateInstances = (versionIds) => {
  return Instance.findByContextVersionIdsAsync(versionIds)
    .each((instanceModel) => {
