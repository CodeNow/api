...
BuildService._addContextVersionAndContextIdToOpts = (opts) => {
  var log = BuildService.logger.child({ opts, method: '_addContextVersionAndContextIdToOpts' })
  log.info('called')
  ...
  .tap((contextVersion) => {
    ...
    if (contextVersionOwner !== opts.owner.github && contextVersionOwner !== process.env.RUNNABLE_DEMO_GITHUB_ID) {
      log.trace({
        RUNNABLE_DEMO_GITHUB_ID: process.env.RUNNABLE_DEMO_GITHUB_ID,
        contextVersionOwner,
        owner: opts.owner.github
      }, 'CV owner doesnt match build owner')
