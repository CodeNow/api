 ...
 * @returns {Promise} Resolved with found Build model
 */
BuildService.findBuild = function (buildId, sessionUser) {
  ...
  return Promise.try(function () {
    if (!utils.isObjectId(buildId)) {
      log.error('Build id is not valid')
      throw Boom.badRequest('Invalid build id')
    }
  }).then(function () {
    return Build.findByIdAsync(buildId)
      .then(function checkBuild (build) {
        if (!build) {
          log.error('Build was not found')
      ...
      .then(function (build) {
        return Promise.any([
          PermisionService.isOwnerOf(sessionUser, build),
          PermisionService.isModerator(sessionUser),
          PermisionService.isHelloRunnableOwnerOf(sessionUser, build)
        ])
        .catch(Promise.AggregateError, function (err) {
          log.error({ err: err }, 'all permission checks failed')
          throw err[0]
        })
        .return(build)
      })
 ...
 * @param {String} buildId internal build id
 * @param {Object} data data to update build: message, triggeredAction etc
 * @param {Object} sessionUser mongo model representing session user
 * @param {Object} domain current request domain that has tid
 * @returns {Promise} Resolved with updated Build model
 */
BuildService.buildBuild = function (buildId, data, sessionUser, domain) {
  ...
  return BuildService.findBuild(buildId, sessionUser)
    ...
    .then(function (build) {
      return ContextVersion.findByIdsAsync(build.contextVersions, { 'build.log': false })
        .then(function validateCvs (contextVersions) {
          log.info('validateCvs')
          ...
          if (contextVersions.length > 1) { // this should not be possible.
            log.error('Cannot build a build with many context versions')
            throw Boom.badRequest('Cannot build a build with many context versions')
          }
          return contextVersions[0]
        })
        .then(function setBuildInProgress (contextVersion) {
          log.info('setBuildInProgress')
          // must be set before dedupe
          return build.setInProgressAsync(sessionUser).return(contextVersion)
        })
        .then(function buildContextVersion (contextVersion) {
          log.info({ contextVersion: contextVersion }, 'buildContextVersion')
          if (contextVersion.build.started) {
            return contextVersion
          }
          return ContextVersion.buildSelf(contextVersion, sessionUser, data, domain)
            .then(function (newContexVersion) {
              log.info({ contextVersion: newContexVersion }, 'build context version self')
              return build.replaceContextVersionAsync(contextVersion, newContexVersion).return(newContexVersion)
            })
            .catch(function (err) {
              log.error({ err: err }, 'error building cv')
              return build.modifyErroredAsync(contextVersion._id)
                .catch(function (err) {
                  log.error({ err: err }, 'error modifying build')
                })
                .return(contextVersion)
        ...
        .then(function updateBuild (contextVersion) {
          log.info({ contextVersion: contextVersion }, 'updateBuild')
