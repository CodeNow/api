...
var createSchema = {
  contextVersions: joi.array().items(joi.objectId()),
  owner: joi.object({
 ...
 *
 * @param    {Object}   opts                   - Options to be set on the build
 * @param    {[String]} opts.contextVersions   - array of ONLY 1 ContextVersion Id {ObjectId}
 *
 * @returns  {Promise}         After the validation is finished
 * @resolves {null}
 * @throws   {Boom.badRequest} When the opts fail the validation check
...
BuildService.validateCreateOpts = function (opts) {
  return BuildService.validateOpts(createSchema, opts)
}
      ...
      if (keypather.get(opts, 'contextVersions.length')) {
        return ContextVersion.findByIdAsync(opts.contextVersions[0])
          .tap(function addContextIdToOpts (contextVersion) {
            ...
            if (contextVersion.owner.github !== opts.owner.github) {
