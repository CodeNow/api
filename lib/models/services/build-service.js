...
BuildService.buildBuild = function (build, data, sessionUser, domain) {
  ...
  log.info('call')
  if (data.triggeredAction) {
    if (!(data.triggeredAction.rebuild || data.triggeredAction.appCodeVersion)) {
  ...
  return ContextVersion.findByIdsAsync(build.contextVersions, { 'build.log': false })
    ...
    .then(function buildContextVersion (contextVersion) {
      ...
      return ContextVersion.buildSelfAsync(contextVersion, sessionUser, data, domain)
