...
module.exports = class DockerComposeClusterService {
   ...
   * @return {undefined}
   */
  static updateCluster (newConfigs, instancesInCluster) {
    return Promise.try(() => {
    ...
    .each(DockerComposeClusterService._updateInstancesWithConfigs)
    .each(DockerComposeClusterService._findConfigsMissingInstanceAndCreate)
  }
  ...
  static _addConfigToInstances (configs, instances) {
    return instances.map((instance) => {
      instance.config = configs.find(hasKeypaths({ 'instance.name': instance.lowerName }))
      return instance
    })
  }
  /**
  ...
  static _isConfigMissingInstance (instances, config) {
    return !~instances.findIndex(hasKeypaths({
      'lowerName': config.instance.name
    }))
  }
   ...
   * @return {undefined}
   */
  static _updateInstancesWithConfigs (instance) {
    return instance.updateAsync({
      ...
      name: instance.config.instance.name
    })
  }
}
