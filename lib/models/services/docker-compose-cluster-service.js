...
module.exports = class DockerComposeClusterService {
  ...
  static createClusterParent (sessionUser, parentComposeData, fullRepoName) {
    ...
    log.info('called')
    const bigPoppaOwnerObject = DockerComposeClusterService._getBpOrgInfoFromRepoName(sessionUser, fullRepoName)
    const orgGithubId = parseInt(bigPoppaOwnerObject.githubId, 10)
    const bigPoppaOrgId = parseInt(bigPoppaOwnerObject.id, 10)
   ...
   */
  static _getBpOrgInfoFromRepoName (sessionUser, fullRepoName) {
    return sessionUser.bigPoppaUser.organizations.find(hasProps({
