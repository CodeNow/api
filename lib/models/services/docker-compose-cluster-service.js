...
module.exports = class DockerComposeClusterService {
   ...
   */
  static createParentContextVersion (contextId, sessionUser, orgGithubId, repoName) {
    var githubToken = sessionUser.accounts.github.accessToken
    var github = new Github({ token: githubToken })
    return github.createHooksAndKeys(repoName)
    .then((opts) => {
      const deployKeys = opts.deployKeys
      const repoInfo = opts.repoInfo
      return ContextVersion.createWithNewInfraCodeAndAppcode({
        context: contextId,
        createdBy: {
          github: sessionUser
        },
        ...
        appCodeVersions: {
          ...
          privateKey: deployKeys.privateKey
        }
      })
    })
  }
