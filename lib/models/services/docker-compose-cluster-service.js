...
module.exports = class DockerComposeClusterService {
  ...
  static create (sessionUser, triggeredAction, repoFullName, branchName, dockerComposeFilePath, newInstanceName) {
    ...
    return github.getRepoContentAsync(repoFullName, dockerComposeFilePath)
      ...
      .tap(function (resp) {
        ...
        const organizationId = resp.ownedByOrg
  ...
  static createClusterParent (sessionUser, parentComposeData, fullRepoName, triggeredAction) {
    ...
    .then((context) => {
      ...
      return DockerComposeClusterService._createParentContextVersion(sessionUser, context._id, orgInfo, fullRepoName, parentComposeData.contextVersion)
    })
    .then((contextVersion) => {
   ...
   * @param  {Object} parsedContextData
   * @param  {Boolean} parsedContextData.advanced
   * @param  {String} parsedContextData.buildDockerfilePath
   * @return {ContextVersion}
   */
  static _createParentContextVersion (sessionUser, contextId, orgInfo, repoName, parsedContextData) {
    const log = DockerComposeClusterService.log.child({
      method: '_createParentContextVersion',
      sessionUser, contextId, orgInfo, repoName, parsedContextData
    })
    log.info('called')
    return InfraCodeVersionService.findBlankInfraCodeVersion()
      .then((parentInfaCodeVersion) => {
        ...
        .then((appCodeVersion) => {
          ...
          const cvOpts = Object.assign(parsedContextData, {
            context: contextId,
            createdBy: {
   ...
   * @return {Instance}
   */
  static createClusterSibling (sessionUser, parsedComposeData, orgInfo, triggeredAction) {
    const log = DockerComposeClusterService.log.child({
      method: 'createClusterSibling',
      sessionUser, parsedComposeData, triggeredAction
    })
    log.info('called')
   ...
   * @return {ContextVersion}
   */
  static _createSiblingContextVersion (sessionUser, contextId, orgInfo, parsedContextData, dockerFileContent) {
    const log = DockerComposeClusterService.log.child({
      method: '_createSiblingContextVersion',
      sessionUser, contextId, orgInfo, parsedContextData, dockerFileContent
    })
    log.info('called')
    return InfraCodeVersionService.findBlankInfraCodeVersion()
      .then((parentInfaCodeVersion) => {
