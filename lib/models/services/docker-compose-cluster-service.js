...
module.exports = class DockerComposeClusterService {
  ...
  static createClusterInstance (sessionUser, parsedComposeData, fullRepoName, triggeredAction) {
    ...
    .then((context) => {
      ...
      return DockerComposeClusterService._createContextVersion(sessionUser, context._id, orgInfo, fullRepoName, parsedComposeData.contextVersion)
    })
    .then((contextVersion) => {
   ...
   */
  static _createContextVersion (sessionUser, contextId, orgInfo, repoName, parsedContextData) {
    const log = DockerComposeClusterService.log.child({
    ...
    return InfraCodeVersionService.findBlankInfraCodeVersion()
      .then((parentInfaCodeVersion) => {
        ...
        .then((appCodeVersion) => {
          ...
          if (parsedContextData.files && parsedContextData.files['/Dockerfile']) {
            const dockerFileContent = parsedContextData.files['/Dockerfile'].body
