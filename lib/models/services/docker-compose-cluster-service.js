...
module.exports = class DockerComposeClusterService {
  ...
  static create (sessionUser, triggeredAction, repoFullName, branchName, dockerComposeFilePath, newInstanceName) {
    ...
    const fullRepoName = repoTokens[1]
    ...
    return github.getRepoContentAsync(repoFullName, dockerComposeFilePath)
      ...
      .then(function (dockerComposeFileString) {
        ...
        const parseInput = {
          ...
          repositoryName: newInstanceName || fullRepoName,
          ownerUsername,
  ...
  static createClusterParent (sessionUser, parentComposeData, fullRepoName) {
    ...
    return DockerComposeClusterService._createContext(sessionUser, bigPoppaOrgId, orgGithubId)
    .then((context) => {
      return DockerComposeClusterService._createParentContextVersion(sessionUser, context._id, orgGithubId, fullRepoName, parentComposeData.contextVersion.buildDockerfilePath)
    })
    .then((contextVersion) => {
   ...
   * @return {Context}
   */
  static _createContext (sessionUser, bigPoppaOrgId, orgGithubId) {
    const log = DockerComposeClusterService.log.child({
   ...
   */
  static _createParentContextVersion (sessionUser, contextId, orgGithubId, fullRepoName, buildDockerfilePath) {
    const log = DockerComposeClusterService.log.child({
      method: '_createParentContextVersion',
      sessionUser, contextId, orgGithubId, fullRepoName
    })
    log.info('called')
  ...
  static createClusterSibling (sessionUser, parsedComposeData, orgInfo) {
    ...
    log.info('called')
    const orgGithubId = orgInfo.githubId
    const bigPoppaOrgId = orgInfo.bigPoppaOrgId

    return DockerComposeClusterService._createContext(sessionUser, bigPoppaOrgId, orgGithubId)
    .then((context) => {
      return DockerComposeClusterService._createSiblingContextVersion(sessionUser, context._id, orgGithubId, parsedComposeData.contextVersion.buildDockerfilePath, parsedComposeData.files['/Dockerfile'].body)
    })
    .then((contextVersion) => {
      return DockerComposeClusterService._createBuild(sessionUser, contextVersion._id)
    })
   ...
   * @return {ContextVersion}
   */
  static _createSiblingContextVersion (sessionUser, contextId, orgGithubId, dockerFileContent) {
    const log = DockerComposeClusterService.log.child({
      method: '_createSiblingContextVersion',
      sessionUser, contextId, orgGithubId, dockerFileContent
    })
    log.info('called')
