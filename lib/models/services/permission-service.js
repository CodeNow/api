 ...
 */
PermisionService.ensureOwnerOrModerator = function (sessionUser, model) {
  var log = PermisionService.logger.child({
    tx: true,
    user: sessionUser,
  ...
  log.info('call')
  return Promise.any([
    PermisionService.isOwnerOf(sessionUser, model),
    PermisionService.isModerator(sessionUser)
  ])
  .catch(Promise.AggregateError, function (err) {
    log.error({ err: err }, 'isOwnerOf: all permission checks failed')
    throw err[0]
  })
  .return(model)
}
 ...
 * @rejects with `access denied (!isOwner|!isModerator)` if `sessionUser` is not owner|moderator
 */
PermisionService.ensureModelAccess = function (sessionUser, model) {
  var log = PermisionService.logger.child({
    tx: true,
    user: sessionUser,
  ...
  return Promise.any([
    ...
    PermisionService.isHelloRunnableOwnerOf(sessionUser, model)
  ])
  .catch(Promise.AggregateError, function (err) {
    log.error({ err: err }, 'isOwnerOf: all permission checks failed')
    throw err[0]
...
PermisionService.isModerator = function (sessionUser) {
  ...
  return Promise.try(function () {
    if (sessionUser.isModerator !== true) {
      log.error('isOwnerOf: access denied')
