 ...
 */
InstanceForkService._forkNonRepoInstance = function (instance, masterInstanceName, isolationId, sessionUser) {
  ...
  return Promise.try(function () {
    ...
    if (!masterInstanceName) {
      throw new Error('_forkNonRepoInstance masterInstanceName is required')
    }
    if (!isolationId) {
    ...
    .then(function (newBuild) {
      return Promise.fromCallback(function (callback) {
        ...
        var body = {
          ...
          name: masterInstanceName + '--' + instance.name,
          env: instance.env,
          owner: { github: instanceOwnerId },
          masterPod: false
        }
