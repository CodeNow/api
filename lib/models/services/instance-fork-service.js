...
InstanceForkService.forkRepoInstance = Promise.method(function (instance, opts, sessionUser) {
  ...
  return BuildService.createNewContextVersion(instance, fakePushInfo)
 ...
 * @returns {Promise} Resolves with forkedInstance
 */
InstanceForkService._forkOne = function (instance, pushInfo) {
  ...
  return BuildService.createAndBuildContextVersion(instance, pushInfo)
    .then(function (result) {
      log.trace('fork master instance')
      var newBuild = result.build
      var user = result.user
      return Promise.fromCallback(function (callback) {
        var runnable = Runnable.createClient({}, user)
