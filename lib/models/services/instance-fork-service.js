...
InstanceForkService.forkMasterInstance = function (masterInst, buildId, branch, sessionUser) {
  ...
  const body = {
    aliases: masterInst.aliases,
    parent: masterInst.shortHash,
    build: buildId,
    name: sanitizedBranch + '-' + masterInst.name,
    shortName: masterInst.shortName || masterInst.name,
    env: masterInst.env,
    ports: masterInst.ports,
    owner: {
    ...
    isTestReporter: masterInst.isTestReporter,
    containerStartCommand: masterInst.containerStartCommand
  }
  const tags = [
  ...
  return Promise
    ...
    .then(function (newBuild) {
      ...
      var body = {
        ...
        isTestReporter: instance.isTestReporter,
