 ...
 * instancen (session user), but is similar to InstanceForkService._forkOne.
...
InstanceForkService.forkRepoInstance = Promise.method(function (instance, opts, sessionUser) {
  ...
  log.info(logData, 'InstanceForkService.forkRepoInstance')
  if (!exists(instance)) {
    ...
    .then(function (newContextVersion) {
      ...
      .then(function (newBuild) {
        var body = {
          ...
          name: instance.shortHash + '--' + instance.name,
          env: instance.env,
        ...
        log.trace(assign({ body: body }, logData), 'InstanceForkService.forkNonRepoInstance createInstance')
...
InstanceForkService._forkOneAndNotify = Promise.method(function (instance, pushInfo) {
  ...
  log.info(logData, 'InstanceForkService._forkOneAndNotify')
  if (!instance) {
  ...
  return InstanceForkService._forkOne(instance, pushInfo)
    ...
    .catch(function (err) {
      log.error(assign({ err: err }, logData), 'InstanceForkService._forkOneAndNotify error')
      throw err
    })
})
...
InstanceForkService.forkNonRepoInstance = function (instance, masterInstanceShortHash, isolationId, sessionUser) {
  ...
  return Promise.try(function () {
    ...
    .then(function () {
      log.trace(logData, 'InstanceForkService.forkNonRepoInstance createNewNonRepoContextVersion')
    ...
    .then(function (newContextVersion) {
      log.trace(logData, 'InstanceForkService.forkNonRepoInstance createBuild')
      ...
      return Promise.fromCallback(function (callback) {
        ...
        .then(function (newBuild) {
          log.trace(logData, 'InstanceForkService.forkNonRepoInstance buildBuild')
          return Promise.fromCallback(function (callback) {
    ...
    .then(function (instance) {
      log.trace(logData, 'InstanceForkService.forkNonRepoInstance findById')
