...
InstanceForkService.forkMasterInstance = function (masterInst, buildId, branch, sessionUser) {
  ...
  return InstanceService.createInstance(body, sessionUser)
    .tap(function () {
      // Since we've created an instance we now have added branches, set the flag.
      return Instance.findOneAndUpdateAsync(
        ...
        { new: true } // Set new to true so we get back the updated master instance
      )
        .tap(function (updatedMasterInst) {
          // Emit this update to the frontend!
          updatedMasterInst.emitInstanceUpdate()
        })
    })
