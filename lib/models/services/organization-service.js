...
var OrganizationService = module.exports = {
  ...
  getByGithubId: function (orgGithubId) {
    ...
    var options = {
      uri: process.env.BIG_POPPA_URL + '/organization/?github_id=' + orgGithubId,
      headers: {
    ...
    return rp(options)
      .then(function (orgs) {
        ...
        if (orgs.length > 0) {
          var now = moment()
          var org = orgs[0]
          log.trace('Organization found', { org: org })
          return assign(org, {
            trial_end: moment(org.trial_end, moment.ISO_8601),
            active_period_end: moment(org.active_period_end, moment.ISO_8601),
            grace_period_end: moment(org.grace_period_end, moment.ISO_8601),
            isActive: org.trial_end.isAfter(now) || org.active_period_end.isAfter(now)
          })
        }
        return null
      })
