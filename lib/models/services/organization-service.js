...
var OrganizationService = module.exports = {
  ...
  create: function (orgGithubName, sessionUser) {
    ...
    return github.getUserAuthorizedOrgsAsync()
      ...
      .then(function (githubOrg) {
        ...
        rabbitMQ.createOrganization({ githubId: githubOrg.id })
      })
  },
  ...
  getByGithubId: function (orgGithubId) {
    ...
    log.trace({ options: options }, 'Make HTTP request')
    return rp(options)
      .then(function (orgs) {
        log.trace({ orgs: orgs }, 'HTTP response')
        if (Array.isArray(orgs) && orgs.length > 0) {
          log.trace({ org: orgs[0] }, 'Organization found')
          return orgs[0]
        }
        return null
      })
  ...
  updateById: function (orgId, updates) {
    ...
    return rp(options)
      .then(function (res) {
        log.trace({ res: res }, 'HTTP response')
      })
  },
