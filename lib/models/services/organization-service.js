...
const BigPoppaClientError = require('@runnable/big-poppa-client/errors/big-poppa-client-error')
...
const OrganizationService = module.exports = {
   ...
   *
   * @param {String} orgName - Github login name for the organization
   *
   * @resolves {Organization}              organization model
   * @throws   {BigPoppaClientError}       if a failure occurs while communicating with bigPoppa
   * @throws   {OrganizationNotFoundError} when no organization could be found
   */
  getByGithubUsername: function (orgName) {
    const log = OrganizationService.log.child({
      orgName,
      method: 'getByGithubUsername'
    })
    log.info('OrganizationService.getByGithubId called')

    return bigPoppaClient.getOrganizations({ lowerName: orgName.toLowerCase() })
      .get('0')
      .catch(function () {
        throw errors.OrganizationNotFoundError('Organization not found', { orgName })
      })
      .tap(OrganizationService.checkOrg(orgName))
  },
  ...
  updatePrivateRegistryOnOrgBySessionUser: function (sessionUser, bpOrgId, privateRegistryOptions) {
    const log = OrganizationService.log.child({
      method: 'updatePrivateRegistryOnOrgBySessionUser',
      sessionUser,
      bpOrgId,
      privateRegistryUrl: keypather.get(privateRegistryOptions, 'url'),
      privateRegistryUsername: keypather.get(privateRegistryOptions, 'username')
    })
    log.info('OrganizationService.updateFlagsOnOrg called')

    return joi.validateOrBoomAsync(privateRegistryOptions, OrganizationService.UPDATE_PRIVATE_REGISTRY_SCHEMA)
      .tap(() => log.trace('validated update schema'))
      .tap(() => UserService.validateSessionUserPartOfOrg(sessionUser, bpOrgId))
      .tap(() => log.trace('validated owner of bp org'))
      .tap(() => bigPoppaClient.updateOrganization(bpOrgId, {
        privateRegistryUrl: privateRegistryOptions.url,
        privateRegistryUsername: privateRegistryOptions.username,
        privateRegistryPassword: privateRegistryOptions.password
      }))
      .catch(BigPoppaClientError, (err) => {
        log.trace({err, data: err.data}, 'Big poppa client error')
        if (err.data.statusCode >= 500) {
          throw err
        }
        if (err.data.statusCode >= 401) {
          throw Boom.unauthorized(err.data.originalError.message)
        }
        throw Boom.badRequest(err.data.originalError.message)
      })
  }
}
