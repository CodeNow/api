...
const OrganizationService = module.exports = {
  ...
  updateFlagsOnOrg: function (orgId, sessionUser, opts) {
    const log = OrganizationService.log.child({ method: 'OrganizationService.updateFlagsOnOrg', orgId, opts })
    ...
    return joi.validateOrBoomAsync(opts, OrganizationService.ORG_FLAG_SCHEMA)
      .then(function validateSessionUser (sessionUser, orgId) {
        log.trace('Request validated')
        return UserService.validateSessionUserPartOfOrg(sessionUser, orgId)
      })
      .then(function updateById () {
        log.trace('Update by Id')
        return OrganizationService.updateById(orgId, opts)
      })
      .tap(function checkOrganization () {
        log.trace('Check organization')
        return OrganizationService.checkOrg(orgId)
      })
  }
}
