...
InfraCodeVersionService.findBlankInfraCodeVersion = () => {
  const log = InfraCodeVersionService.logger.child({
    method: 'findBlankInfraCodeVersin'
  })
  log.info('call')
  const contextQuery = { 'name': 'Blank', 'isSource': true }
  return Context.findOneAsync(contextQuery)
    .tap((context) => {
      if (!context) {
        log.error('Context was not found')
        throw Boom.notFound('InfraCodeVersion not found', { query: contextQuery })
      }
    })
    .then((blankContext) => {
      log.info({ context: blankContext }, 'found blank context')
      const infraQuery = { context: blankContext._id }
      return InfraCodeVersion.findOneAsync(infraQuery)
        .tap(function checkICV (icv) {
          if (!icv) {
            log.error('InfraCodeVersion was not found')
            throw Boom.notFound('InfraCodeVersion not found', { query: infraQuery })
          }
        })
    })
}
...
InfraCodeVersionService.copyInfraCodeToContextVersion = function (targetContextVersion, sourceInfraCodeVersion) {
  ...
  return Promise.try(function validateArgumets () {
    ...
    if (!targetInfraCodeVersionId) {
      throw Boom.badRequest('Target context version has no infracode version.', { targetInfraCodeVersionId: targetInfraCodeVersionId })
    }

    if (!sourceInfraCodeVersion) {
      throw Boom.badRequest('invalid query sourceInfraCodeVersion.', { sourceInfraCodeVersion: sourceInfraCodeVersion })
    }
  })
