...
IsolationService._updateMasterEnv = Promise.method(function (master, children) {
  ...
  var envUpdate = master.env.map(function (env) {
    ...
    searchNames.forEach(function (search) {
      if (found) { return }
      // if it does,
      var foundIndex = env.toLowerCase().indexOf(search)
      if (foundIndex !== -1) {
        // replace it
        found = [
          env.slice(0, foundIndex),
          prefix + search,
          env.slice(foundIndex + search.length)
        ].join('')
      }
    })
    // return the updated line or origional env
    anyUpdate = anyUpdate || !!found
    return found || env
  })
 ...
 *   been sent.
 */
IsolationService.createIsolationAndEmitInstanceUpdates = function (isolationConfig, sessionUser) {
  var logData = {
    tx: true,
  ...
  return Promise.try(function () {
    ...
    .then(function () {
      ...
      return Isolation._validateCreateData(isolationConfig)
        .then(function () { return Isolation._validateMasterNotIsolated(isolationConfig.master) })
 ...
 */
IsolationService.deleteIsolation = function (isolationId) {
  var logData = {
    tx: true,
    isolationId: isolationId
  }
