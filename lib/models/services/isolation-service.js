    ...
    .then(function (nodeMap) {
      ...
      return Promise.all([
        Promise.each(childrenInNodeMap, function AddEachChildForMaster (childInstance) {
          ...
          log.trace({ target: instance._doc.name, child: childInstance.name, elasticHostname: elasticHostname }, 'adding a dependency')
        ...
        Promise.each(nodesThatAreInChildren, function removeEachChildForMaster (childNode) {
          return instance.removeDependency(childNode.instanceId)
        })
      ])
    })
}
