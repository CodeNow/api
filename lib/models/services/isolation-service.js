...
/**
 * Find instances that we are waiting to stop
 * @param isolationId - ID of the isolation
 * @returns {Promise}
 * @resolves {[Instances]} Array of instances that we are waiting to stop
 */
IsolationService.findInstancesToStop = function (isolationId) {
...
/**
 * Redploy the isolation if the isolation is stopping and there are no more instances left to stop
 * @param {String} isolationId - ID of isolation to check
 * @returns {Promise<Boolean>} - Resolves if redeploy triggered
 */
IsolationService.redeployIfAble = function (isolationId) {
  return Promise.props({
    instancesToStop: IsolationService.findInstancesToStop(isolationId),
    isolation: Isolation.find({
      _id: isolationId
    })
  })
    .then(function (results) {
      if (results.isolation.stopping && results.instancesToStop.length === 0) {
