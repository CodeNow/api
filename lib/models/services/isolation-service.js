 ...
 */
IsolationService.forkNonRepoChild = function (instanceId, masterInstanceName, isolationId, sessionUser) {
  return Promise.try(function () {
    if (!exists(instanceId)) {
      throw new Error('forkNonRepoChild instanceId is required')
    }
    if (!exists(masterInstanceName)) {
      throw new Error('forkNonRepoChild masterInstanceName is required')
    }
    ...
    .then(function () {
      ...
      return Isolation._validateCreateData(data)
        ...
        .then(function (models) {
          ...
          var masterInstanceName = models.masterInstance.name
          return Promise.map(
            nonRepoChildren,
            function (child) {
  ...
  return Promise.try(function () {
    ...
    .then(function () {
      ...
      return Promise.props({
        ...
        .then(function (updatedMasterInstance) {
          return IsolationService._emitUpdateForInstances([updatedMasterInstance], sessionUser)
        })
    })
}
