...
IsolationService._updateMasterEnv = Promise.method(function (master, children) {
  ...
  var ownerGitHubUsername = master.owner.username.toLowerCase()
  ...
  if (anyUpdate) {
    return Instance.findOneAndUpdateAsync(
      ...
      .then(function (instance) {
        return instance.setDependenciesFromEnvironmentAsync(ownerGitHubUsername)
      })
  } else {
    return master
  }
...
IsolationService._removeIsolationFromEnv = Promise.method(function (instance) {
  ...
  if (anyUpdate) {
    var ownerGitHubUsername = instance.owner.username
    return Instance.findOneAndUpdateAsync(
      { _id: instance._id },
      { $set: { env: envUpdate } }
    )
