...
var regexForIsolationMasterPrefix = /^[A-z0-9]*-/
/**
  ...
  return Promise.try(function () {
    ...
    .then(function (nodeMap) {
      function returnChildIfInNodeMap (child) {
        var cleanChildName
        if (child.isIsolationGroupMaster) {
          cleanChildName = child.name.replace(regexForIsolationMasterPrefix, '')
        } else {
          cleanChildName = child.name.replace(regexForIsolationPrefix, '')
        }
      ...
      return Promise.all([
        Promise.each(childrenInNodeMap, function removeAndAddEachChildForMaster (childInstance) {
          var elasticHostname = childInstance.getElasticHostname(ownerUsername).replace(regexForIsolationPrefix, '')
  ...
  return Promise.try(function () {
    ...
    .then(function (filteredChildrenList) {
      ...
      return Promise.each(children, function (child) {
        ...
        return IsolationService._updateDependenciesForInstanceWithChildren(child, childrenAndMaster)
      })
    })
    .return(children)
}
