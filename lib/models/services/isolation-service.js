 ...
 * the Dependency graph for each one.  The chil
  ...
  return Promise.try(function () {
    ...
    .then(function pluckOutNames (nodeArray) {
      ...
      nodeArray.forEach(function (node) {
        var cleanName = node.name.replace(regexForIsolationPrefix, '')
    ...
    .then(function (nodeMap) {
      function returnChildIfInNodeMap(child) {
        var cleanChildName = child.name.replace(regexForIsolationPrefix, '')
        return nodeMap[cleanChildName]
      }
      ...
      return Promise.all([
        Promise.each(childrenInNodeMap, function removeAndAddEachChildForMaster (childInstance) {
          return instance.addDependencyAsync(
            childInstance,
            childInstance.getElasticHostname(ownerUsername)
          )
        }),
        Promise.each(nodesThatAreInChildren, function removeAndAddEachChildForMaster (childNode) {
...
IsolationService.updateDependenciesForIsolation = function (master, children) {
  var log = this.logger.child({ method: '_updateDependencies' })
  ...
  return Promise.try(function () {
    ...
    .then(function () {
      // First, do master
      return IsolationService._updateDependenciesForInstanceWithChildren(master, children)
    })
    .then(function (filteredChildrenList) {
      log.trace({
        masterId: master._id.toString(),
        childrenId: filteredChildrenList.map(pluck('_id'))
      }, 'only used these children')
      return Promise.each(children, function (child) {
        // Now do it for every included child, even ones that were filtered out. Some of these
        // children have dependents that weren't dependant on the master
        return IsolationService._updateDependenciesForInstanceWithChildren(child, children)
      })
    })
    .return(children)
}
...
IsolationService.createIsolationAndEmitInstanceUpdates = Promise.method(function (isolationConfig, sessionUser) {
  ...
  return Isolation._validateCreateData(isolationConfig)
    ...
    .then(function (models) {
      var ownerUsername = models.masterInstance.owner.username
      // Set the initial dependencies based on the envs
      return Promise.props({
        masterInstance: models.masterInstance.setDependenciesFromEnvironmentAsync(ownerUsername),
        repoChildren: models.repoChildren.setDependenciesFromEnvironmentAsync(ownerUsername),
        nonRepoChildren: models.nonRepoChildren.setDependenciesFromEnvironmentAsync(ownerUsername)
      })
    })
    .then(function (models) {
      var allChildren = models.repoChildren.concat(models.nonRepoChildren)
      return IsolationService._updateDependenciesForIsolation(models.masterInstance, allChildren)
        .return(models)
    })
    .then(function (models) {
      return Promise.all([
