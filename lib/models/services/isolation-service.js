  ...
  return Promise.try(function () {
    ...
    .then(function pluckOutNames (nodeArray) {
      ...
      nodeArray.forEach(function (node) {
        var cleanName = node.lowerName.replace(regexForIsolationPrefix, '')
    ...
    .then(function (nodeMap) {
      ...
      function returnChildIfInNodeMap (child) {
        // if repo, get the repo name. else use the (non-repo instance's) lowerName
        var searchName
        var repoName = keypather.get(child, 'contextVersion.appCodeVersions[0].lowerRepo')
        if (repoName) {
          // it's a repo instance!
          searchName = repoName
          // repo names have slashed, sir.
          searchName = searchName.split('/').pop()
        } else {
          // it's a non-repo instance
          searchName = child.lowerName
          searchName = searchName.replace(regexForIsolationPrefix, '')
        }
        var returnValue = nodeMap[searchName]
        log.trace({
          instance: child,
