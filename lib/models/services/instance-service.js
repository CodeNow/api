 ...
 * @return {undefined}
 * @throws {errors.UserNotFoundError}                   When owner or owner.login not found
 * @throws {errors.instance.UnbuiltContextVersionError} When build has not started
 * @throws {errors.instance.InstanceCreateFailedError}  When shorthash failed to generate
 */
InstanceService._checkResultsForInstanceData = (opts) => {
  ...
  if (!keypather.get(opts, 'owner.login')) {
    err = new errors.UserNotFoundError('InstanceCreate: The owner of this instance could not be found on Github')
    err.setLevel('critical')
  } else if (!keypather.get(opts.contextVersion, 'build.started')) {
    err = new errors.instance.UnbuiltContextVersionError('InstanceCreate: An unbuilt context Version was attached to an instance')
  } else if (!opts.shortHash) {
    err = new errors.instance.InstanceCreateFailedError('InstanceCreate: We somehow failed to create a shortHash')
  } else {
    // no errors, return!
    return
  }
 ...
 *
 * @throws   {errors.instance.InstanceCreateFailedError} When the instance.save fails
 * @throws   {Boom.notFound}                             When any of the mongo queries fails to return a value
 * @private
 */
InstanceService._saveInstanceAndEmitUpdate = function (instance, newContextVersion, opts, sessionUser) {
  ...
  return instance.saveAsync()
    .tap(function (instance) {
      if (!instance) {
        throw new errors.instance.InstanceCreateFailedError('InstanceCreate: We somehow failed to create a shortHash')
      }
    })
    .tap(function (instance) {
      if (opts.env) {
