...
InstanceService.updateInstanceBuild = function (originalInstance, updates, sessionUserGithubId) {
  ...
  .then(function (instance) {
    ...
    if (keypather.get(originalInstance, 'contextVersion.build.dockerContainer')) {
      log.trace('build container found. delete container')
      // We should be sending the originalInstance, since it has the old Cv data
      InstanceService.deleteBuildContainer(originalInstance)
    }
...
/**
 * Utility function to call `rabbitMQ.deleteImageBuilderContainer`.
 * @param {Instance}       instance                                      - Instance that has been
 *                                                                           updated with a new CV
 * @param {ContextVersion} instance.contextVersion                       - old contextVersion that is
 *                                                                           about to be removed from
 *                                                                           the instance
 * @param {Object}         instance.contextVersion.build.dockerContainer - Id of docker build container
 * @throws ValidationError when requiredKeys are not provided
 */
InstanceService.deleteBuildContainer = function (instance) {
  rabbitMQ.deleteImageBuilderContainer({
    containerId: keypather.get(instance, 'contextVersion.build.dockerContainer'),
    dockerHost: keypather.get(instance, 'contextVersion.dockerHost')
  })
}
