...
InstanceService.updateInstance = function (instance, opts, sessionUser) {
  ...
  return InstanceService.validateUpdateOpts(opts)
    ...
    .tap(function setIsolatedInstancesLocked () {
      ...
      return InstanceService.updateLockedOnInstancesInIsolation(instance.isolated, mainACV.repo, opts.locked)
    })
 ...
 */
InstanceService.updateLockedOnInstancesInIsolation = function (isolationId, repoName, locked) {
  var log = logger.log.child({
    tx: true,
    isolationId: isolationId,
    repoName: repoName,
    locked: locked,
    method: 'InstanceService.updateLockedOnInstancesInIsolation'
  })
  log.info('InstanceService.updateLockedOnInstancesInIsolation called')
  return Instance.updateInstancesInIsolationWithSameRepoAsync(isolationId, repoName, { locked: locked })
}
