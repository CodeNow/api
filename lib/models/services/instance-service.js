...
InstanceService.updateInstanceBuild = function (originalInstance, updates, sessionUserGithubId) {
  ...
  log.info(logData, 'InstanceService.updateInstanceBuild')
  ...
  .then(function (instance) {
    log.trace(logData, 'updateInstanceBuild try to find build')
    return Build.findByIdAsync(buildId)
      .then(function (build) {
        log.trace(logData, 'updateInstanceBuild build found, create container')
        if (instance && build && build.successful) {
