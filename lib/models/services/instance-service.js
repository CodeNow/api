  ...
  function (instance, containerId, containerInspect, cb) {
    ...
    Instance.findOneAndUpdate(query, { $set: $set }, function (err, updatedInstance) {
      if (err) {
        log.error(put({
          err: err
        }, logData), 'InstanceService.prototype.updateOnContainerDie err')
        return cb(err)
      }
      if (!updatedInstance) { // changed or deleted
        log.error(logData,
          'InstanceService.prototype.updateOnContainerDie error instance not found')
        return cb(Boom.conflict("Container inspect was not updated, instance's container has changed"))
      }
      log.trace(logData, 'InstanceService.prototype.updateOnContainerDie success')
      cb(null, updatedInstance)
    })
  }
