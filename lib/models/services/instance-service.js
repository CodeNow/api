...
InstanceService.stopInstance = function (instanceData, sessionUserGithubId) {
  ...
  return Promise.resolve()
    ...
    .then(function () {
      log.trace(put({ instance: instance.toJSON() }, logData), 'stopInstance marking as starting')
      return instance.setContainerStateToStoppingAsync()
        .then(function () {
          log.trace(put({ instance: instance.toJSON() }, logData), 'stopInstance publish stop job')
