      ...
      .spread(function (build, oldBuild) {
        if (oldBuild) {
          log.trace({
            ...
            originalInstanceContextVersion: keypather.get(originalInstance, 'contextVersion._id.toString()'),
            newInstanceContextVersion: keypather.get(instance, 'contextVersion._id.toString()'),
            oldOLDContextVersion: oldContextVersion,
            originalInstance: originalInstance,
            newInstance: instance
          }, 'build found, delete old context version JORGE')
          rabbitMQ.deleteContextVersion({
            contextVersionId: keypather.get(originalInstance, 'contextVersion._id.toString()')
          })
        } else {
