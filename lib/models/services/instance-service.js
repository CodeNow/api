...
InstanceService.emitInstanceUpdate = function (instance, userGithubId, eventName, forceCvRefresh) {
  ...
  return User.findByGithubIdAsync(userGithubId)
    .then(function (user) {
      ...
      // TODO: only do this if build wasn't populated before
      // this one is minor optimization we can add later if (instance.build) {
      populationPromises.push(instance.populateModelsAsync())
      if (forceCvRefresh) {
