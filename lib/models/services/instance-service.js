    ...
    .then(function fetchBuild () {
      return Build.findByIdAsync(opts.build)
        .tap(function useBuildsOwner (build) {
          if (!build) {
            throw Boom.notFound('build not found', { build })
          }
          if (!opts.owner) {
      ...
      return ContextVersionService.findContextVersion(build.contextVersion)
        ...
        .tap(function deleteOldDockerContainer () {
          ...
          if (containerId) {
            ...
            return rabbitMQ.deleteContainer({ containerId })
          }
        })
        ...
        .tap(function matchOtherIsolatedCommits () {
          ...
          const isManual = keypather.get(instance, 'contextVersion.build.triggeredAction.manual')
          const isolationId = keypather.get(instance, 'isolated.toString()')
          // Only match commit for isolated instances manually updated
          if (isolationId && isManual) {
            log.trace({ isolationId }, 'Isolated, enqueing job to match relative isolated instances with same commit')
          ...
          log.trace({ isolationId }, 'Instance is not isolated')
        })
    })
}
...
InstanceService.assertInstanceFound = function (instanceQuery) {
  ...
  return Instance.findOneByShortHashAsync(instanceShortHash)
    .tap(function (instance) {
      if (!instance) {
        ...
        throw Boom.notFound('Instance not found', { instanceShortHash })
      }
    })
}
 ...
 * @param {String} instanceId - short hash to find instance
 * @returns {Promise}
 * @resolves {Object} instance mongo model
 * @rejects   {Instance.NotFoundError} When instance lookup failed
 * @rejects   {Error}                  When Mongo fails
 */
InstanceService.findInstanceById = function (instanceId) {
  const log = logger.child({
    method: 'InstanceService.findInstanceById',
    instanceId
  })
  log.info('InstanceService.findInstanceById called')
  return Instance.findByIdAsync(instanceId)
    .tap(InstanceService.assertInstanceFound({ instanceId }))
}
...
InstanceService.stopInstance = function (instanceShortHash, sessionUser) {
  ...
  return InstanceService.findInstance(instanceShortHash)
    ...
    .tap(function (instance) {
      log.trace({ instance }, 'check if container exists')
    ...
    .then(function (instance) {
      log.trace({ instance }, 'checks passed stopInstance marking as stopping')
      const containerId = instance.container.dockerContainer
      return Instance.markAsStoppingAsync(instance._id, containerId)
        .then(function (instance) {
          log.trace({
...
InstanceService.startInstance = function (instanceShortHash, sessionUser) {
  ...
  return InstanceService.findInstance(instanceShortHash)
    .then(function (instance) {
      return InstanceService._startInstance(instance, sessionUser)
    })
}

InstanceService.startInstanceById = function (instanceId, sessionUser) {
  const log = logger.child({
    method: 'InstanceService.startInstanceById',
    instanceId,
    sessionUser
  })
  log.info('InstanceService.startInstanceById called')
  return InstanceService.findInstanceById(instanceId)
    .then(function (instance) {
      return InstanceService._startInstance(instance, sessionUser)
    })
}

InstanceService._startInstance = function (instance, sessionUser) {
  const log = logger.child({
    method: 'InstanceService._startInstance',
    instance,
    sessionUser
  })
  log.info('InstanceService._startInstance called')
  return Promise.resolve(() => {
    return instance
  })
    .tap(function (instance) {
    ...
    .tap(function (instance) {
      ...
      if (!containerId) {
        log.error('Instance does not have a container')
        throw Boom.badRequest('Instance does not have a container', { instanceId })
      }
    })
    .tap(Instance.assertNotStartingOrStopping)
    .then(function (instance) {
      log.trace({ instance }, 'checks passed')
      const dockRemoved = keypather.get(instance, 'contextVersion.dockRemoved')
      const sessionUserGithubId = keypather.get(sessionUser, 'accounts.github.id.toString()')
      const instanceId = instance._id.toString()
      const containerId = instance.container.dockerContainer
      if (dockRemoved) {
        log.trace('dockRemoved: need to redeploy')
        rabbitMQ.redeployInstanceContainer({ instanceId, sessionUserGithubId })
        return instance
      }

      log.trace('marking as starting')
      return Instance.markAsStartingAsync(instance._id, containerId)
        .then(function (instance) {
          log.trace({
            containerState: keypather.get(instance, 'container.inspect.State')
          }, 'publish start job')
...
InstanceService.restartInstance = function (instanceShortHash, sessionUser) {
  ...
  return InstanceService.findInstance(instanceShortHash)
    ...
    .tap(Instance.assertNotStartingOrStopping)
    .then(function (instance) {
      log.trace({ instance }, 'checks passed, marking as starting')
      const containerId = instance.container.dockerContainer
      return Instance.markAsStartingAsync(instance._id, containerId)
        .then(function (instance) {
          log.trace({
