...
InstanceService.logger = logger.log.child({
  tx: true,
 ...
 *
 * @returns  {Promise}       When the instance has been updated successfully
 * @resolves {Instance}      Newly updated instance
 * @throws   {Boom.conflict} When no instance was found to update
 * @throws   {Error}         When Mongo fails
 */
InstanceService.updateContainerInspect = function (query, setObject) {
  var log = InstanceService.logger.child({
    tx: true,
    query: query,
    setObject: setObject,
  ...
  return Instance.findOneAndUpdateAsync(query, { $set: setObject })
    .tap(function (instance) {
      if (!instance) { // changed or deleted
        log.error('updateContainerInspect error instance not found')
  ...
  function (instanceId, containerId, containerInspect, containerIp) {
    ...
    return Instance.findOneAsync(query)
      .then(function (oldInstance) {
        ...
        return InstanceService.updateContainerInspect(query, $set)
          .tap(function () {
            ...
            }, 'invalidating the dns')
            return Instance.invalidateContainerDNS(oldInstance)
          })
      })
  }
