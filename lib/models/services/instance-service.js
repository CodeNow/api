  ...
  .then(function (instance) {
    ...
    return Build.findByIdAsync(buildId)
      .then(function (build) {
        ...
        var buildMessage = keypather.get(instance, 'contextVersion.attrs.build.message')
        // If parent is isolated and build is not auto-deployed (build.message)
          // Find all isolated instances with same repo and branch
          // If any are found
            // Check in appCodeVersion if commit is different
              // If it's different, change the ACV in child instances
        if (oldContextVersionId !== newContextVersionId && oldContextVersionId) {
          ...
          log.trace({
            ...
            isolatedString: instance.isolated.toString(),
            buildMessage: buildMessage
          }, 'Checking isolation and build message')
          if (instance.isolated && buildMessage && buildMessage.match(/manual.*build/i)) {
            log.trace({
