    ...
    .then(function fireOffRabbitEventsIfBuildSuccessful (results) {
      return results.instance.saveAsync()
        .tap(function (instance) {
          return afterUpdatingInstance(log, instance, results.contextVersion, opts, sessionUser)
        })
    })
    .catch(function (err) {
      log.error({
...
function afterUpdatingInstance (log, instance, newContextVersion, opts, sessionUser) {
...
InstanceService.updateInstance = function (instance, opts, sessionUser) {
  ...
  return InstanceService.validateUpdateOpts(opts)
    .then(function checkIfNewBuild () {
      if (opts.build || opts.contextVersion) {
        return InstanceService._prepareInstanceForNewBuild(instance, opts)
    ...
    .then(function updateInstance (contextVersion) {
      return instance.saveAsync()
        .tap(function (instance) {
          if (!instance) {
            throw Boom.notFound('instance not found')
          }
          return afterUpdatingInstance(log, instance, contextVersion, opts, sessionUser)
        })
    })
    .catch(function (err) {
      log.error({
 ...
 * @param {Instance} instance - original instance to be update
 * @param {Object} updates - ndw instance properties
 * @param {String} sessionUser
 * @param {Promise} promise
 */
InstanceService._prepareInstanceForNewBuild = function (instance, opts) {
  opts = pick(opts, [
    'build',
    'isolated',
    'masterPod'
  ...
  return Build.findByIdAsync(opts.build)
    ...
    .tap(function (contextVersion) {
      if (!keypather.get(contextVersion, 'build.started')) {
        throw Boom.badRequest('Cannot attach a build to an instance with context ' +
          'versions that have been started')
      }
    ...
    .tap(function deleteOtherInstancesWithSameBranch (contextVersion) {
      ...
      if (instance.masterPod && (!opts.isolated && !instance.isolated)) {
        ...
        if (currentMainACV.branch !== newMainACV.branch) {
          log.trace({
    ...
    .tap(function deleteOldDockerContainer () {
      if (container && container.dockerContainer) {
        ...
        return InstanceService.deleteInstanceContainer(instance, container)
      }
    })
}
...
InstanceService.updateBuild = function (instance, build) {
  ...
  return User.findByGithubIdAsync(instance.createdBy.github)
    .then(function (instanceCreator) {
      return InstanceService.updateInstance(instance, { build: build._id }, instanceCreator)
    })
}
