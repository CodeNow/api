...
InstanceService.findInstanceByBranchName = function (atlassianOrg, branchName, sessionUser) {
  const log = logger.child({
    branchName,
    atlassianOrg,
    sessionUser,
  ...
  log.info('call')
  return InstanceService.fetchInstances({'owner.username': atlassianOrg, name: branchName})
    .then((instances) => {
      if (!instances.length) {
