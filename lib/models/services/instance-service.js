 ...
 */
InstanceService.updateInstanceBuild = function (originalInstance, updates, sessionUserGithubId, oldContextVersion) {
  ...
  .then(function (instance) {
    ...
    return Build.findByIdAsync(buildId)
      .then(function (build) {
        ...
        if (build && oldContextVersionId !== newContextVersionId) {
          log.trace({
...
InstanceService._findInstanceAndContextVersion = function (opts, cb) {
  ...
  async.parallel({
    ...
    err = validateMongoData(data, opts)
    if (err) {
   ...
   */
  function validateMongoData (data, opts) {
    var err
    if (!data.instance) {
      err = Boom.notFound('Instance not found', opts)
    } else if (!data.contextVersion) {
      err = Boom.notFound('ContextVersion not found', opts)
    } else if (!objectId.equals(data.instance.contextVersion._id, opts.contextVersionId)) {
      err = Boom.conflict('Queried ContextVersion ID and Instance ContextVersion ID do not match)', opts)
    } else if (data.instance.container) {
      err = Boom.notFound('Instance already has a container', opts)
    } else if (!objectId.equals(data.instance.contextVersion._id, data.contextVersion._id)) {
      err = Boom.conflict('ContextVersion ID and Instance ContextVersion ID do not match (Instance\'s contextVersion has changed)', opts)
    }
    return err
  }
}
