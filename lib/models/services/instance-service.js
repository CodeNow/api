...
/*jshint maxparams: 6 */
InstanceService.prototype.deploy =
  function (instanceId, buildId, userId, ownerUsername, forceDock, cb) {
    if (isFunction(forceDock)) {
      cb = forceDock;
      forceDock = undefined;
    } else if (forceDock === 'body.forceDock' || 'forceDock') {
      forceDock = undefined;
    }
    var logData = {
      tx: true,
