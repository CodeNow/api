...
InstanceService.updateInstance = function (instance, opts, sessionUser) {
  ...
  return InstanceService.validateUpdateOpts(opts)
    ...
    .tap(function setIsolatedInstancesLocked () {
      if (!isBoolean(opts.locked) || !instance.isolated) {
        return
      }
      ...
      return InstanceService.updateLockedOnIsolatedInstances(instance.isolated, mainACV.repo, opts.locked)
    })
...
/**
 * Update all instances in an isolation with same repo name and update their
 * `locked` property (for auto-deploy)
 *
 * @param  {String}  isolationId - Id of isolation model object
 * @param  {String}  repoName    - Name of repo
 * @param  {Boolean} locked      - Locked property to determine auto-deploy
 * @return {Promise}
 * @resolves {Array<Instance>}   - Returns an array of updated instances
 */
InstanceService.updateLockedOnIsolatedInstances = function (isolationId, repoName, locked) {
  var log = logger.log.child({
    tx: true,
    isolationId: isolationId,
    repoName: repoName,
    locked: locked,
    method: 'InstanceService.updateLockedOnIsolatedInstances'
  })
  log.info('InstanceService.updateLockedOnIsolatedInstances called')
  return Instance.findIsolationInstancesWithRepoAsync(isolationId, repoName)
    .then(function (instances) {
      log.trace('Isolation group master found. Update all instances.')
      return Promise.map(instances, function (instance) {
        return instance.setAsync({ locked: locked })
      })
    })
}
