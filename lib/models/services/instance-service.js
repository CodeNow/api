...
InstanceService.updateInstance = function (instance, opts, sessionUser) {
  ...
  return InstanceService.validateUpdateOpts(opts)
    ...
    .tap(function setIsolatedInstancesLocked () {
      if (!exists(opts.locked) || !instance.isolated) {
        log.trace({
          locked: opts.locked,
          isolated: instance.isolated
        }, '`locked` property not set or instance is not isolated')
        return
      }
      ...
      return Instance.findInstancesInIsolationWithSameRepoAsync(instance.isolated, mainACV.repo)
        .then(function (instances) {
          ...
          return Promise.map(instances, function (isolationInstance) {
            log.trace({
              ...
              locked: isolationInstance.locked
            }, 'Update instance with new `locked` property')
            return isolationInstance.updateAsync({ locked: opts.locked })
              .then(function () {
                log.trace({
                  instanceId: isolationInstance._id.toString(),
                  locked: isolationInstance.locked
                }, 'emitInstanceUpdate for isolation instance')
                return isolationInstance.emitInstanceUpdateAsync(sessionUser, 'update')
              })
          })
        })
    })
