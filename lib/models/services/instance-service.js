    ...
    .then(function (build) {
      return ContextVersion.findByIdAsync(build.contextVersion)
        .tap(function validateContextVersion (contextVersion) {
          var err
          if (!contextVersion) {
            err = Boom.notFound('contextVersion not found')
            log.error({
              err: err,
              cvId: build.contextVersion
            }, err.message)
            throw err
          }
          if (!keypather.get(contextVersion, 'build.started')) {
            err = Boom.badRequest('Cannot attach a build to an instance with context ' +
              'versions that have not started building')
            log.error({
              err: err,
              cvId: build.contextVersion
          ...
          if (keypather.get(contextVersion, 'owner.github') !== keypather.get(instance, 'owner.github')) {
            err = Boom.badRequest('Instance owner must match Build owner')
            log.error({
              err: err,
              cvId: build.contextVersion,
              cvOwner: keypather.get(contextVersion, 'owner.github'),
              instanceOwner: keypather.get(instance, 'owner.github')
            }, err.message)
            throw err
          }
        })
