...
InstanceService.updateBuild = function (build) {
  return function (instance) {
    ...
    return User.findByGithubIdAsync(instance.owner.id)
...
InstanceService.updateBuildByRepoAndBranch = function (repo, branch, cvId) {
  ...
  return Build.findByContextVersionIdsAsync([cvId]).then(function (builds) {
    ...
    var build = builds[0]
    Instance.findInstancesLinkedToBranch(repo, branch)
      .then(function (instances) {
        log.trace(logData, 'updateBuildByRepoAndBranch found instances to update')
        return Promise.map(instances, InstanceService.updateBuild(build))
      })
  })
}
