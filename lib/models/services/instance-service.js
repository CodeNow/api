...
InstanceService.createContainer = function (opts, cb) {
  ...
  async.waterfall([
    ...
    function (mongoData, cb) {
      assign(mongoData, opts);
      InstanceService._createDockerContainer(mongoData, cb);
    }
  ], cb);
};
...
InstanceService._createDockerContainer = function (opts, cb) {
  ...
  function createDockerContainer (dockerHost, cb) {
    ...
    // note: do not use any 101 util that clones opts, it will error
    var createOpts = assign({ ownerUsername: opts.ownerUsername }, opts);
