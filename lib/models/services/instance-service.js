...
InstanceService.deleteForkedInstancesByRepoAndBranch = function (instanceId, repo, branch, cb) {
  ...
  log.info('InstanceService.deleteForkedInstancesByRepoAndBranch')
  // do nothing if parameters are missing
  if (!instanceId || !repo || !branch) {
    log.warn('deleteForkedInstancesByRepoAndBranch missing instanceId: ' + instanceId + ' repo: ' + repo + ' branch: ' + branch)
    return cb()
  }
  Instance.findForkedInstancesAsync(repo, branch)
    .then(function (instances) {
      if (instances && instances.length) {
        ...
        if (!thisInstance) {
          var err = new Error('deleteForkedInstancesByRepoAndBranch could not find its original instance')
          log.error({
            err: err
    ...
    .catch(function (err) {
      log.warn({
        err: err
      }, 'deleteForkedInstancesByRepoAndBranch failed')
      throw err
    })
    .asCallback(cb)
}
