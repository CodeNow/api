...
InstanceService.createContainer = function (opts, cb) {
  ...
  async.waterfall([
    ...
    function (mongoData, cb) {
      ...
      var tasks = {
        container: InstanceService._createDockerContainer.bind(InstanceService, createOpts),
        recovery: mongoData.contextVersion.handleRecovery.bind(mongoData.contextVersion)
      }
      async.series(tasks, function (err, results) {
        if (err) { return cb(err) }
        cb(null, results.container)
      })
    }
  ], cb)
}
...
InstanceService._findInstanceAndContextVersion = function (opts, cb) {
  ...
  async.parallel({
    ...
    if (err) {
      log.error(put({ err: err }, logData), 'InstanceService._findInstanceAndContextVersion dbErr')
      return cb(err)
    }
    err = validateMongoData(data)
    if (err) {
      log.error(put({ err: err }, logData), '_findInstanceAndContextVersion validationErr')
      return cb(err)
    }
    log.trace(put({ data: data }, logData), '_findInstanceAndContextVersion success')

    if (data.instance.parent) {
      log.trace(put({ data: data }, logData),
        '_findInstanceAndContextVersion find if parent exists')
      Instance.findOneByShortHash(data.instance.parent, function (err, parent) {
        if (err) {
          log.error(put({ data: data, err: err }, logData),
...
InstanceService._handleImageNotFoundErr = function (opts, err, cb) {
  ...
  try {
    ...
    log.error(put(logData, { err: dataErr }),
      '_handleImageNotFoundErr data validation error')
