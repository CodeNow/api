...
InstanceService._createDockerContainer = function (opts, cb) {
  ...
  docker.createUserContainer(opts, function (err, container) {
    ...
    if (err) {
      log.info(logData, 'InstanceService._createDockerContainer error')
      var originalMessage = keypather.get(err.data.err.json)
      var opts = map({
        orriginalErr: err,
        message: err.message,
        originalMessage: originalMessage,
        imageNotFound: Docker.isImageNotFoundForCreateErr(err),
        isBoom: err.isBoom,
        is5xx: err.output.statusCode === 500 || err.output.statusCode === 502,
        is500: err.output.statusCode === 500,
        is502: err.output.statusCode === 502,
        messageImageNotFound: /image.*not found/.test(err.message),
        orignalMessage: /image.*not found/.test(originalMessage),
        imageNotFoundMessage: (/image.*not found/.test(err.message) || /image.*not found/.test(originalMessage))
      }, toJSON)
