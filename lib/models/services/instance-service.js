  ...
  return Build.findByIdAsync(opts.build)
    ...
    .then(function (build) {
      return ContextVersion.findByIdAsync(build.contextVersion)
        ...
        .tap(function matchOtherIsolatedCommits (contextVersion) {
          if (!instance.isolated) {
            log.trace({
              isolationId: instance.isolated
            }, 'Instance is not isolated. Wont match commit')
            return
          }
          var mainACV = ContextVersion.getMainAppCodeVersion(keypather.get(contextVersion, 'appCodeVersions'))
          if (!mainACV) {
            log.trace({
              isolationId: instance.isolated
            }, 'Instance has no main appCodeVersion. Wont match commit')
            return
          }
          log.trace({
            isolationId: instance.isolated,
            mainACV: mainACV
          }, 'Enqueue job to match instance commit in al isolation instances with same repo (Instance is isolated)')
          return rabbitMQ.matchCommitWithIsolationMaster({
            isolationId: instance.isolated,
            repo: mainACV.repo,
            branch: mainACV.branch,
            commit: mainACV.commit,
