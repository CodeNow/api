 ...
 * @param sessionUserId - user that should perform instance deletion action
 ...
 */
InstanceService.prototype.deleteForkedInstancesByRepoAndBranch = function (instanceId, sessionUserId, repo, branch, cb) {
  var logData = {
    tx: true,
  ...
  if (!instanceId || !sessionUserId || !repo || !branch) {
 ...
 * @return {Promise}
 */
InstanceService.deleteAllInstanceForks = function (instance, sessionUserId) {
  var logData = {
    tx: true,
    instance: instance,
    sessionUserId: sessionUserId
  ...
  if (!instance.masterPod) {
    // return emoty array since nothing was deleted
    log.trace(logData, '_deleteAllInstanceForks nothing to delete')
