 ...
 * @return {undefined}
 * @throws {Boom.notFound} when owner or owner.login not found
 * @throws {Boom.badRequest} when build has not started
 * @throws {Boom.badImplementation} when shorthash failed to generate
 */
InstanceService._checkResultsForInstanceData = (opts) => {
  ...
  let err = null
  let critErrorMessage = null
  if (!keypather.get(opts, 'owner.login')) {
    critErrorMessage = 'InstanceCreate: The owner of this instance could not be found on Github'
    err = Boom.notFound(errorMessages.user.notFound)
  } else if (!keypather.get(opts.contextVersion, 'build.started')) {
    critErrorMessage = 'InstanceCreate: An unbuilt context Version was attached to an instance'
    err = Boom.badRequest(errorMessages.instances.create.unbuiltCv)
  } else if (!opts.shortHash) {
    critErrorMessage = 'InstanceCreate: We somehow failed to create a shortHash'
    err = Boom.badImplementation(errorMessages.instances.create)
  }
  if (critErrorMessage) {
    error.log(new CriticalError(critErrorMessage, {
      opts: opts,
...
InstanceService._saveInstanceAndEmitUpdate = function (instance, newContextVersion, opts, sessionUser) {
  ...
  return instance.saveAsync()
    .tap(function (instance) {
      if (!instance) {
        error.log(new CriticalError('The Instance failed to create', {
          instance: instance,
          opts: opts
        }))
        throw Boom.badImplementation(errorMessages.instances.create)
      }
    })
    .tap(function (instance) {
      if (opts.env) {
