...
InstanceService.updateBuild = function (build) {
  return function (instance) {
    ...
    return User.findByGithubIdAsync(instance.owner.github)
      .then(function (instanceCreator) {
        log.info({ instanceCreator: instanceCreator }, 'updateBuild found instance creator')
        var runnableClient = new Runnable({}, instanceCreator)
        return Promise.fromCallback(function (cb) {
          log.info('updateBuild instance')
  ...
  return Build.findByContextVersionIdsAsync([cvId]).then(function (builds) {
    ...
    return Instance.findInstancesLinkedToBranchAsync(repo, branch)
      .then(function (instances) {
        log.trace(logData, 'updateBuildByRepoAndBranch found instances to update')
        return Promise.map(instances, InstanceService.updateBuild.bind(InstanceService, build))
      })
  })
}
