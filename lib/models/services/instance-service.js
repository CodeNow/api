 ...
 * @param query {object} - query to find matching instances to update
 * @param setObject {object} - object set as the $set value of the mongo update
 * @param cb {function}- standard Node.js callback
 */
 ...
 * @param instance {object} - instance that should be updated
 * @param containerId {string} - docker container id
 * @param containerInspect {object} - docker inspect data
 * @param containerIp {string} (optional)- docker container ip address
 * @param cb {function}- standard Node.js callback
 */
...
InstanceService.emitInstanceUpdate = function (instance, userGithubId, eventName, forceCvRefresh) {
  ...
  return User.findByGithubIdAsync(userGithubId)
    ...
    .then(function () {
      return messenger.emitInstanceUpdateAsync(instance, eventName)
    })
}
