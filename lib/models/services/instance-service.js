...
const boomError = require('dat-middleware').Boom
...
InstanceService.fetchInstanceByContainerIdAndEnsureAccess = function (containerId, sessionUser) {
  ...
  return Instance.findOneByContainerIdOrBuildContainerIdAsync(containerId)
    .then((instance) => {
      if (instance) {
    ...
    .tap(res => {
      if (!res || !res.instance) {
        const notFound = boomError.create(404, 'Missing instance', { containerId })
