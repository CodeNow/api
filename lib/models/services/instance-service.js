...
InstanceService._findInstanceAndContextVersion = function (opts, cb) {
  var log = logger.child({
    method: 'InstanceService._findInstanceAndContextVersion',
    opts: opts
  })
  log.info('InstanceService._findInstanceAndContextVersion called')
  ...
  async.parallel({
    ...
    if (err) {
      log.error({ err: err }, 'dbErr')
      return cb(err)
    }
    err = validateMongoData(data)
    if (err) {
      log.error({ err: err }, 'validationErr')
      return cb(err)
    }
    log.trace({ data: data }, 'success')

    if (!data.instance.parent) {
      return cb(null, data)
    }
    log.trace({ data: data }, 'check if parent exists')
    Instance.findOneByShortHash(data.instance.parent, function (err, parent) {
      var log = log.child({ data: data, err: err })
      if (err) {
        log.error('parent lookup error')
        return cb(err)
      }
      if (!parent) {
      ...
      log.trace('parent lookup success')
      cb(null, data)
    })
  })
  /**
