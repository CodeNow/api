...
function wrapMethods (logger) {
  [
    ...
    var original = logger[method];
    logger[method] = function (data) {
      if(isObject(data)) {
        try{
          ...
          console.log(e);
          console.log(data);
          console.log('error flattening data');
          data = {message: 'error with data'};
        }
      }
      return original.apply(logger, arguments);
    }.bind(logger);
  });
}
wrapMethods(logger);
