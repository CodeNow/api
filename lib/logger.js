...
var serializers = put(bunyan.stdSerializers, {
  ...
  container: function (container) {
    ...
    container = removeEnvsAtPropertyPath(['inspect.Config.Env'])(container)
  ...
  containerInspect: removeEnvsAtPropertyPath(['config.Env']),
  elapsedTimeSeconds: function (date) {
    return (new Date() - date) / 1000
  },
  err: removeEnvsAtPropertyPath([
    'data.job.inspectData.Config.Env',
    'data.job.container.inspect.Config.Env',
    'data.docker.opts.Env'
  ]),
  ...
  data: function (data) {
    data = removeEnvsAtPropertyPath([
      ...
      'job.container.inspect.Config.Env',
      'job.inspect.Config.Env',
      'job.inspectData.Config.Env',
      'docker.opts.Env'
    ])(data)
    return _removeExtraKeys(data)
  },
