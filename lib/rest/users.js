...
module.exports = function (parentDomain) {
  ...
  var fetchuser = function (req, res, next) {
    if (!req.params.userid.match(/^[0-9a-fA-F]{24}$/)) {
      ...
      users.findUser(req.domain, { _id: req.params.userid }, req.domain.intercept(function (user) {
        if (!user) {
          res.json(404, { message: 'user not found' });
        } else {
  ...
  app.post('/token', function (req, res) {
    ...
    if (req.body.username == null && req.body.email == null) {
      return res.json(400, { message: 'username or email required' });
    } else {
      if (req.body.password == null) {
        return res.json(400, { message: 'password required' });
      } else {
            ...
            if (!token) {
              ...
              return redis_client.get(token, function (err, old_user_id) {
                ...
                if (!old_user_id) {
                  ...
                  return users.findUser(req.domain, { _id: old_user_id }, function (err, old_user) {
                    if (err) {
                      return res.json(err.code, { message: err.msg });
                    } else {
  ...
  app.all('*', function (req, res, next) {
    ...
    if (/\/runnables\?map=true|\/channels\?map=true/.test(url.parse(req.url).path)) {
      ...
      token = req.get('runnable-token');
      if (!token) {
        return res.json(401, { message: 'access token required' });
      } else {
        return redis_client.get(token, function (err, user_id) {
          ...
          if (!user_id) {
            return res.json(401, { message: 'must provide a valid access token' });
          } else {
            req.user_id = user_id;
            return next();
          }
  ...
  var getusers = function (req, res, next) {
    ...
    if (req.query.ids) {
      ...
      return users.publicList(req.domain, { lower_username: req.query.username.toLowerCase() }, sendUsers);
    } else if (req.query.channel) {
      return users.channelLeaders(req.domain, req.query.channel, req.query.idsOnly, sendUsers);
    } else {
      return res.json(400, { message: 'must provide ids or username for users to get' });
    }
  };
  ...
  var getuser = function (req, res) {
    return users.findUser(req.domain, { _id: req.user_id }, function (err, user) {
      ...
      if (err) {
        ...
        if (!user) {
          return res.json(404, { message: 'user doesnt exist' });
        } else {
          json_user = user.toJSON();
          json_user.votes = user.getVotes();
          delete json_user.password;
          return images.count({ owner: user._id }, req.domain.intercept(function (imagesCount) {
  ...
  var putuser = function (req, res) {
    return users.findUser(req.domain, { _id: req.user_id }, function (err, user) {
      var data;
      if (err) {
        return res.json(err.code, { message: err.message });
      } else {
        if (user.permission_level !== 0) {
          return res.json(403, { message: 'you are already registered' });
        } else {
          if (req.body.email == null) {
            return res.json(400, { message: 'must provide an email to register with' });
          } else {
            if (req.body.username == null) {
              return res.json(400, { message: 'must provide a username to register with' });
            } else {
              if (req.body.password == null) {
                return res.json(400, { message: 'must provide a password to register with' });
              } else {
  ...
  var postvote = function (req, res) {
    if (req.body.runnable == null) {
      return res.json(400, { message: 'must include runnable to vote on' });
    } else {
      ...
      if (err) {
        ...
        return user.removeVote(req.domain, req.params.voteid, function (err) {
          if (err) {
            ...
            return res.json({ message: 'removed vote' });
          }
        });
      }
    });
  ...
  var postrunnable = function (req, res) {
    if (req.query.from == null) {
      return res.json(400, { message: 'must provide a runnable to fork from' });
    } else {
  ...
  var getrunnables = function (req, res) {
    ...
    return runnables.listContainers(req.domain, req.user_id, query, function (err, containers) {
      if (err) {
        return res.json(err.code, { message: err.msg });
      } else {
  ...
  var getrunnable = function (req, res) {
    return runnables.getContainer(req.domain, req.user_id, req.params.runnableid, function (err, container) {
      if (err) {
        return res.json(err.code || 500, { message: err.msg || err.message });
      } else {
  ...
  var createfile = function (req, res, next) {
    ...
    if (contentType === 'application/json') {
      if (req.body.dir) {
        if (req.body.name == null) {
          return res.json(400, { message: 'dir must include a name field' });
        } else {
          if (req.body.path == null) {
            return res.json(400, { message: 'dir must include a path field' });
          } else {
        ...
        if (req.body.name == null) {
          return res.json(400, { message: 'file must include a name field' });
        } else {
          if (req.body.content == null) {
            return res.json(400, { message: 'file must include a content field' });
          } else {
            if (req.body.path == null) {
              return res.json(400, { message: 'file must include a path field' });
            } else {
      ...
      if (/multipart\/form-data/.test(contentType)) {
        ...
        return res.json(400, { message: 'content type must be application/json or multipart/form-data' });
      }
    }
  };
      ...
      return form.parse(req, function (err, fields, files) {
        ...
        return runnables.readFile(req.domain, req.user_id, req.params.id, req.params.fileid, function (err, root) {
          if (err) {
            ...
            if (!root.dir) {
              return res.json(403, { message: 'resource is not of directory type' });
            } else {
              return async.mapSeries(files_array, function (file, cb) {
                var filestream;
                filestream = fs.createReadStream(file.path);
    ...
    if (contentType === 'application/json') {
      return async.waterfall([
        ...
        if (err) {
          ...
          if (!file) {
            return res.json(400, { message: 'must provide content, name, path or tag to update operation' });
          } else {
            return res.json(file);
          }
        }
  ...
  var writemountedfiles = function (req, res) {
    return res.json(403, { message: 'mounted file-system is read-only' });
  };
