...
var mailchimpApi = configs.mailchimp != null ?
  new mailchimp.MailChimpAPI(configs.mailchimp.key) : void 0;
...
if (mailchimpApi != null) {
  Object.keys(configs.mailchimp.lists).forEach(function (list) {
    app.post('/campaigns/' + list, function (req, res) {
      ...
      mailchimpApi.listSubscribe(opts, req.domain.intercept(function () {
        res.json(201, req.body);
      }));
    });
  });
}
app.post('/request/improve', function (req, res) {
  ...
  requestEmail.send(function (err) {
    if (!err) {
      ...
      console.error('ERROR sending new request mail', err, requestEmailBody);
