...
module.exports = function (baseUrl) {
  ...
  // FIXME: files will be drastically changed or no longer exist
...
// TODO: updateContainer needs rework, especially the commit interactions with harbourmaster..
  var updateContainer =
    series(
      ...
      params.decodeId('containerId'),
      or(me.isUser, me.isModerator),
      utils.ternary(body.contains('status', 'Committing'),
        containers.findById('params.containerId'), // include files if committing
        containers.findById('params.containerId', { files: 0 })),
      containers.checkFound,
      or(me.isOwnerOf('container'), me.isModerator),
      ...
      containers.model.unset('files'), // dont return files
      containers.respond);
  ...
  app.delete(path.join(baseUrl, ':containerId'),
    params.isObjectId64('containerId'),
    params.decodeId('containerId'),
    or(me.isUser, me.isModerator),
  ...
  var getContainerForStartStop = series(
    ...
    containers.checkFound,
    mw.query().ifExists('servicesToken')
      .then(queryAndContainerTokensMatch)
      .else(
        or(me.isOwnerOf('container'), me.isModerator)
      )
  );
...
function queryAndContainerTokensMatch (req, res, next) {
  if (req.query.servicesToken &&
      req.container.servicesToken !== req.query.servicesToken) {
    next(error(403, 'access denied (token)'));
  }
  else {
    next();
  }
