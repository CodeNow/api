...
// TODO: updateContainer needs rework, especially the commit interactions with harbourmaster..
  var updateContainer =
    series(
      ...
      utils.if(body.contains('status', 'Committing'),
        body.trim('status'),
        containers.model.atomicUpdateCommitStatusAndName('body'),
        body.unset('status'), // to prevent nonatomic set below
        utils.ternary(containers.checkFound, // atomic update uses findAndModify
          harbourmaster.commitContainer,              // container updated successfull
          containers.findById('params.containerId', { files: 0 }))),// refetch container
  ...
  app.del(path.join(baseUrl, ':containerId'),
    ...
    params.decodeId('containerId'),
    or(me.isUser, me.isModerator),
    containers.findById('params.containerId', { _id: 1 }),
    containers.checkFound,
    or(me.isOwnerOf('container'), me.isModerator),
