...
app.post('/github',
  ...
  function (req, res) {
      ...
      function createContainerFromImageAndSync (cb) {
        ...
        async.waterfall([
          ...
          function createContainerViaHarbourmaster (container, waterfall_cb) {
            console.log('harbourmaster');
            req.container = container;
            req.image     = image;
            console.log(image.revisions[0], container.servicesToken);
            harbourmaster.createContainer(req.domain,
              {Image: 'registry.runnable.com/runnable/' + image.revisions[0].repo, servicesToken: container.servicesToken},
              function harbourmasterCreateContainerCB(err, response) {
              console.log('wtf is this err?', err);
              waterfall_cb(err);
            });
          },
          function syncFiles (waterfall_cb) {
            console.log('syncing files');
            files.sync(req, res, function (err) {
    ...
    ], req.domain.intercept(function (err) {
      console.log('annnnnd done.');
