...
app.post('/github',
  ...
  function (req, res) {
    ...
    async.series([
      ...
      function addTag (cb) {
        Channel.find({name: req.query.stack}, function (err, channel) {
          if (err) {
            cb(err);
          } else {
            var channelId = channel._id || channel;
            image.tags.push({ channel: channelId });
            var tag = _.last(image.tags).toJSON();
            var query = {
              _id: image._id,
            ...
            Image.findOneAndUpdate(query, update, function (err, updatedImage) {
              if (err) {
                return cb(err);
              }
              if (!updatedImage) {
                return cb(error(400, 'image already tagged with ' + tag.name));
              }
              cb();
            });
          }
