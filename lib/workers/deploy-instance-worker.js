 ...
 *
 *
 * @module lib/workers/deploy-instance-worker
 */
'use strict';
...
DeployInstanceWorker.prototype.handle = function (done) {
  ...
  if (this.instanceId) {
    ...
    instanceQuery['build._id'] = this.buildId;
  } else {
      ...
      return self.pFindBuild({
        ...
        .then(function (cv) {
          return self._filterAndSaveCvToInstances(instances, cv)
            .then(function () {
              return self._getDockHost(cv);
            })
            .then(function (dockerHost) {
              return self._enqueueCreateContainerWorkers(instances, cv, dockerHost);
            });
        });
    })
...
DeployInstanceWorker.prototype._findInstances = function (query) {
  log.trace(this.logData, 'DeployInstanceWorker.prototype._findInstances');
  var self = this;
...
DeployInstanceWorker.prototype._updateInstance = function (instance, updateQuery) {
  log.trace(this.logData, 'DeployInstanceWorker.prototype._updateInstance');
  var self = this;
  return new Promise(function (resolve, reject) {
    instance.update({
      ...
      if (err) {
        self.logError(err, 'DeployInstanceWorker _updateInstance failed');
        return reject(err);
      } else {
...
DeployInstanceWorker.prototype._filterAndSaveCvToInstances = function (instances, cv) {
  ...
  if (!self.instances.length) {
    return new AcceptableError('No instances were found to deploy');
  }
  return Promise.all(self.instances.map(function (instance) {
...
DeployInstanceWorker.prototype._getDockHost = function (cv) {
  log.trace(this.logData, 'DeployInstanceWorker.prototype._getDockHost' + cv);
  var self = this;
  return new Promise(function (resolve, reject) {
    if (self.forceDock) {
      return resolve(self.forceDock);
    }
    var mavis = new Mavis();
    mavis.findDockForContainer(cv, function (err, dockerHost) {
      if (err) {
        return reject(err);
      } else {
...
DeployInstanceWorker.prototype._enqueueCreateContainerWorkers =
  ...
  instances.forEach(function (instance) {
    ...
    var createContainerJobData = {
      ...
      buildId: keypather.get(instance, 'build._id.toString()'),
...
DeployInstanceWorker.prototype._emitEvents = function (instances) {
  ...
  var self = this;
  return new Promise(function (resolve, reject) {
    self.pFindUser(self.sessionUserGithubId, function (err) {
      if (err) {
        return reject(err);
      } else {
        return resolve();
    ...
    .then(function () {
      return Promise.all(instances.map(function (instance) {
        return self.pUpdateInstanceFrontend({ '_id': instance._id }, 'deploy');
      }));
    });
};
