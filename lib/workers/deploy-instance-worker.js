...
DeployInstanceWorker.prototype._getDockHost = Promise.method(function (cv) {
  log.trace(this.logData, 'DeployInstanceWorker.prototype._getDockHost' + cv);
  if (this.forceDock) {
    return this.forceDock;
  }
  var mavis = new Mavis();
  return Promise.promisify(mavis.findDockForContainer)(cv);
});
...
DeployInstanceWorker.prototype._emitEvents = function (instances) {
  ...
  var self = this;
  return self.pFindUser(self.sessionUserGithubId)
    .then(function () {
      return Promise.all(instances.map(function (instance) {
