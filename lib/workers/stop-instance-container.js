...
StopInstanceContainerWorker.prototype.handle = function (done) {
  ...
  async.series([
    this._baseWorkerFindInstance.bind(this, {
      ...
      'container.dockerContainer': this.data.dockerContainerId
    }),
    ...
    sauron.detachHostFromContainer.bind(sauron,
      this.data.networkIp, this.data.hostIp, this.data.dockerContainerId)
  ], function (err) {
...
StopInstanceContainerWorker.prototype._stopContainer = function (stopContainerCb) {
  ...
  async.retry({
    ...
    self.docker.stopContainer(self.data.dockerContainerId, function (err) {
      attemptCount++;
      if (err) {
        log.warn(put({
          err: err,
