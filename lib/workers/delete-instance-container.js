...
DeleteInstanceContainerWorker.prototype.handle = function(done) {
  ...
  this._findGitHubUsername(data.sessionUserId, instanceOwnerGithubId, function(err, instanceOwnerUsername) {
    if (err) {
    ...
    async.series([
      sauron.detachHostFromContainer.bind(sauron, networkIp, hostIp, containerId),
