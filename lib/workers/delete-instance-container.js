...
DeleteInstanceContainerWorker.prototype.handle = function (done) {
  ...
  var dockerContainer = data.dockerContainer;
  var dockerHost = container.dockerHost;
  var networkIp = data.networkIp;
  ...
  this._findGitHubUsername(data.sessionUserId, instanceOwnerGithubId,
    function (err, instanceOwnerUsername) {
      ...
      async.series([
        sauron.detachHostFromContainer.bind(sauron, networkIp, hostIp, dockerContainer),
        hosts.removeHostsForInstance.bind(hosts, instanceOwnerUsername,
          data.instanceMasterBranch, data.instanceShortHash, instanceOwnerGithubId,
          data.instanceMasterPod, data.instanceName, container),
