...
function InstanceContainerCreatedWorker (job) {
  ...
  return joi.validateOrBoomAsync(job, schema)
      ...
      return Promise.fromCallback(function (cb) {
        ...
        .catch(function (err) {
          ...
          if (statusCode === 409) {
            ...
            return docker.removeContainerAsync(job.id)
              .catch(function (err) {
                if (keypather.get(err, 'output.statusCode') < 500) {
                  throw new TaskFatalError('instance.container.created', 'Failed to delete container', { job: job, err: err })
                }
                throw err
              })
              .then(function () {
    ...
    .then(function (instance) {
      ...
      var tid = labels.tid || uuid.v4()
      return InstanceService.startInstance(instance, labels.sessionUserGithubId, tid)
    })
}
