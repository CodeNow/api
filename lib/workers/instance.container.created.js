...
module.exports = class InstanceContainerCreatedWorker {
  constructor (job) {
    this.log = logger.child({ method: 'InstanceContainerCreatedWorker' })
    this.job = job
   ...
   */
  static task (job) {
    const worker = new InstanceContainerCreatedWorker(job)
    return worker.run()
  }
  ...
  _findAndSetCreatingInstance () {
    ...
    .catch(Instance.NotFoundError, this._publishRemoveAndStopWorker)
    .bind(this)
  }
   ...
   */
  _publishRemoveAndStopWorker (err) {
    rabbitMQ.publishContainerRemove({
