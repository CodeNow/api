...
module.exports.worker = function (data, done) {
  ...
  workerDomain.run(function () {
    log.info(put({
      tx: true
    }, data), 'hermes.subscribe on-instance-container-start-worker start');
...
function OnInstanceContainerStartWorker (data) {
  log.info('OnInstanceContainerStartWorker constructor');
  var labels = keypather.get(data, 'inspectData.Config.Labels');
  this.data = data;
  this.data.ports = data.inspectData.NetworkSettings.Ports;
...
OnInstanceContainerStartWorker.prototype.handle = function (done) {
  log.info(this.logData, 'OnInstanceContainerStartWorker.prototype.handle');
  var self = this;
  ...
  async.series([
    ...
    if (err) {
      ...
      log.info(self.logData, 'OnInstanceContainerStartWorker.prototype.handle final success');
    }
...
OnInstanceContainerStartWorker.prototype._attachContainerToNetwork = function (cb) {
  log.info(this.logData, 'OnInstanceContainerStartWorker.prototype._attachContainerToNetwork');
  ...
  async.series([
    ...
    hosts.upsertHostsForInstance.bind(hosts, self.ownerUsername, self.instance, self.instance.name, self.data)
  ], function (err) {
    if (err) {
      log.warn(
        put({
...
OnInstanceContainerStartWorker.prototype._updateInstance = function (updateInstanceCb) {
  var self = this;
  log.info(this.logData, 'OnInstanceContainerStartWorker.prototype._updateInstance');
