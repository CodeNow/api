...
module.exports.task = (job) => {
  ...
  .tap(function checkIfFirstDockCreated (org) {
    log.info('Org result', { org })
    if (org.firstDockCreated) {
      throw new WorkerStopError('firstDockCreated was set before', {}, { level: 'info' }
      )
    }
  })
  .tap(function (org) {
    ...
    return dockerClient.createContainerAsync({
      ...
      .catch((err) => {
        log.error({err}, 'Error when spinning up first container')
      })
  })
  ...
  .then(function () {
    ...
    .catch(function (err) {
      log.error({ err }, 'failed to notify')
      throw new WorkerStopError('Failed to create job or send websocket event', { originalError: err })
    })
  })
  .catch(function (err) {
    if (~err.message.indexOf('Organization not found')) {
      log.warn({ err }, 'Organization not found')
      throw new WorkerStopError('Organization not found', { originalError: err })
    }
