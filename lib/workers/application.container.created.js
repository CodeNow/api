...
class Worker {
  ...
  _findAndSetCreatingInstance () {
    ...
    .bind(this)
    .catch(Instance.NotFoundError, this._cleanupContainer.bind(this))
  }
   ...
   */
  _cleanupContainer (err) {
    this.log.trace({ err }, '_cleanupContainer called')
    const containerId = this.containerId
    console.log('hello', containerId, this.containerId)
    return Promise.try(() => {
      if (containerId) {
        rabbitMQ.deleteInstanceContainer({ containerId })
      }
    })
  }
  ...
  _startInstance (instance) {
    ...
    return User.findByGithubIdAsync(this.sessionUserGithubId)
      ...
      .then((user) => {
        return InstanceService.startInstance(this.instanceId, user)
      })
  }
}
