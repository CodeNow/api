...
function startInstanceContainerWorker (data, callback, done) {
  ...
  var instance;
  var user;
  log.info(logData, 'startContainerWorker');
  ...
  async.series([
    ...
    if (err) {
      ...
      if (instance) {
        instance.populateOwnerAndCreatedBy(user, function () {
        });
  ...
  function updateFrontend (eventName) {
    ...
    Instance.findById(data.instanceId, function (err, instance) {
      if (err) {
        ...
        log.error(put({
          err: err
        }, logData), 'updateFrontend fetchInstance instance not found');
        return;
      }
  ...
  function startContainer (startContainerCb) {
    ...
    async.retry({
      ...
      docker.startUserContainer(data.dockerContainer, data.ownerGithubId, function (err) {
        if (err) {
          log.warn(put({
            err: err,
  ...
  function inspectContainerAndUpdate (inspectContainerAndUpdateCb) {
    ...
    async.retry({
      ...
      if (err) {
        ...
        instance.modifyContainerInspect(data.dockerContainer, result, function (err2) {
          if (err2) {
            log.warn(put({
              err: err2
            }, logData), 'startInstanceContainerWorker inspectContainer '+
