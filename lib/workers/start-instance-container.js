...
StartInstanceContainerWorker.prototype._startContainer = function (startContainerCb) {
  ...
  }, this.data.dockerContainer, this.data.sessionUserGithubId, function (err) {
    if (err) {
      log.warn(put({
        err: err,
...
._inspectContainerAndUpdate = function (inspectContainerAndUpdateCb) {
  ...
  var self = this;
  this.docker.inspectContainerWithRetry({
    times: process.env.WORKER_INSPECT_CONTAINER_NUMBER_RETRY_ATTEMPTS
  }, this.data.dockerContainer, function (err, result) {
    if (err) {
      log.warn(put({
        err: err,
      }, self.logData), 'inspectContainerAndUpdate: inspectContainer final error');
      self.instance.modifyContainerInspectErr(self.data.dockerContainer, err, function (err2) {
        if (err2) {
          log.warn(put({
            ...
            'final error updateInspectError error');
        }
      ...
      log.trace(self.logData,
        'inspectContainerAndUpdate: inspectContainer final success');
