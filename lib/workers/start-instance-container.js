...
function startInstanceContainerWorker (data, callback, done) {
  ...
  function findInstance (findInstanceCb) {
    ...
    Instance.findOne({
      ...
      if (err) {
        log.warn(put({
          err: err
        }, logData), 'findInstance findOne error');
        return findInstanceCb(err);
      }
      else if (!result) {
        log.warn(logData, 'findInstance not found');
      ...
      }, logData), 'findInstance findOne success');
      instance = result;
  ...
  function startContainer (startContainerCb) {
    log.info(logData, 'startContainer');
    var attemptCount = 1;
    async.retry({
      ...
      docker.startUserContainer(data.dockerContainer, data.sessionUserGithubId, function (err) {
        if (err) {
          ...
          }, logData), 'startContainer attempt failure');

          if (err.reason === 'container already started') {
            log.warn(put({
              err: err,
              attemptCount: attemptCount
            }, logData), 'startContainer docker container already started');
            return cb(null);
          }
          ...
          log.trace(put({
            attemptCount: attemptCount
          }, logData), 'startContainer success');
        }
      ...
      if (err) {
        log.warn(put({
          err: err,
          attemptCount: attemptCount
        }, logData), 'startContainer final failure');
      }
      else {
        log.trace(put({
          attemptCount: attemptCount
        }, logData), 'startContainer final success');
      }
      instance.removeStartingStoppingStates(function (err2) {
        if (err2) {
          log.warn(put({
            err: err2,
            attemptCount: attemptCount
          }, logData), 'startContainer final removeStartingStoppingStates failure');
        }
        else {
          log.trace(logData, 'startContainer final removeStartingStoppingStates success');
        }
  ...
  function inspectContainerAndUpdate (inspectContainerAndUpdateCb) {
    log.info(logData, 'inspectContainerAndUpdate');
    var attemptCount = 1;
    async.retry({
      ...
      docker.inspectContainer(data.dockerContainer, function (err, result) {
        if (err) {
          log.warn(put({
            err: err,
            attemptCount: attemptCount
          }, logData), 'inspectContainerAndUpdate inspectContainer error');
          attemptCount++;
          return cb(err);
        }
        log.trace(put({
          inspect: result
        }, logData), 'inspectContainerAndUpdate inspectContainer success');
        cb(null, result);
      });
    }, function (err, result) {
      if (err) {
        log.warn(put({
          err: err,
          attemptCount: attemptCount
        }, logData), 'inspectContainerAndUpdate inspectContainer async.retry final error');
        instance.modifyContainerInspectErr(data.dockerContainer, err, function (err2) {
          if (err2) {
            log.warn(put({
              err: err2
            }, logData), 'inspectContainerAndUpdate inspectContainer '+
        ...
        log.trace(put({
          attemptCount: attemptCount
        }, logData), 'inspectContainerAndUpdate inspectContainer async.retry final success');
        instance.modifyContainerInspect(data.dockerContainer, result, function (err2, _instance) {
          if (err2) {
            log.warn(put({
              err: err2
            }, logData), 'inspectContainerAndUpdate modifyContainerInspect '+
              'async.retry final error updateInspectError error');
          }
  ...
  function attachContainerToNetwork (attachContainerToNetworkCb) {
    log.info(logData, 'attachContainerToNetwork');
    ...
    async.series([
      ...
      if (err) {
        log.warn(put({
          err: err
        }, logData), 'attachContainerToNetwork async.series error');
      }
      else {
        log.trace(logData, 'attachContainerToNetwork async.series success');
      }
