...
startInstanceContainerWorker.prototype.handle = function (data, done) {
  ...
  async.series([
    ...
    this._inspectContainerAndUpdate,
    this._attachContainerToNetwork
  ], function (err) {
    if (err) {
      log.warn(put({
        err: err
      ...
      if (self.instance) {
        self._updateFrontend('start-error');
      }
    }
    else {
      ...
      if (err) {
        ...
        if (err.reason === 'container already started') {
          log.warn(put({
            err: err,
            attemptCount: attemptCount
          }, self.logData), 'startContainer docker container already started');
          return cb(null);
        }
        attemptCount++;
      }
 ...
 */
startInstanceContainerWorker.prototype
.attachContainerToNetwork = function (attachContainerToNetworkCb) {
    log.info(this.logData, 'startInstanceContainerWorker.prototype.attachContainerToNetwork');
