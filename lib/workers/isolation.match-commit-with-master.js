...
function MatchCommitWithIsolationGroupMaster (job) {
  ...
  return joi.validateOrBoomAsync(job, schema)
    ...
    .spread(function (childInstancesToUpdate, commitHash) {
      log.trace({
      ...
      // return Promise.map(childInstancesToUpdate, function (instance) {
        // // originalInstance, updates, sessionUserGithubId
        // return InstanceService.updateInstanceBuild(instance, { build: newBuildId  }, job.sessionUserGithubId)
      // })
    })
    .catch(function (err) {
      log.trace({
