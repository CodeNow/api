...
function updateInstanceCommitToNewCommit (commit, sessionUser, instance) {
  ...
  return ContextVersion.findByIdAsync(instance.contextVersion._id)
    .then(function (contextVersion) {
      ...
      return BuildService.createNewContextVersion(instance, pushInfo, 'autodeploy')
    })
    .then(function newContextVersion (newCV) {
      log.trace({
        newCV: newCV
      }, 'New contextVersion created. Creating new build. updateInstanceCommitToNewHash')
      var runnableClient = new Runnable({}, sessionUser)
      return Promise.fromCallback(function (callback) {
        var newBuildPayload = {
          contextVersions: [ newCV._id.toString() ],
          owner: { github: sessionUser.accounts.github.id }
        }
        log.trace({ body: newBuildPayload }, 'calling createBuild updateInstanceCommitToNewHash')
        runnableClient.createBuild({ json: newBuildPayload }, callback)
      })
    })
    .then(function buildNewBuild (newBuild) {
      log.trace({
        newBuild: newBuild
      }, 'createNewBuild updateInstanceCommitToNewHash')
      var message = 'Commit changed in isolation master'
      var runnableClient = new Runnable({}, sessionUser)
      return Promise.fromCallback(function (callback) {
        var buildBuildPayload = { message: message }
        log.trace({ body: buildBuildPayload }, 'building our new build updateInstanceCommitToNewHash')
        runnableClient.buildBuild(newBuild, { json: buildBuildPayload }, callback)
      })
    })
