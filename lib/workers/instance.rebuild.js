...
function InstanceRebuildWorker (job) {
  var log = logger.child({ method: 'InstanceRebuildWorker' })
  ...
  return workerUtils.validateJob(job, schema)
    ...
    .then(function (instance) {
      log.info('findUser and login to Runnable API client')
    ...
    .then(function (data) {
      ...
      log.info({ build: data.build }, 'build deep copied. building')
    ...
    .then(function (data) {
      log.info({ build: data.build }, 'build was build. Update instance')
