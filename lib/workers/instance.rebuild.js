...
function InstanceRebuildWorker (job) {
  ...
  return joi.validateOrBoomAsync(job, schema)
    ...
    .then(function (data) {
      ...
      return Promise.fromCallback(function (cb) {
        var buildId = data.build.attrs._id
        log.info({ buildId: buidId }, 'InstanceRebuildWorker goting to build a buil')
        ...
        log.info({build: build}, 'InstanceRebuildWorker build build')
        return {
          build: build
        }
      })
    ...
    .then(function (data) {
      ...
      return Promise.fromCallback(function (cb) {
        var buildId = data.build.attrs._id
        log.info({ buildId: buidId }, 'InstanceRebuildWorker patch instance with a build')
        var instanceModel = runnableClient.newInstance(job.instanceShortHash)
