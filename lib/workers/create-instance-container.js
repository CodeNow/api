...
function createInstanceContainer (job) {
   ...
   * @return {Promise} Resolves if we can recover; fatally rejects if we cannot.
   */
  function errorHandler (err) {
    return Promise.resolve()
      .then(function () {
        ...
        log.error(logData, 'createInstanceContainer errorHandler')
        if (error.is4XX(err)) {
          ...
          return Promise.fromCallback(function (cb) {
            ...
            .then(function (ctxVersion) {
              ...
              if (completed && moment(completed) < moment().subtract(2, 'minutes')) {
                ...
                var twoMinuteBuildError = new Error('Publishing instance rebuild. More than two minutes have elapsed since last build.')
                error.log(twoMinuteBuildError)
