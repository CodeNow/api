    ...
    ContextVersion.findById(data.cvId, function (err, contextVersion) {
      ...
      docker.createUserContainer(contextVersion, {
        ...
        if (err) {
          ...
          if (statusCode === 404) {
            // no need to retry. application error
            // retry only if error happened in _handle404 itself
            self._handle404(contextVersion, data, err, cb);
            ...
            // no need to retry. application error
            // retry only if error happened in _handle404 itself
            self._handleAppError(data.instanceId, data.cvId, err, cb);
          }
        }
        else {
...
Worker.prototype._handleAppError = function (instanceId, cvId, err, cb) {
