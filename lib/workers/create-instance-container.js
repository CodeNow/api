  ...
  ContextVersion.findById(data.cvId, function (err, contextVersion) {
    ...
    docker.createUserContainer(contextVersion, {
      ...
      if (err) {
        ...
        if (statusCode === 404) {
          handle404(contextVersion, data, err, done);
        } else if (statusCode === 504) {
          // timeout we need to retry
          console.log('done');
        } else {
          // some real failure. not timeout
          modifyInstanceWithError(data.instanceId, data.cvId, err, done);
        }
      }
      else {
        log.info(put({
...
function handle404 (contextVersion, data, err, cb) {
  findUserAndInstance(data.userId, data.instanceId, function (err, res) {
    if (err) {
      return cb(err);
    }
...
function modifyInstanceWithError (instanceId, cvId, err, cb) {
...
function findUserAndInstance (userId, instanceId, cb) {
  async.parallel({
