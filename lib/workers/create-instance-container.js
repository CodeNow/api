...
CreateInstanceContainerWorker.prototype._handleAppError = function (instanceId, cvId, appErr, cb) {
  ...
  Instance.findById(instanceId, function (err, instance) {
    ...
    instance.modifyContainerCreateErr(cvId, appErr, function (err2) {
      if (!err2) {
        // update frontend to show build as failed
        return self._baseWorkerUpdateInstanceFrontend(
          self.data.instanceId, self.data.sessionUserGithubId, 'update', cb);
      }
      cb();
    });
  });
