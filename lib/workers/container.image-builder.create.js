 ...
 *   4. Populate the infra-code verison information
...
function ContainerImageBuilderCreate (job) {
  ...
  return joi.validateOrBoomAsync(job, schema)
    ...
    .then(function fetchRequiredModels () {
      return Promise.join(
        function getUser () {
          log.debug('ContainerImageBuilderCreate: Fetching User')
          return User.findByGithubIdAsync(job.sessionUserGithubId)
        },
        function getContext () {
          log.debug('ContainerImageBuilderCreate: Fetching Context')
          return Context.findOneAsync(job.contextId)
        },
        function getContextVersion () {
          ...
          log.debug(
            { query: query },
            'ContainerImageBuilderCreate: Fetching ContextVersion'
          )
          return ContextVersion.findOneAsync(query)
        }
      )
    })
    .spread(function populateICV (user, context, contextVersion) {
      log.debug('ContainerImageBuilderCreate: Populating InfraCodeVersion')
      contextVersion.populateAsync('infraCodeVersion')
        .then(function createImageBuilderContainer () {
          return Promise
            .fromCallback(function createImageBuilderContainer (cb) {
              log.debug('ContainerImageBuilderCreate: Creating Container')
              new Docker().createImageBuilder({
        ...
        .then(function updateContextVersionWithContainer (container) {
          log.debug('ContainerImageBuilderCreate: Setting CV Build Container')
        ...
        .then(function markContextVersionAsRecovered () {
          log.debug('ContainerImageBuilderCreate: Marking CV as Recovered')
