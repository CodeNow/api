...
module.exports.task = (job) => {
  ...
  return ClusterBuildService.findActiveByIdAndState(job.clusterBuildId, 'created')
    .then((clusterBuild) => {
      return UserService.getCompleteUserByBigPoppaId(clusterBuild.createdByUser)
        .then(sessionUser => {
          const props = [ 'filePath', 'isTesting', 'clusterName', 'clusterCreateId', 'testReporters', 'parentInputClusterConfigId' ]
          const opts = pick(job, props)
          if (opts.parentInputClusterConfigId === '') {
            delete opts.parentInputClusterConfigId
          }
          opts.triggeredAction = clusterBuild.triggeredInfo.action
          opts.repoFullName = clusterBuild.triggeredInfo.repo
          opts.branchName = clusterBuild.triggeredInfo.branch
