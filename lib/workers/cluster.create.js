    ...
    .then(sessionUser => {
      ...
      return ClusterConfigService.create(sessionUser, opts)
        .then((inputClusterConfig) => {
          messenger.messageRoom('org', job.githubId, { task: 'compose-cluster-created', clusterName: job.clusterName, parentInputClusterConfigId: inputClusterConfig._id })
        })
        .catch(err => {
          log.error({ err }, 'Creating the cluster failed')
          messenger.messageRoom('org', job.githubId, { task: 'compose-cluster-created', err: err.message, clusterName: job.clusterName })
