...
OnInstanceContainerDie.prototype.handle = function (done) {
  ...
  if (!this.instanceId) {
    log.info(this.logData,
      'OnInstanceContainerDie.prototype.handle exit because instanceId is null')
    return done()
  }
  var self = this
  async.series([
    ...
    if (err) {
      ...
      log.error(newLogData, 'OnInstanceContainerDie.prototype.handle final error')
      error.workerErrorHandler(err, newLogData)
    } else {
      log.trace(self.logData, 'OnInstanceContainerDie.prototype.handle final success')
    }
    done()
  })
}
...
OnInstanceContainerDie.prototype._updateFrontend = function (cb) {
  if (this.instance) {
    log.info(this.logData, 'OnInstanceContainerDie.prototype._updateFrontend do update')
    this._baseWorkerUpdateInstanceFrontend(
      this.instanceId, this.sessionUserGithubId, 'update', cb)
  } else {
    log.info(this.logData, 'OnInstanceContainerDie.prototype._updateFrontend noop')
    cb()
  }
}
/**
...
OnInstanceContainerDie.prototype._updateInstance = function (cb) {
  ...
  instanceService.updateOnContainerDie(
    this.instance,
    this.containerId,
    this.inspectData,
    function (err) {
