...
OnImageBuilderContainerDie.prototype._emitInstanceUpdates = function (emitInstanceUpdatesCb) {
  ...
  Instance.findBy('build.dockerContainer', this.data.id, function (err, instances) {
    if (err) {
      log.error(put({
        err: err
      }, self.logData), '_emitInstanceUpdates: Instance.findBy error');
      return emitInstanceUpdatesCb(err);
    }
    else if (!instances) {
      log.warn(self.logData, '_emitInstanceUpdates: Instance.findBy !instances');
      return emitInstanceUpdatesCb();
    }
    else {
      log.trace(put({
        instances: instances.map(pluck('name'))
      }, self.logData), '_emitInstanceUpdates: Instance.findBy success');
