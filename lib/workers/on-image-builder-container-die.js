...
OnImageBuilderContainerDie.prototype._emitInstanceUpdates = function (emitInstanceUpdatesCb) {
  ...
  Instance.findBy('build.dockerContainer', this.data.id, function (err, instances) {
    if (err) {
      ...
      log.trace(self.logData, '_emitInstanceUpdates: Instance.findBy success');
      async.each(instances, function (instance, cb) {
        instance.populateOwnerAndCreatedBy(function (err2, instance) {
          if (err2) {
            log.error(put({
              err: err
            }, self.logData), '_emitInstanceUpdates populateOwnerAndCreatedBy error');
            return cb(err2);
          }
