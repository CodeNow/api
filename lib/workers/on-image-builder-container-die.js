...
OnImageBuilderContainerDie.prototype._deployInstance = function (buildId) {
  ...
  rabbitMQ.deployInstance({
    ...
    ownerUsername: Labels.ownerUsername || Labels.sessionUserUsername
  }, Labels);
};
  ...
  Build.find({
    ...
    async.each(builds, function (build, cb) {
      ...
      if (!buildInfo) {
        ...
        build.modifyCompleted(buildInfo.failed, function (err, count) {
          console.trace('modifyCompleted', arguments);
          if (err || !count) {
            log.error(put({
