  ...
  return ContextVersion.updateBuildCompletedByContainerAsync(job.id, buildInfo)
    .then(function updateInstances (versions) {
      ...
      var buildPromise = updateBuildByCVPromise.then(function (builds) {
        log.info(logData, '_handleBuildComplete: builds updated')
        log.info({ builds: builds }, '_handleBuildComplete: builds updated 2')
        if (!builds || !builds[0]) {
          return
        }
        ...
        return Promise.map(versions, function (cv) {
          log.info(logData, '_handleBuildComplete: process cv')
          ...
          log.info(logData, '_handleBuildComplete: is manual action')
...
OnImageBuilderContainerDie._onBuildsCompleted = function (builds) {
