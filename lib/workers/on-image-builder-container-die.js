...
OnImageBuilderContainerDie.prototype.handle = function (done) {
  ...
  async.series([
    ...
    self._removeContainer(err, done)
  })
}

/**
 * Logs error and removed build container regardless of error
 * @param {Object} err
 * @param {Function} done - sends ACK signal to rabbitMQ
 */
OnImageBuilderContainerDie.prototype._removeContainer = function (err, done) {
  log.info(this.logData, 'OnImageBuilderContainerDie.prototype._removeContainer')
  var self = this
  if (err) {
    log.warn(put({
      err: err
    }, self.logData), 'OnImageBuilderContainerDie.prototype._removeContainer final error')
  } else {
    log.trace(self.logData, 'OnImageBuilderContainerDie.prototype._removeContainer final success')
  }
  var docker = new Docker()
  // remove container, ignore errors khronos will cleanup missed containers
  docker.removeContainer(this.data.id, function (err) {
    if (err) {
      log.error(put({
        err: err
      }, self.logData), 'OnImageBuilderContainerDie.prototype._removeContainer removeContainer error')
    } else {
      log.trace(self.logData, 'OnImageBuilderContainerDie.prototype._removeContainer removeContainer success')
    }
    done()
  })
}
...
OnImageBuilderContainerDie.prototype._getBuildInfo = function (getBuildInfoCb) {
  ...
  docker.getBuildInfo(this.data.id, exitCode, function (err, buildInfo) {
    if (err) {
      ...
      log.trace(self.logData, '_getBuildInfo: docker.getBuildInfo success')
  ...
  ContextVersion.updateBuildCompletedByContainerAsync(self.data.id, buildInfo)
    .then(function updateInstances (versions) {
      ...
      return Promise.all([
        ...
        Promise.fromCallback(function updateBuidlsByContextVersions (cb) {
          if (buildInfo.failed) {
            Build.updateFailedByContextVersionIds(versionIds, cb)
          } else {
