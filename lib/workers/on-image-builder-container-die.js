...
OnImageBuilderContainerDie.prototype.handle = function (done) {
  ...
  async.series([
    ...
    this._baseWorkerFindGithubUser.bind(this, this.contextVersion.owner.github),
...
OnImageBuilderContainerDie.prototype._deployInstance = function (buildId) {
  ...
  var ownerUsername = keypather.get(this.user, 'accounts.github.username');

  if (!sessionUserGithubId || !ownerUsername) {
    throw error(400, 'missing sessionUserGithubId or ownerUsername');
  }
  var opts = {
    buildId: buildId,
  ...
  rabbitMQ.deployInstance(put(opts, this.data.inspectData.Config.Labels));
};
