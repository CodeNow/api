...
OnImageBuilderContainerDie.prototype._handleBuildSuccess = function (versionIds, cb) {
  ...
  async.parallel({
    // mark builds as completed
    updateBuilds:
      Build.updateCompletedByContextVersionIds
        .bind(Build, versionIds),
    // update instance w/ latest context version data, and emit
    instances:
      Instance.emitInstanceUpdates
        .bind(Instance, { 'contextVersion._id': { $in: versionIds } })
  }, function (err, results) {
    if (err) {
      log.error(
        put({ err: err }, self.logData),
        '_handleBuildSuccess: async.parallel final failure'
      );
      return cb(err);
    }
    log.trace(
      self.logData,
      '_handleBuildSuccess: async.parallel final success'
    );
    // create create-instance jobs
...
/**
 * update builds w/ versionIds to be completed
 * @param  {Function} cb [description]
 * @return {[type]}      [description]
 */
OnImageBuilderContainerDie.prototype._updateBuildsWithCompleted = function (versionIds, cb) {
  var self = this;
  Build.updateCompletedByContextVersionIds(versionIds, function (err) {
    if (err) {
      log.warn(
        put({ err: err }, self.logData),
        '_handleBuildSuccess: Build.updateCompletedByContextVersionIds failure'
      );
      return cb(err);
    }
    log.trace(
      self.logData,
      '_handleBuildSuccess: Build.updateCompletedByContextVersionIds success'
    );
    cb();
  });
};
...
OnImageBuilderContainerDie.prototype._deallocImageBuilderNetwork =
  function (deallocImageBuilderNetworkCb) {
    ...
    var self = this;
    var cv = this.contextVersions[0]; // any will work
    Sauron.deleteHostFromContextVersion(cv, function (err) {
      if (err) {
        log.error(put({
          err: err
