...
OnImageBuilderContainerDie._handleAutoDeploy = function (versions) {
  ...
  return Promise.map(versions, function (cv) {
        ...
        .tap(function (instances) {
          ...
          if (instances) {
            instances.forEach(function (instance) {
              ...
              if (manual) {
                // Only if it's manual because updateBuildByRepoAndBranch calls deployed in rabbit if its automatic
    ...
    .filter(exists)
    .then(function (instanceLists) {
      return instanceLists.reduce(function (memo, curr) {
        if (Array.isArray(curr)) {
          Array.prototype.push.apply(memo, curr)
        } else {
          log.warn('result was not an array')
        }
        return memo
      }, [])
    })
