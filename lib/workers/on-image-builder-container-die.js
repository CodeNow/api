...
OnImageBuilderContainerDie.prototype.handle = function (done) {
  ...
  var sessionUserGithubId =
        keypather.get(this.data, 'inspectData.Config.Labels.sessionUserGithubId');
  var self = this;
  async.series([
    ...
    function (cb) {
      self._findUser(sessionUserGithubId, cb);
    },
    function (cb) {
      self._updateInstanceFrontend({
...
OnImageBuilderContainerDie.prototype._deployInstance = function (buildId) {
  ...
  console.log('\n\n\n\n\n\_deployInstance _deployInstance\n', Labels);
  rabbitMQ.deployInstance({
    buildId: buildId,
    sessionUserGithubId: Labels.sessionUserGithubId || Labels.sessionUserId
  }, Labels);
};
...
OnImageBuilderContainerDie.prototype._findBuildAndEmitUpdate = function (buildInfo, findBuildCb) {
  log.info(this.logData, 'OnImageBuilderContainerDie.prototype._findBuild');
  var self = this;
  Build.find({
      contextVersions: self.contextVersion._id
    }, function (err, builds) {
    console.log('_findBuild: Build.findById', arguments);
    if (err) {
      log.error(put({
        err: err
      }, self.logData), '_findBuild: Build.findById error');
    }
    async.each(builds, function (build, cb) {
      console.log('\n\n\nBUILDS', build);
      if (!buildInfo) {
        build.modifyErrored(self.contextVersion._id, function () {
          cb();
        });
      } else {
        build.modifyCompleted(buildInfo.failed, function () {
          if (!buildInfo.failed) {
...
/**
 * @param {Function} emitInstanceUpdatesCb
 */
OnImageBuilderContainerDie.prototype._emitInstanceUpdates = function (emitInstanceUpdatesCb) {
  var self = this;
  log.info(this.logData, 'OnImageBuilderContainerDie.prototype._emitInstanceUpdates');
  Instance.find({
    'contextVersion.build.dockerContainer': this.data.id
  }, function (err, instances) {
    if (err) {
      log.error(put({
        err: err
      }, self.logData), '_emitInstanceUpdates: Instance.find error');
      return emitInstanceUpdatesCb(err);
    }
    else if (!instances) {
      log.warn(self.logData, '_emitInstanceUpdates: Instance.find !instances');
      return emitInstanceUpdatesCb();
    }
    else {
      log.trace(put({
        instances: instances.map(pluck('name'))
      }, self.logData), '_emitInstanceUpdates: Instance.find success');
      async.each(instances, function (instance, cb) {
        log.trace(self.logData, 'emit instance patch update '+instance._id.toString());
        instance.emitInstanceUpdate('patch', cb);
      }, emitInstanceUpdatesCb);
    }
  });
};
