...
OnImageBuilderContainerDie.prototype._getBuildInfo = function (getBuildInfoCb) {
  ...
  return docker.getBuildInfoAsync(this.data.id, exitCode)
    .then(function (buildInfo) {
      ...
      return self._handleBuildComplete(buildInfo)
    })
    .catch(function (err) {
      log.error(put({
        err: err,
      ...
      return self._handleBuildError(err)
    })
    .asCallback(getBuildInfoCb)
}
 ...
 * @return {Promise}
 */
OnImageBuilderContainerDie.prototype._handleBuildError = function (err) {
  var logData = put({ buildErr: err }, this.logData)
  log.info(logData, 'OnImageBuilderContainerDie.prototype._handleBuildError')
  return ContextVersion.updateBuildErrorByContainerAsync(this.data.id, err)
 ...
 * @return {Promise}
 */
OnImageBuilderContainerDie.prototype._handleBuildComplete = function (buildInfo) {
  var self = this
  ...
  return ContextVersion.updateBuildCompletedByContainerAsync(self.data.id, buildInfo)
