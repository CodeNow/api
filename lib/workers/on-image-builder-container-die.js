...
OnImageBuilderContainerDie._createContainersIfSuccessful = function (job, instances, buildInfo) {
  ...
  if (!buildInfo.failed) {
    instances.forEach(function (instance) {
      ...
      // Is Isolation group master and auto deployed
      if (!manual && instance.isIsolationGroupMaster) {
        instance.getParentAsync()
          .then(function (fullParentInstance) {
            return AutoIsolationConfig.findOneAsync({
              instance: fullParentInstance._id
            })
          })
          .then(function (autoIsolationConfig) {
            if (autoIsolationConfig.autoRestart) {
              OnImageBuilderContainerDie._restartAllIsolatedInstances(job, instance)
            }
          })

        // isolation has autoRestart configured
       OnImageBuilderContainerDie._restartAllIsolatedInstances(job, instance)
      }
...
OnImageBuilderContainerDie._restartAllIsolatedInstances = function (job, isolationGroupMasterInstance) {
  var tid = keypather.get(job, 'job.inspectData.Config.Labels.tid')
  tid = tid || uuid.v4()
  var log = logger.child({
    method: '_restartAllIsolatedInstances',
    job: OnImageBuilderContainerDie.getJobLogProperties(job),
    tid: tid
  })
  log.info('call')
  ...
  // Fetch all the child instances
  Instance.find({ isolation: objectId(isolationGroupMasterInstance.isolation), isolationGroupMaster: false })
