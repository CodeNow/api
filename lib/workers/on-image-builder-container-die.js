...
OnImageBuilderContainerDie.prototype.handle = function (done) {
  ...
  var sessionUserGithubId =
      ...
      keypather.get(this.data, 'inspectData.Config.Labels.sessionUserId');
  var self = this;
  async.series([
    ...
    this._baseWorkerUpdateInstanceFrontend.bind(this, {
      'contextVersion._id': this.contextVersion._id
    }, sessionUserGithubId, 'patch')
  ], function (err) {
...
OnImageBuilderContainerDie.prototype._findBuildAndEmitUpdate = function (buildInfo, findBuildCb) {
  log.info(this.logData, 'OnImageBuilderContainerDie.prototype._findBuildAndEmitUpdate');
  var self = this;
  Build.find({
    ...
    if (err) {
      log.error(put({
        err: err
      }, self.logData), '_findBuildAndEmitUpdate: Build.find error');
    }
      ...
      if (!buildInfo) {
        ...
        build.modifyCompleted(buildInfo.failed, function (err) {
          if (err || !build) {
            ...
            }, self.logData), '_findBuildAndEmitUpdate: Build.modifyCompleted error');
          }
          if (!buildInfo.failed) {
