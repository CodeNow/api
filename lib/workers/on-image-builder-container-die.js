...
function (buildInfo, handleBuildErrorCb) {
  ...
  ContextVersion.updateBuildErrorByContainer(this.data.id, err, function (err) {
    if (err) {
      ...
      ContextVersion.updateBuildCompletedByContainer(self.data.id, buildInfo, function (err2) {
        if (err2) {
          log.error(put({
            err: err
          }, self.logData),
            '_handleBuildError: contextVersion.updateBuildErrorByContainer '+
              'updateBuildCompletedByContainer error');
          handleBuildErrorCb(err);
        }
        else {
          log.error(self.logData,
            '_handleBuildError: contextVersion.updateBuildErrorByContainer '+
              'updateBuildCompletedByContainer success');
          handleBuildErrorCb();
        }
      });
    }
