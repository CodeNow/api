...
CreateImageBuilderContainerWorker.prototype.handle = function (done) {
  ...
  async.series([
    function (cb) {
      self._baseWorkerFindUser(self.sessionUserGithubId, cb);
    },
    ...
    function (cb) {
      self._baseWorkerUpdateContextVersionFrontend('build_started', cb);
    }
  ], function (err) {
    if (err) {
      log.error(put({
...
CreateImageBuilderContainerWorker.prototype._findContext = function (findContextCb) {
  log.trace(this.logData, 'CreateImageBuilderContainerWorker.prototype._findContext');
  var self = this;
  Context.findOne({
    '_id': self.contextId
  }, function (err, result) {
