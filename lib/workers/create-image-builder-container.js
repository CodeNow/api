...
/**
 * find context
 * @param {Function} findContextCb (err)
 */
 ...
 * @param {Function} findCreateCb (err)
 * @private
 */
 ...
 * @param {Function} createImageCb (err)
 */
 ...
 * @param {Object} error passed in error
 * @param {Function} onErrorCb (err)
 * @private
 */
CreateImageBuilderContainerWorker.prototype._onError = function (error, onErrorCb) {
  ...
  if (this.network) {
    this.sauron.deleteHost(self.network.networkIp, self.network.hostIp, function (err) {
      if (err) {
        log.error(put({
          err: err
