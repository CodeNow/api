...
module.exports.worker = function (data, done) {
  ...
  workerDomain.on('error', function (err) {
    ...
    // Make sure we update the build status on the cv
    if (worker.contextVersion) {
      return worker._onError(err, done)
    }
  ...
  workerDomain.run(function () {
    ...
    worker = new CreateImageBuilderContainerWorker(data)
    worker.handle(done)
  })
}
...
function CreateImageBuilderContainerWorker (data) {
  ...
  this.ownerGitHubUsername = data.ownerGitHubUsername
...
CreateImageBuilderContainerWorker.prototype._createImageBuilder = function (createImageCb) {
  ...
  docker.createImageBuilderWithRetry(
    ...
    },
    {
      ...
      ownerGitHubUsername: this.ownerGitHubUsername,
...
CreateImageBuilderContainerWorker.prototype._updateContextVersionWithContainer = function (upCvCb) {
  ...
  ContextVersion.updateContainerByBuildId({
    ...
    ContextVersion.recover(self.contextVersion._id, upCvCb)
  })
}
