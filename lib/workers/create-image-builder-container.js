...
CreateImageBuilderContainerWorker.prototype._updateContextVersionWithContainer = function (upCvCb) {
  ...
  ContextVersion.updateContainerByBuildId({
    ...
    self.contextVersion.handleRecovery(upCvCb)
  })
}
...
CreateImageBuilderContainerWorker.prototype._updateCvOnError = function (originalError, cb) {
  var self = this
  ContextVersion.updateBuildErrorByBuildId(self.contextVersion.build._id, originalError, function (err) {
    if (err) {
      log.error(
        put({
          err: err
