...
OnCreateStartImageBuilderContainerWorker.prototype.handle = function (data, done) {
  ...
  async.series([
    ...
    if (err) {
      ...
      log.info(self.logData, 'OnCreateStartImageBuilderContainerWorker.prototype.handle final success');
    }
    done();
  });
};
 ...
 */
OnCreateStartImageBuilderContainerWorker.prototype._updateContextVersionWithContainer = function (upCvCb) {
  log.info(this.logData, 'OnCreateStartImageBuilderContainerWorker.prototype._findContextVersion');
  var self = this;
...
OnCreateStartImageBuilderContainerWorker.prototype._updateFrontend = function (error, cb) {
  ...
  ContextVersion.updateBuildErrorByBuildId(self.contextVersion.build._id, error, function (err) {
    if (err) {
      log.warn(put({
        err: err
      }, self.logData), 'OnCreateStartImageBuilderContainerWorker updateBuildErrorByBuildId failed');
    } else {
