...
function DockRemovedWorker (job) {
  ...
  return joi.validateOrBoomAsync(job, schema)
    ...
    .then(DockRemovedWorker._stopStoppingInstances.bind(DockRemovedWorker, job))
    .then(DockRemovedWorker._redeployAndNotify.bind(DockRemovedWorker, job))
    .then(DockRemovedWorker._rebuildAndNotify.bind(DockRemovedWorker, job))
}
...
DockRemovedWorker._redeploy = function (job) {
  ...
  return Promise.fromCallback(function (cb) {
    Instance.findInstancesRunningOrStartingByDockerHost(job.host, function (err, instances) {
      if (err) {
        log.error(put({ err: err }, logData), '_redeploy failed to found instances to redeploy')
        return cb(err)
      }
