    ...
    .each(function (instance) {
      return PermissionService.checkOwnerAllowed(instance)
        .catch(errors.OrganizationNotAllowedError, function (err) {
          ...
          throw new TaskFatalError(queueName, 'Organization is not allowed, no need to redeploy', { originalError: err })
        })
        .catch(errors.OrganizationNotFoundError, function (err) {
          ...
          throw new TaskFatalError(queueName, 'Organization is not whitelisted, no need to redeploy', { originalError: err })
        })
        .then(function () {
    ...
    .each(function (instance) {
      return PermissionService.checkOwnerAllowed(instance)
        .catch(errors.OrganizationNotAllowedError, function (err) {
          ...
          throw new TaskFatalError(queueName, 'Organization is not allowed, no need to rebuild', { originalError: err })
        })
        .catch(errors.OrganizationNotFoundError, function (err) {
          log.error({ instance: instance }, 'Organization is not whitelisted, no need to rebuild')
          throw new TaskFatalError(queueName, 'Organization is not whitelisted, no need to rebuild', { originalError: err })
        })
        .then(function () {
