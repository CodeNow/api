    ...
    .each(function (instance) {
      return PermissionService.checkOwnerAllowed(instance)
        .catch(errors.OrganizationNotAllowedError, function (err) {
          log.info({ instance: instance }, 'Organization is not allowed, no need to redeploy')
        })
        .catch(errors.OrganizationNotFoundError, function (err) {
          log.info({ instance: instance }, 'Organization is not whietlized, no need to redeploy')
        })
        .then(function () {
  ...
  return Instance.findInstancesBuildingOnDockerHostAsync(job.host)
    .each(function (instance) {
      return PermissionService.checkOwnerAllowed(instance)
        ...
        .catch(errors.OrganizationNotFoundError, function (err) {
          log.info({ instance: instance }, 'Organization is not whietlized, no need to rebuild')
        })
        .then(function () {
