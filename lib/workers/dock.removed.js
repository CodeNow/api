...
function DockRemovedWorker (job) {
  ...
  return joi.validateOrBoomAsync(job, schema)
    ...
    // })
    .then(function () {
      log.trace(logData, 'DockRemovedWorker handle rebuild')
      return Promise.fromCallback(function (cb) {
        Instance.findInstancesBuildingOnDockerHost(job.host, function (err, instances) {
          if (err) {
            log.error(put({ err: err }, logData), 'DockRemovedWorker failed to found instances to rebuild')
            return cb(err)
          }
          log.trace(logData, 'DockRemovedWorker found instances to rebuild')
