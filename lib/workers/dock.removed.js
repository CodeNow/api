...
const checkInstance = function (instance) {
  ...
  return PermissionService.checkOwnerAllowed(instance)
    .catch(errors.OrganizationNotAllowedError, function (err) {
      log.error({ instance, err }, 'Organization is not allowed, no need to redeploy/rebuild')
    ...
    .catch(errors.OrganizationNotFoundError, function (err) {
      log.error({ instance, err }, 'Organization is not whitelisted, no need to redeploy/rebuild')
...
const updateFrontendInstances = function (instances) {
  ...
  log.info({ count: instances.length }, 'updateFrontendInstances called')
  return Promise.all(
    instances
