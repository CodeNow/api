...
function DockRemovedWorker (job) {
  ...
  return joi.validateOrBoomAsync(job, schema)
    ...
    .then(function () {
      // console.log('qqq11', DockRemovedWorker._redeploy())
      return DockRemovedWorker._redeploy(logData, job)
    })
    .then(function (instances) {
      // console.log('qqq22', DockRemovedWorker._updateFrontendInstances())
...
/**
 * Find all instances that should be redeployed and create new job for each of them
 */
DockRemovedWorker._redeploy = function (logData, job) {
  log.trace(logData, 'DockRemovedWorker._redeploy')
  return Promise.fromCallback(function (cb) {
...
DockRemovedWorker._rebuild = function (logData, job) {
  log.trace(logData, 'DockRemovedWorker._rebuild')
  return Promise.fromCallback(function (cb) {
    Instance.findInstancesBuildingOnDockerHost(job.host, function (err, instances) {
      if (err) {
