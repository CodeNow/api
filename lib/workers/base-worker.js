...
BaseWorker.prototype._pBaseWorkerFindInstances = function (query) {
  log.info(this.logData, 'BaseWorker.prototype._pFindInstances');
  var self = this;
...
BaseWorker.prototype._pBaseWorkerFindBuild = function (query) {
  ...
  return new Promise(function (resolve, reject) {
    Build.findOne(query, function (err, build) {
      ...
      log.info(self.logData, 'BaseWorker.prototype._pBaseWorkerFindBuild success');
      self.build = build;
      return build;
    })
    .catch(function (err) {
 ...
 * @param query Mongo query for fetching the instances
  ...
  async.retry({
    ...
    if (err) {
      ...
      self.instance.modifyContainerInspect(self.data.dockerContainer,
        ...
        log.info(self.logData, '_baseWorkerInspectContainerAndUpdate: modifyContainerInspect '+
