...
BaseWorker.prototype._updateFrontendWithContextVersion = function (event, cb) {
  ...
  this._findContextVersion({
    ...
    if (err) {
      log.error(put({
        err: err
      }, self.logData), 'OnCreateStartImageBuilderContainerWorker emitting update failed');
    } else if (contextVersion) {
      log.info(self.logData, 'OnCreateStartImageBuilderContainerWorker emitting update success');
 ...
 */
BaseWorker.prototype._findInstance = function (findInstanceCb) {
 ...
 */
BaseWorker.prototype._findUser = function (findUserCb) {
