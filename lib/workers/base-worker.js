...
function BaseWorker (data) {
  log.trace('BaseWorker constructor');
  this.data = data;
  this.logData = {
    tx: true,
...
BaseWorker.prototype._pBaseWorkerFindInstances = function (query) {
  log.trace(this.logData, 'BaseWorker.prototype._pFindInstances');
  var self = this;
  return new Promise(function (resolve, reject) {
    Instance.find(query, function (err, instances) {
      ...
      }, self.logData), 'BaseWorker: _pFindInstances success');
    ...
    .catch(function (err) {
      log.error(put({
        err: err
      }, self.logData), 'BaseWorker: _pFindInstances error');
      throw err;
    });
};
...
BaseWorker.prototype._pBaseWorkerFindBuild = function (query) {
  log.trace(this.logData, 'BaseWorker.prototype._pBaseWorkerFindBuild');
  var self = this;
  return new Promise(function (resolve, reject) {
...
BaseWorker.prototype._baseWorkerFindContextVersion = function (query, findCvCb) {
  log.trace(this.logData, 'BaseWorker.prototype._baseWorkerFindContextVersion');
  var self = this;
 ...
 * @param {String} query - query or instanceId (if string) of mongoose Instance model
 * @param {String} userGithubId - instance of mongoose user model
 * @param {String} eventName
 * @param {Function} cb
 */
BaseWorker.prototype._baseWorkerUpdateInstanceFrontend =
  function (query, userGithubId, eventName, cb) {
    log.trace(put({
      query: query,
...
BaseWorker.prototype._baseWorkerUpdateContextVersionFrontend = function (eventName, cb) {
  log.trace(this.logData, 'BaseWorker.prototype._baseWorkerUpdateContextVersionFrontend');
...
BaseWorker.prototype._baseWorkerValidateDieData = function (validateDieDataCb) {
  log.trace(this.logData, 'BaseWorker.prototype._baseWorkerValidateDieData');
  var requiredKeys = [
    'from',
    'host',
    'id',
...
BaseWorker.prototype._baseWorkerFindInstance = function (query, findInstanceCb) {
  log.trace(this.logData, 'BaseWorker.prototype._baseWorkerFindInstance');
  var self = this;
...
BaseWorker.prototype._baseWorkerFindUser = function (userGithubId, findUserCb) {
  log.trace(this.logData, 'BaseWorker.prototype._baseWorkerFindUser');
  var self = this;
...
._baseWorkerInspectContainerAndUpdate = function (inspectContainerAndUpdateCb) {
  log.trace(this.logData, 'BaseWorker.prototype._baseWorkerInspectContainerAndUpdate');
  var self = this;
  var attemptCount = 0;
  async.retry({
