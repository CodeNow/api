...
/**
 * find github user by Id. Id can be orgId or userId
 * @param githubId
 * @param {Function} findUserCb
 */
BaseWorker.prototype._baseWorkerFindGithubUser = function (githubId, findUserCb) {
  var logData = put({
    githubId: githubId
  }, this.logData);
  log.info(logData, 'BaseWorker.prototype._baseWorkerFindGithubUser');
  var self = this;
  var user = new User({
    accounts: {
      github: {
        accessToken: process.env.HELLO_RUNNABLE_GITHUB_TOKEN
      }
    }
  });
  user.findGithubUserByGithubId(githubId, function (err, result) {
    if (err) {
      log.warn(put({
        err: err
      }, logData), '_baseWorkerFindGithubUser: findGithubUserByGithubId error');
      return findUserCb(err);
    } else if (!result) {
      log.warn(logData, '_baseWorkerFindGithubUser: findGithubUserByGithubId not found');
      return findUserCb(new Error('user not found'));
    }
    log.trace(put({
      user: result
    }, logData), '_baseWorkerFindGithubUser: findGithubUserByGithubId success');
    self.githubUser = result;

    findUserCb(null, result);
  });
};
