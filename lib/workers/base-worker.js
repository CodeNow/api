...
function BaseWorker (data, logData) {
  // If this worker is in production, log everything
  if (!logData || envIs('production')) {
    logData = data || {};
  }
  log.info('BaseWorker');
  data = data || {};
...
BaseWorker.prototype.pFindInstance = Promise.promisify(BaseWorker.prototype._findInstance);
...
BaseWorker.prototype.pFindInstances = function (query) {
  log.info(this.logData, 'BaseWorker.prototype.pFindInstances');
  var self = this;
  return new Promise(function (resolve, reject) {
    ...
    .then(function (instances) {
      ...
      }, self.logData), 'BaseWorker: pFindInstances success');
    ...
    .catch(function (err) {
      log.error(put({
        err: err
      }, self.logData), 'BaseWorker: pFindInstances error');
      throw err;
    });
};
...
BaseWorker.prototype.pFindBuild = function (query) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    .then(function (build) {
      ...
      log.trace(self.logData, 'BaseWorker.prototype.pFindBuild success');
      self.build = build;
      return build;
    })
    .catch(function (err) {
...
BaseWorker.prototype._baseWorkerFindContextVersion = function (query, findCvCb) {
  log.info(this.logData, 'BaseWorker.prototype._baseWorkerFindContextVersion');
  var self = this;
...
BaseWorker.prototype._baseWorkerUpdateInstanceFrontend =
  function (instanceId, userGithubId, eventName, cb) {
    ...
    this._baseWorkerFindUser(userGithubId, function (err, user) {
      ...
      self._baseWorkerFindInstance({_id: instanceId}, function (err, instance) {
        if (err) {
          return cb(err);
        }
