 ...
 * @param {Function} [requiredKeypaths] keys to validate the existance of
 * @param {Function} cb
 */
BaseWorker.prototype._baseWorkerValidateDieData = function (requiredKeypaths, cb) {
  log.info(this.logData, 'BaseWorker.prototype._baseWorkerValidateDieData');
  if (isFunction(requiredKeypaths)) {
    cb = requiredKeypaths;
    requiredKeypaths = null;
  }
  requiredKeypaths = requiredKeypaths || [];
  requiredKeypaths = requiredKeypaths.concat([
    'from',
    'host',
    'id',
    'time',
  ...
  if (missingKeypath) {
    var err = new Error(
      '_baseWorkerValidateDieData: die event data missing keypath: '+missingKeypath
    );
    log.error(put({
      err: err,
      key: missingKeypath
    }, self.logData), '_baseWorkerValidateDieData: missing required keypath');
    return cb(err);
  }
  cb();
