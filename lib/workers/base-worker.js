 ...
 * @param {Function} cb
 */
BaseWorker.prototype._validateData = function (requiredKeypaths, cb) {
  var missingKeypath = find(requiredKeypaths, keypathNotExists.bind(null, this.data));
  if (missingKeypath) {
    var err = new Error(
      '_validateData: event data missing keypath: '+missingKeypath
    );
    log.error(put({
      err: err,
      key: missingKeypath
...
BaseWorker.prototype._baseWorkerValidateDieData = function (cb) {
  ...
  this._validateData(requiredKeypaths, cb);
};
