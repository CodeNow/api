 ...
 * @param {Function} cb
 */
BaseWorker.prototype._baseWorkerFindInstance = function (query, cb) {
  var logData = put({ query: query }, this.logData)
  log.info(logData, 'BaseWorker.prototype._baseWorkerFindInstance')
  var self = this
  Instance.findOne(query, function (err, result) {
    if (err) {
      log.warn(put({
        err: err
      }, logData), '_baseWorkerFindInstance: findOne error')
      return cb(err)
    }
    if (!result) {
      log.warn(logData, '_baseWorkerFindInstance: not found')
    ...
    log.trace(put({
      ...
      build: result.build
    }, logData), '_baseWorkerFindInstance: findOne success')
