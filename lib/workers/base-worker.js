...
BaseWorker.prototype.pFindInstances = function (query) {
  ...
  var self = this;
  return Promise.promisify(Instance.find)(query)
    ...
    .catch(function (err) {
      log.error(put({
        err: err
      }, self.logData), 'BaseWorker: Build.pFindInstances error');
      throw err;
    });
};
...
BaseWorker.prototype.pFindBuild = function (query) {
  ...
  var self = this;
  return Promise.promisify(Build.findOne)(query)
    .then(function (build) {
      if (!build) {
...
BaseWorker.prototype._updateFrontendWithContextVersion = function (event, cb) {
  var self = this;
  console.trace('_updateFrontendWithContextVersion');
  ...
  this._findContextVersion({
    ...
    if (err) {
      ...
      console.trace('_updateFrontendWithContextVersion failed');
    } else if (contextVersion) {
      log.info(self.logData, 'BaseWorker emitting update success');
      console.trace('_updateFrontendWithContextVersion success');
