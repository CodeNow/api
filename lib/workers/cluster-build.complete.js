...
const publisher = require('../models/rabbitmq/index')
const transform = require('../middlewares/transformations')
...
class Worker {
  ...
  _populateClusterBuildWithSpecifications (clusterBuild) {
    return Promise.map(clusterBuild.specifications, (specification) => {
      return Instance.findByIdAsync(specification.instanceId)
    }).bind(this)
    .map(transform.toJSON)
    .map(this._convertInstanceToSpecification)
    .then(transform.addResultToObjWithKey(clusterBuild, 'specifications'))
  }
