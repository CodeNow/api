...
module.exports.task = (job) => {
  ...
  const mainInstanceMeta = parsedInstances.find((inst) => {
  ...
  .then(() => {
    return UserService.getCompleteUserByBigPoppaId(job.sessionUserBigPoppaId)
      .then(function (sessionUser) {
        return DockerComposeClusterService.createClusterParent(sessionUser, mainInstanceMeta, job.orgBigPoppaId, job.repoName)
      })
