...
DeleteInstanceWorker.prototype.handle = function (done) {
  ...
  this._findInstance({_id: data.instanceId}, function (err, instance) {
    ...
    async.series([
      // if "Neo.ClientError.Statement.EntityNotFound" happened
      // we still need to proceed to another step.
      // basically it might be deleted on the firest attempt
      instance.removeSelfFromGraphAndIgnore404.bind(instance),
...
DeleteInstanceWorker.prototype._deleteForks = function (instance, sessionUserId, cb) {
  ...
  Instance.findInstancesByParent(instance.shortHash, function (err, instances) {
    if (err) {
      log.error(put({ err: err }, self.logData),
    ...
    instances.forEach(function (fork) {
      rabbitMQ.deleteInstance({instanceId: fork._id, sessionUserId: sessionUserId});
    });
    cb();
  }.bind(this));
};
