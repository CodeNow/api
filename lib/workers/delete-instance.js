...
function deleteInstanceWorker (data, callback, done) {
  ...
  findActionUser(data.creatorGitHubId, data.pushUserId, function (err, user) {
    if (err) {
      if (callback) {
      ...
      runnable.destroyInstance(data.instanceId, function (err) {
        if (callback) {
          callback.apply(this, arguments);
        }
        clearTimeout(timeout);
...
function findActionUser (githubId, userId, cb) {
  if (githubId) {
  ...
  return User.findById(userId, cb);
}
