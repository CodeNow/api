...
module.exports.task = function MatchCommitInIsolationInstances (job) {
  ...
  return Instance.findByIdAsync(job.instanceId)
    ...
    .then(function (instanceToMatch) {
      const instanceIsolation = instanceToMatch.isolated.toString()
      if (instanceIsolation !== job.isolationId) {
        throw new WorkerStopError('Instance does not have the isolation that needs to matched', { instanceIsolation })
      }
      ...
      if (!fullRepoName || !commitHash) {
        throw new WorkerStopError('Instance does not have repo and/or commit hash', { fullRepoName, commitHash })
      }
      log.trace({ fullRepoName, commitHash }, 'Fetching instances')
      return Instance.findInstancesInIsolationWithSameRepoAndBranchAsync(job.isolationId, fullRepoName, branchName)
        .then(function (instances) {
          log.trace({ instances: instances.length }, 'Finished fetching instances')
          if (instances.length === 0) {
            log.trace('No instances with same repo found. No instances to update.')
            throw new WorkerStopError('No instances with same repo found. No instances to update.', {}, { level: 'debug' })
          }
    ...
    .spread(function (childInstancesToUpdate, sessionUser, commitHash) {
      ...
      return Promise.map(childInstancesToUpdate, function (instance) {
        ...
        log.trace({ instanceId }, 'updateInstanceCommitToNewCommit for child instance')
        return InstanceService.updateInstanceCommitToNewCommit(instance, commitHash, sessionUser)
          .catch(function (err) {
            log.warn({ instanceId, err }, 'Failed to updateInstanceCommitToNewCommit for child instance')
