...
function OnImageBuilderContainerCreate (job) {
  ...
  return joi
    ...
    .then(function setBuildStarting () {
      log.info(logData, 'OnImageBuilderContainerCreate: setBuildStarting')
      var contextVersionBuildId = job.inspectData.Config.Labels['contextVersion.build._id']

      return ContextVersion.setBuildStarting(contextVersionBuildId, job.host)
    })
    ...
    .then(function emitContextVersionUpdate (contextVersions) {
      log.info(put(logData, { contextVersions: contextVersions }), 'OnImageBuilderContainerCreate: emitContextVersionUpdate')
