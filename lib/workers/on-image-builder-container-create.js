...
module.exports = OnImageBuilderContainerCreateWorker

/**
 * Main handler for image builder create event
 * Should ...
 * @param {Object} job - Job info
 * @returns {Promise}
 */
function OnImageBuilderContainerCreateWorker (job) {
  return new Promise(function (resolve, reject) {
    var workerDomain = domain.create()
    workerDomain.runnableData = BaseWorker.getRunnableData()
    workerDomain.on('error', function (err) {
      error.workerErrorHandler(err, job)
      reject(err)
    })
    workerDomain.run(function () {
      log.info(put({
        tx: true
      }, job), 'hermes.subscribe on-image-builder-container-create start')
      var worker = new OnImageBuilderContainerCreate(job)
 ...
 * @param {Function} cb
 */
OnImageBuilderContainerCreate.prototype._startContainer = function (cb) {
  ...
  ContextVersion.updateBy('build._id', self.contextVersion.build._id, update, { multi: true },
    function (err) {
      if (err) {
        log.error(put({
          err: err
        }, self.logData), '_updateContextVersion: updateBy error')
        return updateCvCb(err)
      }
      log.trace(self.logData, '_updateContextVersion: updateBy success')
      updateCvCb()
    }
  )
}
