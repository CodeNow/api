...
function OnImageBuilderContainerCreate (job) {
  ...
  return joi
    ...
    .then(function startImageBuilderContainer () {
      ...
      var created = job.inspectData.Created

      return docker.startContainerAsync(dockerContainerId)
        .catch(function (err) {
          // there is a race where swarm does not know about a container
          // before we try and start it. If we get 404 and its been 5 min, its gone
          if (keypather.get(err, 'output.statusCode') === 404 &&
            moment(created) < moment().subtract(5, 'minutes')) {
            throw new TaskFatalError(
              'on-image-builder-container-create',
              'Container does not exist after 5 minutes of being created',
