  ...
  return joi
    ...
    .then(function emitContextVersionUpdate (contextVersions) {
      log.info(put(logData, {
        contextVersions: contextVersions.map(function (cv) {
          return cv._id
        })
      }), 'OnImageBuilderContainerCreate: emitContextVersionUpdate')

      return Promise.map(contextVersions, function (contextVersion) {
        return messenger.emitContextVersionUpdate(contextVersion, 'build_started')
      })
    })
    .then(function emitInstanceUpdate () {
      return InstanceService.emitInstanceUpdateByBuildId(contextVersionBuildId, 'build_started', true)
    })
}
