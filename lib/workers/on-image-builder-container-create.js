...
OnImageBuilderContainerCreate.prototype.handle = function (done) {
  ...
  log.trace(this.logData, 'OnImageBuilderContainerCreate.prototype.handle');
  async.series([
    function (cb) {
    ...
    function (cb) {
      self._baseWorkerUpdateContextVersionFrontend('build_running', cb);
    }
  ], function (err) {
    if (err) {
      log.error(put({
...
OnImageBuilderContainerCreate.prototype._startContainer = function (startContainerCb) {
  log.trace(this.logData, 'OnImageBuilderContainerCreate.prototype._startContainer');
  var self = this;
  var attemptCount = 1;
  async.retry({
    ...
    self.docker.startImageBuilderContainer(self.dockerContainer, function (err) {
      if (err) {
        log.warn(put({
          ...
          'OnImageBuilderContainerCreate _startContainer attempt failure'
        );
        ...
        if (err.statusCode === 304) {
          log.trace(put({
            ...
            'OnImageBuilderContainerCreate _startContainer: ' + err.reason
          );
          return cb(null);
        }
        ...
        log.trace(put({
          attemptCount: attemptCount
        }, self.logData), 'OnImageBuilderContainerCreate _startContainer success');
      }
    ...
    if (err) {
      ...
      log.trace(put({
        attemptCount: attemptCount
      }, self.logData), 'OnImageBuilderContainerCreate _startContainer final success');
    }
...
OnImageBuilderContainerCreate.prototype._updateContextVersion = function (updateCvCb) {
  log.trace(this.logData, 'OnImageBuilderContainerCreate.prototype._updateContextVersion');
  var self = this;
  var update = {
    $set: {
...
OnImageBuilderContainerCreate.prototype._onError = function (error, onErrorCb) {
  var self = this;
  log.trace(self.logData, 'OnImageBuilderContainerCreate.prototype._onError');
  var sauron = new Sauron(this.sauronHost);
  sauron.deleteHost(self.networkIp, self.hostIp, function (err) {
    if (err) {
      log.error(put({
        err: err
      }, self.logData), 'OnImageBuilderContainerCreate deleteSauronHost failed');
    } else {
      log.trace(put({
      ...
      }, self.logData), 'OnImageBuilderContainerCreate deleteSauronHost success');
    }
