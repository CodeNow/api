...
function OnCreateImageBuilderContainer (data) {
  log.trace('OnCreateImageBuilderContainer constructor');
...
OnCreateImageBuilderContainer.prototype.handle = function (done) {
  ...
  log.trace(this.logData, 'OnCreateImageBuilderContainer.prototype.handle');
  async.series([
    function (cb) {
...
OnCreateImageBuilderContainer.prototype._startContainer = function (startContainerCb) {
  log.trace(this.logData, 'OnCreateImageBuilderContainer.prototype._startContainer');
  var self = this;
  var attemptCount = 1;
  async.retry({
    ...
    self.docker.startImageBuilderContainer(self.dockerContainer, function (err) {
      if (err) {
        log.warn(put({
          ...
          'OnCreateImageBuilderContainer _startContainer attempt failure'
        );
        ...
        if (err.statusCode === 304) {
          log.trace(put({
            ...
            'OnCreateImageBuilderContainer _startContainer: ' + err.reason
          );
          return cb(null);
        }
        ...
        log.trace(put({
          attemptCount: attemptCount
        }, self.logData), 'OnCreateImageBuilderContainer _startContainer success');
      }
    ...
    if (err) {
      log.error(put({
        err: err,
        attemptCount: attemptCount
      }, self.logData), 'OnCreateImageBuilderContainer _startContainer final failure');
    }
    else {
      log.trace(put({
        attemptCount: attemptCount
      }, self.logData), 'OnCreateImageBuilderContainer _startContainer final success');
    }
...
OnCreateImageBuilderContainer.prototype._updateContextVersion = function (updateCvCb) {
  log.trace(this.logData, 'OnCreateImageBuilderContainer.prototype._findContextVersion');
  var self = this;
  var update = {
    $set: {
...
OnCreateImageBuilderContainer.prototype._onError = function (error, onErrorCb) {
  var self = this;
  log.trace(self.logData, 'OnCreateImageBuilderContainer.prototype._onError');
