...
module.exports = class Worker {
   ...
   */
  static task (job) {
    const log = logger.child({ method: 'ContainerImageBuilderDied' })
    log.info('ContainerImageBuilderDied called')

    const contextVersionBuildId = keypather.get(job, 'inspectData.Config.Labels["contextVersion.build._id"]')

    return Promise.try(Worker._publishParallelTasks(job))
    .then(Worker._getBuildInfo(job))
    .then(function (buildInfo) {
      if (buildInfo.failed) {
        Worker._reportBuildFailure(job, buildInfo)
      }
      return BuildService.handleBuildComplete(contextVersionBuildId, buildInfo)
        .then(function (contextVersions) { return [buildInfo, contextVersions] })
    })
    .spread(function (buildInfo, contextVersions) {
      return InstanceService.emitInstanceUpdateByCvBuildId(contextVersionBuildId, 'patch')
    ...
    .spread(function (instances, buildInfo, versions) {
      return Worker._handleAutoDeploy(versions)
        ...
        .then(function (isolationInstances) {
          return Worker._killIsolationIfNeeded(job, isolationInstances)
        })
        .then(function (nonKilledInstances) {
          Worker._createContainersIfSuccessful(job, nonKilledInstances, buildInfo)
        })
    })
    .catch(function (err) {
   ...
   * @return {Promise}
   */
  static _publishParallelTasks (job) {
    return Promise.method(() => {
      const dockerImageTag = keypather.get(job, 'inspectData.Config.Labels.dockerTag')

      rabbitMQ.pushImage({ imageTag: dockerImageTag })
      rabbitMQ.clearContainerMemory({ containerId: job.id })
    })
  }
  /**
  ...
  static _handleAutoDeploy (versions) {
   ...
   *           {string}     BuildInfo.dockerImage  docker image name of build
   *           {string}     BuildInfo.error.message  if build timed out
   *           {string}     BuildInfo.dockerHost     host build was built on
   */
  static _getBuildInfo (job) {
    var log = logger.child({
   ...
   */
  static _reportBuildFailure (job, buildInfo) {
   ...
   */
  static _createContainersIfSuccessful (job, instances, buildInfo) {
   ...
   * @private
   */
  static _killIsolationIfNeeded (job, instances) {
    var log = logger.child({
      job: job,
