...
class Worker {
    ...
    return this._publishParallelTasks()
      ...
      .then((contextVersions) => {
        return InstanceService.emitInstanceUpdateByCvBuildId(this.contextVersionBuildId, 'patch')
          ...
          .catch((err) => {
            this.log.error({ err: err }, 'failed to emit instance updates')
            throw err
          })
      })
      .spread((instances, versions) => {
        return this._handleAutoDeploy(versions)
          ...
          .then((nonKilledInstances) => {
            this._createContainersIfSuccessful(nonKilledInstances)
          })
      })
      .catch((err) => {
  ...
  _updateBuildAndGetUpdatedVersions () {
