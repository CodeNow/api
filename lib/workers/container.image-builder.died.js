...
class Worker {
   ...
   */
  task () {
    return this._clearBuildResources().bind(this)
  ...
  _handleAutoDeploy (contextVersions) {
    ...
    return Promise.try(() => {
      if (!contextVersions || !contextVersions[0]) {
        return []
      }
  ...
  _emitDeployedIfExists (instances) {
    this.log.trace({ instances }, 'updated instances with a build')
    if (!instances) { return }
  ...
  _createContainersIfSuccessful (instances) {
    const log = logger.child({ method: '_createContainersIfSuccessful' })
  ...
  _filterAndKillIfIsolationMaster (instance) {
    this.log.trace({ instance }, 'Searching for isolation')
  ...
  _filterAndKillIfIsolationExists (instance, isolation) {
    this.log.trace({ isolation }, 'isolation search results')
