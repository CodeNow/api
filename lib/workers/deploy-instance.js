...
DeployInstanceWorker.prototype.handle = function (done) {
  ...
  if (this.data.instanceId) {
    ...
    instanceQuery._id = this.data.instanceId;
  } else if (this.data.buildId) {
    // If we are only given a buildId, we'll be fetching all instances with this build attached
    instanceQuery.build = this.data.buildId;
  } else {
...
DeployInstanceWorker.prototype._validateSelfData = function () {
  if (!this.data.sessionUserGithubId) {
    this.logError(
      this.logData,
    ...
    return false;
  } else if (!this.data.ownerUsername) {
    this.logError(
      this.logData,
  ...
  return self._pBaseWorkerFindInstances(query)
    ...
    .then(function (instances) {
      ...
      if (!self.data.buildId) {
        self.data.buildId = instances[0].build;
      }
      return instances;
    });
};
...
DeployInstanceWorker.prototype._pGetDockHost = function (cv) {
  ...
  if (this.data.forceDock) {
    return Promise.resolve(this.data.forceDock);
  }
  var self = this;
  var mavis = new Mavis();
...
function (instances, contextVersion, dockerHost) {
  ...
  instances.forEach(function (instance) {
    ...
    var labels = {
      ...
      sessionUserGithubId : self.data.sessionUserGithubId.toString()
    };
    var createContainerJobData = {
      cvId: contextVersion._id.toString(),
      sessionUserId: self.data.sessionUserGithubId,
