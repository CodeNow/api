...
InstanceContainerRedeploy.prototype.handle = function (done) {
  ...
  async.series([
    ...
    function findCv (cb) {
      var cvId = keypather.get(self.instance, 'build.contextVersions[0]')
    ...
    if (err) {
      ...
      log.error(newLogData, 'InstanceContainerRedeploy.prototype.handle final error')
      error.workerErrorHandler(err, newLogData)
    } else {
      log.trace(self.logData, 'InstanceContainerRedeploy.prototype.handle final success')
    }
    done()
  })
}
