 ...
 *
 * @module lib/workers/start-image-build-container
 */
'use strict';
...
module.exports = StartImageBuildContainerWorker;
...
module.exports.worker = function (data, done) {
  ...
  workerDomain.run(function () {
    log.info(put({
      tx: true
    }, data), 'hermes.subscribe start-image-build-container-worker start');
    var worker = new StartImageBuildContainerWorker();
    worker.handle(data, done);
  });
};
function StartImageBuildContainerWorker () {
  this.logData = {
    tx: true,
    elapsedTimeSeconds: new Date(),
    uuid: uuid.v4()
  };
  log.info('StartImageBuildContainerWorker constructor');
}
 ...
 * @param done
 */
StartImageBuildContainerWorker.prototype.handle = function (data, done) {
  ...
  async.series([
    ...
    if (err) {
      log.warn(put({
        err: err
      }, self.logData), 'StartImageBuildContainerWorker.prototype.handle final error');
      if (self.contextVersion) {
        return self._onError(err, done);
      }
    }
    else {
      log.info(self.logData, 'StartImageBuildContainerWorker.prototype.handle final success');
    }
    done();
  });
};
 ...
 */
StartImageBuildContainerWorker.prototype._findContextVersion = function (findCvCb) {
  log.info(this.logData, 'StartImageBuildContainerWorker.prototype._findContextVersion');
  var self = this;
 ...
 */
StartImageBuildContainerWorker.prototype._updateContextVersionWithContainer = function (upCvCb) {
  log.info(this.logData, 'StartImageBuildContainerWorker.prototype._findContextVersion');
  var self = this;
 ...
 */
StartImageBuildContainerWorker.prototype._startContainer = function (startContainerCb) {
  log.info(this.logData, 'StartImageBuildContainerWorker.prototype._startContainer');
  var self = this;
  var attemptCount = 1;
  async.retry({
 ...
 */
StartImageBuildContainerWorker.prototype._updateContextVersion = function (updateCvCb) {
  log.info(this.logData, 'StartImageBuildContainerWorker.prototype._findContextVersion');
  var self = this;
 ...
 */
StartImageBuildContainerWorker.prototype._updateFrontend = function (error, cb) {
  log.info(put({
    eventName: 'build_complete'
  }, this.logData), 'StartImageBuildContainerWorker.prototype._updateFrontend');
  var self = this;
  ContextVersion.updateBuildErrorByBuildId(self.contextVersion.build._id, error, function (err) {
    if (err) {
      log.warn(put({
        err: err
      }, self.logData), 'StartImageBuildContainerWorker updateBuildErrorByBuildId failed');
    } else {
 ...
 * @private
 */
StartImageBuildContainerWorker.prototype._onError = function (error, onErrorCb) {
  var self = this;
  log.info(self.logData, 'StartImageBuildContainerWorker.prototype._onError');

  self.sauron.deleteHost(self.networkIp, self.hostIp, function (err) {
    if (err) {
      log.error(put({
        err: err
      }, self.logData), 'StartImageBuildContainerWorker deleteSauronHost failed');
    } else {
      log.trace(put({
