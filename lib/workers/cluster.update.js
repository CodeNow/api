...
module.exports.task = (job) => {
  ...
  return Promise.props({
    ...
    .then(result => {
      ...
      const instanceName = mainInstance.name
      log.info({ result }, 'fetched data for cluster update')
      return ClusterConfigService.fetchComposeFileFromGithub(bigPoppaUser, fullRepo, filePath)
        .then(composeFileData => {
          log.info({ composeFileData }, 'Compose file data')
          return ClusterConfigService.parseComposeFile(composeFileData, fullRepo, instanceName)
        })
        .then(octobearInfo => {
          log.info({ octobearInfo }, 'Compose file parsed')
          return ClusterConfigService.updateCluster(sessionUser, mainInstance, githubPushInfo, octobearInfo)
        })
    })
    .catch(Instance.NotFoundError, err => {
      throw new WorkerStopError('Instance not found', { err, job }, { level: 'info' })
    })
    .catch(BaseSchema.NotFoundError, err => {
      throw new WorkerStopError('Config not found', { err, job }, { level: 'info' })
    })
}
