...
module.exports.task = (job) => {
  ...
  return Promise.props({
    ...
    .then(result => {
      ...
      let branchOrCommit
      if (result.config.repo === job.pushInfo.repo) {
        // if the repo we're updating is the same as the one which owns the compose file,
        // we need to set the branch to be the one from the pushinfo.  Otherwise, we're updating
        // a repo with an external compose file, so we need to use the info given in the config
        branchOrCommit = job.pushInfo.commit
      } else {
        branchOrCommit = result.config.branch
      }
      ...
      return ClusterConfigService.fetchFileFromGithub(bigPoppaUser, fullRepo, filePath, branchOrCommit)
