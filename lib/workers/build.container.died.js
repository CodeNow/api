...
class Worker {
  ...
  task () {
    return Promise.try(this._clearBuildResources.bind(this))
      .bind(this)
      .tap(this._publishDeploymentComplete)
  ...
  _publishDeploymentComplete (){
    if (this._isSuccessfulBuild()) {
      return this._getDeploymentFromContextVersionBuildId()
        .then(this._checkIfDeploymentBuilt)
        .then(this._publishDeploymentBuilt)
        .catch(Deployment.IncorrectStateError, () => {
          this.log.trace('deployment not yet complete')
        })
    }
  }
  /**
