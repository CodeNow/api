...
ContainerImageBuilderDied.jobSchema = joi.object({
  ...
  inspectImageData: joi.object({
    Config: joi.object({
      ExposedPorts: joi.alternatives().try(joi.object(), null).required()
    }).unknown().required()
  }).unknown(),
...
class Worker {
  constructor (job) {
    ...
    this.inspectImageData = job.inspectImageData
  ...
  task () {
    return Promise.try(this._clearBuildResources.bind(this))
      ...
      .each(this._updatePortsOnInstance)
  ...
  _handleAutoDeploy (contextVersions) {
    ...
    return Promise.try(() => {
      ...
      if (!isAutoDeploy) {
        log.trace({ triggeredActionName }, 'Instance is not autodeployed. Stopping.')
        return []
      }
   ...
   */
  _updatePortsOnInstance (instance) {
    const log = this.log.child({ instance, method: '_updatePortsOnInstance' })
    log.info('_updatePortsOnInstance called')
    const imagePorts = keypather.get(this, 'inspectImageData.Config.ExposedPorts')
    const instancePortLength = keypather.get(instance, 'ports.length')
    if (instancePortLength || !imagePorts) {
      return
    }

    const ports = Object.keys(imagePorts).map((portAndProtocol) => {
      return parseInt(portAndProtocol.split('/')[0])
    })
