...
ContainerImageBuilderDied.jobSchema = joi.object({
  ...
  inspectData: joi.object({
    Config: joi.object({
      ExposedPorts: joi.alternatives(joi.object(), null).unknown().required()
    }).unknown().required()
  }).unknown().required(),
  inspectData: joi.object({
    Config: joi.object({
...
class Worker {
  ...
  _updatePortsOnInstance (instance) {
    const log = this.log.child({ instances, method: '_updatePortsOnInstance' })
    log.info('_updatePortsOnInstance called')
    const ExposedPorts = keypather.get(this, 'inspectImageData.Config.ExposedPorts')
    if (!ExposedPorts) { return }

    const ports = Object.keys(ExposedPorts).map((portAndProtocol) => {
      return portAndProtocol.split('/')[0]
    })

    const mergedPorts = Object.assign(instance.ports || {}, ports)
