...
function CatchableError (message) {
...
function ContainerNetworkAttachedWorker (job) {
  ...
  return joi.validateOrBoomAsync(job, schema)
    ...
    .then(function (instance) {
      return InstanceService.modifyExistingContainerInspect(
        ...
        .catch(function (err) {
          ...
          if (statusCode === 409) {
            var fatalError = new TaskFatalError('container.network.attached', 'Instance not found', { job: job })
      ...
      if (instance.isolated) {
        return Isolation.findOneAsync({_id: instance.isolated})
          .then(function (isolation) {
            if (isolation && isolation.status === 'killing') {
              return rabbitMQ.killInstance(instance)
                .then(function () {
                  throw new CatchableError('Bypass Instance Update!')
                })
            }
          })
      }
    ...
    .then(function (instance) {
      return InstanceService.emitInstanceUpdate(instance, null, 'start', false)
    })
    .catch(CatchableError, function () { })
    .return(undefined)
}
