 ...
 * @param data.inspectData.Config.Labels.ownerGitHubUsername
...
function ContainerNetworkAttachedWorker (data) {
  ...
  this.ownerGitHubUsername = labels.ownerGitHubUsername
...
ContainerNetworkAttachedWorker.prototype.handle = function (done) {
  ...
  if (!this.instanceId) {
    log.trace(this.logData, 'handle exit because instanceId is null')
    return done()
  }
  var self = this
  var hosts = new Hosts()
  async.series([
    ...
    if (err) {
      // FIXME: We've seen that sometimes there is already an error here
      // with an empty object which prevents us from passing/handling the
      // error correctly in lib/error.js.
      keypather.set(self, 'logData.err', null)
  ...
  InstanceService.modifyExistingContainerInspect(
    ...
    function (err) {
      if (err) {
        log.error(put({
          err: err
        }, self.logData), '_updateInstance: modifyExistingContainerInspect error')
      } else {
        log.trace(self.logData, '_updateInstance: modifyExistingContainerInspect final success')
      }
      return cb(err)
    })
}
