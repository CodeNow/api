        ...
        .then(function (instanceKilled) {
          if (!instanceKilled) {
            const payload = {
              ...
              githubOrgId: keypather.get(instance, 'owner.github')
            }
            const bigPoppaUserId = keypather.get(job, 'inspectData.Config.Labels.sessionUserBigPoppaId')
            if (bigPoppaUserId) {
              payload.bigPoppaUserId = parseInt(bigPoppaUserId, 10)
            }
            const githubUserId = keypather.get(job, 'inspectData.Config.Labels.sessionUserGithubId')
            if (githubUserId) {
              payload.githubUserId = parseInt(githubUserId, 10)
            }
