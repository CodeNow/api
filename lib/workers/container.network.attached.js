...
module.exports.task = (job) => {
  ...
  log.trace('XXX container.network.attached')
  const containerId = job.id
  ...
  return Promise.resolve()
    .then(() => {
      ...
      log.trace({ updateData }, 'update query')
    ...
    .then(() => {
      log.trace('markAsStartingAsync')
      return Instance.markAsStartingAsync(instanceId, containerId)
    })
    .then(() => {
      log.trace('findOneByContainerIdAsync')
    ...
    .then(function (instance) {
      return InstanceService.modifyExistingContainerInspect(
        ...
        job.id,
        job.inspectData,
        job.inspectData.NetworkSettings.IPAddress
      )
      .catch(function (err) {
    ...
    .catch((err) => {
      if (~err.message.indexOf('Instance not found')) {
        log.error({isolation: isolation}, 'Instance not found, stopping task')
        return
      }
      throw err
    })
