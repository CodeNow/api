  ...
  return Promise.fromCallback(function (cb) {
    ...
    .then(function (instance) {
      ...
      return Promise.fromCallback(function (cb) {
        ...
        if (!user) {
          throw new WorkerStopError('User not found', {}, { level: 'info' })
        }
    ...
    .then(function (data) {
      ...
      .catch(function (err) {
        log.error({ err }, 'findGithubUsernameByGithubId error')
...
ApplicationContainerRedeployWorker._createNewContainer = function (job, data) {
  const contextVersionId = keypather.get(data, 'build.contextVersions[0]')
