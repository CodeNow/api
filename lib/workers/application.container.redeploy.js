...
function ApplicationContainerRedeployWorker (job) {
  const log = logger.child({ job, method: 'ApplicationContainerRedeployWorker' })
  return Promise.fromCallback(function (cb) {
    ...
    .then(function (instance) {
      ...
      return Promise.fromCallback(function (cb) {
        ...
        if (!user) {
          throw new WorkerStopError('User not found')
        }
        return {
          instance: instance,
          user: user
    ...
    .then(function (data) {
      ...
      .then(function (build) {
        if (!build.successful) {
          throw new WorkerStopError('Cannot redeploy an instance with an unsuccessful build')
        }
        data.build = build
        return data
      })
    ...
    .then(function (data) {
      log.trace('find cv')
      const cvId = keypather.get(data, 'build.contextVersions[0]')
      return Promise.fromCallback(function (cb) {
    ...
    .then(function (data) {
      ...
      const oldContainer = data.instance.container
      return Promise.fromCallback(function (cb) {
    ...
    .then(function (data) {
      ...
      const containerId = keypather.get(data, 'oldContainer.dockerContainer')
      if (containerId) {
