      ...
      return docker.startUserContainerAsync(job.containerId, data.contextVersion)
        .catch(function (err) {
          if (keypather.get(err, 'output.statusCode') === 404) {
            var created = keypather.get(data.instance, 'container.inspect.Created')
        ...
        .catch(TaskFatalError, function (err) {
          return Instance.setContainerErrorAsync(job.instanceId, job.containerId, err)
            .catch(function (updateErr) {
              log.error({ err: updateErr }, 'setContainerError failed, ignore')
            })
            .finally(function () {
              throw err
            })
