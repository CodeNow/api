...
module.exports.task = function InstanceStartWorker (job) {
  ...
  log.info('InstanceStartWorker called')
  return Instance.findOneStarting(job.instanceId, job.containerId)
    ...
    .catch(Instance.NotFound, function (err) {
      throw WorkerStopError(err.message, { job: job, err: err })
    })
    ...
    .catch(Instance.IncorrectState, function (err) {
      throw WorkerStopError(err.message, { job: job, err: err })
    })
    .return()
}
