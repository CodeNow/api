...
// Add CSRF protection!
app.use(require('csurf')({sessionKey: 'session'}))
app.use(function (req, res, next) {
  var domainChunks = url.parse(process.env.FULL_API_DOMAIN).hostname.split('.')
  var rawDomain = '.' + domainChunks[domainChunks.length - 2] + '.' + domainChunks[domainChunks.length  - 1]
