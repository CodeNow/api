...
error.errorHandler = function (err, req, res, next) {
  ...
  err.reformat();
  if (configs.logErrors) {
    if (err.output.statusCode >= 500) {
      console.error('Bad App Error: ', err.output.statusCode, req.method, req.url);
      error.log(err);
    }
    else {
      console.error('Acceptable App Error: ', err.output.statusCode, req.method, req.url);
    }
  }
  // if (err.output.statusCode === 500) {
  //   throw err;
