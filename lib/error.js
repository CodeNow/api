...
function report (err) {
  ...
  logger.trace({ err: err, payload: payload }, 'rollbar.handleErrorWithPayloadData')
  if (process.env.ROLLBAR_KEY) {
...
error.log = function (err, req) {
  ...
  if (!err.isBoom) {
    err = error(500, err.message || 'unknown', { err: err })
    err.reformat()
  }
...
error.csrfHandler = function (err, req, res, next) {
  if (err.code !== 'EBADCSRFTOKEN') {
    return next(err)
  }
  next(Boom.create(403, 'Invalid CSRF validation token'))
}
  ...
  function handMongoError (err) {
    if (error.isMongoAlreadyExistsError(err)) {
      ...
      if (match && match.length > 1) {
        ...
        message = resource + ' with ' + field + ' already exists'
      }
...
error.workerErrorHandler = function (err, data) {
  var log = logger.child({
    method: 'error.workerErrorHandler',
    err: err,
    data: data
  })
  log.info('error.workerErrorHandler called')
  if (process.env.LOG_ERRORS) {
    error.log(err)
    log.error('rollbar.handleErrorWithPayloadData')
    if (process.env.ROLLBAR_KEY) {
...
/**
 * determine if an error is 4XX class error
 * @param  {Error}  err error
 * @return {Boolean} whether error is a 4XX error
 */
error.is4XX = function (err) {
  var ret = err &&
    err.isBoom &&
