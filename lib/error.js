...
function log (err) {
  ...
  if (err.data) {
    ...
    if (err.data.docker) {
      errorData = assign(error, pick(err.data.docker,
        ['host', 'port', 'cmd', 'containerId', 'log']));
      logger.log.error(errorData, 'log docker error');
    }
    if (err.data.krain) {
      errorData = assign(error, pick(err.data.krain, ['uri', 'statusCode', 'info']));
      logger.log.error(errorData, 'log krain error');
    }
    if (err.data.mavis) {
      errorData = assign(error, pick(err.data.mavis, ['uri', 'statusCode', 'info']));
      logger.log.error(errorData, 'log mavis error');
    }
    if (err.data.s3) {
      errorData = assign(error, pick(err.data.s3,
        ['bucket', 'contextId', 'sourcePath', 'sourceUrl']));
      logger.log.error(errorData, 'log s3 error');
    }
    if (err.data.res) {
      errorData = assign(error, pick(err.data.res, ['body', 'statusCode']));
      logger.log.error(errorData, 'log http response error');
    }
    if (err.data.debug) {
      errorData = assign(error, pick(err.data.debug, ['debug']));
