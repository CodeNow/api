...
function logFn (err) {
  ...
  if (err.data) {
    if (err.data.err) {
      ...
      log.trace({ dataErr: err.data.err, err: err }, 'error logFn')
...
function report (err) {
  ...
  log.trace({ err: err }, 'error report')
  var custom = err.data || {}
...
error.workerErrorHandler = function (err, data) {
  ...
  if (process.env.LOG_ERRORS) {
    error.log(err) // logFn, report
    log.trace({ err: err, data: data }, 'error handleErrorWithPayloadData workerErrorHandler')
