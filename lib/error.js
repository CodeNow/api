...
error.stringify = function (err, dataErrKeys) {
  dataErrKeys = dataErrKeys || [];
  dataErrKeys = dataErrKeys
    .concat(['message', 'stack'])
    .concat(Object.keys(err));
  if (err.isBoom) {
    if (err.data.err) {
      err.data.err = pick(err.data.err, dataErrKeys); // so it stringifies correctly
    }
    err.reformat();
    return JSON.stringify(pick(err, ['message', 'stack', 'output', 'data', 'isBoom']));
  }
  else {
    return JSON.stringify(pick(err, dataErrKeys));
  }
};

error.parse = function (errStr) {
  var json = JSON.parse(errStr);
  var err;
  if (json.isBoom) {
    err = new Boom(json.output.statusCode || 500, json.ouput.message || 'Unknown', json.data);
  }
  else {
