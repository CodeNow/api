  ...
  getOwners = function(domain, containers, cb) {
    ...
    return users.find(query, fields, domain.intercept(function(users) {
      ...
      users.forEach(function(user) {
        return userHash[user._id] = user;
      });
      containers.forEach(function(container) {
        return container.ownerJSON = userHash[container.owner];
      });
  ...
  cleanupContainersNotIn = function(domain, whitelist, cb) {
    ...
    return async.parallel([
      function(cb) {
        ...
        notInWhitelist = {
          _id: {
            $nin: whiteContainerIds
          }
        };
        return containers.count(notInWhitelist, domain.intercept(function(count) {
    ...
    return async.series([
      function(cb) {
        ...
        return users.findUser(domain, {
          ...
          if (!user) {
            return cb({
              message: 'permission denied: no user'
            });
          } else {
            if (!user.isModerator) {
              return cb({
      ...
      if (err) {
        ...
        return res.json(200, {
          message: 'successfuly sent prune request to harbourmaster and cleaned mongodb'
        });
      }
    });
  };
