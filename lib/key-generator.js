...
module.exports.start = function () {
  log.trace('keypair generator has started');
  var self = this;
...
function checkAndGenerateKeypairs (cb) {
  ...
  log.trace('starting to generate keys');
  Keypair.getRemainingKeypairCount(function (err, count) {
    if (err) {
      ...
      log.trace('starting to generate keys (%d)', keysToCreate);
      async.whilst(
