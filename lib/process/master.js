...
Master.prototype.cycleWorkers = function () {
  ...
  return setInterval(function () {
    var worker = self.workers[0]; // kill worker in first position
    if (worker) {
      self.logWorkerEvent('kill by interval', worker);
      self.workerKillTimeouts[worker.id] = setTimeout(
        self.handleWorkerKillTimeout.bind(self, worker),
