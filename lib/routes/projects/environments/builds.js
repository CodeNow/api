...
function buildVersionsAndTailLogs (req, res, next) {
  var noop = function () {};
  var versionsStarted = 0;
  var versionsErrored = 0;
  var responded = false;
  async.forEach(req.contextVersions, function (contextVersion, cb) {
    async.waterfall([
      ...
      if (err) {
        ...
        var build = req.build;
        async.parallel({
          contextErr: contextVersion.updateBuildError.bind(contextVersion, err),
          buildErr: build.pushErroredContextVersion.bind(build, contextVersion._id)
        }, function (err) {
          if (err) { error.logIfErr(err); }
          checkToRespond();
        });
      }
      // Whether or not the build succeeded or failed, end the build stream connection.
    ...
    function buildVersion (dockerHost, cb) {
      ...
      docker.buildVersion(contextVersion, req.sessionUser, function () {
        versionsStarted++;
        checkToRespond();
      }, cb);
    }
    function versionSetBuildCompleted (dockerInfo, cb) {
      contextVersion.setBuildCompleted(dockerInfo, function (err) {
        cb(err); // only pass err
      });
    }
    function checkToRespond () {
      if (!responded && (versionsStarted + versionsErrored) === req.contextVersions.length) {
        if (versionsErrored) {
          req.build.setCompleted(function (err) {
            if (err) { error.logIfErr(err); }
            // just respond 201 frontend ui will show build failed
