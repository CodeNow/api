...
var createBuildWithVersionsCopies = flow.series(
  ...
  mw.log('!!!build.contextVersions', 'build.contextVersions'),
  contextVersions.findByIds('build.contextVersions'),
  mw.log('!!!FOUND', 'contextVersions'),
  contextVersions.createDeepCopies('sessionUser', 'contextVersions'),
...
var createBuildWithVersionsAndS3Copy = flow.series(
  mw.res.send(504)
);
var createFromParent = flow.series(
  ...
  mw('build')('environment.toString()').validate(validations.equalsKeypath('params.envId.toString()'))
