...
var createBuildWithVersionsCopies = flow.series(
  ...
  mw.body('shallow').require()
    .then(
      ...
      contextVersions.createShallowCopies(
        'sessionUser',
        'contextVersions',
...
function buildVersionsAndTailLogs (req, res) {
  ...
  async.forEach(req.contextVersions, function (contextVersion, cb) {
    ...
    function versionSetBuildStarted (dockerHost, cb) {
      var buildProps = {};
      buildProps.triggeredAction = req.body.triggeredAction;
      contextVersion.setBuildStarted(req.sessionUser, buildProps, function (err) {
        versionsStarted++;
