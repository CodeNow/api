...
function buildVersionsAndTailLogs (req, res) {
  var noop = function () {};
  var respond = createCount(req.contextVersions, function() {
  ...
  async.forEach(req.contextVersions, function (contextVersion, cb) {
    ...
    function buildVersion (dockerHost, cb) {
      ...
      docker.buildVersion(contextVersion, req.sessionUser, respond.next, cb);
    }
    function handleBuildFinish (cb) {
      return function (err, dockerInfo) {
        if (err) {
          var build = req.build;
          async.parallel({
        ...
        contextVersion.setBuildCompleted(dockerInfo, cb);
      };
    }
  }, function () {
