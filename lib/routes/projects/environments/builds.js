 ...
 * is good, then we'll start the process.  We should create deep copies of all of the
 * ContextVersions (Copy their build info, just link to the infra-code versions)
...
app.post('/:projectId/environments/:envId/builds',
  ...
  function (req, res, next) {
    req.oldContextVersions = req.contextVersions.filter(hasProps('build'));
    if (req.oldContextVersions.length === 0) {
    ...
    if (req.oldContextVersions.filter(not(hasKeypaths('build.error', 'build.completed'))).length > 0) {
    ...
    req.contextVersions = [];
    req.oldContextVersions.forEach(function(version) {
      req.contextVersions.push(contextVersions.createShallowCopy(version, 'sessionUser._id'))
    });
    next();
  },
  // Now save the models in the database
  function(req, res, next) {
    console.log(runnable);
    next();
  },
  contextVersions.models.save(),
  // Create a copy of the build, and add all of the cloned context versions
  builds.model.createCopyWithClonedVersions('contextVersions'),
  // Call building route
  runnable.create(),mw.log('HEYYYYY'),
