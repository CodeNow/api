...
var github = apiMiddlewares.github;
...
app.post('/:projectId/environments/:envId/builds/:id/actions/build',
  ...
  mw.body('triggeredAction.appCodeVersion').require()
    ...
    .else(github.create({ token: 'sessionUser.accounts.github.accessToken' })),
  ...
  function (req, res, next) {
    ...
    // make sure none of the context versions are built
  ...
  mw.body('environment').require() // builds environment can be changed at build time
    .then(
...
function buildVersionsAndTailLogs (req, res) {
  ...
  async.forEach(contextVersions, function (contextVersion, cb) {
    ...
    function getLatestCommitHash (dockerHost, cb) {
      var commit = keypather.get(buildProps, 'triggeredAction.appCodeVersion.commit');
      var repo = keypather.get(buildProps, 'triggeredAction.appCodeVersion.repo');

      async.each(contextVersion.appCodeVersions, function (acv, cb) {
        if (acv.commit) {
          // it's all set, we don't have to get the latest commit for the branch
          cb(null);
        } else if (repo && repo === acv.repo) {
          updateACVersionWithCommit(acv._id, { sha: commit }, function (err) {
            cb(err, dockerHost);
          });
        } else {
          // we gotta find out what commit we're going to be building for this branch
          github.getLatestCommit(acv.repo, acv.branch, function (err, commit) {
            if (err) { return cb(err); }
            updateACVersionWithCommit(acv._id, commit, function (err) {
              cb(err, dockerHost);
            });
          });
        }
      ...
      function updateACVersionWithCommit(id, commit, callback) {
        ContextVersions.update({
          _id: contextVersion._id,
          'appCodeVersions._id': id
        }, {
          $set: {
            'appCodeVersions.$.commit': commit.sha
          }
        }, callback);
      }
    }
