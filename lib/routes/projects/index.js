...
app.get('/',
  mw.query('search', 'owner', 'name', 'githubOwner', 'sort', 'page', 'limit').pick(),
  mw.query({ or: ['owner', 'name', 'githubOwner', 'search', 'sort'] }).require(),
  mw.query('githubOwner').require()
    .then(
      users.findOneByGithubUsername('query.githubOwner'),
      checkFound('user'),
      mw.query().set('owner.github', 'user.accounts.github.id'),
      mw.query().unset('githubOwner')),
