...
app.post('/auto-isolation-configs',
  ...
  function (req, res, next) {
    ...
    req.body.requestedDependencies = req.body.requestedDependencies.map(function (d) {
      if (err) { return }
      if (!isString(d.lowerRepo)) {
        err = Boom.badRequest('lowerRepo is required for each requested dependency')
      }
      if (!isString(d.lowerBranch)) {
        err = Boom.badRequest('lowerBranch is required for each requested dependency')
      }
      return {
        lowerRepo: d.lowerRepo.toLowerCase(),
        lowerBranch: d.lowerBranch.toLowerCase()
      }
    })
    next(err)
  },
