...
var updateUser = series(
  ...
  mw.body('accounts.slack.orgs[0]').require()
    .then(
      ...
      mw.req('slackUser.id').require()
        .else(
          mw.next(mw.Boom.notFound('Slack user with provided username was not found'))
          ),
      mw.req().set('slackUser.enabled', 'slackAccount.enabled'),
      slack.model.saveSlackAccount('params.userId', 'slackUser', 'slackAccount.githubId'),
