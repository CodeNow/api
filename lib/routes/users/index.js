...
const userNotFoundHandler = function (err) {
...
app.get('/users/:userId',
  ...
  flow.mwIf(me.isUser)
    ...
    .else(function (req, res, next) {
      UserService.getCompleteUserById(req.params.userId)
      .tap(function (user) {
        if (user && !req.sessionUser.isModerator) {
          user.protectEmail()
        }
        req.user = user
      })
      .catch(errors.UserNotFoundError, userNotFoundHandler)
      .asCallback(function (err) {
        next(err)
      })
    }),
