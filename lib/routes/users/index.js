...
app.get('/users/:userId',
  ...
  flow.mwIf(or(me.isUser, me.isModerator))
    .then(
      function (req, res, next) {
        logger.log.trace('Get user')
        var sessionUser = req.sessionUser.toJSON()
        logger.log.trace({ json: sessionUser }, 'Get user #2')
        logger.log.trace({ bigPoppa: req.sessionUser.get('bigPoppaUser'), keys: Object.keys(req.sessionUser) }, 'Get user #2.2')
        Promise.all([
          User.findByIdAsync(req.params.userId),
          UserService.getByGithubId(1981198)
        ])
        .tap(function (res) {
          logger.log.trace({ res }, 'Get user #3')
