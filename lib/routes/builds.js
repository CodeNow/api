...
app.post('/builds/:id/actions/build',
  ...
  mw.req('contextVersion.build.started').require()
    .then(
      mw.req('contextVersion.build.completed').require()
        .then(
          builds.model.modifyCompleted('contextVersion.build.failed'),
  ...
  mw.req().set('orgId', 'context.owner.github'),

  /**
   * TODO move below logic into "on-image-builder-container-die" worker
   * SAN-2525
   */
  socketClient.createSocketClient('orgId'),
  function (req, res, next) {
    log.trace({
      tx: true,
      contextVersion: req.contextVersion
    }, 'onBuildCompleted pre-waiting');
    req.socketClient.onBuildCompleted(req.contextVersion._id, function (err, cv) {
      if (err || !cv || keypather.get(cv, 'build.error')) {
        log.warn({
          tx: true,
          contextVersion: req.contextVersion,
          cv: cv
        }, 'onBuildCompleted complete error');
        ...
        log.warn({
          ...
          contextVersion: req.contextVersion,
          cv: cv
        }, 'onBuildCompleted complete success');
        builds.model.modifyCompleted(cv.build.failed)(req, res, next);
      }
    });
  },
  function (req, res, next) {
    socketClient.deleteSocketClient(req.orgId)(req, res, next);
  },
...
function logIfErrAndNext (next, req) {
  return function (err) {
