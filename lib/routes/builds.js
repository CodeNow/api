...
app.post('/builds/:id/actions/build',
  ...
  mw.log('build marked as completed'),
  mw.req('build.successful').validate(validations.equals(true)).then(
    mw.log('break5'),
    mw.log('contextVersions[0].appCodeVersions[0]', 'appCodeVersions'),
    instances.findInstancesLinkedToBranch('contextVersions[0].appCodeVersions[0].lowerRepo', 'contextVersions[0].appCodeVersions[0].lowerBranch'),
    mw.log('instances', 'break5.5'),
    mw.req('instances').each(
      ...
      runnable.create({}, 'sessionUser'),
      mw.log('break6', 'build', 'instance._id'),
    ...
    //   },
    //   runnable.create({}, 'sessionUser'),
    //   mw.log('starting redeploy'),
    //   runnable.model.patchInstance('instance._id', 'build'))),
...
function checkToRespond (eachReq, res, next) {
  ...
  if (req.attachCount === req.contextVersions.length) {
    ...
    console.log('build build just send response');
