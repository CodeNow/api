          ...
          mw.req('contextVersion.build.completed').require()
            .then(
              mw.req('contextVersion.build.error').require()
                .then(
                  // If the context version has errored out, then push it to the
                  // Errored list
                  builds.model.pushErroredContextVersion('contextVersion._id'))
            ).else(
              // If it hasn't completed, then we should poll mongo until we see it finish
              pollMongo('contextVersion', contextVersions, 'build.completed', 'build.error',
                stop,builds.model.pushErroredContextVersion('contextVersion._id')))
        ).else(
...
/*jshint maxparams: 6 */
function pollMongo(polledModel, database, successKeyPath, failureKeyPath, successCb, failureCb) {
  setTimeout(function() {
    database.findOne({_id: polledModel._id}, function(err, model) {
      if (keypather.get(model, failureKeyPath)) {
        failureCb(model);
      } else if (keypather.get(model, successKeyPath)) {
        successCb(model);
      } else {
        setTimeout(pollMongo(polledModel, database, successKeyPath, failureKeyPath, successCb,
          failureCb), process.env.BUILD_END_TIMEOUT);
      }
    });
