...
var findBuild = flow.series(
  ...
  checkFound('build'),
  mw.body('public').validate(validations.equals(true))
    .else(
    flow.or(
      me.isOwnerOf('build'),
      me.isModerator)));
...
app.post('/builds/:id/actions/copy',
  ...
  mw.query('deep').require()
    ...
    function (req, res, next) {
      req.body.newBuildOwner = (req.body.public) ?
      {
        github: req.sessionUser.accounts.github.id,
        username: req.sessionUser.accounts.github.login
      } : req.build.owner;
      next();
    },
      builds.create({
