...
app.post('/builds/',
  ...
  mw.body('contextVersions').require().validate(validations.isObjectIdArray)
    .then(
      ...
      mw.req('contexts').each(function(context, req, eachReq, res, next) {
        eachReq.context = context;
        next();
      },
        mw.req('body.owner.github')
          .validate(validations.equalsKeypath('context.owner.github'))
          .else(mw.next(Boom.badRequest('Context version\'s owner must match build owner')))), function(req, res, next) {
...
app.get('/builds/',
  ...
  mw.query('completed').require()
    .then(mw.query('completed').mapValues(transformations.boolToExistsQuery)), function(req, res, next) {
...
app.post('/builds/:id/actions/copy',
  ...
  mw.query('deep').require()
    .then(
      ...
      mw.req().set('copiedContextVersions', 'runnableResult'), function(req, res, next) {
        req.copiedContextVersions = req.copiedContextVersions.map(pluck('json()'));
...
app.post('/builds/:id/actions/build',
  ...
  mw.req('contextVersion.build.started').require()
    .then(builds.model.modifyCompletedIfFinished('contextVersion.build'))
    .else( // build has an unbuilt contextVersion
      flow.try(
        ...
        // Check if building it caused a dedup
