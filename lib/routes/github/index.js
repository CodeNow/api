...
proxy.on('proxyRes', function (proxyRes, req) {
  ...
  if (proxyRes.statusCode === 304 && req.cachedData) {
    ...
    debug('cache hit', proxyRes.statusCode);
    proxyRes.headers = omit(proxyRes.headers, corsOmitHeaders);
    ...
    res.status(req.cachedData.statusCode.toString());
...
function checkForTokenAndGenerateKey (req, res, next) {
  ...
  var data = {
    method: req.method,
    url: req.url // url has the query on it, no need to duplicate
  };
  debug('digesting this', data);
  req.redisDigest = 'github-proxy-cache:' + jsonHash.digest(data);
  next();
}
