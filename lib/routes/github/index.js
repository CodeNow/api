  ...
  if (proxyRes.statusCode === 304) { // Cache HIT
      ...
      if (err) {
        ...
        jsonObjectify([
          ...
          if (err) {
            ...
            zlib.gzip(resData.body, function (err, newNewBody) {
              if (err) { return error.errorHandler(err, req, res); }
              res.status(statusCode).send(newNewBody);
            });
          }
        });
      }
    ...
    var saveToRedis = concat(function (body) {
      ...
      async.waterfall([
        ...
        function (hs, cb) {
          if (~proxyRes.headers['content-type'].indexOf('application/json')) {
            cb(null, hs, body);
          } else {
