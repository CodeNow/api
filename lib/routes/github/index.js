...
proxy.on('proxyReq', function (proxyReq, req) {
  ...
  if (req.cachedData && req.headers['cache-control'] !== 'no-cache') {
    ...
    var quotedEtag = '"' + etag + '"'
    log.trace({ tx: true, etag: quotedEtag }, 'proxyReq')
  ...
  log.trace({
    ...
    proxyHeaders: JSON.stringify(proxyReq._headers, true, 2)
...
function handleGithubCacheMissRes (req, proxyRes) {
  ...
  function toCache (body) {
    ...
    // Also we don't we cleanup quotes and slaches
    // etag are received in the following format: W/"22e8fcac0a8d2a7220fc9f94ac46923d"
    // wehn we send etag back to github it's fine to send it like "22e8fcac0a8d2a7220fc9f94ac46923d"
