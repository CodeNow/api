...
proxy.on('proxyReq', function (proxyReq, req) {
  ...
  console.log('proxyReq headers 1', JSON.stringify(req.headers, true, 2))
  Object.keys(req.headers).forEach(function (key) {
    // if(key !== 'user-agent' && key !== 'accept' && key !== 'origin' && key !== 'origin' && key !== 'referer'  && key !== 'host') {
    // if(key !== 'user-agent' && key !== 'accept' && key !== 'host') {
      delete req.headers[key]
      proxyReq.setHeader(key, undefined)
    // }
  })
  proxyReq.setHeader('host', 'api.github.com')
  proxyReq.setHeader('User-Agent', 'curl/7.43.0')
  proxyReq.setHeader('Accept', '*/*')
  console.log('proxyReq headers 2', JSON.stringify(req.headers, true, 2))
  if (req.cachedData && req.headers['cache-control'] !== 'no-cache') {
    ...
    if (etag) {
      ...
      console.log('cleanuped up etag', etag)
    }
    log.trace({
      tx: true,
      etag: etag
    ...
    var sendEtag = '"' + etag + '"'
    console.log('sending etag to github', sendEtag)
    proxyReq.setHeader('If-None-Match', sendEtag)
  }
...
function proxyRequest (req, res) {
  ...
  // Don't check github if it's been less than 15 seconds
  if ((keypather.get(req, 'cachedData.date') || 0) + 15 > Date.now()) {
    log.trace({tx: true}, 'Bypassing github call, its been less than 15 seconds')
    res.headers = req.cachedData.headers
    res.statusCode = 304
    handleGithubCacheOkayRes(req, res)
  } else {
