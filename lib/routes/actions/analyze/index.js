...
var checkForDependencyFiles = flow.or(
  ...
  mw.req('repositoryRootDir')
    ...
    .then(mw.req().set('languageFramework', javascript_nodejs),
          mw.req().set('supportedLanguageVersions', stacks.node.versions),
  ...
  mw.req('repositoryRootDir')
    ...
    .then(mw.req().set('languageFramework', ruby_ror),
          mw.req().set('supportedLanguageVersions', stacks.ruby.versions),
  ...
  mw.req('repositoryRootDir')
    ...
    .then(mw.req().set('languageFramework', python),
          mw.req().set('supportedLanguageVersions', stacks.python.versions),
...
function extractJS (req, res, next) {
  ...
  req.inferredLanguageVersion = keypath(depFile, 'engine.node') ||
    req.supportedLanguageVersions.default;
  next();
}
...
function extractRuby (req, res, next) {
  ...
  req.inferredLanguageVersion = req.supportedLanguageVersions.default;
  next();
}
