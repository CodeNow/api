...
function extractDependencyList (req, res, next) {
  debug('extracting dependencies for languageFramework: %s', req.languageFramework);
  var dependencies;
...
function inferDependenciesFromDependencyList (req, res, next) {
  var inferredServices = Object.keys(suggestableServices).filter(function (key) {
    ...
    return suggestableServices[key].filter(function (mm) { // matching module
      return !!req.dependencyList.filter(function (pm) { // project module
        // match any module where known module name is subset
        // TODO: logging
        return (pm.toLowerCase().indexOf(mm.toLowerCase()) !== -1);
      });
    });
  });
