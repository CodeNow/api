...
var followBranch = flow.series(
  ...
  mw.req().set('ownerGithub', 'instances[0].owner.github'),
  mw.req().set('creatorGithub', 'instances[0].createdBy.github'),
  ...
  runnable.create({}, githubUser),
  runnable.model.createNewBuild('newCVs', 'ownerGithub'),
  ...
  // we will use GitHub API using creator account - which is not ideal.
  // we should use maybe runnable account in the future?
  users.create(),
  users.model.findGithubUserByGithubId('creatorGithub'),
  users.create('user'),
  instances.models.getGithubUsername('user'),
  // find all the context versions we just created, and return them
  contextVersions.findByIds('newCVs'),

  settings.findSettingsForOwnerGihubId('ownerGithub'),
  notifications.create('setting.notifications'),
...
app.post('/actions/github/',
  ...
  mw.headers('x-github-event').matches(/^push$/).then(
    ...
    mw.log('github user', 'githubResult', 'headCommit.author.username'),
