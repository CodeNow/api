  ...
  if (req.jsonNewBuild) {
    ...
    onBuildCompleted(cvId, req.socketClient,
      ...
      function buildCompleted () {
        ...
        var runnableClient = new Runnable({}, req.instanceCreator);
        runnableClient.forkMasterInstance(instance, buildId, req.githubPushInfo.user.id,
          req.githubPushInfo.branch,
          handleInstanceDeployed(req, instance, 'github_push_autofork_',
          function instanceDeploySuccess () {
            // report event to the heap. fire and forget
            trackInstanceEvent(req.githubPushInfo, instance, 'github_hook_autofork');
            sendSlackAutoForkNotification(req.githubPushInfo, instance);
          }));
      }));
  }
  next();
