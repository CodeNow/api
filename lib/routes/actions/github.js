...
app.post('/actions/github/',
  ...
  mw.headers('x-github-event').matches(/^push$/).then(
    ...
    function (req, res, next) {
      ...
      //   github: req.body.organization.id
      // };
      // TODO (anton) owner can be not only org but user.
      // Use this code just tmp for testing
      console.log('owner data>>>>', req.repoOwner);
      next();
    },
    ...
    function (req, res, next) {
      Instances.aggregate([
        {
          $match: {
            'contextVersion.appCodeVersions.lowerRepo': req.lowerRepo,
            'contextVersion.appCodeVersions.lowerBranch': req.lowerBranch
          }
        },
        {
          $match: {
    ...
    runnable.create({}, githubUser),
    mw.log('githubResult', 'github user data'),
    runnable.model.createNewBuild('newCVs', 'githubResult.id', 'githubResult.username'),
    mw.log('runnableResult', '<<<<new build'),
    runnable.create({}, githubUser),
    runnable.model.buildBuild('runnableResult', {message: 'auto-update'}),
    mw.log('runnableResult', '<<<<build was built'),
    // // TODO notifications should be sent in bg. Don't need to block response
    // notifications.create('contextVersions'),
    // notifications.model.notifyOnBuild(),
    mw.res.status(201),
