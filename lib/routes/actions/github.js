...
var Experiment = require('utils/runnable-experiment')
...
function parseGitHubPushData (req, res, next) {
  ...
  log.trace(req.githubPushInfo, 'parseGitHubPushData')
  next()
}
...
function newContextVersion () {
  return function (req, res, next) {
    var cloneReq = pick(req, ['domain', 'contextVersion', 'githubPushInfo'])
    var cloneRes = {}
    var experiment = new Experiment('newContextVersion')
    experiment.compare(function (control, candidate) {
      ...
      skipFields.forEach(function (f) {
        keypather.set(control.value, f, undefined)
        keypather.set(candidate.value, f, undefined)
      })
      console.log(JSON.stringify(control.value), JSON.stringify(candidate.value))
      var expect = require('code').expect
      try {
        expect(control.value).to.deep.equal(candidate.value)
      } catch (e) {
        return false
      }
      return true
    })
    experiment.use(function () { return oldWay(req, res) })
    experiment.try(function () { return newWay(cloneReq, cloneRes) })
    experiment.run().asCallback(function (err) {
      next(err)
    })
  }

  function oldWay (req, res) {
    var pushSessionUser = {
      permissionLevel: 5,
      accounts: {
        github: {
    ...
    return Promise.fromCallback(function (next) {
      ...
      .then(function () {
        return req.contextVersion.toJSON()
      })
  }

  function newWay (req, res) {
    return Promise.fromCallback(function (next) {
      flow.series(
