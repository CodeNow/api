...
app.post('/actions/github/',
  ...
  mw.headers('x-github-event').matches(/^push$/).then(
    parseGitHubPushData,
    checkRepoOrganizationAgainstWhitelist,
              ...
              .else(
                ...
                function (req, res, next) {
                  InstanceForkService.autoFork(req.instances, req.githubPushInfo)
                    ...
                    .then(function (newInstances) {
                      return IsolationService.autoIsolate(
                        newInstances,
                        req.githubPushInfo
                      )
                      .catch(function (err) {
                    ...
                    .catch(function () {
                      // we have already logged this error, just stopping it
                    })
                }
              )
          )
 ...
 */
function checkRepoOrganizationAgainstWhitelist (req, res, next) {
  var orgName = keypather.get(req, 'githubPushInfo.repoOwnerOrgName.toLowerCase()')
  var logData = {
    tx: true,
    pushInfo: req.githubPushInfo,
    method: 'checkRepoOrganizationAgainstWhitelist'
  }
  var localLog = log.child(logData)
  localLog.info('checking against whitelist')
  UserWhitelist.findOne({ lowerName: orgName }, function (err, record) {
    if (err) {
      localLog.error({ err: err }, 'mongo error')
      return next(err)
    }
    if (!record) {
      localLog.warn('organization not in whitelist')
      return res.status(403).send('Repo owner is not registered on Runnable')
    }
    if (!record.allowed) {
      localLog.warn('organization in whitelist, but is disallowed')
      return res.status(403).send('Your organization has been suspended on Runnable')
    }
    next()
  })
}
...
function autoDeploy (req, res, next) {
  ...
  Promise.map(instances, function (instance) {
    ...
    instanceLog.trace('Creating build for instance')
    return BuildService.createAndBuildContextVersion(instance, req.githubPushInfo, 'autodeploy')
      .tap(function (buildResults) {
        if (instance.isTesting) {
          // Usually we'd do an express request here....
          var instanceUpdate = {
            build: buildResults.build._id,
            contextVersion: buildResults.contextVersion.toJSON()
          }

          // Keep lastBuiltSimpleContextVersion up to date
          if (keypather.get(instance, 'contextVersion.advanced') === false) {
            instanceUpdate.lastBuiltSimpleContextVersion = {
              id: keypather.get(instance, 'contextVersion._id'),
              created: Date.now()
            }
          }
          var buildGithubId = keypather.get(buildResults, 'user.accounts.github.id')
          instanceLog.trace({
            updateParams: instanceUpdate,
            buildGithubId: buildGithubId
          }, 'Updating testing instance with build')
          // Update the build
          return InstanceService.updateInstanceBuild(instance, instanceUpdate, buildGithubId)
            .then(function () {
              instanceLog.trace('Updated instance build, emitting instance update event')
              return instance.emitInstanceUpdateAsync(buildGithubId, 'patch')
            })
        } else {
          instanceLog.trace('Instance is not for testing')
        }
      })
  })
  .then(function (newResults) {
    ...
    log.trace({
      newContextVersions: contextVersionIds
    }, 'Created builds')
    res.status(200)
    res.json(contextVersionIds)
  })
...
module.exports._autoDeploy = autoDeploy
