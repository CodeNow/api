...
var followBranch = flow.series(
  ...
  pollMongo({
    ...
    failureCb: mw.log('failed to build!') // TODO how to handle this error
  }),
  mw.req('instances').each(
    function (instance, req, eachReq, res, next) {
      eachReq.req = req;
  ...
  contextVersions.findByIds('newCVs'),
  // TODO: we need to group instances by ownerId and send notification to each owner
  // skipping that for one. will implement shortly
  mw.log('first instance owner', 'instances[0].owner.github'),
  settings.findSettingsForOwnerGihubId('instances[0].owner.github'),
  mw.log('settings', settings),
  notifications.create('settings.notifications'),
