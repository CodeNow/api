...
var deployments = require('middlewares/notifications').github;
  ...
  mw.headers('x-github-event').matches(/^pull_request$/).then(
    mw.body('action').validate(validations.equals('opened'))
      .else(
        mw.body('action').validate(validations.equals('closed'))
          ...
          .then(
            parseGitHubPRData,
            deployments.create(),
            deployments.model.deletePullRequestComment('githubPushInfo'),
            mw.res.status(201),
      ...
      .then(
        ...
        deployments.create(),
        deployments.model.notifyOnPullRequest('githubPushInfo', 'instances'),
        mw.res.status(201),
...
function followBranch (instancesKey) {
  return flow.series(
    ...
    github.model.createDeploymentStatus('githubPushInfo.repo', 'deploymentId', {state: 'pending'}),
    // RESPOND
    ...
    flow.try(
      ...
      mw.req('deployedInstances.length').validate(validations.notEquals(0))
        .then(
          ...
          github.model.createDeploymentStatus('githubPushInfo.repo', 'deploymentId', {state: 'success'}),
          settings.findOneByGithubId('ownerGithubId'),
          mw.req('setting').require().then(
            notifications.create('setting.notifications'),
