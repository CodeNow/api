...
app.post('/actions/github/',
  ...
  mw.headers('x-github-event').matches(/^pull_request$/).then(
    mw.body('deleted').validate(equals(true))
      .else(
        mw.res.status(202),
        mw.res.send('Deleted the PR no work to be done.')),
    parseGitHubPRData,
    mw.log('githubPushInfo>>', 'githubPushInfo'),
    ...
    mw.req('instances.length').validate(validations.equals(0))
      .then(
        ...
        //newBranch()
        mw.log('no linked instances was found')
      )
      .else(
        // instances following particular branch were found. Redeploy them with the new code
        mw.log('found linked instance')
      ),
    mw.res.status(201),
    mw.res.send('We processed PR event')
  ),
  // handle push events
  mw.headers('x-github-event').matches(/^push$/).then(
    ...
    mw.req('githubPushInfo.commitLog.length').validate(validations.equals(0))
      ...
      .else(
        ...
        notifications.model.notifyOnGitHubPullRequest('githubPushInfo', 'instances'))
  ),
  mw.res.status(202),
