        ...
        mw.body('created').validate(validations.equals(true))
          .then(
            ...
            mw.req('instances.length').validate(validations.equals(0))
              .then(autoDeployOrNewBranch())
              .else(autoFork('instances')))
          .else(autoDeployOrNewBranch())
      )
  ),
  mw.res.status(202),
 ...
 * @return middleware
 */
function autoDeployOrNewBranch () {
  return flow.series(
    ...
    mw.req('instances.length').validate(validations.equals(0))
      ...
      .else(autoDeployBranch('instances'))
    );
}
 ...
 */
function autoDeployBranch (instancesKey) {
  return flow.series(
  ...
  if (req.jsonNewBuild) {
    ...
    runnableClient.forkMasterInstance(instance, buildId,
      ...
      function instanceForkSuccess (forkedInstance) {
        // instance was foked. now we need to check/wait when it was deployed with the build
        onInstanceDeployed(forkedInstance, req, function (err, deployedInstance) {
          if (!err) {
            // report event to the heap. fire and forget
            trackInstanceEvent(req.githubPushInfo, deployedInstance, 'github_hook_autofork');
            sendSlackAutoForkNotification(req.githubPushInfo, deployedInstance);
...
function processBuildComplete (req, onBuildCompletedSuccess) {
  ...
  onBuildCompleted(cvId, req.socketClient, handleBuildCompleted(req, cb));
}
...
/**
 * Call `handler` when instance is running with the new build.
 * If instance has not `containers`  it's no running yet and we need to redeploy it.
 * Also listen for the deploy event
 */
function onInstanceDeployed (instance, req, handler) {
  ...
  if (instance.containers && instance.containers.length > 0) {
    ...
    var runnableClient = new Runnable({}, forkedInstanceCreator);
    var buildId = req.jsonNewBuild._id;
    runnableClient.deployInstanceWithRetry(instance, {
      json: { build: buildId }}, function (err, deployed) {
        if (!err) {
          safeCallback(null, deployed)
        }
      });
  }
  var called = false;
