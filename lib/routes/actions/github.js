  ...
  return flow.series(
    ...
    mw.req('newContextVersionIds').each(
      ...
      mw.req('infoObjects').each(
        ...
        flow.try(
          pullRequest.model.deploymentStarted('githubPushInfo',
            'deploymentId', 'instance.name', 'targetUrl'),
          redeployInstance(),
          pullRequest.model.deploymentSucceeded('githubPushInfo',
            'deploymentId', 'instance.name', 'targetUrl')
        ).catch(
          error.logIfErrMw,
          pullRequest.model.deploymentErrored('githubPushInfo',
            'deploymentId', 'instance.name', 'targetUrl')
        ),
