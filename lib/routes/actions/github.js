...
function noServers () {
  return flow.series(
    ...
    function (req, res, next) {
      var selectionUrl = createServerSelectionUrl(req.contextOwner.username, req.githubPullRequest);
...
function createServerSelectionUrl (owner, gitInfo) {
  return 'https://' + process.env.DOMAIN + '/' + owner + '/serverSelection/' +
    gitInfo.repo + '?branch=' + gitInfo.branch + '&commit=' + gitInfo.commit + '&message=fixme';
}
