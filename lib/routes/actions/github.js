...
function autoDelete () {
  return flow.series(
    ...
    // TODO: fix we should use either git push user or creator as a fallback only
    users.findByGithubId('instances[0].createdBy.github'),
    mw.req('instances').each(
      function (instance, req, eachReq, res, next) {
        var data = instance.toJSON();
        logger.log.info(data, 'create delete-instance job');
        // add more data about user
        rabbitMQ.publish('delete-instance', data);
        next();
      }
    ),
