  ...
  return flow.series(
    ...
    mw.req('instances').each(
      function (instance, req, eachReq, res, next) {
        ...
        eachReq.targetUrl = createTargetUrl(instance, req.instanceCreator.accounts.github.username);
        next();
      },
      github.create({token:'instanceCreator.accounts.github.accessToken'}),
      github.model.createBuildStatus('githubPushInfo.repo', 'githubPushInfo.commit', {state: 'pending', target_url: 'targetUrl'}),
      newContextVersion('contextVersion'), // replaces context version!
      // Note: pushSessionUser has moderator permissions,
      // can only be used for loopback methods that don't require a githubToken
      runnable.create({}, pushSessionUser),
      ...
      mw.req().set('jsonNewBuild', 'runnableResult'),
      github.model.createBuildStatus('githubPushInfo.repo', 'githubPushInfo.commit', {state: 'success', target_url: 'targetUrl'}),

      github.model.createDeployment('githubPushInfo.repo', 'githubPushInfo.commit', {
        instanceIds: 'instance._id.toString()',
      }),
    ...
    flow.try(
      ...
      mw.req('instancesToWaitFor').each(
        function (instance, req, eachReq, res, next) {
          ...
          eachReq.deploymentId  = req.instanceNewInfo[instance._id.toString()].deploymentId;
          eachReq.targetUrl = createTargetUrl(instance, req.instanceCreator.accounts.github.username);
          next();
        },
        github.create({token:'instanceCreator.accounts.github.accessToken'}),
        ...
        heap.model.track('githubPushInfo.user.id', 'github_hook_autodeploy', {
          ...
          instancesName: 'instance.name',
          boxOwnerGithubId: 'instanceCreator.accounts.github.id',
          boxOwnerGithubUsername: 'instanceCreator.accounts.github.username',
