...
var newVar = require('middlewares/new-var');
var eachIter = require('middlewares/each-iter');
...
// TODO: need to revisit timers and timers names
function autoFork (instancesKey) {
  return flow.series(
    createNewInstancesBuilds(instancesKey),
    // send http response & continue in background
    resSendAndNext(201, 'newContextVersionIds'),
    // background
    mw.req('newContextVersionIds').each(
      eachIter('contextVersionId'),
      mw.req().set('infoObjects', 'contextVersionNewInfo[contextVersionId]'),
      newVar.newArray('deployedInstances'),
      ...
      mw.req('infoObjects').each(
        eachIter('infoObject'),
        mw.req().set('instance', 'infoObject.instance'),
        mw.req().set('buildId', 'infoObject.buildId'),
        mw.req().set('targetUrl', 'infoObject.targetUrl'),
        mw.req().set('creatorGithubId', 'instance.createdBy.github'),
        mw.req().set('instanceCreator', 'infoObject.instanceCreator'),
        function (req, res, next) {
          eachReq.timerId = 'github_push_autofork:' + req.instance.shortHash;
          next();
        },
...
function createNewInstanceBuild (instanceKey) {
  return flow.series(
    mw.req().set('instance', instanceKey),
    mw.req().set('contextVersion', 'instance.contextVersion'),
    mw.req().set('creatorGithubId', 'instance.createdBy.github'),
    users.findByGithubId('creatorGithubId'),
...
function createNewInstancesBuilds (instancesKey) {
  return flow.series(
    mw.req().set('instances', instancesKey),

    fetchPullRequests(),
    newVar.newArray('newContextVersionIds'),
    newVar.newObject('contextVersionNewInfo'),
    mw.req('instances').each(
      eachIter('instance'),
      createNewInstanceBuild('instance'),
      function (req, res, next) {
        if (req.jsonNewBuild) {
  ...
  return flow.series(
    ...
    mw.req('newContextVersionIds').each(
      ...
      mw.req('infoObjects').each(
        ...
        newVar.newArray('deploymentsIds'),
        buildCompletedStatus(),
        timers.create(),
        timers.model.startTimer('timerId'),
...
function gitHubDeploymentsSucceeded () {
  return flow.series(
    mw.req('pullRequests').each(
      attachPullRequestInfo,
      pullDeploymentId,
...
function createAndBuildBuild (githubInfoKey) {
  return flow.series(
    mw.req().set('githubInfo', githubInfoKey),
    // Note: pushSessionUser has moderator permissions,
    // can only be used for loopback methods that don't require a githubToken
    ...
    runnable.model.buildBuild('newBuild', { json: {
      triggeredAction: {
        manual: false,
        appCodeVersion: {
...
function forkMasterInstance () {
  return flow.series(
    runnable.create({}, 'instanceCreator'),
    runnable.model.forkMasterInstance('instance',
      'buildId', 'githubPushInfo.user.id', 'githubPushInfo.branch'),
    mw.req().set('forkedInstance', 'runnableResult'),
    mw.log('forked instance', 'forkedInstance'),
    // we must reinstantiate runnable model for each call bc of a bug
    runnable.create({}, 'instanceCreator'),
    runnable.model.waitForInstanceDeployed('forkedInstance.shortHash'),
    mw.req().set('instanceDeployed', 'runnableResult'),
    function (req, res, next) {
      if (req.instanceDeployed) {
        req.deployedInstances.push(req.forkedInstance);
      }
      next();
    }
  );
