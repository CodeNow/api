...
function createSocketClient (orgId) {
      ...
      flow.try(
        ...
        function (req, res, next) {
          if (req.jsonNewBuild) {
            ...
            console.log('new build id', req.jsonNewBuild);
            ...
            console.log('subscribing to', buildCompletedEvent);
...
function sendAndForgetGitHubStatus (req, accessToken, status) {
  ...
  if (status === 'error') {
    ...
    pullRequest.model.buildStarted(req.githubPushInfo, req.targetUrl, noop);
  } else {
...
function createAndStartGitHubDeployments (req, accessToken, status, callback) {
  req.githubPullRequests.forEach(function(githubPullRequest) {
    var prInfo = createPullRequestInfo(githubPullRequest, req.githubPushInfo);
    if (req.creatorGithubId === prInfo.creator.id) {
      var pullRequest = new PullRequest(accessToken);
      pullRequest.createDeployment(prInfo, req.instance.name, {instanceId: req.instance._id.toString()});
    }
  });
