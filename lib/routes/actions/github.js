...
/**
 * Deploy build to the instance but check that autodepploy is still enabled.
 */
function deployBuildToInstance (instanceHash, buildId, runnableUser, cb) {
  var runnableClient = new Runnable({}, runnableUser);
  runnableClient.isAutoDeployEnabled(instanceHash, function (err, enabled) {
    ...
    runnableClient = new Runnable({}, runnableUser);
    runnableClient.updateInstance(instanceHash, payload, cb);
  });
}
...
function processInstanceAutoDeployEvents (req, res, next) {
  if (req.jsonNewBuild) {
    ...
    onBuildCompleted(cvId, req.socketClient,
      handleBuildCompleted(req, instance,
      // build was build successfuly
      function buildCompleted () {
        // deploy new build to the instance
        deployBuildToInstance(instance.shortHash, buildId, req.instanceCreator,
          handleInstanceDeployed(req, instance, 'github_push_autodeploy_',
          function instanceDeploySuccess (deployedInstance) {
            // if deployedInstance is null tha  nothing was deployed.
            // probably autodeploy was disabled in the flight
            if (deployedInstance) {
 ...
 */
function handleBuildCompleted (req, instance, onSuccess) {
...
function sendSlackAutoDeployNotificationIfFinished (githubPushInfo, instances, deployedInstances) {
  ...
  Settings.findOneByGithubId(ownerGitHubId, function (err, setting) {
    if (!err && setting) {
      ...
      slack.notifyOnAutoDeploy(githubPushInfo, deployedInstances, noop);
    }
  });
}
...
function getAccessToken (req) {
  var pushUserToken = keypather.get(req, 'pushUser.accounts.github.accessToken');
  if (pushUserToken) {
    return pushUserToken;
  }
  return keypather.get(req, 'instanceCreator.accounts.github.accessToken');
}
