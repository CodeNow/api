...
function followBranch (instancesKey) {
  return flow.series(
    ...
    users.findByGithubId('ownerGithubId'),
    // session user is needed for getGithubUsername and patchInstance
    mw.req().set('instanceOwner', 'user'),
    function (req, res, next) {
    ...
    mw.req('instances').each(
      function (instance, req, eachReq, res, next) {
        ...
        eachReq.targetUrl = createTargetUrl(instance, req.instanceOwner.accounts.github.username);
        next();
      },
      pullRequest.create('instanceCreator.accounts.github.accessToken'),
      pullRequest.model.buildStarted('githubPushInfo.repo', 'githubPushInfo.commit', 'targetUrl'),
      newContextVersion('contextVersion'), // replaces context version!
      // Note: pushSessionUser has moderator permissions,
      // can only be used for loopback methods that don't require a githubToken
      runnable.create({}, pushSessionUser),
    ...
    mw.req('newContextVersionIds').each(
      function (contextVersionId, req, eachReq, res, next) {
        ...
        var username = req.instanceOwner.accounts.github.username;
      ...
      pollMongo({
        ...
        failureCb: function (failureKeyPathValue, req, res, next) {
          pullRequest.model.buildErrored('githubPushInfo.repo', 'githubPushInfo.commit', 'targetUrl')(req, res, next);
        }
      }),
      pullRequest.model.buildSucceed('githubPushInfo.repo', 'githubPushInfo.commit', 'targetUrl'),
      pullRequest.model.createDeployment('githubPushInfo.repo', 'githubPushInfo.commit', {
        instanceId: 'instance._id.toString()',
      }),
      mw.req().set('deploymentId', 'pullrequestResult.id'),
      flow.try(
        pullRequest.model.deploymentStarted('githubPushInfo.repo', 'deploymentId', 'targetUrl'),
        ...
        pullRequest.model.deploymentSucceed('githubPushInfo.repo', 'deploymentId', 'targetUrl'),
        ...
        heap.model.track('githubPushInfo.user.id', 'github_hook_autodeploy', {
          ...
          boxOwnerGithubId: 'instanceOwner.accounts.github.id',
          boxOwnerGithubUsername: 'instanceOwner.accounts.github.username',
        ...
        error.logIfErrMw,
        pullRequest.model.deploymentErrored('githubPushInfo.repo', 'deploymentId', 'targetUrl')
      )
    ),
    noop
  );
