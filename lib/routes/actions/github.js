...
function autoDelete() {
  return flow.series(
    ...
    users.findByGithubId('instances[0].createdBy.github'), function(req, res, next) {
...
function autoDeploy(instancesKey) {
  return flow.series(
    mw.req().set('allInstances', instancesKey), function(req, res, next) {
  ...
  if (req.jsonNewBuild) {
    ...
    processBuildComplete(req, function buildCompleted() {
      ...
      runnableClient.updateInstance(instance.shortHash, payload,
        handleInstanceDeployed(req, instance, 'github_push_autodeploy_', function instanceDeploySuccess() {
          // finish timer
...
function processInstanceAutoForkEvents(req, res, next) {
  if (req.jsonNewBuild) {
    ...
    runnableClient.forkMasterInstance(instance, buildId,
      ...
      handleInstanceDeployed(req, instance, 'github_push_autofork_', function instanceForkSuccess(forkedInstance) {
        // instance was forked. now we need to check/wait when it was deployed with the build
        req.socketClient.onInstanceDeployed(forkedInstance, buildId, function(err, deployedInstance) {
          // finish timer
