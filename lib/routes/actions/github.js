...
function parseGitHubPushData (req, res, next) {
  ...
  // Repo owner username can be in the `login` or `name` property depending if
  // its an org or not
...
function checkCommitterIsRunnableUser () {
  function respondCommitterNotRunnableUserError (req, res) {
    var committerNotRunnableUserError = Boom.forbidden('Commit author/committer is not a Runnable user', {
      repo: req.githubPushInfo.repo,
      repoName: req.githubPushInfo.repoName,
      committer: req.githubPushInfo.committer,
  ...
  return flow.series(
    function (req, res, next) {
      log.info(req.githubPushInfo, 'checkCommitterIsRunnableUser')
      next()
    },
    users.findOneByGithubUsername('githubPushInfo.committer'),
    // if the committer isn't in a Runnable user
    flow.mwIf(checkFound('user')).else(respondCommitterNotRunnableUserError)
  )
}
