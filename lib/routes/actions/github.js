...
function reportMixpanelUserPush () {
  return flow.series(
    ...
    // only report to mixpanel if push-user is a user of our platform
    mw.req('pushUser').validate().then(
      mixpanel.create('pushUser'),
 ...
 * Auto-build will be triggered if instance auto-build not disabled
 * Report auto-build triggered to Mixpanel
  ...
  return flow.series(
    ...
    mw.req('instances').each(
      ...
      flow.try(
        ...
        /**
         * If build was successful, organize instance & build models/data
         * onto infoObject for use later in route
         */
        function (req, res, next) {
          if (req.jsonNewBuild) {
     ...
     * Background tasks
     *
     * Poll mongodb until build has finished building, then update PR status
     * with Github Status API
     */
...
/**
 * If Github pull request creator matches instance creator ID
 * call github comments API to update PR status
 * @return {Function} - middleware
 */
...
/**
 * Initiates internal API request to create & build a build
