...
function listenOnBuildStarted (req, res, next) {
...
function processInstanceEvents (req, res, next) {
  if (req.jsonNewBuild) {
    ...
    req.newContextVersionIds.push(req.jsonNewBuild.contextVersions[0]);
    req.socketClient.addHandler(buildCompletedEvent, function (contextVersion) {
      if (contextVersion.build.error) {
        ...
        pullRequest.createAndStartDeployment(githubPushInfo, instance.name, payload, targetUrl, function (err, deployment) {
          if (err) {
          ...
          // TODO this should be `deploy` and not `patch`. But somehow we receive deploy only occasionally
