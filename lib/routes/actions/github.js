...
function followBranch (instancesKey) {
  return flow.series(
    ...
    githubNotifications.create('instanceCreator.accounts.github.accessToken'),
    githubNotifications.model.createDeployment('githubPushInfo', {
      instanceIds: 'instanceIds',
      newContextVersionIds: 'newContextVersionIds'
    }),
    mw.log('deployment', 'githubNotificationsResult'),
    mw.req().set('deploymentId', 'githubNotificationsResult.description.id'),
    mw.log('deployment id', 'deploymentId'),
    githubNotifications.model.createDeploymentStatus('githubPushInfo', 'deploymentId', 'pending'),
    // RESPOND
    ...
    flow.try(
      ...
      mw.req('deployedInstances.length').validate(validations.notEquals(0))
        .then(
          ...
          githubNotifications.model.createDeploymentStatus('githubPushInfo', 'deploymentId', 'success'),
          settings.findOneByGithubId('ownerGithubId'),
          mw.req('setting').require().then(
            notifications.create('setting.notifications'),
