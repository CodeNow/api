      ...
      flow.try(
        ...
        function (req, res, next) {
          if (req.jsonNewBuild) {
            ...
            console.log('new build', req.jsonNewBuild);

            // listen on build build event
            ...
            req.socketClient.addHandler(buildCompletedEvent, function (contextVersion) {
              console.log('finished building', contextVersion);
              var accessToken = req.instanceCreator.accounts.github.accessToken;
              if (contextVersion.build.error) {
                // if error send github statuses
                sendAndForgetGitHubStatus(req, accessToken, 'error');
              } else {
                // build was successful
                // 1. update github statuses
                sendAndForgetGitHubStatus(req, accessToken, 'success');
                // 2. send start deployment github request
                createAndStartGitHubDeployments(req, accessToken, function (err, deployment) {
                  console.log('github deployments result', err, deployment);
                  if (err) {
                ...
                // 3. deploy it to the instance
                var runnableClient = new Runnable({}, req.instanceCreator);
                var buildId = req.jsonNewBuild._id;
                console.log('updating instance', instance.shortHash, 'with build', buildId);
                runnableClient.updateInstance(instance.shortHash, {
                  json: {build: buildId}
                }, function (err, result) {
                  // is deployment failed if this call failed????
                  console.log('updated instance result', err, result);
                });
              }

            });

            // listen on instance deploy event
            var instanceDeployedEvent = ['INSTANCE_UPDATE', 'patch', instance.shortHash].join(':');
            req.socketClient.addHandler(instanceDeployedEvent, function (instance) {
              console.log('finished deploy', instance);
              var instanceStatus = req.deployments[instance.shortHash];
              if (instanceStatus) {
                var pullRequest = new PullRequest(accessToken);
                pullRequest.deploymentSucceeded(req.githubPushInfo, instanceStatus.deploymentId,
                  req.instance.name, req.targetUrl, noop);
              }
            });
...
function sendAndForgetGitHubStatus (req, accessToken, status) {
  var pullRequest = new PullRequest(accessToken);
  if (status === 'error') {
    pullRequest.buildErrored(req.githubPushInfo, req.targetUrl, noop);
  } else if (status === 'pending'){
    pullRequest.buildStarted(req.githubPushInfo, req.targetUrl, noop);
  } else {
    pullRequest.buildSucceeded(req.githubPushInfo, req.targetUrl, noop);
  }
}
// TODO investigate problem with multiple deployments.
// we need one callback here?
function createAndStartGitHubDeployments (req, accessToken, callback) {
  var pullRequest = new PullRequest(accessToken);
  var payload = {
    instanceId: req.instance._id.toString()
  };
  pullRequest.createAndStartDeployment(req.githubPushInfo, req.instance.name,
    payload, req.targetUrl, callback);
}


function buildStartedStatus (req, res, next) {
  var accessToken = keypather.get(req, 'instanceCreator.accounts.github.accessToken');
  sendAndForgetGitHubStatus(req, accessToken, 'pending');
  next();
}
