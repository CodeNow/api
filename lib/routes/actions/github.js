...
function checkCommiterIsRunnableUser () {
  function respondCommiterNotRunnableUserError (req, res) {
    var commiterNotRunnableUserError = Boom.forbidden('Commit author/commiter is not a Runnable user', {
      ...
      commiter: req.githubPushInfo.commiter,
      repoOwnerOrgName: req.githubPushInfo.repoOwnerOrgName
    })
    log.error({
      tx: true,
      err: commiterNotRunnableUserError
    }, 'Commit author/commiter is not a Runnable user')
    return res.status(403).send('Commit author/commiter is not a Runnable user')
  }

  return flow.series(
    function (req, res, next) {
      log.info(req.githubPushInfo, 'checkCommiterIsRunnableUser')
      next()
    },
