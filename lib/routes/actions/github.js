...
function followBranch (instancesKey) {
  return flow.series(
    ...
    mw.req('instances').each(
      initInstancesIter,
      populateInstanceOwnerAndCreator(),
      flow.try(
 ...
 */
function populateInstanceOwnerAndCreator () {
  return flow.series(
    users.findByGithubId('creatorGithubId'),
...
function processInstanceAutoDeployEvents (req, res, next) {
  if (req.jsonNewBuild) {
    ...
    onBuildCompleted(cvId, req.socketClient, function (contextVersion) {
      if (contextVersion.build.error) {
        ...
        pullRequest.createAndStartDeployment(githubPushInfo, serverName, targetUrl,
            ...
            runnableClient.updateInstance(instance.shortHash, payload, function (err) {
              ...
              if (err) {
                ...
                if (req.instances.length === req.deployedInstances.length) {
                  sendSlackAutoDeployNotification(githubPushInfo, req.deployedInstances);
                }
 ...
 * 1. handle when new build was created and completed
 * 2. handle when instance was created form the master
 */
      ...
      if (contextVersion.build.error) {
        ...
        pullRequest.createAndStartDeployment(githubPushInfo, serverName, targetUrl,
          function (err, deployment) {
            ...
            // 4. deploy it to the instance
            var runnableClient = new Runnable({}, req.instanceCreator);
            runnableClient.forkMasterInstance(instance, buildId, githubPushInfo.user.id,
              githubPushInfo.branch, function (err) {
                var deploymentId = req.deployments[instance.shortHash].deploymentId;
                if (err) {
                  pullRequest.deploymentErrored(req.githubPushInfo, deploymentId,
                    serverName, targetUrl);
 ...
 */
function sendSlackAutoDeployNotification (githubPushInfo, deployedInstances) {
