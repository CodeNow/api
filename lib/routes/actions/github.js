...
var analytics = require('middlewares/analytics').index;
...
app.post('/actions/github/',
  ...
  mw.headers('x-github-event').matches(/^push$/).then(
    ...
    parseGitHubPushData,
    analytics.create(),
    analytics.model.track('githubPushInfo.user.id', 'github_hook_start', {
      repo: 'githubPushInfo.repo',
      branch: 'githubPushInfo.branch',
      commit: 'githubPushInfo.headCommit.id',
      githubUsername: 'githubPushInfo.user.id'
    }),
  ...
  return flow.series(
    ...
    flow.try(
      ...
      mw.req('deployedInstances.length').validate(validations.notEquals(0))
        .then(
          function (req, res, next) {
            req.instancesNames = req.deployedInstances.map(pick('name'));
            next();
          },
          analytics.create(),
          analytics.model.track('githubPushInfo.user.id', 'github_hook_autodepoy', {
            repo: 'githubPushInfo.repo',
            branch: 'githubPushInfo.branch',
            commit: 'githubPushInfo.headCommit.id',
            githubUsername: 'githubPushInfo.user.id',
