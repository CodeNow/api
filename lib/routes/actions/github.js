...
function newContextVersion () {
  return flow.series(
    timers.create(),
    timers.model.startTimer('actions.github.newContextVersion'),
    // Create new Context Version
    contexts.findOne({ _id: 'contextVersion.context' }),
    timers.model.startTimer('actions.github.newContextVersion-deepCopy'),
    ...
    timers.model.stopTimer('actions.github.newContextVersion-deepCopy'),
    timers.model.startTimer('actions.github.newContextVersion-modifyACV'),
    ...
    timers.model.stopTimer('actions.github.newContextVersion-modifyACV'),
    timers.model.stopTimer('actions.github.newContextVersion')
...
function autoDeploy (instancesKey) {
  return flow.series(
    ...
    timers.create(),
    timers.model.startTimer('actions.github.autodeploy'),
    function (req, res, next) {
    ...
    timers.model.stopTimer('actions.github.autodeploy'),
...
function autoFork (instancesKey) {
  return flow.series(
    ...
    timers.create(),
    timers.model.startTimer('actions.github.autofork'),
    socketClient.createSocketClient('instances[0].owner.github'),
    // init vars that should be used inside `each` loop
    initVars,
    mw.req('instances').each(
    ...
    timers.model.stopTimer('actions.github.autofork'),
    mw.res.json('newContextVersionIds')
  )
}
