...
// FIXME: TJ hates this with a firey passion... get rid of it.
app.patch('/:id',
  function (req, res, next) {
    if (process.env.NODE_ENV !== 'test') {
      next(mw.Boom.forbidden('ONLY AVAILABLE FOR TESTING'));
    }
    next();
  },
  findVersion,
  function (req, res, next) {
    req.version.files.push(req.body.file);
    req.version.save(next);
  },
  mw.res.send('version'));
