...
app.get('/billing/plan/',
  ...
  function (req, res, next) {
    log.trace({ organizationId: req.body.organizationId }, 'plan route')
    CreamAPI.getPlanForOrganization(req.body.organizationId)
    .tap(function (body) {
...
app.get('/billing/invoices/',
  flow.mwIf(or(me.isUser, me.isModerator)),
  function (req, res, next) {
    log.trace({ organizationId: req.body.organizationId }, 'invoices route')
    CreamAPI.getInvoicesForOrganization(req.body.organizationId)
    .tap(function (body) {
      log.trace({ body: body }, 'final http response')
      req.creamResponse = body
    })
...
app.get('/billing/payment-method/',
  flow.mwIf(or(me.isUser, me.isModerator)),
  function (req, res, next) {
    log.trace({ organizationId: req.body.organizaitonId }, 'payment-method route')
    // TODO: Only show payment method to owner
    CreamAPI.getPaymentMethodForOrganization(req.body.organizationId)
    .tap(function (body) {
      log.trace({ body: body }, 'final http response')
      req.creamResponse = body
    })
...
app.post('/billing/payment-method/',
  ...
  function (req, res, next) {
    ...
    .then(users => {
      ...
      return CreamAPI.postPaymentMethodForOrganization(req.body.organizationId, stripeToken, userId)
    })
    .tap(function (body) {
      log.trace({ body: body }, 'final http response')
      req.creamResponse = body
