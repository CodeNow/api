...
app.post('/contexts/:contextId/versions/:versionId/appCodeVersions/:appCodeVersionId/actions/applyTransformRules',
  ...
  function applyTransformRules(req, res, next) {
    ...
    var rules = [];
    if (exclude.length > 0) {
      var excludeFiles = exclude.concat(['translation_rules.sh']);
      rules.push({ action: 'exclude', files: excludeFiles });
    }
    else {
      rules.push({ action: 'exclude', files: ['translation_rules.sh'] });
    }
    ...
    optimus.transform(
      ...
      handleOptimusError(next, function (err, response) {
        var fullpath = '/translation_rules.sh';
        console.log('\n\n\n--------  OPTIMUS SCRIPT --------\n');
        console.log(response.body.script);
        console.log('\n\n\n');
