...
/** Call out to optimus to get the results of the transformation rules.
*  @param {ObjectId} contextId ID of the {@link module:models/context Context}
*  @param {ObjectId} versionId ID of the {@link module:models/version Version}
*  @param {ObjectId} id ID of the appCodeVersion
*  @event POST rest/contexts/:contextId/versions/:id/appCodeVersions
*  @memberof module:rest/contexts/versions/app-code-versions/actions/applyTransformRules */
app.post('/contexts/:contextId/versions/:versionId/appCodeVersions/:appCodeVersionId/actions/applyTransformRules',
  ...
  function (req, res) {
    ...
    // TODO This may change in the future, assuming only one app code version
    //      for now.
    var appCodeVersion = contextVersion.appCodeVersion[0];
    var rules = ;

    var optimusParams = {
      repo: appCodeVersion.repo,
      commitish: appCodeVersion.commit,
      rules: appCodeVersion.transformRules
    };

    optimus.transform(optimusParams, function (err, response) {
      if (err) {
