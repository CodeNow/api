...
app.post('/contexts/:contextId/versions/:versionId/appCodeVersions/:appCodeVersionId/actions/testTransformRule',
  ...
  function (req, res) {
    ...
    if (testRule.action === 'replace') {
      testIndex = (exclude.length > 0 ? 1 : 0) + replace.length;
      replace.push(testRule);
    }
    else if (testRule.action === 'rename') {
      testIndex = (exclude.length > 0 ? 1 : 0) + replace.length + rename.length;
    ...
    optimus.transform(
      ...
      function (err, response) {
        if (err) { return res.status(500).send(err); }
        return res.status(200).send(response.body.results[testIndex]);
      }
    );
  });
