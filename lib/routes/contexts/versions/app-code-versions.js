...
app.post('/contexts/:contextId/versions/:versionId/appCodeVersions/:appCodeVersionId/actions/testTransformRule',
  ...
  function (req, res) {
    ...
    // that point.
    if (testRule._id) {
      var oldRule = ruleCollection.find(function (rule) {
        return rule._id === testRule._id;
      });
      if (!oldRule) {
        return res.status(400).send(new Error(
      ...
      ruleCollection = ruleCollection.slice(0, replace.indexOf(oldRule));
    }

    // Add the rule to the end of the collection and push the rules into the
    // final rule list
    ruleCollection.push(testRule);
    ruleCollection.forEach(function (rule) { rules.push(rule); });
    ...
    optimus.transform(
      ...
      function (err, response) {
        if (err) { return res.status(500).send(err); }
        return res.status(200).send(last(response.body.results));
      }
    );
  });
