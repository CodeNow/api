...
var findInfraCodeVersion = flow.series(
  infraCodeVersions.findById('version.infraCodeVersion'),
...
app.get('/:contextId/versions/:id/files',
  ...
  mw.res.json(200, 'infraCodeVersion.listFiles(query.prefix)'));
...
app.patch('/:contextId/versions/:id/files/:key*',
  ...
  findVersion,
  findInfraCodeVersion,
  mw.body('Key', 'Prefix', 'Body').pick(),
  mw.body({ or: ['Key', 'Prefix', 'Body'] }).require(),
  flow.if(mw.body({ or: ['Key', 'Prefix'] }).require())
    .then(
