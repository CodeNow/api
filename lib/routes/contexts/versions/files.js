...
app.get('/contexts/:contextId/versions/:id/files/:fullpath*',
  ...
  function (req, res, next) {
    ...
    // NOTE: Never used to fetch a directory
    req.infraCodeVersion.findFile(fullpath, function (err, fileStream) {
      if (err) {
        next(err)
      } else if (!fileStream) {
...
app.patch('/contexts/:contextId/versions/:id/files/:fullpath*',
  ...
  mw.body('body').require()
    .then(
      mw.body('body').string(),
      function (req, res, next) {
        var fullpath = keypather.get(req, 'params.fullpath')
        var body = keypather.get(req, 'body.body')
        req.infraCodeVersion.updateFile(fullpath, body, next)
      },
...
app.delete('/contexts/:contextId/versions/:id/files/:fullpath*',
  ...
  function (req, res, next) {
    var fullpath = keypather.get(req, 'params.fullpath')
    req.infraCodeVersion.removeFsAsync(fullpath)
    .asCallback(function (err) {
      next(err)
    })
  },
