...
app.get('/:contextId/versions/:id/files',
  ...
  function (req, res, next) {
    req.infraCodeVersion.getFile(req.query.path, function (err, fs) {
      if (err) {
        next(err);
      }
      else if (!fs.isDir) {
        next(Boom.badRequest('Cannot create a file in a file'));
      }
      else {
        next();
      }
...
app.post('/:contextId/versions/:id/files',
  ...
  function (req, res, next) {
    req.infraCodeVersion.getFile(req.body.path, function (err, fs) {
      if (err) {
        next(err);
      }
      else if (!fs.isDir) {
