...
app.post('/contexts/:contextId/versions/:id/actions/build',
  ...
  mw.req('contextVersion.build.started').require()
    ...
    .else( // build NOT started,
      ...
      contextVersions.model.dedupeBuild(),
      mw.req('contextVersion.build.dupeFound').validate(validations.equals(true))
        ...
        .else(
          flow.try(
            ...
            // FIXME: get rid of these loops put it on mongo
            contextVersions.models.setBuildCompleted('dockerResult', 'container.Id'),
            noop // done.
          ).catch(
            mw.req().setToErr('err'),
            contextVersions.findBy('build._id', 'contextVersion.build._id'),
            function (req, res, next) {
              async.each(req.contextVersions, function (cv, cb) {
                // FIXME: get rid of these loops put it on mongo
                if (req.container) {
                  cv.updateBuildError(req.err, req.container.Id, cb);
                }
                else {
                  cv.updateBuildError(req.err, cb);
                }
