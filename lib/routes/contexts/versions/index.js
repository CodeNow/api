...
app.post('/contexts/:contextId/versions/:id/actions/build',
  ...
  mw.req('contextVersion.build.started').require()
    ...
    .else( // no dupe found, build NOT started yet
      ...
      contextVersions.model.setBuildStarted('sessionUser', 'body'),
...
app.patch('/contexts/:contextId/versions/:id',
  ...
  mw.body({
    or: [ 'advanced', 'dockRemovedNeedsUserConfirmation' ]
  }).require().pick(),
  mw.body('advanced').require()
    .then(mw.body('advanced').boolean()),
  mw.body('dockRemovedNeedsUserConfirmation').require()
    .then(mw.body('dockRemovedNeedsUserConfirmation').boolean()),
