    ...
    .then(
      ...
      mw('infraCodeVersion')('context.toString()')
        ...
        .then(
          ...
          contextVersions.findOneAndUpdate('contextVersion._id', {
            $set: { infraCodeVersion: 'infraCodeVersion._id' }
          })
        )
        .else(
...
app.post('/:contextId/versions/:id/actions/build',
  findContext,
  contextVersions.findById('params.id'),
  checkFound('contextVersion'),
  ...
  flow.try(
    ...
    function (req, res, next) {
      ...
      if (req.build) {
        req.build.pushErroredContextVersion(req.contextVersion._id,
          logIfErrAndNext(count.next.bind(count)));
      }
    }
  ),
