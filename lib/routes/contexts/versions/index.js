...
app.patch('/contexts/:contextId/versions/:id',
  ...
  function (req, res, next) {
    Instance.updateContextVersionAsync(req.params.id, req.body)
    .asCallback(next)
  },
...
app.put('/contexts/:contextId/versions/:versionId/infraCodeVersion/actions/copy',
  ...
  function (req, req, next) {
    req.infraCodeVersion.removeSourceDirAsync(icv)
    .then(function () {
      icv.copyFilesFromSourceAsync(keypather.get(req, 'query.sourceInfraCodeVersion'))
      .then(function (icv) {
        icv.updateByIdAsync({
          $set: {
