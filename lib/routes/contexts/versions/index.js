...
app.post('/contexts/:contextId/versions/:id/actions/copy',
  ...
  mw.req('contextVersion.owner.github').require()
...
function deepCopyCvFromHelloRunnable (cvKey) {
  return flow.series(
    ...
    runnable.createContext('body'),
    mw.req().set('context', 'runnableResult'),
    // make a new context-version
    runnable.create({}, 'sessionUser'),
    runnable.createContextVersion('context._id'),
        ...
        .else(
          flow.try(
            ...
            function (req, res, next) {
              flow.series(
                contextVersions.findBy('build._id', 'contextVersion.build._id'),
                contextVersions.models.updateBuildError('err')
              )(req, res, function (err) {
                error.logIfErr(err);
                // don't next error, might be done below
                next();
              });
            },
