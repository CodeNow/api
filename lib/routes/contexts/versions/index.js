...
app.get('/contexts/:contextId/versions',
  ...
  mw.query('appCodeVersions').require()
    .then(
      ...
      mw.query('appCodeVersions').each(function(item, req, eachReq, res, next) {
        eachReq.appCodeVersion = item; next();
      },
...
app.post('/contexts/:contextId/versions/:id/actions/build',
  ...
  mw.req('contextVersion.build.started').require()
    .then( // dupe found
      logger(['contextVersion'], 'ROUTE: contextVersion.build.started', 'trace'), function(req, res, next) {
    ...
    .else( // no dupe found, build NOT started yet
      ...
      mw.req('contextVersion.build._id.toString()')
        ...
        .else(
          contextVersions.model.populateOwner('sessionUser'), function(req, res, next) {
...
app.delete('/contexts/:contextId/versions/:id', function(req, res) {
  res.send(405);
});
...
app.put('/contexts/:contextId/versions/:versionId/infraCodeVersion/actions/copy',
  ...
  mw.query('sourceInfraCodeVersion').require().validate(isObjectId), function(req, res, next) {
    var InfraCodeVersion = require('models/mongo/infra-code-version');
    InfraCodeVersion.findById(req.query.sourceInfraCodeVersion, function(err, infraCodeVersion) {
      if (err) {
        next(err);
