...
app.post('/contexts/:contextId/versions/:id/actions/copy',
  mw.log('copy cv step0'),
  findContext,
  mw.log('copy cv step1'),
  contextVersions.findById('params.id'),
  mw.log('copy cv step2'),
  checkFound('contextVersion'),
  mw.log('copy cv step3'),
  // only supports deep copy for now
  mw.query('deep').require().validate(validations.equals('true')),
  mw.log('copy cv step4', 'sessionUser', 'contextVersion'),
  contextVersions.createDeepCopy('sessionUser', 'contextVersion'),
  mw.log('copy cv step5', 'contextVersion'),
  mw.res.status(201),
  mw.res.json('contextVersion'));
...
app.post('/contexts/:contextId/versions/:id/actions/build',
  isInternalRequest, // used internally, session user may not be real (github hook)
  findContext,
  contextVersions.findById('params.id'),
  checkFound('contextVersion'),
  ...
  mw.log('building cv. start', 'contextVersion.build'),
  // mw.req('contextVersion.build.started').require()
  //   .else(
  //     mw.next(Boom.conflict('cannot build a context version that is already built'))),
  mw.req().set('origContextVersion', 'contextVersion'),
  mw.log('building cv'),
  contextVersions.model.dedupe(),
  mw.log('building cv. deduped', 'contextVersion.build.started'),
  mw.req('contextVersion.build.started').require()
