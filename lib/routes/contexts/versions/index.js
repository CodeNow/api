...
var findContext = flow.series(
  ...
  flow.or(
    ...
    mw.req('context.public').validate(validations.equals(true)),
...
app.post('/contexts/:contextId/versions/:id/actions/copy',
  ...
  flow.mwIf(flow.or(
    mw.req('context.public').validate(validations.equals(false)),
    me.isOwnerOf('context')))
    .then(
    contextVersions.createDeepCopy('sessionUser', 'contextVersion')
  ).else(
    contextVersions.createDeepCopyForNewUser('sessionUser', 'contextVersion')),
  mw.res.status(201),
  mw.res.json('contextVersion'));
