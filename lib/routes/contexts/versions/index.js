...
app.get('/contexts/:contextId/versions',
  ...
  mw.query('build').require()
    .then(
      mw.query('build.completed').mapValues(transformations.toBool),
      mw.query('build.triggeredAction.manual').mapValues(transformations.toBool),
      mw.query('build.started').mapValues(transformations.toBool),
      function (req, res, next) {
        var buildCompletedQuery = contextVersion
          .generateExistsQuery(req.query, ['build.completed', 'build.started'])
        if (Object.keys(req.query.build).length === 0) {
          delete req.query.build
        }
        assign(req.query, buildCompletedQuery)
        next()
      }),
