...
//var logInfoMiddleware = require('log-info-middleware')('routes:contexts:versions:index');
...
var sauron = middlewarize(require('models/apis/sauron'));
...
app.get('/contexts/:contextId/versions',
  ...
  mw.query('notId', 'limit', 'sort', 'advanced', 'buildCompleted', 'appCodeVersions', 'infraCodeVersion').pick(),
  ...
  mw.query('sort').require()
    ...
    mw.query('sort').validate(validations.equalsAny(
      'started', '-started', 'created', '-created', '-build.started')),
    mw.req().set('opts.sort', 'query.sort'),
    mw.query().unset('sort')),
  mw.query('notId').require()
    .then(
    mw.query('notId').validate(isObjectId),
    function (req, res, next) {
      req.query._id = {$ne: req.query.notId};
      next();
    },
    mw.query().unset('notId')),
  mw.query('advanced').require()
    .then(mw.query('advanced').mapValues(transformations.toBoolean)),
  mw.query('buildCompleted').require()
    .then(
    mw.query('buildCompleted').mapValues(transformations.boolToExistsQuery),
    mw.query().unset('buildCompleted')),
      ...
      mw.req('contextVersion.build._id.toString()')
        ...
        .else(
          flow.try(
            ...
            mw.req().set('dockerTag', 'docker.getDockerTag(sessionUser, contextVersion)'),
            sauron.new('mavisResult'),
            sauron.instance.findOrCreateHostForContext('context', 'cb'),
            docker.model.createImageBuilder(
              'sessionUser',
              'contextVersion', 'dockerTag',
              'sauronResult', 'body.noCache'),
            mw.req().set('container', 'dockerResult'),
            contextVersions.updateContainerByBuildId({
            ...
            docker.model.startImageBuilderContainer('container')
          ).catch(
            mw.req().setToErr('err'),
            mw.req('sauron').require().then(
              sauron.instance.deleteHost('sauronResult.networkIp', 'sauronResult.hostIp', 'cb')),
            contextVersions.updateBuildErrorByBuildId(
              'contextVersion.build._id', 'err'),
            mw.next('err') // skips non error mw below
          ),
          // create and start was successful, respond cv
          mw.req().set('json', 'contextVersion.toJSON()'),
          // unpopulate infraCode
          mw.req().set('json.infraCodeVersion', 'json.infraCodeVersion._id.toString()'),
