...
app.post('/workers/on-instance-container-create',
  ...
  logger(['body'], 'POST_WORKERS_ON_INSTANCE_CONTAINER_CREATE', 'info'),
  isInternalRequest,
  ...
  logger(reqLogKeypaths, 'update instance start', 'info'),
  runnable.model.updateInstance('instanceShortHash', 'updateOpts', 'cb').async('instance'),
  logger(reqLogKeypaths, 'update instance finish, startContainer start', 'info'),
  runnable.model.startInstance('instanceShortHash', 'cb').async(),
  logger(reqLogKeypaths, 'startContainer finish', 'info'),
  mw.res.json(200, {})
);
