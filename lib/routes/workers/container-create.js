...
var logInfoMiddleware = require('middlewares/log-info-middleware')(
  'routes:workers:container-create'
);
...
var continuationLocalStorage = require('middlewares/continuation-local-storage');
...
app.post('/workers/container-create',
  ...
  function (req, res, next) {
    var namespace = require('middlewares/continuation-local-storage').namespace;
    console.log('top', namespace.get('tid'));
    next();
  },
  ...
  logInfoMiddleware(['instanceShortHash', 'instance'], 'worker update instance finish'),


  logInfoMiddleware(['instanceShortHash'], 'worker start instance start'),
  runnable.model.startInstance('instanceShortHash', 'cb').async(),
  logInfoMiddleware(['instanceShortHash'], 'worker start instance finish'),

  function (req, res, next) {
    var namespace = require('middlewares/continuation-local-storage').namespace;
    console.log('final', namespace.get('tid'));
    next();
  },
