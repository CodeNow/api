...
app.post('/workers/container-create',
  ...
  mw.req().set('userGithubId', 'body.inspectData.Config.Labels.userGithubId'),
  mw.req().set('dockerContainer', 'body.id'),
  mw.req().set('dockerHost', 'body.host'),
  containerCreateValidationFlow,

  // could use tags to pass entire strigified sessionUser object?
  user.findByGithubId('userGithubId').exec('sessionUser'),
  mw.req('sessionUser._id').require(), //validate(validations.isObjectId()),
  function (req, res, next) {
    //User.findByGithubId.bind(User, '----githubid', function () {
      // set to sessionUser
    //});
    //req.sessionUser = {};
    next();
  ...
  runnable.model.deployInstance('instance', 'cb').async(),
  mw.res.json(200, {})
);
