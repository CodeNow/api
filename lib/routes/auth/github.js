...
function enforceAuthWhitelist() {
  return !!process.env.ENABLE_USER_WHITELIST;
}
...
app.get('/auth/github',
  mw.query('redirect').require()
    ...
    .else(function(req, res, next) {
      ...
      req.session.authCallbackRedirect = reqUtils.getProtocol(req) + process.env.DOMAIN;
      log.trace({
        tx: true,
  ...
  mw.query('requiresToken').require()
    .then(
      ...
      mw.query('forceLogin').require()
        .else(
          token.createWithSessionCookie('session', 'headers.cookie', 'cb'), function(req, res) {
            log.trace({
              tx: true,
              authCallbackRedirect: req.session.authCallbackRedirect
            }, 'redirecting');
