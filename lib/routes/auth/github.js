...
function logoutWhitelistFalse (req, res) {
  if (process.env.ENABLE_AUTO_WHITELISTING_FEATURE) {
    return
  }
...
function validateAgainstWhitelist () {
  return flow.series(
    ...
    logger([ 'orgIds', 'userId' ], 'validateAgainstWhitelist', 'trace'),
    ...
    logger([ 'userOrgs' ], 'validateAgainstWhitelist userOrgs', 'trace'),
    userWhitelist.findOne({
      lowerName: { $in: 'userOrgs' }
    }),
    logger([ 'userwhitelist' ], 'validateAgainstWhitelist userWhitelist', 'trace'),
