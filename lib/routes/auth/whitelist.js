...
app.patch('/auth/whitelist/:id',
  ...
  //mw.body('metadata.hasConfirmedSetup').mapValues(transformations.toBool),
  //mw.body('metadata.hasAha').mapValues(transformations.toBool),
  function (req, res, next) {
    return Promise
      .try(function () {
        const bigPoppaId = (keypather.get(req, 'params.id'))
        const metadata = pick(req.body.metadata, ['hasConfirmedSetup', 'hasAha'])
