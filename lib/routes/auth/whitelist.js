...
var OrganizationService = require('models/services/user-service')
...
app.get('/auth/whitelist/',
  function (req, res, next) {
    var githubId = keypather.get(req, 'sessionUser.accounts.github.githubId')
    var accessToken = keypather.get(req, 'sessionUser.accounts.github.accessToken')
    UserService.getByGithubId(githubId)
    .tap(function (user) {
      return Promise.map(user.orgs, function (org) {
        return OrganizationService.getByGithubIdWithGithubOrg(org.githubId, accessToken)
      })
    })
    .tap(function (orgs) {
      req.userwhitelists = orgs
    })
...
app.post('/auth/whitelist',
  function (req, res, next) {
    ...
    github.getUserByUsernameAsync(orgName)
      .then(function (githubOrg) {
        return OrganizationService.create(githubOrg.id)
      })
      .then(function () {
...
app.delete('/auth/whitelist/:name',
  ...
  function (req, res, next) {
    var name = keypather.get(req, 'params.name.toLowerCase()')
    var accessToken = keypather.get(req, 'sessionUser.accounts.github.accessToken')
    return OrganizationService.getByGithubUsername(name, accessToken)
      .tap(function (userwhitelist) {
        req.userwhitelist = userwhitelist
      })
      .asCallback(function (err) {
  ...
  checkFound('userwhitelist'),
  function (req, res, next) {
    var orgGithubId = keypather(req, 'userWhitelist.github_id')
    return OrganizationService.delete(orgGithubId)
  },
  mw.res.send(202))
