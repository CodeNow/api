...
/**
 * Checks if instance container has a Starting or Stopping property
 * and if set, return error
 */
function checkIfStartingOrStopping (req, res, next) {
  var State = keypather.get(req, 'instance.container.inspect.State');
  if (State.Starting) {
    return next(Boom.badRequest('Instance is already starting'));
  }
  if (State.Stopping) {
    return next(Boom.badRequest('Instance is already stopping'));
  }
  next();
}
...
app.put('/instances/:id/actions/start',
  ...
  checkIfStartingOrStopping,
  // get owner username
  mw.req().set('user', 'sessionUser'),
  users.model.findGithubUserByGithubId('instance.owner.github'),
  checkFound('user', 'Owner not found'),
  mw.req().set('ownerUsername', 'user.login'),
  // set state starting and notify frontend
  instances.findByIdAndUpdate('instance._id', {$set: {'container.inspect.State.Starting': true}}),
  instances.model.populateOwnerAndCreatedBy('sessionUser'),
  ...
  .catch(
    mw.req().setToErr('err'),
    instances.model.modifyContainerInspectStartErr(),
    instances.model.populateOwnerAndCreatedBy('sessionUser'),
...
app.put('/instances/:id/actions/stop',
  ...
  checkFound('user', 'Owner not found'),
  mw.req().set('ownerUsername', 'user.login'),
  // set state stopping and notify frontend
  instances.findByIdAndUpdate('instance._id', {$set: {'container.inspect.State.Stopping': true}}),
  instances.model.populateOwnerAndCreatedBy('sessionUser'),
  ...
  .catch(
    mw.req().setToErr('err'),
    instances.model.modifyContainerInspectStopErr(),
    instances.model.populateOwnerAndCreatedBy('sessionUser'),
