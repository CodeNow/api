...
app.put('/instances/:id/actions/restart',
  ...
  flow.try(
    ...
    docker.create('dockerHost'),
    // ignore next die
    flags.set('instance.container.dockerContainer', '-die', 'ignore'),
    flow.try(
      docker.model.restartContainer('instance.container')
    ).catch(
      mw.req().setToErr('restartErr'),
      // don't ignore next die
      flags.del('instance.container.dockerContainer', '-die-flag'),
      mw.next('restartErr')
    ),
    docker.model.inspectContainer('instance.container'),
    mw.req().set('containerInspect', 'dockerResult'),
...
app.put('/instances/:id/actions/stop',
  ...
  flow.try(
    ...
    // ignore next die
    flags.set('instance.container.dockerContainer', '-die-flag', 'ignore'),
    flow.try(
      ...
      mw.req().setToErr('restartErr'),
      // don't ignore next die
      flags.del('instance.container.dockerContainer', '-die'),
      mw.next('restartErr')
    ))
  .catch(
    mw.req().setToErr('err'),
