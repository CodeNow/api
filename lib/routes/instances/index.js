...
app.get('/instances/',
  ...
  flow.or(
    me.isOwnerOf('query'),
    mw.query('owner.github').validate(validations.equals(process.env.HELLO_RUNNABLE_GITHUB_ID))
  ),
...
app.post('/instances/',
  ...
  // make all instances created by HELLORUNNABLE user public by default
  mw.req('sessionUser.accounts.github.id')
    .validate(validations.equals(process.env.HELLO_RUNNABLE_GITHUB_ID))
    .then(mw.req().set('public', true))
    .else(mw.req().set('public', false)),
  instances.create({
    public: 'public',
    shortHash: 'nextHash',
