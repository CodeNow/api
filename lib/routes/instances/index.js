...
var removeInstanceContainer = // TODO: this has a lot of overlap with instance stop
  function (instanceKey, oldContainerKey, instanceNameKey, sessionUserIdKey) {
    return flow.series(
      ...
      mw.req('container.dockerContainer').require()
        .then( // has a docker container
          function (req, res, next) {
            var appCodeVersions = keypather.get(req, 'instance.contextVersion.appCodeVersions');
            ...
            rabbitMQ.deleteInstanceContainer({
              ...
              container: req.instance.container,
...
var updateInstanceBuild = flow.series(
  ...
  removeInstanceContainer(
    'instance', 'oldContainer', 'oldName', 'sessionUser.id'),
...
app.delete('/instances/:id',
  ...
  flow.try(
    ...
    // remove instance container
    removeInstanceContainer(
      'deletedInstance', 'deletedInstance.container', 'deletedInstance.name', 'sessionUser.id'))
  .catch(
    mw.req().setToErr('err'),
    mw.next('err')),
...
app.post('/instances/:id/actions/redeploy',
  ...
  flow.try(
    ...
    // remove old container
    removeInstanceContainer(
      'instance', 'oldContainer', 'instance.name', 'sessionUser.id'),
