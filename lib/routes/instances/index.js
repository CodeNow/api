  ...
  mw.body('container').require()
    .then(
      ...
      mw.query('contextVersion').pick().require()
        .else(
          mw.next(mw.Boom(400, 'query.contextVersion is required when updating container'))),
      instances.findSelf('query'),
      mw.req('instance').require()
        .else(
          mw.next(mw.Boom(409, 'instances\'s contextVersion does match query')))
    ),
