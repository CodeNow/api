...
app.post('/instances/',
  ...
  flow.try( // if anything after network-allocation fails, dealloc networkIp/hostIp
    middlewareLogger('POST_INSTANCES flow.try start'),
    ...
    mw.req('build.successful').validate(validations.equals(true))
      .then(
        middlewareLogger('POST_INSTANCES flow.try build successful'),
  ...
  .catch( // dealloc networkIp/hostIp
    middlewareLogger('POST_INSTANCES flow.try error'),
    mw.req().setToErr('err'),
    flow.try(
  ...
  middlewareLogger('POST_INSTANCES flow.try complete success'),
  instances.model.populateOwnerAndCreatedBy('sessionUser'),
