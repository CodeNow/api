...
var flags = require('middlewares/redis').flags;
...
app.put('/instances/:id/actions/restart',
  ...
  flow.try(
    ...
    // ignore next die
    flags.create(),
    flags.model.set('instance.container.dockerContainer', '-die', 'ignore'),
    flow.try(
      docker.model.restartContainer('instance.container')
    ).catch(
      mw.req().setToErr('restartErr'),
      // don't ignore next die
      flags.model.del('instance.container.dockerContainer', '-die-flag'),
      mw.next('restartErr')
    ),
    docker.model.inspectContainer('instance.container'),
    mw.req().set('containerInspect', 'dockerResult'),
...
app.put('/instances/:id/actions/stop',
  ...
  flow.try(
    ...
    // ignore next die
    flags.create(),
    flags.model.set('instance.container.dockerContainer', '-die-flag', 'ignore'),
    flow.try(
      docker.model.stopContainer('instance.container', 'body.force'),
      instances.model.setContainerFinishedState(new Date().toISOString(), 0)
    ).catch(
      mw.req().setToErr('restartErr'),
      // don't ignore next die
      flags.model.del('instance.container.dockerContainer', '-die'),
      mw.next('restartErr')
    ))
  .catch(
    mw.req().setToErr('err'),
