...
var createSaveAndNetworkContainer = flow.series(
  ...
  mw.req('forceHost').require()
    .then(
      mw.req().set('dockerHost', 'forceHost'))
    .else(
      mavis.create(),
  ...
  .catch(
    ...
    mw.req('containerCreateErr.output.statusCode').validate(validations.equals(404))
      .then(
        docker.model.pullImage('dockerResult'),
        // emitter.emit('dockerResult'),
        runnable.create({}, 'sessionUser'),
        runnable.model.redeployInstance('instance') // redeploy instance - with host
        )),
