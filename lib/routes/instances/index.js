...
var bodyValidations = flow.series(
  ...
  mw.body('env').require()
    .then(
      ...
      mw.body('env').each(function(env, req, eachReq, res, next) {
        eachReq.env = env;
        next();
      },
  ...
  mw.body('isolated').require()
    .then(mw.body('isolated').validate(validations.isObjectId)),
...
app.get('/instances/',
  ...
  mw.query({
    or: ['owner.github', 'githubUsername', '["owner.github"]']
  }).require(),
  timers.create(),
  ...
  instances.find('query', {
    'contextVersion.build.log': false
  }),
...
app.post('/instances/',
  ...
  mw.body('name').require()
    .else( // if name is not provided generate one
      ...
      instanceCounter.nextForOwner('body.owner'), function(req, res, next) {
...
app.delete('/instances/:id',
  ...
  flow.or(
    me.isOwnerOf('instance'),
    me.isModerator), function(req, res, next) {
...
app.put('/instances/:id/actions/start',
   ...
   *   won race)
   */
  instances.model.isNotStartingOrStopping(), function(req, res, next) {
