...
var createSaveAndNetworkContainer = flow.series(
  ...
  flow.background( // background container create task
    function (req, res, next) {
      // mongoose is stupid:
      // mongoose properties that look like arrays
      // are not actually arrays. let's make it an
      // array before we pass it to createUserContainer
      req.envArray = req.instance.toJSON().env;
      next();
    },
    flow.try(
...
app.delete('/instances/:id',
  ...
  mw.req('instance.masterPod').validate(validations.equals(true))
    .then(
      runnable.create({}, 'sessionUser'),
      runnable.model.destroyForkedInstances('instance')),
