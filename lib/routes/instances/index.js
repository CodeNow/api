...
var updateInstanceBuild = flow.series(
  ...
  mw.req('build.successful').validate(validations.equals(true))
    .then(
      logger([ 'build' ], 'ROUTE: updateInstanceBuild build.successful', 'trace'),
      function (req, res, next) {
        console.log('update instance with succesfull build', req.contextVersion)
...
app.post('/instances/',
  ...
  mw.req().set('ownerUsername', 'user.login'),
  function (req, res, next) {
    console.log('fork instance 5')
    next()
  },
  findBuildContextVersion,
  function (req, res, next) {
    console.log('fork instance 6')
    next()
  },
  ...
  function (req, res, next) {
    console.log('fork instance 8', req.build, req.build.successful)
    next()
  },
  mw.req('build.successful').validate(validations.equals(true))
    .then(
      function (req, res, next) {
        console.log(req.contextVersion.build.triggeredAction, req.build.successful, 'build cv>>>>')
        ...
        if (!manual) {
          NotificationService.instanceDeployed(req.instance._id, req. build.contextVersions[0]).asCallback(function () {
            console.log('fork instance notification')
          })
        }
