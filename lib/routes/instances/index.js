...
var createSaveAndNetworkContainer = flow.series(
  ...
  mw.req('containerCreateErr').require()
    .else( // container create was successful
      instances.model.modifySetContainer('containerInfo', 'dockerHost'),
 ...
 *  @param {String} oldContainerKey  instance's old container
 *  @param {String} instanceNameKey  instance's name (in hosts to remove)
 *  @param {String} ownerUsernameKey instance's owner's username
 *  // also assumes req.instance exists
 * */
var removeInstanceContainer = function (oldContainerKey, instanceNameKey, ownerUsernameKey) {
  return flow.series(
    // host LOCK must be acquired PRIOR to calling this (not necessary for post instances)!
...
var updateInstanceBuild = flow.series(
  ...
  mw.body('name').require()
    ...
    .else(
      mw.req().set('oldName', 'instance.name'),// new name and old name are the same here
  ...
  flow.try(
    ...
    removeInstanceContainer('oldContainer', 'oldName', 'ownerUsername'),
...
app.delete('/instances/:id',
  ...
  mw.req().set('ownerUsername', 'user.login'),
  // remove instance container
  removeInstanceContainer('instance.container', 'instance.name', 'ownerUsername'),
...
app.put('/instances/:id/actions/start',
  ...
  mw.req().set('instance.owner.username', 'ownerUsername'),
  mw.req().set('instance.container.inspect', 'containerInspect'),
  mw.req().set('instance.containers[0].inspect', 'containerInspect'),
  mw.res.json('instance'));
