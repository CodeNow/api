...
app.post('/instances/:id/actions/deploy',
  ...
  flow.try(
    function (req, res, next) {
      logger.log.debug({
        tx: true
      }, 'POST_INSTANCES_ID_ACTIONS_DEPLOY flow.try start');
      next();
    },
  ...
  .catch(
    function (req, res, next) {
      logger.log.debug({
        tx: true
      }, 'POST_INSTANCES_ID_ACTIONS_DEPLOY flow.try error');
      next();
    },
    mw.req().setToErr('err'),
    mw.next('err')),
  function (req, res, next) {
    logger.log.debug({
      tx: true
    }, 'POST_INSTANCES_ID_ACTIONS_DEPLOY flow.try complete success');
    next();
  },
  instances.model.populateModels(),
