...
app.get('/instances/',
  mw.query('url').require()
    .then(
      mw.query('url').string(),
      mw.query('name').require().string(),
      hosts.create(),
      hosts.model.parseUsernameFromUrl('query.url', 'query.name'),
      mw.query().unset('url'),
      mw.query().set('githubUsername', 'hostsResult')),
  mw.query({ or: ['owner.github', 'githubUsername', '["owner.github"]', 'url'] }).require(),
  timers.create(),
  ...
  mw.query('owner', 'shortHash', 'name' ,
    'owner.github', 'contextVersion.appCodeVersions.repo', 'container.dockerContainer').pick(),
  mw.query('["container.dockerContainer"]').require()
    .then(
      mw.query('["container.dockerContainer"]').string()),
