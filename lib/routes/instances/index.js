...
var bodyValidations = flow.series(
  ...
  mw.body('publicParent').require().then(
    mw.body('publicParent').boolean()),
...
app.post('/instances/',
  ...
  mw.body('name', 'owner', 'env', 'parent', 'build', 'publicParent').pick(),
  // validate body types
    ...
    mw.req('build.successful').validate(validations.equals(true))
      ...
      runnable.model.redeployInstance('instance', {
        json: {
          ...
          publicParent: 'body.publicParent'
        }
      }),
...
app.post('/instances/:id/actions/copy',
  ...
  flow.or(
    ...
    instances.model.isPublic(),
    me.isModerator),
  flow.mwIf(instances.model.isPublic())
    .then(
    mw.body().set('publicParent', true)),
  ...
  flow.mwIf(instances.model.isPublic())
    .then(
    runnable.create({}, 'sessionUser'),
    runnable.model.deepCopyPublicBuild('build'),
    mw.req().set('build', 'runnableResult'),
    //runnable.create({}, 'sessionUser'),
    runnable.model.buildBuild('build', 'body')),
