...
/**
 * update hosts for container (hipache)
 */
...
app.patch('/instances/:id',
  ...
  mw.body('build').validate(validations.equals('instance.build.toString()'))
    .then(
      mw.body().unset('build')),
  mw.body('oldBuild').require().then(
    mw.body('oldBuild').validate(validations.notEqualsKeypath('instance.build.toString()'))
      .then(
        mw.next(
          mw.Boom.conflict('Cancel update if oldBuild doesn\'t match instance.build')))),
  mw.body({
