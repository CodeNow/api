...
app.get('/instances/',
  ...
  mw.query('owner', 'shortHash', 'name', 'owner.github', 
    'contextVersion.context',
    'contextVersion.appCodeVersions.repo', 
    'container.dockerContainer').pick(),
  ...
  mw.query('["contextVersion.context"]').require()
    .then(
      mw.query('["contextVersion.context"]')
        ...
        .mapValues(transformations.toObjectId) // mongoose isn't autocasting this..
    ),
  mw.query('owner.github')
    .mapValues(transformations.toInt).number(),
  flow.or(
    me.isOwnerOf('query'),
