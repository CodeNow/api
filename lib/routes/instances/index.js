...
app.post('/instances/',
  ...
  bodyValidations,
  function (req, res, next) {
    req.body.shortName = req.body.name
    InstanceService.createInstance(req.body, req.sessionUser)
      .then(function (instance) {
        return InstanceService.populateInstanceModels(instance)
      })
      .then(function (instance) {
        req.instance = instance
      })
...
app.get('/instances/:id',
  findInstance,
  function (req, res, next) {
    InstanceService.populateInstanceModels(req.instance)
      .tap(function (instance) {
        req.instance = instance
      })
      .asCallback(function (err) {
...
/** Get in a instance's history
 *  @event GET rest/instances/:id/history
 *  @memberof module:rest/instances */
app.get('/instances/:id/history',
  findInstance,
  function (req, res, next) {
    clioClient.fetchInstanceHistory(req.params.id)
      .then((body) => {
        res.json(body)
      })
      .catch(next)
...
app.post('/instances/:id/actions/redeploy',
  ...
  function (req, res, next) {
    return InstanceService.populateInstanceModels(req.instance)
    .tap(function (instance) {
      req.instance = instance
    })
    .asCallback(function (err) {
...
/**
 * Filter all instances by JIRA issue id, requires body of { issueId: String }
 * @param {String} issueId - Jira issue id used to find instance's branch name
 * @event GET rest/allInstances/
 * @memberof module:rest/instances
 */
app.get('/allInstances/',
  me.isModerator,
  function (req, res, next) {
    const issueToCheck = new RegExp('.*' + req.query.issueId + '.*')
    const githubId = req.query.orgId || null
    InstanceService.findInstanceByBranchName(githubId, issueToCheck)
      .tap(function (instances) {
        req.instances = instances
      })
      .asCallback(function (err) {
