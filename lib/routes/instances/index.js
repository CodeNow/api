...
var updateInstanceBuild = flow.series(
  ...
  mw.req('build.successful').validate(validations.equals(true))
    .then(
      ...
      function (req, res, next) {
        console.log('fork notification')
        NotificationService.autoFork(req.instanceId, req.build.contextVersions[0])
          .asCallback(function () {
...
app.post('/instances/',
  ...
  function (req, res, next) {
    console.log('fork instance 8', req.build)
    next()
  },
  mw.req('build.successful').validate(validations.equals(true))
    .then(
      function (req, res, next) {
        console.log(req.contextVersion.build.triggeredAction, 'build cv>>>>')
        rabbitMQ.createInstanceContainer({
