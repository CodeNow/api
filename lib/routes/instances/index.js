...
app.post('/',
  ...
  mw.req('build.successful').require().then(createAllContainers),
...
app.patch('/:id',
  ...
  mw.body({ or: ['name', 'build']}).require()
    .then(
      mw.body('build').require()
        .then(mw.req('build.successful').require().then(createAllContainers))
...
app.post('/:id/actions/redeploy',
  ...
  flow.or(
    ...
    me.isModerator),
  findBuild,
  mw.req('instance.containers').validate(validations.existsArray())
