...
app.get('/instances/',
  ...
  mw.query('githubUsername').require()
    .then(
      ...
      mw.req().set('instance.owner.username', 'query.githubUsername'),
  ...
  instances.models.populateModels('sessionUser'),
...
app.post('/instances/',
  ...
  instances.model.populateModels('sessionUser'),
...
app.patch('/instances/:id',
  ...
  mw.body({ or: ['name', 'env'] }).require()
    .then(
      // getGithubUsername will cb immediately, if already populated
      // github username will not be previously populated if env is updated without name
      // acquire lock
      instances.model.getGithubUsername('sessionUser'),
      mw.req().set('ownerUsername', 'instance.owner.username'),
      hosts.create(),
