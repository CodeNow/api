...
var bodyValidations = flow.series(
  ...
  mw.body('build').require()
    .then(
      ...
      mw.req('instance.owner.github').require()
        .then(
...
var createSaveAndNetworkContainer = flow.series(
  ...
  .catch(
    ...
    instances.model.modifyContainerCreateError('containerCreateErr')),
      ...
      function (req, res, next) { // BACKGROUND remove - sauron and docker
        flow.series(
          ...
          flow.try(
            ...
            docker.model.inspectContainer('container.dockerContainer'),
            mw.req().set('containerInfo', 'dockerResult'),
          ...
          mw.req('build.successful').validate(validations.equals(true))
            .then(
              ...
              flow.try(
                ...
                createSaveAndNetworkContainer,
                // upsert new hosts
                hosts.model.upsertHostsForInstance('user.login', 'instance'),
                // remove old hosts - LAST least important
                mw.body('name').require()
                  .then(hosts.model.removeHostsForInstance('ownerUsername', 'instance')))
              .catch(
                mw.req().setToErr('err'),
                // release locsk if error occurred
                hosts.model.releaseHostLock('ownerUsername', 'newName'),
                mw.body('name').require()
                  .then(hosts.model.releaseHostLock('ownerUsername', 'oldName')),
                mw.next('err')),
              // release locks
              hosts.model.releaseHostLock('ownerUsername', 'newName'),
                mw.body('name').require()
