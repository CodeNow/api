  ...
  mw.req('build.successful').validate(validations.equals(true))
    .then(
      ...
      instanceService.model.deploy(
        'instanceId', 'build._id', 'sessionUser.accounts.github.id',
        'ownerUsername', 'body.forceDock'))
    .else(
      logger(['body', 'build'], 'ROUTE: updateInstanceBuild !build.successful', 'trace')
    )
);
...
app.post('/instances/',
  ...
  mw.req('build.successful').validate(validations.equals(true))
    .then(
      instanceService.create(),
      instanceService.model.deploy(
        'instance._id', 'build._id', 'sessionUser.accounts.github.id',
        'ownerUsername', 'body.forceDock')),
...
app.post('/instances/:id/actions/redeploy',
  ...
  flow.try(
    ...
    instanceService.create(),
    instanceService.model.deploy(
      'instanceId', null, 'sessionUser.accounts.github.id',
      'ownerUsername', 'body.forceDock'))
  .catch(
    mw.req().setToErr('err'),
    mw.next('err')),
