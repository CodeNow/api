...
app.get('/',
  mw.query({or: ['owner.github', 'shortHash', 'project']}).require(),
  mw.query('owner', 'shortHash', 'project').pick(),
  mw.query('owner.github').require().then(
    mw.query('owner.github').mapValues(transformations.toInt).number(),
    me.isOwnerOf('query')),
  mw.query('project').require().then(
    mw.query('project').validate(validations.isObjectId),
    projects.findById('query.project'),
    checkFound('project'),
    me.isOwnerOf('project')),
...
app.post('/',
  ...
  instanceCounter.nextForOwner('project.owner'),
  function (req, res, next) {
    req.instanceName = 'Instance'+req.instanceCounter;
    next();
  },
  mw.body('name').require()
    .else(mw.body().set('name', 'instanceName')),
  instances.create({
    shortHash: 'nextHash',
