...
var createAllContainers = flow.series(
  instances.model.set({ deploy: { started: Date.now() } }),
  flow.try(
    ...
    instances.updateDeployError('instance._id', 'err'),
    mw.next('err')
  ));

/**
 * removes all containers from docker and instance document
 * @type {Function} middleware
 */
var removeAllContainers = flow.series(
  ...
  instances.model.update({ $set: { containers: [] } }),
  function (req, res, next) {
    // remove containers from docker in background
    mw.req('instance.containers').each(
      ...
      docker.model.removeContainer('container'))(req, res, error.logIfErr);
    // next immediately
    next();
  });
 ...
 *  @params body.parentInstance  id of the parent instance
 *  @event POST rest/instances
 *  @memberof module:rest/instances */
app.post('/instances/',
  mw.body('build').require().validate(validations.isObjectId),
  findBuild,
  mw.body('name', 'owner', 'env', 'parentInstance').pick(),
  // validate body types
  ...
  mw.body('env').require().then(
    instances.model.set({ env: 'body.env' })),
  mw.body('parentInstance').require().then(
    instances.model.set({ parent: 'body.parentInstance' })),
  contextVersions.findByIds('build.contextVersions'),
  mw.req('contextVersions').validate(validations.existsKeypathArray('build.started',
    'Cannot attach a build to an instance with context versions that have not started building')),
  instances.model.set({ contextVersions: 'contextVersions' }),
  // If the build has completed, but hasn't failed, create the containers
  mw.req('build.successful').require().validate(validations.equals(true))
    .then(
...
app.patch('/instances/:id',
  ...
  mw.body('name').require().then(
    ...
    instances.model.set({ name: 'body.name' }),
    instances.model.save()
  ),
  // validate body types
  mw.body('env').require()
    ...
    .then(
      mw.body('build').require()
        ...
        mw.req('build.owner.github').validate(validations.equalsKeypath('instance.owner.github'))
          ...
          // Grab all of the context versions from the build, add them to the instance, and update
          contextVersions.findByIds('build.contextVersions'),
          mw.req('contextVersions').validate(validations.existsKeypathArray('build.started',
            'Cannot attach a build to an instance with context versions that ' +
            'have not started building')),
          removeAllContainers, // remove all containers always even if build it not built yet
          mw.body().set('contextVersions', 'contextVersions')
        ).else(
          // If build is included, don't do removeHipacheRoutes
          // (It already happens when we destroy the containers)
          removeHipacheRoutes)),
    ...
    .then(
      mw.body('build').require()
        .then(mw.req('build.successful').validate(validations.equals(true))
          ...
          // If build is included, don't do updateHipacheRoutes
          // (It already happens when we destroy the containers)
          updateHipacheRoutes)),
  instances.model.getGithubUsername('sessionUser'),
  instances.model.populateModelsAndContainers(),
