...
app.patch('/instances/:id',
  ...
  mw.body('build').require()
    .then( // build was updated
      findBuildContextVersion,
      // save prevBuiltNonAdvancedContextVersion if eligible
      mw.req().set('oldContextVersion', 'instance.contextVersion'),
      mw.req('oldContextVersion.advanced').require().validate(validations.equals(true))
        .then(
          mw.req('oldContextVersion.build.completed').require().validate(validations.equals(true))
            .then(
              mw.body().set('prevBuiltNonAdvancedContextVersion', 'oldContextVersion._id'))),
      // without toJSON - mongoose inf loops.
      mw.body().set('contextVersion', 'contextVersion.toJSON()'),
      updateInstanceBuild)
    .else(
