...
var middlewareLogger = function (msg, level) {
  return function (req, res, next) {
    logger.log[level]({tx: true}, msg);
    next();
  };
};
...
var createContainer = flow.series(
  function (req, res, next) {
    logger.log.info({
      tx: true
    }, 'createContainer middleware start');
    next();
  },
...
app.post('/instances/',
  ...
  middlewareLogger('POST_INSTANCES', 'info'),
  ...
  flow.try( // if anything after network-allocation fails, dealloc networkIp/hostIp
    middlewareLogger('POST_INSTANCES flow.try start', 'debug'),
    ...
    mw.req('build.successful').validate(validations.equals(true))
      .then(
        middlewareLogger('POST_INSTANCES flow.try build successful', 'debug'),
  ...
  .catch( // dealloc networkIp/hostIp
    middlewareLogger('POST_INSTANCES flow.try error', 'debug'),
    mw.req().setToErr('err'),
    flow.try(
  ...
  middlewareLogger('POST_INSTANCES flow.try complete success', 'debug'),
  instances.model.populateOwnerAndCreatedBy('sessionUser'),
...
app.post('/instances/:id/actions/deploy',
  requestTrace('POST_INSTANCES_ID_ACTIONS_DEPLOY'),
  middlewareLogger('POST_INSTANCES_ID_ACTIONS_DEPLOY', 'info'),
  isInternalRequest,
  findInstance,
  flow.or(
    me.isOwnerOf('instance'),
  ...
  mw.req().set('ownerUsername', 'user.login'),
  flow.try(
    middlewareLogger('POST_INSTANCES_ID_ACTIONS_DEPLOY flow.try start', 'info'),
    ...
    instances.model.populateOwnerAndCreatedBy('sessionUser'))
  .catch(
    middlewareLogger('POST_INSTANCES_ID_ACTIONS_DEPLOY flow.try error', 'info'),
    mw.req().setToErr('err'),
    mw.next('err')),
  middlewareLogger('POST_INSTANCES_ID_ACTIONS_DEPLOY flow.try complete success', 'info'),
  instances.model.populateModels(),
...
app.put('/instances/:id/actions/start',
  ...
  function (req, res, next) {
    rabbitMQ.hermesClient.publish('start-instance-container', {
      ...
      ownerUsername: req.sessionUser.login,
  ...
  // required otherwise frontend API client collection-store
  // hash json will break
  instances.model.populateOwnerAndCreatedBy('sessionUser'),
  mw.res.json('instance'));
...
app.put('/instances/:id/actions/restart',
  ...
  flow.try(
    ...
    flow.try(
      ...
      instances.model.removeStartingStoppingStates(),
      instances.model.populateOwnerAndCreatedBy('sessionUser'),
...
app.put('/instances/:id/actions/stop',
  logger(['body'], 'PUT_INSTANCES_ID_ACTIONS_STOP', 'info'),
  findInstance,
  logger(['instance'], 'PUT_INSTANCES_ID_ACTIONS_STOP findInstance result', 'info'),
  flow.or(
    me.isOwnerOf('instance'),
    me.isModerator),
  mw.req('instance.container.dockerContainer').require()
  ...
  mw.req().set('ownerUsername', 'user.login'),
  instances.model.populateOwnerAndCreatedBy('sessionUser'),
  logger(['instance'], 'PUT_INSTANCES_ID_ACTIONS_STOP instance post populateOwnerAndCreatedBy', 'info'),
  ...
  flow.try(
    ...
    userStoppedContainer.create('instance.container.dockerContainer'),
    userStoppedContainer.model.lock(),
    logger([], 'PUT_INSTANCES_ID_ACTIONS_STOP debug point', 'info'),
    flow.try(
      docker.model.stopContainer('instance.container', 'body.force'),
      logger([], 'PUT_INSTANCES_ID_ACTIONS_STOP debug point 2', 'info'),
      instances.model.inspectAndUpdate(),
      logger([], 'PUT_INSTANCES_ID_ACTIONS_STOP debug point 3', 'info')
    ).catch(
      mw.req().setToErr('restartErr'),
      // don't ignore next die
      instances.model.removeStartingStoppingStates(),
  ...
  .catch(
    mw.req().setToErr('err'),
    logger(['instance'], 'PUT_INSTANCES_ID_ACTIONS_STOP instance error', 'info'),
    instances.model.populateOwnerAndCreatedBy('sessionUser'),
