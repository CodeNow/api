...
app.get('/instances/',
  mw.query('hostname').require()
    .then(
      ...
      mw.query({ or: ['name', 'githubUsername'] }).require().string(),
      hosts.create(),
      mw.query('name').require()
        .then(
          hosts.model.parseUsernameFromHostname('query.hostname', 'query.name'),
          mw.query().set('githubUsername', 'hostsResult'),
          mw.query().unset('hostname')
        ).else( // assumes: mw.query('githubUsername').require()
          hosts.model.parseInstanceNameFromHostname('query.hostname', 'query.githubUsername'),
          mw.query().set('name', 'hostsResult'),
          mw.query().unset('hostname')
        )
    ),
