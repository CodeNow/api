    ...
    .then(
      ...
      function (req, res, next) {
        rabbitMQ.createInstanceContainer({
          ...
          ownerGitHubUsername: req.ownerGitHubUsername
        })
        next()
      }
  ).else(
...
app.post('/instances/',
  ...
  instances.model.setDependenciesFromEnvironment('ownerGitHubUsername'),
...
app.post('/instances/:id/actions/redeploy',
  ...
  instances.model.populateOwnerAndCreatedBy('sessionUser'),
  instances.model.populateModels(),
  mw.req().set('instance.owner.username', 'ownerGitHubUsername'),
  mw.res.json('instance'))
...
app.put('/instances/:id/actions/restart',
  ...
  flow.try(
    ...
    hosts.create(),
    hosts.model.upsertHostsForInstance('ownerGitHubUsername', 'instance'))
    .catch(
      mw.req().setToErr('err'),
      mw.next('err')),
  // success!
...
app.put('/instances/:id/actions/stop',
  ...
  function (req, res, next) {
    rabbitMQ.stopInstanceContainer({
      ...
      ownerGitHubUsername: req.instance.owner.username,
      sessionUserGithubId: req.sessionUser.accounts.github.id,
      tid: req.domain.runnableData.tid
    })
    next()
