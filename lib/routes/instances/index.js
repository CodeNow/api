...
var bodyValidations = flow.series(
  mw.body('name').require().then(
    ...
    // save the name to oldLowerName so we can change it in the graph
    mw.req().set('oldLowerName', 'instance.lowerName')
  ),
  mw.body('env').require()
    .then(
      mw.body('env').array()
...
app.get('/instances/',
  ...
  mw.req('instances').each(
    ...
    graph.model.getDependencyGraph('instance')),
...
app.post('/instances/',
  ...
  graph.create(),
  graph.model.graphInstanceDependencies('instance'),
  graph.model.getDependencyGraph('instance'),
...
app.patch('/instances/:id',
  ...
  mw.body({ or: ['name', 'env'] }).require()
    .then(
      graph.create(),
      graph.model.graphInstanceDependencies('instance', 'oldLowerName'),
      graph.model.getDependencyGraph('instance')),
  instances.model.getGithubUsername('sessionUser'),
  instances.model.populateModelsAndContainers(),
  mw.res.json('instance'));
...
app.put('/instances/:id/actions/stop',
  ...
  flow.try(
    ...
    // stop container - dont worry about updating mongo
    docker.create('dockerHost'),
