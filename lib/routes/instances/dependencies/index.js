...
app.get('/instances/:id/dependencies',
  ...
  mw.query('hostname').require().then(
    ...
    mw.query().set('elasticHostname', 'query.hostname')),
  mw.query('elasticHostname').require().then(
    mw.query('elasticHostname').string(),
    mw.query().set('elasticHostname', 'query.elasticHostname.toLowerCase()')),
...
app.put('/instances/:id/dependencies/:hostname',
  ...
  instances.model.emitInstanceUpdate('sessionUser', 'update'), // Used to trigger the link Update worker
