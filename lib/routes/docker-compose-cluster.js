...
const restartSchema = joi.object({
  ...
  isolationGroupMasterInstanceId: joi.string().required(),
  testDockerImageId: joi.string().required()
}).unknown().required()
...
const restartRoute = function (req, res, next) {

  const isolationId = req.body.isolationId
  const log = logger.child({
    method: 'restartRoute',
    isolationId,
    instanceId,
    isolationGroupMasterInstanceId
  })
  log.info('called')
  return joi.validateOrBoomAsync(req.body, restartSchema)
    .then(function () {
      log.trace('restart isolation master')
...
app.post('/docker-compose-cluster/restart/', restartRoute)

module.exports = app
module.exports.postRoute = postRoute
module.exports.deleteRoute = deleteRoute
module.exports.restartRoute = restartRoute
