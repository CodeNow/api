...
const postRoute = function (req, res, next) {
  ...
  return joi.validateOrBoomAsync(req.body, postSchema)
    .then(() => {
      const organization = UserService.getBpOrgInfoFromRepoName(req.sessionUser, repoFullName)
      const triggeredInfo = {
        action: 'manual',
        repo: repoFullName,
        branch: branchName
      }
      return ClusterBuildService.create(sessionUserBigPoppaId, organization.id, triggeredInfo)
    })
    .then((clusterBuild) => {
      log.trace({ clusterBuild }, 'create cluster')
