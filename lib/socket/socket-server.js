  ...
  this.primus.on('connection', function (socket) {
    ...
    primusDomain.run(function () {
      ...
      if (!userId && !authToken && !envIs('test')) {
        ...
        logger.error({ error: noAuthError }, 'socker-server: no auth for primus')
        return socket.write({
      ...
      function onSocketData (message) {
        ...
        */
        if (!isDataValid(message)) {
          ...
          logger.error({ err: invalidStreamError, message: message }, 'socker-server error input')
          return socket.write({
            error: 'invalid input',
            data: message
          })
        ...
        if (typeof handlers[message.event] !== 'function') {
          ...
          logger.error({ error: invalidEventError, message: message }, 'socker-server invalid event')
          return socket.write({
            error: 'invalid event',
            data: message
          })
