...
var createRedisClient = require('models/redis').createClient;
...
function SocketServer (server) {
  ...
  this.primus.use('rooms', require('primus-rooms'));
  this.primus.use('substream', require('substream'));
  this.primus.use('cluster', require('primus-cluster'));
  ...
  server.on('close', function () {
    var len = redisClients.length;
    for (var i = 0; i < len; i++) {
      redisClients.pop().quit();
    }
  });
}
