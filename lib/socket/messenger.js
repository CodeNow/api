...
var Warning = require('error-cat/errors/warning')
...
Messenger.prototype.canJoin = function (socket, data, cb) {
  ...
  if (!userId) {
    ...
    return cb(new Warning('No authentication data'))
  }
...
Messenger.prototype.subscribeStreamHandler = function (socket, id, data) {
  ...
  return Promise.try(function () {
    if (!data.name || !data.type || !data.action) {
      ...
      throw new Warning('name, type and action are required')
    }
  }).bind(this)
    .then(function () {
      if (~data.action.indexOf('join')) {
        return this.canJoinAsync(socket, data)
          .catch(function () {
            throw new Warning('access denied', data)
          })
        ...
        throw new Warning('invalid action', data)
      }
    })
    .then(function () {
