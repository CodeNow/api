...
Messenger.prototype.emitInstanceUpdate = function (instance, action) {
  ...
  var valid = requiredKeypaths.every(function (requiredKeypath) {
    return exists(keypather.get(instance, requiredKeypath))
  })

  if (!valid) {
    log.error(put({ requiredKeypaths: requiredKeypaths }, logData),
...
/**
 * Handler for the room subscription socket
...
Messenger.prototype.subscribeStreamHandler = function (socket, id, data) {
  ...
  if (!data.name ||
    ...
    return Promise.reject(new Error('name, type and action are required'))
  }
