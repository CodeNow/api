...
Messenger.prototype.messageRoomHenry = function (githubId, task)  {
  let data = {githubId: githubId, task: task}
  const log = logger.child({ logName: githubId, type: 'org', data: data })
  log.info('messageRoom Henry')
  this.server.room(genRoomName('org', githubId)).write({
    id: uuid(),
    event: 'ROOM_MESSAGE',
    type: 'org',
    name: githubId,
...
Messenger.prototype._emitInstanceUpdateAction = function (instance, action) {
  ...
  // this creates jobs that other services use
  if (action === 'delete') {
