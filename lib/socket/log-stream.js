...
function logHandler (socket, id, data) {
  ...
  return commonStream.validateDataArgs(data, reqArgs)
    ...
    .then(instance => {
      timer.stop()
      return setupLogs(socket, id, data, instance)
    })
    .catch(commonStream.onValidateFailure('logHandler', socket, id, tags))
    .finally(() => timer.stop())
}
...
function setupLogs (socket, id, data, instance) {
  ...
  return commonStream.pipeLogsToClient(destLogStream, data.containerId, { tailLimit, baseDataName, tags })
    .catch(function (err) {
