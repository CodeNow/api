  ...
  docker.getLogs(data.containerId, process.env.DOCKER_LOG_TAIL_LIMIT,
    function (err, dockerLogStream) {
      ...
      dockerLogStream.on('data', function (data) {
        log.trace({arguments: arguments}, 'LogStream data.')
      });
      dockerLogStream.on('end', function () {
        log.trace({arguments: arguments}, 'LogStream END.')
      });
      dockerLogStream.on('finish', function () {
        log.trace({arguments: arguments}, 'LogStream FINISH.')
      });
      if (err) {
