 ...
 */
function onClientSocketDisconnect(socket) {
  return function() {
    var socketId = socket.id;
    //TODO remove these listener removals when the route process does it
    socket.removeListener('end', onClientSocketDisconnect(socket));
    socket.removeListener('close', onClientSocketDisconnect(socket));
    if (socketsConnected[socketId]) {
      Object.keys(socketsConnected[socketId]).forEach(function (streamId) {
...
function setupSocketDisconnectEvents(socket, streamId) {
  if (!socketsConnected[socket.id]) {
    socketsConnected[socket.id] = {};
    //TODO remove these listeners when the route process attaches them
    socket.on('close', onClientSocketDisconnect(socket));
    socket.on('end', onClientSocketDisconnect(socket));
  }
  socketsConnected[socket.id][streamId] = true;
...
function clearMapsOfClient(streamId, socketId) {
  ...
  if (socketsConnected[socketId] && socketsConnected[socketId][streamId]) {
    delete socketsConnected[socketId][streamId];
  }
}
...
module.exports.onClientSocketDisconnect = onClientSocketDisconnect;
