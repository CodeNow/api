...
function BuildStream (socket, id, data) {
  ...
  this.authToken = keypather.get(socket, 'request.query.token')
  // user id should exist when connecting from browser
  this.userId = keypather.get(socket, 'request.session.passport.user')
  this.sessionUser = {}
  keypather.set(this.sessionUser, 'accounts.github.id', this.userId)
  keypather.set(this.sessionUser, 'accounts.github.accessToken', this.authToken)
...
BuildStream.prototype._checkOwnership = function (version) {
  var self = this
  var logData = {
    tx: true,
    version: version,
  ...
  log.info(logData, 'BuildStream.prototype._checkOwnership')
  return Promise.any([
    Promise.fromCallback(function (callback) {
      me.isOwnerOf(version.attrs)(req, {}, callback)
    }),
    Promise.fromCallback(function (callback) {
      me.isModerator(version.attrs)(req, {}, callback)
    })
  ]).catch(function (err) {
    log.warn(put({
      tx: true,
      errMessage: err
    }, logData), '_checkOwnership failed')
    self._writeErr('You don\'t own this build')
  })
}
