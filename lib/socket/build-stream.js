...
BuildStream.prototype.handleStream = function () {
  ...
  return Promise.fromCallback(function (callback) {
    ContextVersion.findOne({ _id: self.data.id }, callback)
  })
    .catch(function (err) {
      dogstatsd.increment(baseDataName + '.err.no_ContextVersion_1')
      self._writeErr('could not find build in database')
      throw err
    })
    .then(function (version) {
      if (!self._validateVersion(version)) {
        return SocketServer.checkOwnership(self.sessionUser, version)
          .then(function () {
          ...
          .catch(function (err) {
            ...
            dogstatsd.increment(baseDataName + '.err.nonowner_buildstream')
            throw err
          })
      }
    })
