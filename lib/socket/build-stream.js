...
BuildStream.prototype.handleStream = function () {
  ...
  return ContextVersion.findOneAsync({ _id: self.data.id })
    .bind(this)
    .catch(function (err) {
      dogstatsd.increment(baseDataName + '.err.no_ContextVersion_1')
      this._writeErr('could not find build in database')
      throw err
    })
    .then(function (version) {
      if (!version) {
        this._writeErr('version not found')
    ...
    .then(function (version) {
      return commonStream.checkOwnership(self.sessionUser, version)
        ...
        .catch(function (err) {
          dogstatsd.increment(baseDataName + '.err.buildstream_nonowner')
          throw err
        })
    })
    .then(function (version) {
      // Grab the stream from the socket using the containerId
      var clientStream = this.socket.substream(self.data.streamId)
    ...
    .catch(function (err) {
      this._writeErr('You don\'t have access to this stream')
      throw err
    })
}
...
BuildStream.prototype._validateVersion = function (version) {
  ...
  if (!keypather.get(version, 'build.dockerContainer')) {
    dogstatsd.increment(baseDataName + '.err.invalid_version')
    this._writeErr('dockerContainer not found in version')
    return true
  }
}
