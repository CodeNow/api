...
var uuid = reuqire('uuid')

var pubnub = require("pubnub")({
  ssl: true,  // <- enable TLS Tunneling over TCP
  publish_key: "pub-c-1e787ab8-b80b-4493-ae05-871011899cf2",
  subscribe_key: "sub-c-f89868a8-e6f3-11e5-b07b-02ee2ddab7fe",
  secret_key: 'sec-c-MjRmNmVkZmEtODY1Zi00ZmI3LTlkMzAtZGEwZjA1MTc1MDFj',
  restore: true
})
...
function createJSONParser (client, onErr) {
  ...
  function onRootEvent (data) {
    ...
    client.write(data)
    pubnub.publish(this.channel, data)
  }
...
BuildStream.prototype.handleStream = function () {
  ...
  return commonStream.validateDataArgs(this.data, reqArgs)
    ...
    .then(function () {
      ...
      return Promise.fromCallback(function (cb) {
        Pubnub.channel_group_list_channels({
          channel_group: self.id,
          callback: cb
        })
      })
    })
    .then(function (channelGroupList) {
      if (channelGroupList.payload.channels.length) {
        throw new Error('Skipping processing')
      }
    })
    .then(function () {
      var self = this;
    ...
    .catch(function (err) {
      if (err.message === 'Skipping processing') {
        return
      }
      throw err
    })
