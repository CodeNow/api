...
subscriber.on('message', function(channel, message) {
  if (clientDataCallbackMap.hasOwnProperty(message)) {
    for(var socketId in clientDataCallbackMap[message]) {
      if (clientDataCallbackMap[message].hasOwnProperty(socketId))
      clientDataCallbackMap[message][socketId]();
    }
  }
});
...
function buildStreamHandler (socket, id, data) {
  ...
  socket.on('error', onClientSocketDisconnect(data.id, socket.id));
  socket.on('close', onClientSocketDisconnect(data.id, socket.id));
  socket.on('disconnection', onClientSocketDisconnect(data.id, socket.id));
  socket.on('end', onClientSocketDisconnect(data.id, socket.id));
...
//    console.log('Grabbing log from ' + data.id, bufferData);
    if (clientPendingDataMap.hasOwnProperty(data.id)) {
      clientDataCallbackMap[data.id][socket.id] = onSendDataToClient(data.id, socket.id, clientStream);
...
function onBuildStreamData(id) {
  return function(data) {
    if (data) {
      ...
      publisher.append(id, data, function(err) {
        if (err) {
          debug('Build Appending Error: ', err);
        }
        if (clientPendingDataMap.hasOwnProperty(id)) {
          for (var socketId in clientPendingDataMap[id]) {
            if (clientPendingDataMap[id].hasOwnProperty(socketId)) {
              clientPendingDataMap[id][socketId] += data.length;
            }
          }
        }
        // Fire off subscriber
