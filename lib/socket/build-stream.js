...
function onFirstConnection(queue, streamId, client, socket, socketEndingMessage) {
  return function (err, ttl) {
    if (ttl < -1) {
      ...
      if (ttl === -1) {
        // TTL of -1 means the data is in redis, but it doesn't have a TTL.  That means the stream is
        // alive, and we need to schedule the first data packet
...
/**
 * Queue Callback that should be used for normal 'piping'.  Whenever data is put into the queue to be
 * written through the substream, this is the callback that should be used.
 * @param err err
