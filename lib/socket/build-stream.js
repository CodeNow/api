...
BuildStream.prototype.handleStream = function () {
  const tags = {
    ...
    userId: keypather.get(this.sessionUser, 'accounts.github.id')
  }
  var log = logger.child(put(tags, { method: 'BuildStream.handleStream' }))
  log.info('BuildStream.handleStream called')
  ...
  return commonStream.validateDataArgs(this.data, reqArgs)
    ...
    .then(version => {
      ...
      timer.stop()
      return commonStream.pipeLogsToClient(clientStream, containerId, { parseJSON, baseDataName, tags })
    })
    .catch(commonStream.onValidateFailure('buildStream', this.socket, this.data.streamId, tags))
    .finally(() => timer.stop())
}
