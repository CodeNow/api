...
module.exports._setupStream = Promise.method(function (socket, data) {
  ...
  return Promise
    .try(function () {
      if (terminalConnections[terminalId] && terminalConnections[terminalId].containerId === data.containerId) {
    ...
    .then(function (execStream) {
      ...
      connection.execStream = execStream
    ...
    .then(function (connection) {
      ...
      const cleanedExecStream = connection.execStream.pipe(streamCleanser).pipe(buff2String)
      // Send terminal output to the user
      cleanedExecStream.pipe(clientTermStream)
      clientTermStream.on('end', function () {
        connection.execStream.unpipe(streamCleanser)
      })

      // Listen for terminal output so we can handle restore nicely
      cleanedExecStream.on('data', function (lastMessage) {
        connection.lastMessage = lastMessage
      })

      connection.execStream.on('error', function () {
