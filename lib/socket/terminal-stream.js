...
module.exports._setupStream = function (socket, data) {
  ...
  return Promise
    ...
    .then(execStream => {
      ...
      var messageHistory = connection.lastMessage.toarray().reverse().join('')
      ...
      clientTermStream.on('exit', function () {
        // If we don't disconnect, we'll get repeated data in the terminal
        bufferStream.removeListener('data', queueLastMessage)
      })
