...
module.exports._setupStream = Promise.method(function (socket, data) {
  ...
  return Promise.resolve()
    .then(function () {
      ...
      terminalConnections[terminalId].connection = Promise.fromCallback(function (cb) {
          docker.execContainerAndRetryOnTimeout(data.containerId, cb)
        })
        .then(function (execStream) {
          ...
          return {
            containerId: data.containerId,
            cleanedStream: execStream.pipe(streamCleanser).pipe(buff2String),
    ...
    .then(function (connection) {
      if (connection.containerId !== data.containerId) {
        var err = new Error('You are not authorized to access this stream.')
        log.warn({ err: err }, 'Unauthorized attempt to access stream.')
        throw err
      }
      return connection;
    })
    .then(function (connection) {
      ...
      connection.cleanedStream.pipe(clientTermStream)

      // Listen for terminal output so we can handle restore nicely
      connection.cleanedStream.on('data', function (lastMessage) {
      ...
      clientTermStream.on('data', function (clientData) {
        if (terminalConnections[terminalId]) {
          ...
          connection.rawStream.write(clientData)
        } else {
