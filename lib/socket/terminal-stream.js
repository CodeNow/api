...
function setupStream (socket, data) {
  ...
  if (terminalConnections[terminalId]) {
    console.log('FINDME: re-using terminal');
    connectStream()
  } else {
    console.log('FINDME: Creating a new terminal');
  ...
  function connectStream () {
    ...
    clientTermStream.on('data', function (clientData) {
      console.log('FINDME: Client Data', clientData);
      ...
      if (terminalConfig) {
        ...
        log.warn('FINDME: Attempt to write to a terminal that no longer exists')
      }
    })
