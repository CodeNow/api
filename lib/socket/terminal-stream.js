...
function proxyStreamHandler (socket, id, data) {
  ...
  return commonStream.validateDataArgs(data, reqArgs)
    ...
    .then(function (model) {
      timer.stop()
      return module.exports._setupStream(socket, data, model)
    })
    .catch(commonStream.onValidateFailure('proxyStreamHandler', socket, id, tags, timer))
}

module.exports._setupStream = function (socket, data, model) {
  const instanceId = data.isDebugContainer ? model.instance : keypather.get(model, '_id.toString()')
  const sessionUser = keypather.get(socket, 'request.sessionUser')
  const modelOwnerGitHubId = keypather.get(model, 'owner.github')
  ...
  function writeUserDataToTerminal (clientData) {
    if (terminalConnections[terminalId]) {
      ...
      rabbitMQ.publishTerminalDataSent({
        // TODO: get branch and repo from acv and other instance data
        isDebugContainer: data.isDebugContainer,
        containerId: data.containerId,
        instanceId,
        user: {
  ...
  return Promise
    ...
    .tap(() => {
      ...
      rabbitMQ.publishTerminalConnected({
        // TODO: get branch and repo from acv and other instance data
        isDebugContainer: data.isDebugContainer,
        containerId: data.containerId,
        instanceId,
