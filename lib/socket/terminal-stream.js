...
function setupStream (socket, id, data) {
  ...
  docker.execContainer(data.containerId, function (err, execStream) {
    ...
    docker.modem.demuxStream(execStream, clientTermStream, clientTermStream)

    // pump(clientTermStream, execStream)
    // pump(clientEventStream, destEventStream)
    // pump(execStream, clientTermStream)
    // pump(destEventStream, clientEventStream)

    dogstatsd.captureSteamData(baseDataName + '.clientTermStream', clientTermStream)
    dogstatsd.captureSteamData(baseDataName + '.execStream', execStream)
