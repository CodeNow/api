 ...
 */
function pipeLogsToClient (clientStream, containerId, opts) {
   ...
   */
  const timer = monitorDog.timer(baseDataName + '.connections.timeTillFirstByte', true)
  ...
  pipeDomain.run(function () {
    ...
    return docker.getLogsAsync(containerId, tailLimit)
      .catch(err => {
        monitorDog.increment(baseDataName + '.connections', {result: 'success'})
        throw err
      })
      .then(function (dockerLogStream) {
        dockerLogStream.once('data', () => timer.stop)
        dockerLogStream.once('error', () => timer.stop)
        monitorDog.increment(baseDataName + '.connections', {result: 'failure'})
