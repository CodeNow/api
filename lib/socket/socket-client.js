 ...
 */
SocketClient.prototype.joinOrgRoom = function (orgId, cb) {
  debug('joinOrgRoom', formatArgs(arguments));
  this.primusClient.on('data', function onData (data) {
    if (data.event === 'ROOM_ACTION_COMPLETE') {
      cb();
    }
  });
  this.primusClient.write({
    // TODO: anton do we need to use uuid here? Like in fixtures?
    id: orgId,
    event: 'subscribe',
    data: {
      action: 'join',
