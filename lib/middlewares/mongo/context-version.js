...
var contextVersions = module.exports = createMongooseMiddleware(Version, {
  findVersion: function (key) {
    return flow.series(
      ...
      flow.or(
        mw.req('me').require().then(me.isOwnerOf('contextVersion')),
        mw.req('me').require().then(me.isModerator),
        // FIXME: this is a hack to authorize github push to use version
        mw.headers('x-github-event').require()
          .then(mw.headers('x-github-event').matches(/^push$/))));
  },
  ...
  buildVersion: function () {
    ...
    return flow.series(
      ...
      contextVersions.updateById('params.id', {
        $set: {
