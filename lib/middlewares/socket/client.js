 ...
 * @param {Object} socketClient - socket client which is used for listening events
 * @param {Object} runnableUser - runnable user
 * @param {Function} cb - standard callback
 */
 module.exports.onInstanceDeployed = function (instance, buildId, socketClient, runnableUser, cb) {
  ...
  if (instance.containers && instance.containers.length > 0) {
    ...
    // NOTE instance creator should be user that commited code or
    // `masterInstance` creator if committer user doesn't exists in Runnable
    ...
    runnableClient.deployInstanceWithRetry(instance, payload, safeCallback);
  }
  var called = false;
