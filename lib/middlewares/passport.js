...
function manageUser (accessToken, refreshToken, profile, done) {
  ...
  function updateOrCreateUser (user, cb) {
    ...
    if (user && user._id) {
      ...
      logger.log.trace({ profile }, 'save jorginho')
      return user.saveAsync()
        .then(function () {
          logger.log.trace({ profile }, 'Create or Update User... jorginho')
          return UserService.createOrUpdateUser(profile.id, profile.accessToken)
        })
        .then(function () {
          logger.log.trace({ profile }, 'getCompleteUserByGithubId jorginho')
    ...
    return UserService.createCompleteUser(data)
      .asCallback(cb)
  }
}
