...
function fetchById (id, cb) {
  ...
  .catch(function (err) {
    logger.log.warn({ err }, 'Error fetching user by id')
    throw err
  })
  .asCallback(cb)
}

function fetchUserByGithubId (githubId, cb) {
  logger.log.info({ githubId }, 'Fetching user by github id')
  Promise.all([
    User.findByGithubIdAsync(githubId),
    UserService.getByGithubId(githubId)
  ])
  .spread(function (user, bigPoppaUser) {
    user.set('bigPoppaUser', bigPoppaUser)
    logger.log.trace('Attaching bp user to session user')
  })
  .catch(function (err) {
    logger.log.warn({ err }, 'Error getting bigpoppa user')
    throw err
  })
}
  ...
  function updateOrCreateUser (user, cb) {
    ...
    if (user && user._id) {
      ...
      user.save(function (err) {
        ...
        return fetchUserByGithubId(profile.id, cb)
      })
    } else {
