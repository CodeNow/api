...
function Slack (settings, contextOwner) {
  ...
  // TODO (remove) token
  var apiToken = keypather.get(this.settings, 'notifications.slack.apiToken') || 'xoxb-4069689649-JB4Uxf0d5P8VxGLBpD6yI88F';
  this.slackClient = new SlackAPI(apiToken);
}

Slack.prototype.sendDirectMessage = function (gitUser, message, cb) {
  var mapping = keypather.get(this.settings, 'notifications.slack.githubUsernameToSlackIdMap') || {};
