...
Slack.prototype.sendDirectMessage = function (gitUser, message, cb) {
  ...
  if (!gitUser.username) {
    // committer.username can be null if git user wasn't configured properly
    return cb(null);
  }
...
Slack.prototype.notifyOnNewBranch = function (gitInfo, cb) {
  ...
  if (process.env.ENABLE_SLACK_MESSAGES !== 'true') {
    return cb(null);
  }
  ...
  var committer = keypather.get(gitInfo, 'headCommit.committer');
  if (!committer) {
    return cb(null);
  }
  ...
  this.sendDirectMessage(committer, message, cb);
};
