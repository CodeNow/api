...
Slack.prototype.notifyOnNewBranch = function (gitInfo, cb) {
  ...
  dogstatsd.increment('api.slack.new_branch', 1, tags);
};
...
// Utility function to double encode string.
...
Slack.prototype._createAutoForkText = function (gitInfo, instance) {
  ...
  text += ' are deployed on the new server:\n';
  text += this._createServerLink(instance);
  return text;
};
...
Slack.prototype._createServerSelectionText = function (owner, gitInfo) {
  var url = 'https://' + process.env.DOMAIN + '/' + owner + '/serverSelection/' +
    gitInfo.repoName + '?branch=' + doubleEncode(gitInfo.branch) +
    '&commit=' + gitInfo.commit +
    '&message=' + doubleEncode(gitInfo.headCommit.message) +
    '&' + REF_SLACK;
  return this._createSlackLink(url, 'Choose a server') +
    ' to run ' + gitInfo.branch + ' (' + gitInfo.repo + ')';
};
