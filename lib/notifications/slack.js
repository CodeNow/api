...
Slack.prototype.notifyOnAutoUpdate = function (gitInfo, instances, cb) {
  ...
  var text = instances[0].owner.username + '\\' + instances[0].name;
  text += 'Your changes (' + gitInfo.headCommit.messages + ') to ';
  text += 'CodeNow/runnable-angular ' + gitInfo.branch + ' are deployed on servers: ';
...
function createServerUrl (owner, gitInfo) {
...
function createServerSelectionUrl (instance) {
