...
images.find({}, function(err, images) {
  ...
  async.forEachSeries(images, function(image, cb) {
    ...
    request.post({url:"http://10.0.2.235:4243/v1.4/images/registry.runnable.com/runnable/" + encodeId(image._id) + "/push"}, function(err1, resp1, body1) {
		console.log("Trying to push all images for project http://runnable.com/" + encodeId(image._id));
		if (err1)
			console.log("Error is:", err1);
	    console.log("body1 is", body1);

	 	async.eachSeries(image.revisions, function(item, callback) {
	        request.post({url:"http://10.0.2.235:4243/v1.4/images/registry.runnable.com/runnable/" + encodeId(item.id)+ "/push"}, function(err2, resp2, body2) {
				console.log("http://10.0.2.235:4243/v1.4/images/registry.runnable.com/runnable/" + encodeId(item.id)+ "/push returned");
				if (err2)
