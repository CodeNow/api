...
mock.start = function (cb) {
  ...
  if (AWS.Route53 !== Route53) {
    debug('route53 mock already started', !!AWS.Route53, !!Route53);
    return cb();
  }
...
var resp = {
  nameNotPermittedErr: function (name) {
    var message = 'RRSet with DNS name {name}. is not permitted in zone {domain}.'
      .replace('{name}', name)
      .replace('{domain}', process.env.DOMAIN_HOST);
...
function mockUpsert (resourceRecordSet, cb) {
  ...
  var domainRe = new RegExp(escapeRegExp(process.env.DOMAIN_HOST)+'$');
